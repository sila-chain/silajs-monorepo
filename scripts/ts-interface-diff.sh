...
# 
# Usage: ./scripts/ts-interface-diff.sh silajs-vm packages/vm/
# Run from repository root
# 
# To send stdout to VS Code to leverage better tools to visualize diff, you can run:
# ./scripts/ts-interface-diff.sh silajs-blockchain packages/blockchain/ | code-
# 
...
# git diff --no-index -- .tmp/silajs-vm/A .tmp/silajs-vm/B
# 
# or variations of it:
# git diff --word-diff=color --no-index -- .tmp/silajs-vm/A .tmp/silajs-vm/B
###
