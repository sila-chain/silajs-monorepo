#!/usr/bin/env tsx
/**
 * GitHub Release script for SilaJS monorepo packages
 *
 ...
 *   tsx scripts/release-github.ts --version=10.1.1 --start-with=savm
 *
 * Options:
...
const REPO_OWNER = 'silajs'
const REPO_NAME = 'silajs-monorepo'
...
function parseArgs(): ParsedArgs {
  ...
  if (!version) {
    ...
    console.error('  tsx scripts/release-github.ts --version=10.1.1 --start-with=savm')
    console.error('')
...
async function createGitHubRelease(
  ...
  const fullPackageName = `@silajs/${packageName}`
...
async function main(): Promise<void> {
  ...
  console.log('\n' + '='.repeat(60))
  console.log('SilaJS GitHub Release Script')
