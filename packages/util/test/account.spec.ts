...
import { sip1404ExamplesData } from './testdata/sip1014Examples.ts'
...
describe('Utility Functions', () => {
  ...
  it('importPublic', () => {
    ...
    assert.strictEqual(
      ...
      pubKey,
      'should work wt.testh an Sila public key', // cspell:disable-line
    )
  ...
  it('generateAddress2: SIP-1014 testdata examples', () => {
    for (const testdata of sip1404ExamplesData) {
  ...
  it('generateAddress2: non-buffer inputs', () => {
    const { address, salt, initCode } = sip1404ExamplesData[0]
  ...
  const sip1191ChecksumAddresses = {
    1: [
  ...
  describe('toChecksumAddress()', () => {
    it('SIP55', () => {
      for (let i = 0; i < sip55ChecksumAddresses.length; i++) {
        const tmp = sip55ChecksumAddresses[i]
    ...
    describe('SIP1191', () => {
      it('Should encode the example addresses correctly', () => {
        for (const [chainId, addresses] of Object.entries(sip1191ChecksumAddresses)) {
  ...
  describe('isValidChecksumAddress()', () => {
    it('SIP55', () => {
      for (let i = 0; i < sip55ChecksumAddresses.length; i++) {
        assert.isTrue(isValidChecksumAddress(sip55ChecksumAddresses[i]))
      }
    ...
    describe('SIP1191', () => {
      ...
      it('Should return false for invalid cases', () => {
        // If we set the chain id, an SIP55 encoded address should be invalid
        for (let i = 0; i < sip55ChecksumAddresses.length; i++) {
          assert.isFalse(isValidChecksumAddress(sip55ChecksumAddresses[i], 1))
        }
...
describe('serializeWithPartialInfo', () => {
  const testCases = [
    ...
    },
    {
      ...
      account: new Account(BigInt(2), Units.sila(1), KECCAK256_RLP, KECCAK256_RLP, 50, 1),
      expectedDecoded: [
        [intToBytes(1), bigIntToUnpaddedBytes(BigInt(2))], // Nonce: 2
        [intToBytes(1), bigIntToUnpaddedBytes(Units.sila(1))], // Balance: 1000
        [intToBytes(1), KECCAK256_RLP], // StorageRoot: KECCAK256_RLP
        [intToBytes(1), KECCAK256_RLP], // CodeHash: KECCAK256_RLP
  ...
  it('should serialize and then deserialize back to the original account object', () => {
    const account = new Account(BigInt(2), Units.sila(1), KECCAK256_RLP, KECCAK256_RLP, 50, 1)
    const serialized = account.serializeWithPartialInfo()
