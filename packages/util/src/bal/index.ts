 ...
 *
 * @remarks Experimental (SilaAmsterdam): public API and behaviour may change on patch releases.
...
export class BlockLevelAccessList {
  ...
  public revert(): void {
    ...
    // SIP-7928: When storage writes are reverted, the slot keys MUST still
    ...
    for (const [address, access] of Object.entries(current)) {
      ...
      // SIP-7928: Convert reverted storageChanges to storageReads
      for (const slot of Object.keys(access.storageChanges)) {
  ...
  public addStorageWrite(
    ...
    // SIP-7928: Check if this is a no-op write (value equals pre-transaction value)
    ...
    // SIP-7928: Zeroing a slot (pre-value exists, post-value is zero) IS a write
    if (isNoOp) {
      // SIP-7928: A no-op write within a transaction only affects that tx's
    ...
    // Per SIP-7928: A successful storage write subsumes any prior read of the same slot.
  ...
  public addStorageRead(address: BALAddressHex, storageKey: BALStorageKeyBytes): void {
    ...
    const strippedKey = normalizeStorageKeyHex(bytesToHex(stripLeadingZeros(storageKey)))
    // Per SIP-7928: Don't add to storageReads if the slot was already written.
  ...
  public addBalanceChange(
    ...
    // SIP-7928: Track the original (pre-transaction) balance for net-zero detection
  ...
  public cleanupNetZeroBalanceChanges(): void {
    for (const [address, originalBalance] of this.originalBalances.entries()) {
      ...
      // SIP-7928: If final balance == original balance, remove all balanceChanges
  ...
  public cleanupSelfdestructed(addresses: Array<BALAddressHex>): void {
    for (const address of addresses) {
      ...
      // SIP-7928: If the account had a positive pre-transaction balance,
...
function shouldIncludeAddress(_address: BALAddressHex, _access: Accesses[BALAddressHex]): boolean {
  ...
  // fixture JSON). SIP-7928 requires touched addresses to appear even with empty change
