...
/**
 * SIP-7928 gas cost attributed to each BAL item (one address or one storage key).
 *
...
function throwIncorrectBlockFormat(detail: string): never {
  throw SilaJSErrorWithoutCode(`invalid header: ${detail}`)
}

function throwInvalidBlockAccessList(detail: string): never {
  throw SilaJSErrorWithoutCode(`invalid block access list: ${detail}`)
}

function throwInvalidBalHash(detail: string): never {
  throw SilaJSErrorWithoutCode(`invalid block access list hash: ${detail}`)
}

function throwBlockAccessListGasLimitExceeded(): never {
  throw SilaJSErrorWithoutCode('block access list gas limit exceeded')
}
