...
export const ETHER_TO_WEI = BigInt(10 ** 18) // Multiplier to convert from Sila to Wei
...
export class Units {
  static validateInput(amount: number | bigint): void {
    if (typeof amount === 'number' && !Number.isInteger(amount)) {
      throw SilaJSErrorWithoutCode('Input must be an integer number')
    }
    if (BigInt(amount) < 0) {
      throw SilaJSErrorWithoutCode('Input must be a positive number')
    }
  }
   ...
   *
   * @param {number | bigint} amount amount of units of sila to convert to wei
   * @returns {bigint} amount of units in wei
   */
  static sila(amount: number | bigint): bigint {
