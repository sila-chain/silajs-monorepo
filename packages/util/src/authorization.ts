...
import { SilaJSErrorWithoutCode, RLP } from '@silajs/rlp'
  ...
  for (const field of requiredFields) {
    if (authorizationList[field] === undefined) {
      throw SilaJSErrorWithoutCode(
        `SIP-7702 authorization list invalid: ${field} is not defined`,
      )
    }
  }
...
export function eoaCode7702AuthorizationMessageToSign(
  ...
  if (Array.isArray(input)) {
    ...
    if (address.length !== 20) {
      throw SilaJSErrorWithoutCode('Cannot sign authority: address length should be 20 bytes')
    }
    return concatBytes(
