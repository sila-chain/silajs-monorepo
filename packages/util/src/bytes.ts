...
export const hexToBytes = (hex: PrefixedHexString): Uint8Array => {
  if (!hex.startsWith('0x')) throw SilaJSErrorWithoutCode('input string must be 0x prefixed')
  return hexToBytesNoble(padToEven(stripHexPrefix(hex)))
}

export const unprefixedHexToBytes = (hex: string): Uint8Array => {
  if (hex.startsWith('0x')) throw SilaJSErrorWithoutCode('input string cannot be 0x prefixed')
...
export const bytesToInt = (bytes: Uint8Array): number => {
  ...
  if (!Number.isSafeInteger(res)) throw SilaJSErrorWithoutCode('Number exceeds 53 bits')
  return res
}
...
export const intToHex = (i: number): PrefixedHexString => {
  if (!Number.isSafeInteger(i) || i < 0) {
    throw SilaJSErrorWithoutCode(`Received an invalid integer type: ${i}`)
  }
...
export const toBytes = (v: ToBytesInputTypes): Uint8Array => {
  ...
  if (typeof v === 'bigint') {
    if (v < BIGINT_0) {
      throw SilaJSErrorWithoutCode(`Cannot convert negative bigint to Uint8Array. Given: ${v}`)
    }
    let n = v.toString(16)
    if (n.length % 2) n = '0' + n
  ...
  throw SilaJSErrorWithoutCode('invalid type')
}
