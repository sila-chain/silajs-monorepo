...
function handleNullIndicator(values: NestedUint8Array | Uint8Array): Uint8Array | null {
  ...
  if (nullIndicator > 1) {
    throw SilaJSErrorWithoutCode(`Invalid isNullIndicator=${nullIndicator}`)
  }
  if (values.length < 2) {
    throw SilaJSErrorWithoutCode(`Invalid values length=${values.length}`)
  }
...
export class Account {
  ...
  private _validate() {
    if (this._nonce !== null && this._nonce < BIGINT_0) {
      throw SilaJSErrorWithoutCode('nonce must be greater than zero')
    }
    if (this._balance !== null && this._balance < BIGINT_0) {
      throw SilaJSErrorWithoutCode('balance must be greater than zero')
    }
    if (this._storageRoot !== null && this._storageRoot.length !== 32) {
      throw SilaJSErrorWithoutCode('storageRoot must have a length of 32')
    }
    if (this._codeHash !== null && this._codeHash.length !== 32) {
      throw SilaJSErrorWithoutCode('codeHash must have a length of 32')
    }
    if (this._codeSize !== null && this._codeSize < BIGINT_0) {
      throw SilaJSErrorWithoutCode('codeSize must be greater than zero')
    }
  }
...
export function createAccountFromRLP(serialized: Uint8Array) {
  ...
  if (!Array.isArray(values)) {
    throw SilaJSErrorWithoutCode('Invalid serialized account input. Must be array')
  }
...
export function createPartialAccountFromRLP(serialized: Uint8Array) {
  ...
  for (const value of values) {
    ...
    if (!Array.isArray(value)) {
      throw SilaJSErrorWithoutCode('Invalid partial encoding. Each item must be an array')
    }
  }
...
export const toChecksumAddress = function (
  hexAddress: string,
  sip1191ChainId?: BigIntLike,
): PrefixedHexString {
  assertIsHexString(hexAddress)
  const address = stripHexPrefix(hexAddress).toLowerCase()

  let prefix = ''
  if (sip1191ChainId !== undefined) {
    const chainId = bytesToBigInt(toBytes(sip1191ChainId))
 ...
 *
 * See toChecksumAddress' documentation for details about the sip1191ChainId parameter.
 */
export const isValidChecksumAddress = function (
  hexAddress: string,
  sip1191ChainId?: BigIntLike,
): boolean {
  return isValidAddress(hexAddress) && toChecksumAddress(hexAddress, sip1191ChainId) === hexAddress
}
...
export const generateAddress2 = function (
  ...
  if (from.length !== 20) {
    throw SilaJSErrorWithoutCode('Expected from to be of length 20')
  }
  if (salt.length !== 32) {
    throw SilaJSErrorWithoutCode('Expected salt to be of length 32')
  }
...
export const pubToAddress = function (pubKey: Uint8Array, sanitize: boolean = false): Uint8Array {
  ...
  if (pubKey.length !== 64) {
    throw SilaJSErrorWithoutCode('Expected pubKey to be of length 64')
  }
  // Only take the lower 160bits of the hash
...
/**
 * Returns the sila public key of a given private key.
...
/**
 * Returns the sila address of a given private key.
 * @param privateKey A private key must be 256 bits wide
 */
...
/**
 * Converts a public key to the Sila format.
 */
