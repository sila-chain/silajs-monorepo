...
export class BinaryTree {
  ...
  root(value?: Uint8Array | null): Uint8Array {
    if (value !== undefined) {
      ...
      if (value.length !== this._hashLen) {
        throw SilaJSErrorWithoutCode(`Invalid root length. Roots are ${this._hashLen} bytes`)
      }
  ...
  async get(stem: Uint8Array, suffixes: number[]): Promise<(Uint8Array | null)[]> {
    if (stem.length !== 31)
      throw SilaJSErrorWithoutCode(`expected stem with length 31; got ${stem.length}`)
  ...
  async put(stem: Uint8Array, suffixes: number[], values: (Uint8Array | null)[]): Promise<void> {
    ...
    if (foundPath.stack.length === 0)
      throw SilaJSErrorWithoutCode(`Root node not found in trie`)
  ...
  async findPath(keyInBytes: Uint8Array): Promise<Path> {
    ...
    if (rawNode === undefined) throw SilaJSErrorWithoutCode('root node should exist')
    ...
    while (!finished) {
      ...
      if (rawNode === undefined)
        throw SilaJSErrorWithoutCode(`missing node at ${childNode.path}`)
  ...
  createReadStream(): any {
    throw SilaJSErrorWithoutCode('Not implemented')
  }
  ...
  protected hash(msg: Uint8Array | null): Uint8Array {
    ...
    if (msg.length !== 32 && msg.length !== 64) {
      throw SilaJSErrorWithoutCode('Data must be 32 or 64 bytes')
    }
