...
import { SilaJSErrorWithoutCode, bytesToHex, isDebugEnabled } from '@silajs/util'
...
export class EthashConsensus implements Consensus {
  ...
  async validateConsensus(block: Block): Promise<void> {
    ...
    if (!valid) {
      throw SilaJSErrorWithoutCode('invalid POW')
    }
    this.DEBUG &&
      this._debug(
  ...
  async validateDifficulty(header: BlockHeader) {
    ...
    if (header.ethashCanonicalDifficulty(parentHeader) !== header.difficulty) {
      throw SilaJSErrorWithoutCode(`invalid difficulty ${header.errorStr()}`)
    }
    this.DEBUG &&
      this._debug(
