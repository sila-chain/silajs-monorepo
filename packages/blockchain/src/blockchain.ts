import { Block, BlockHeader, createBlock } from '@silajs/block'
...
import type { CliqueConfig, GenesisState } from '@silajs/common'
import type { BigIntLike, DB, DBObject } from '@silajs/util'
...
export class Blockchain implements BlockchainInterface {
  ...
  constructor(opts: BlockchainOptions = {}) {
    ...
    if (opts.common) {
      ...
      const DEFAULT_CHAIN = SilaMainnet
  ...
  async getCanonicalHeadHeader(): Promise<BlockHeader> {
    return this.runWithLock<BlockHeader>(async () => {
      if (!this._headHeaderHash) throw SilaJSErrorWithoutCode('No head header set')
  ...
  async getCanonicalHeadBlock(): Promise<Block> {
    return this.runWithLock<Block>(async () => {
      if (!this._headBlockHash) throw SilaJSErrorWithoutCode('No head block set')
  ...
  async resetCanonicalHead(canonicalHead: bigint) {
    ...
    await this.runWithLock<void>(async () => {
      ...
      if (hash === undefined) {
        throw SilaJSErrorWithoutCode(`no block for ${canonicalHead} found in DB`)
      }
  ...
  public async validateHeader(header: BlockHeader, height?: bigint) {
    ...
    if (number !== parentHeader.number + BIGINT_1) {
      throw SilaJSErrorWithoutCode(`invalid number ${header.errorStr()}`)
    }

    if (header.timestamp <= parentHeader.timestamp) {
      throw SilaJSErrorWithoutCode(`invalid timestamp ${header.errorStr()}`)
    }
    ...
    // check blockchain dependent SIP1559 values
    if (header.common.isActivatedEIP(1559)) {
      ...
      if (header.baseFeePerGas! !== expectedBaseFee) {
        throw SilaJSErrorWithoutCode(`Invalid block: base fee not correct ${header.errorStr()}`)
      }
    }
    ...
    if (header.common.isActivatedEIP(7685)) {
      if (header.requestsHash === undefined) {
        throw SilaJSErrorWithoutCode(`requestsHash must be provided when SIP-7685 is active`)
      }
    }
  }
  ...
  private async _validateUncleHeaders(block: Block) {
    ...
    // See Geth: https://github.com/sila-chain/go-sila/blob/b63bffe8202d46ea10ac8c4f441c582642193ac8/consensus/ethash/consensus.go#L207
    ...
    uncleHeaders.map((uh) => {
      ...
      if (canonicalChainHashes[uncleHash]) {
        throw SilaJSErrorWithoutCode(`The uncle is a canonical block ${block.errorStr()}`)
      }
    })
  }
  ...
  async getBlock(blockId: Uint8Array | number | bigint): Promise<Block> {
    ...
    if (block === undefined) {
      if (typeof blockId === 'object') {
        throw SilaJSErrorWithoutCode(`Block with hash ${bytesToHex(blockId)} not found in DB`)
      } else {
        throw SilaJSErrorWithoutCode(`Block number ${blockId} not found in DB`)
      }
    }
    return block
  }
  ...
  public async getTotalDifficulty(hash: Uint8Array, number?: bigint): Promise<bigint> {
    if (number === undefined) {
      ...
      if (number === undefined) {
        throw SilaJSErrorWithoutCode(`Block with hash ${bytesToHex(hash)} not found in DB`)
      }
    }
  ...
  private async findCommonAncestor(newHeader: BlockHeader) {
    ...
    if (header.number !== newHeader.number) {
      throw SilaJSErrorWithoutCode('Failed to find ancient header')
    }
  ...
  private async _getHeader(hash: Uint8Array, number?: bigint) {
    if (number === undefined) {
      number = await this.dbManager.hashToNumber(hash)
      if (number === undefined)
        throw SilaJSErrorWithoutCode(`no header for ${bytesToHex(hash)} found in DB`)
    }
  ...
  async getCanonicalHeader(number: bigint) {
    ...
    if (hash === undefined) {
      throw SilaJSErrorWithoutCode(`header with number ${number} not found in canonical chain`)
    }
  ...
  get genesisBlock(): Block {
    if (!this._genesisBlock)
      throw SilaJSErrorWithoutCode('genesis block not set (init may not be finished)')
