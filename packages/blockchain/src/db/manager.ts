...
import type { Block, BlockBodyBytes, BlockBytes, BlockOptions } from '@silajs/block'
import type { Common } from '@silajs/common'
import type { BatchDBOp, DB, DBObject, DelBatch, PutBatch } from '@silajs/util'
...
export class DBManager {
  ...
  async getBlock(blockId: Uint8Array | bigint | number): Promise<Block | undefined> {
    ...
    if (blockId instanceof Uint8Array) {
      ...
      throw SilaJSErrorWithoutCode('Unknown blockId type')
    }
    ...
    if (body === undefined) {
      ...
      if (!equalsBytes(header.transactionsTrie, KECCAK256_RLP)) {
        throw SilaJSErrorWithoutCode('transactionsTrie root should be equal to hash of null')
      }

      if (!equalsBytes(header.uncleHash, KECCAK256_RLP_ARRAY)) {
        throw SilaJSErrorWithoutCode('uncle hash should be equal to hash of empty array')
      }
  ...
  async hashToNumber(blockHash: Uint8Array): Promise<bigint | undefined> {
    ...
    if (value === undefined) {
      throw SilaJSErrorWithoutCode(`value for ${bytesToHex(blockHash)} not found in DB`)
    }
  ...
  async get(dbOperationTarget: DBTarget, key?: DatabaseKey): Promise<any> {
    ...
    if (cacheString !== undefined) {
      if (this._cache[cacheString] === undefined) {
        throw SilaJSErrorWithoutCode(`Invalid cache: ${cacheString}`)
      }
