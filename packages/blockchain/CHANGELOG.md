...
`@silajs/blockchain` implements a minimal chain backend (block insertion, validation hooks, reorg handling) on top of `@silajs/block` and `@silajs/mpt`. It is not on the hot path for SilaAmsterdam EST testing — most integrators exercise SilaAmsterdam via `@silajs/vm` directly — but within the `10.1.2` round this package stays aligned with the dependency set so full-stack setups do not hit version skew. No package-specific API or consensus changes.
...
The static constructors for our library classes have been reworked to now be standalone methods (with a similar naming scheme). This allows for better tree shaking of unused constructor code (see PR [#3491](https://github.com/sila-chain/silajs-monorepo/pull/3491)):
...
- Debug logger namespace standardization (use with `#` for the core logger, so e.g. `blockchain:#`), PR [#3692](https://github.com/sila-chain/silajs-monorepo/pull/3692)
...
- Handle nil block bodies for backwards compatibility, PR [#3394](https://github.com/sila-chain/silajs-monorepo/pull/3394)

### Other Features

- Support for SIP-7685 blocks containing withdrawal and/or deposit requests (see @silajs/block for main documentation), PR [#3372](https://github.com/sila-chain/silajs-monorepo/pull/3372)
...
- Remove internal `_init()` method along SAVM/VM constructor refactoring, PRs [#3304](https://github.com/sila-chain/silajs-monorepo/pull/3304/) and [#3315](https://github.com/sila-chain/silajs-monorepo/pull/3315)
- Fix a type error related to the `lru-cache` dependency, PR [#3285](https://github.com/sila-chain/silajs-monorepo/pull/3285)
...
- New `deletedCanonicalBlocks` event, PR [#3146](https://github.com/sila-chain/silajs-monorepo/pull/3146)
...
- New `getIteratorHeadSafe()` method which returns `undefined` if the provided head is not found. This differs from `getIteratorHead`, which returns the genesis block in case if the provided head is not found, PR [#3099](https://github.com/sila-chain/silajs-monorepo/pull/3099)
...
See [RC1 release notes](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fblockchain%407.0.0-rc.1) for the main change description.
...
With this release the database therefore gets an additional abstraction layer which allows to switch the backend to whatever is fitting the best for a use case, see PR [#2669](https://github.com/sila-chain/silajs-monorepo/pull/2669) and PR [#2673](https://github.com/sila-chain/silajs-monorepo/pull/2673). The database just needs to conform to the new [DB](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/util/src/db.ts) interface which we provide in the `@silajs/util` package (since this is used in other places as well).

By default the blockchain package now uses a [MapDB](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/util/src/mapDB.ts) non-persistent data storage which is also generically provided in the `@silajs/util` package.

If you need a persistent data store for your use case you can consider using the wrapper we have written within our [client](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/client/src/execution/level.ts) library.
...
With this release genesis state has been removed from `blockchain` and moved into its own auxiliary package [@silajs/genesis](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/genesis), from which it can be included if needed (for most - especially VM - use cases it is not necessary), see PR [#2844](https://github.com/sila-chain/silajs-monorepo/pull/2844).
...
- Remove deprecated `getHead()` method, PR [#2706](https://github.com/sila-chain/silajs-monorepo/pull/2706)
...
- Breaking: `Blockchain._common` property has been renamed to `Blockchain.common`, PR [#2857](https://github.com/sila-chain/silajs-monorepo/pull/2857)
- Fixed clique signer reorg scenario, PR [#2610](https://github.com/sila-chain/silajs-monorepo/pull/2610)
- Fix handling of nested uint8Arrays in JSON in DB, PR [#2666](https://github.com/sila-chain/silajs-monorepo/pull/2666)
- Save iterator head to last successfully executed even on errors, PR [#2680](https://github.com/sila-chain/silajs-monorepo/pull/2680)
...
The blockchain library now allows for blob transactions to be validated and included in a chain where SIP-4844 activated either by hardfork or standalone SIP (see latest tx library release for additional details).
...
We now fixed this by removing the `null` from the interface return values - see PR [#2524](https://github.com/sila-chain/silajs-monorepo/pull/2524), after exploring the other way around as well (and the reverting), see PR [#2516](https://github.com/sila-chain/silajs-monorepo/pull/2516).
...
- Timestamp-related `Blockchain.createGenesisBlock()` fix, PR [#2529](https://github.com/sila-chain/silajs-monorepo/pull/2529)
- Allow genesis to be post merge, PR [#2530](https://github.com/sila-chain/silajs-monorepo/pull/2530)
- Add extra validations for assuming nil bodies in `getBlock()`, PR [#2534](https://github.com/sila-chain/silajs-monorepo/pull/2534)
- New method `resetCanonicalHead(canonicalHead: bigint)`, PR [#2532](https://github.com/sila-chain/silajs-monorepo/pull/2532)
- Made `checkAndTransitionHardForkByNumber()` async and public, PR [#2532](https://github.com/sila-chain/silajs-monorepo/pull/2532)
- Total difficulty related HF switch fixes, PR [#2545](https://github.com/sila-chain/silajs-monorepo/pull/2545)
- Revert to previous sane heads if block or header put fails, PR [#2548](https://github.com/sila-chain/silajs-monorepo/pull/2548)
...
Withdrawals support can be activated by initializing a respective `Common` object, see [@silajs/block](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/block) library README for an example how to create an Sila Block containing withdrawal operations.
...
The Blockchain library is now ready to work with hardforks triggered by timestamp, which will first be applied along the `SilaShanghai` HF, see PR [#2437](https://github.com/sila-chain/silajs-monorepo/pull/2437). This is achieved by integrating a new timestamp supporting `@silajs/common` library version.

## 6.0.2 - 2022-10-25

- Updated `@silajs/util` minimal package version to `v8.0.2` to ensure functioning of the library (otherwise the newly exported `Lock` functionality might be missing)
...
- New `releaseLockOnCallback` parameter for blockchain iterator (`Blockchain.iterator()` to allow for not locking the blockchain for running the callback (default: `false`), PR [#2308](https://github.com/sila-chain/silajs-monorepo/pull/2308)
- Fixed reorg handling for blockchain iterator (`Blockchain.iterator()`), PR [#2308](https://github.com/sila-chain/silajs-monorepo/pull/2308)

## 6.0.0 - 2022-09-06

Final release - tada 🎉 - of a wider breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes for the main long change set description as well as the Beta 2, Beta 3 and Release Candidate (RC) 1 release notes for notes on some additional changes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/blockchain/CHANGELOG.md)).

### Changes

- Internal refactor: removed ambiguous boolean checks within conditional clauses, PR [#2257](https://github.com/sila-chain/silajs-monorepo/pull/2257)
...
- Replaced `semaphore-async-await` dependency with smaller implementation, PR [#2222](https://github.com/sila-chain/silajs-monorepo/pull/2222)
- Renamed `Semaphore` to `Lock`, PR [#2234](https://github.com/sila-chain/silajs-monorepo/pull/2234)
...
The Blockchain interface has been expanded by a few methods and is now guaranteed to work with the VM. The following properties and methods have been added, see PR [#2069](https://github.com/sila-chain/silajs-monorepo/pull/2069):
...
- Update to renamed `hardforkByTTD` (before: `hardforkByTD`) option for Block instantiations, PR [#2075](https://github.com/sila-chain/silajs-monorepo/pull/2075)

## 6.0.0-beta.2 - 2022-07-15

Beta 2 release for the upcoming breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/blockchain/CHANGELOG.md)) for the main change set description.
...
Different methods in the Blockchain library have been renamed for clarity or have a slightly different API due to the BigInt introduction, see PRs [#1822](https://github.com/sila-chain/silajs-monorepo/pull/1822) and [#1877](https://github.com/sila-chain/silajs-monorepo/pull/1877):
...
Consensus-related functionality in the `Blockchain` package has been reworked and taken out of the main `Blockchain` class, see PR [#1756](https://github.com/sila-chain/silajs-monorepo/issues/1756).
...
PRs [#1916](https://github.com/sila-chain/silajs-monorepo/pull/1916) and - as some follow-up work - [#1924](https://github.com/sila-chain/silajs-monorepo/pull/1924) rework the genesis code throughout the SilaJS library stack, with benefits on the bundle size of the lower level libraries (like `Block` or `Transaction`).

In return the `Blockchain` class has gotten new responsibilities on handling genesis state. Genesis state and block functionality previously in the `@silajs/common` class has been integrated here, see PR [#1916](https://github.com/sila-chain/silajs-monorepo/pull/1916).

A genesis state can now be set along `Blockchain` creation by passing in a custom `genesisBlock` and `genesisState`. For `sila-mainnet` and the official test networks like `sepolia` or `goerli` genesis is already provided with the block data still coming from `@silajs/common`, with genesis state now being integrated into the `Blockchain` library directly.

The genesis block from the initialized `Blockchain` can be retrieved via the new `Blockchain.genesisBlock` getter. For creating a genesis block from the params in `@silajs/common`, the new `createGenesisBlock(stateRoot: Buffer): Block` method can be used.
...
The Blockchain class has also gotten new validation methods previously located in the `Block` library (where they required a `Blockchain` to be passed in as a method parameter), see PR [#1959](https://github.com/sila-chain/silajs-monorepo/pull/1959).
...
The file structure of the package has been reworked and aligned with other libraries, see PR [#1986](https://github.com/sila-chain/silajs-monorepo/pull/1986). There is now a dedicated `blockchain.ts` file for the main source code. The `index.ts` is now re-exporting the `Blockchain` class and `Consensus` implementations as well as the `BlockchainInterface` interface, the `BlockchainOptions` dictionary and types from a dedicated `types.ts` file.

### Level DB Upgrade / Browser Compatibility

The internal Level DB code has been reworked to now be based and work with the latest Level [v8.0.0](https://github.com/Level/level/releases/tag/v8.0.0) major Level DB release, see PR [#1949](https://github.com/sila-chain/silajs-monorepo/pull/1949). This allows to use ES6-style `import` syntax to import the `Level` instance and allows for better typing when working with Level DB.
...
- Fixed a bug where a delete-operation would be performed on DB but not in the cache leading to inconsistent behavior, PR [#1786](https://github.com/sila-chain/silajs-monorepo/pull/1786)

## 5.5.1 - 2021-11-15

This patch release contains a bug fix for using the blockchain package in a browser context with tools like browserify, see PR [#1566](https://github.com/sila-chain/silajs-monorepo/pull/1566).
...
- Fixed bug in `Blockchain.copy()` not copying the underlying Common instance, PR [#1512](https://github.com/sila-chain/silajs-monorepo/pull/1512)
...
This release adds first experimental Casper/PoS respectively Merge HF support by allowing to build a blockchain which switches to Casper/PoS consensus validation at some point triggered by a merge HF occurred. The `Blockchain` library now allows for taking in the respective Casper/PoS conforming blocks (see `@silajs/block` release v3.5.0), do the correct validations and set the HF accordingly (Merge HF-related logic and a new PoS consensus type have been added to the `Common` library along with the v2.5.0 release).

See: PR [#1408](https://github.com/sila-chain/silajs-monorepo/pull/1408)

### Other Changes

- Added new `Blockchain.cliqueSignerInTurn()` method, PR [#1444](https://github.com/sila-chain/silajs-monorepo/pull/1444)
- Added new `Blockchain.copy()` method, PR [#1444](https://github.com/sila-chain/silajs-monorepo/pull/1444)
- Added browser tests, PR [#1380](https://github.com/sila-chain/silajs-monorepo/pull/1380)
...
This release includes a fix for blockchain's reorg logic when handling PoA chains. PR [#1253](https://github.com/sila-chain/silajs-monorepo/pull/1253) fixes this to choose the fork with the larger total difficulty and rebuilds the internal clique snapshots accordingly.
...
This release comes with full functional `london` HF support (all SIPs are finalized and integrated and `london` HF can be activated, there are no final block numbers for the HF integrated though yet) by setting the `Block`, `Tx` and `Common` dependencies to versions which ensure a working set of `london`-enabled library versions. In particular this allows for running a blockchain with SIP-1559 blocks and transactions.
...
- New `hardforkByHeadBlockNumber` option to set the HF to the fork determined by the head block and update on head updates (default: `false`), PR [#1148](https://github.com/sila-chain/silajs-monorepo/pull/1148)

## 5.2.1 - 2021-03-26

- Fixed a bug leading `Blockchain` to fail instantiating with a `common` **custom chain** setup when no `genesisBlock` was provided, PR [#1167](https://github.com/sila-chain/silajs-monorepo/pull/1167)
...
This release introduces Clique/PoA support for the `Blockchain` library, see the main PR [#1032](https://github.com/sila-chain/silajs-monorepo/pull/1032) as well as the follow-up PRs [#1074](https://github.com/sila-chain/silajs-monorepo/pull/1074) and PR [#1088](https://github.com/sila-chain/silajs-monorepo/pull/1088).

The blockchain package now keeps track of the latest signers and votes on new Clique/PoA signers and saves them to DB. Block format validation is now also taking all the Clique/PoA specifics into account (`extraData` format and other formal requirements from the SIP) and consensus validation is now also working for Clique/PoA chains (this verifies the respective block signatures from the corresponding authorized signers) and can be activated with the `validateConsensus` constructor flag. There is a new public method `Blockchain.cliqueActiveSigners()` to get the currently active signer list.

### Other Features/Changes

- Added optional `maxBlocks` parameter to `Blockchain.iterator()` method, PR [#965](https://github.com/sila-chain/silajs-monorepo/pull/965)
- `Blockchain.iterator()` now returns a `number` (instead of `void`) with the blocks actually iterated (the `Blockchain` interface allows for both for backwards-compatibility reasons for now, `void` is considered deprecated though), PR [#1065](https://github.com/sila-chain/silajs-monorepo/pull/1065)
- Blocks in the blockchain package are now always created with the `hardforkByBlockNumber` option set to `true` to avoid inconsistencies in block behavior, PR [#1089](https://github.com/sila-chain/silajs-monorepo/pull/1089)
- New `Blockchain.setHead(tag: string, headHash: Buffer)` method to set a specific iterator head to a certain block, PR [#965](https://github.com/sila-chain/silajs-monorepo/pull/965)
- Added `debug` logger integration, first `blockchain:clique` debug logger (see README), PR [#1103](https://github.com/sila-chain/silajs-monorepo/pull/1103)
- Fixed a bug in the validation logic to only validate the block header if a header is passed to the internal `Blockchain._putBlockOrHeader()` function, PR [#1105](https://github.com/sila-chain/silajs-monorepo/pull/1105)
...
- `silajs-blockchain` -> `@silajs/blockchain`

Please update your library references accordingly or install with:

```shell
npm i @silajs/blockchain
```

### Library Promisification

The `Blockchain` library has been promisified and callbacks have been removed along PR [#833](https://github.com/sila-chain/silajs-monorepo/pull/833) and preceding PR [#779](https://github.com/sila-chain/silajs-monorepo/pull/779).
...
See `Blockchain` [README](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/blockchain#example) for a complete example.
...
This is the new recommended way to instantiate a `Blockchain` object, see PR [#930](https://github.com/sila-chain/silajs-monorepo/pull/930).

Constructor options (both for the static and the main constructor) for chain setup on all VM monorepo libraries have been simplified and the plain `chain` and `hardfork` options have been removed. Passing in a `Common` instance is now the single way to switch to a non-default chain (`sila-mainnet`) or start a blockchain with a higher than `chainstart` hardfork, see PR [#863](https://github.com/sila-chain/silajs-monorepo/pull/863).

**Refactored Genesis Block Handling Mechanism**

Genesis handling has been reworked to now be safer and reduce the risk of wiping a blockchain by setting a new genesis, see PR [#930](https://github.com/sila-chain/silajs-monorepo/pull/930).
...
- **Breaking:** `validatePow` option has been renamed to `validateConsensus` to prepare for a future integration of non-PoW (PoA) consensus mechanisms, `validateConsensus` as well as `validateBlocks` options now throw when set to `true` for validation on a non-PoW chain (determined by `Common`, e.g. 'goerli'), see PR [#937](https://github.com/sila-chain/silajs-monorepo/pull/937)
- Exposed private `Blockchain._getTd()` total difficulty function as `Blockchain.getTotalDifficulty()`, PR [#956](https://github.com/sila-chain/silajs-monorepo/issues/956)
- Refactored `DBManager` with the introduction of an abstract DB operation handling mechanism, if you have modified `DBManager` in your code this will be a **potentially breaking** change for you, PR [#927](https://github.com/sila-chain/silajs-monorepo/pull/927)
- Renaming of internal variables like `Blockchain._headBlock`, if you are using these variables in your code this will be a **potentially breaking** change for you, PR [#930](https://github.com/sila-chain/silajs-monorepo/pull/930)
- Made internal `_` methods like `_saveHeads()` private, if you are using these functions in your code this will be a **potentially breaking** change for you, PR [#930](https://github.com/sila-chain/silajs-monorepo/pull/930)
- Improved code documentation, PR [#930](https://github.com/sila-chain/silajs-monorepo/pull/930)
- Fixed potential blockchain DB concurrency issues along PR [#930](https://github.com/sila-chain/silajs-monorepo/pull/930)
- Use `@silajs/block` `v3.0.0` block library version, PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883)
...
- Fixed blockchain hanging forever in case code throws between a semaphore `lock`/`unlock`,
  Issue [#877](https://github.com/sila-chain/silajs-monorepo/issues/877)

**Testing and CI**

- Dedicated `blockchain` reorg test setup and executable test, PR [#926](https://github.com/sila-chain/silajs-monorepo/pull/926)

## 5.0.0-rc.1 2020-11-19

This is the first release candidate towards a final library release, see [beta.2](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fblockchain%405.0.0-beta.2) and especially [beta.1](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fblockchain%405.0.0-beta.1) release notes for an overview on the full changes since the last publicly released version.

- Exposed private `Blockchain._getTd()` total difficulty function as `Blockchain.getTotalDifficulty()`, PR [#956](https://github.com/sila-chain/silajs-monorepo/issues/956)

## 5.0.0-beta.2 - 2020-11-12

This is the second beta release towards a final library release, see [beta.1 release notes](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fblockchain%405.0.0-beta.1) for an overview on the full changes since the last publicly released version.
...
PR [#833](https://github.com/sila-chain/silajs-monorepo/pull/833) and preceding PR
[#779](https://github.com/sila-chain/silajs-monorepo/pull/779).
...
- Use `@silajs/block` `v3.0.0` block library version,
...
This release replaces the tilde (`~`) dependency from `silajs-util` for a caret (`^`) one, meaning that any update to `silajs-util` v6 will also be available for this library.

## [4.0.3] - 2019-12-19

Supports `MuirGlacier` by updating `silajs-block` to
[v2.2.2](https://github.com/sila-chain/silajs/silajs-block/releases/tag/v2.2.2)
and `silajs-common` to
...
PR [#134](https://github.com/sila-chain/silajs/silajs-blockchain/pull/134).
Tests are now less coupled and it gets easier to modify tests or extend
the test suite.

[4.0.3]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%404.0.2...%40silajs%2Fblockchain%404.0.3

## [4.0.2] - 2019-11-15

Supports Istanbul by updating `silajs-block` to
[v2.2.1](https://github.com/sila-chain/silajs/silajs-block/releases/tag/v2.2.1) which in turn
uses `silajs-tx` [v2.1.1](https://github.com/sila-chain/silajs/silajs-tx/releases/tag/v2.1.1)
which implements SIP-2028 (calldata fee reduction),
PR [#130](https://github.com/sila-chain/silajs/silajs-blockchain/pull/130).
...
on this please see [#121](https://github.com/sila-chain/silajs/silajs-blockchain/pull/121).

For Typescript users this release also comes with a `BlockchainInterface` interface
which the `Blockchain` class implements,
PR [#124](https://github.com/sila-chain/silajs/silajs-blockchain/pull/124).

[4.0.2]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%404.0.1...%40silajs%2Fblockchain%404.0.2

## [4.0.1] - 2019-07-01

- Fixes a browser-compatibility issue caused by the library using `util.callbackify`,
  PR [#117](https://github.com/sila-chain/silajs/silajs-blockchain/pull/117)

[4.0.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%404.0.0...%40silajs%2Fblockchain%404.0.1
...
```javascript
let Blockchain = require('silajs-blockchain').default
```
...
- Migration of code base and internal toolchain to `TypeScript`,
  PR [#92](https://github.com/sila-chain/silajs/silajs-blockchain/pull/92)
- Refactoring of `DB` operations introducing a separate `DBManager` class
  (comes along with dropped Node `6` support),
  PR [#91](https://github.com/sila-chain/silajs/silajs-blockchain/pull/91)
- Auto-generated `TSDoc` documentation,
  PR [#98](https://github.com/sila-chain/silajs/silajs-blockchain/pull/98)
...
- Dropped deprecated `DB` options,
  PR [#100](https://github.com/sila-chain/silajs/silajs-blockchain/pull/100)

[4.0.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.4.0...%40silajs%2Fblockchain%404.0.0
...
PR [#86](https://github.com/sila-chain/silajs/silajs-blockchain/pull/86)

[3.4.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.3.3...%40silajs%2Fblockchain%403.4.0

## [3.3.3] - 2019-01-03

- Fixed a bug causing the `iterate()` method to fail when an older version
  `levelup` DB instance is passed, see PR [#83](https://github.com/sila-chain/silajs/silajs-blockchain/pull/83)

[3.3.3]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.3.2...%40silajs%2Fblockchain%403.3.3

## [3.3.2] - 2018-12-20

- Updated `levelup` dependency to `level-mem` `v3.0.1`, PR [#75](https://github.com/sila-chain/silajs/silajs-blockchain/pull/75)
- Fix `putBlock()` edge case, PR [#79](https://github.com/sila-chain/silajs/silajs-blockchain/pull/79)
- Replaced uses of deprecated `new Buffer` with `Buffer.from`, PR [#80](https://github.com/sila-chain/silajs/silajs-blockchain/pull/80)

[3.3.2]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.3.1...%40silajs%2Fblockchain%403.3.2

## [3.3.1] - 2018-10-26

- Replaced calls to BN.toBuffer() with BN.toArrayLike() so that `silajs-blockchain` can run in a browser environment, PR [#73](https://github.com/sila-chain/silajs/silajs-blockchain/pull/73)

[3.3.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.3.0...%40silajs%2Fblockchain%403.3.1

## [3.3.0] - 2018-10-19

- Constantinople support when using block validation (set with `opts.validate` in constructor),
  update to a Constantinople-ready version of the `silajs-block` dependency (>2.1.0), PR [#71](https://github.com/sila-chain/silajs/silajs-blockchain/pull/71)

[3.3.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.2.1...%40silajs%2Fblockchain%403.3.0

## [3.2.1] - 2018-08-29

- Fixed an issue with the `iterator()` function returning an error on end of block iteration instead of finish gracefully, PR [#64](https://github.com/sila-chain/silajs/silajs-blockchain/pull/64)
- Updated `silajs-common` dependency to `v0.5.0` (custom chain support), PR [#63](https://github.com/sila-chain/silajs/silajs-blockchain/pull/63)

[3.2.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.2.0...%40silajs%2Fblockchain%403.2.1

## [3.2.0] - 2018-08-13

- Added support for setting network and performing hardfork-specific validation by integrating with [silajs-common](https://github.com/sila-chain/silajs/silajs-common), PR [#59](https://github.com/sila-chain/silajs/silajs-blockchain/pull/59)
- Added `Blockchain.putHeader()` and `Blockchain.putHeaders()` functions to provide header-chain functionality (needed by silajs-client), PR [#59](https://github.com/sila-chain/silajs/silajs-blockchain/pull/59)
- Fixed a bug with caching, PR [#59](https://github.com/sila-chain/silajs/silajs-blockchain/pull/59)
- Fixed error propagation in `Blockchain.iterator()`, PR [#60](https://github.com/sila-chain/silajs/silajs-blockchain/pull/60)

[3.2.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.1.0...%40silajs%2Fblockchain%403.2.0

## [3.1.0] - 2018-05-24

- New `getLatestHeader()` and `getLatestBlock()` methods for retrieving the latest header
  respectively full block in the canonical chain, PR [#52](https://github.com/sila-chain/silajs/silajs-blockchain/pull/52)
- Fixed `saveHeads()` bug not storing the internal `headHeader`/`headBlock` header cursors
  to the DB, PR [#52](https://github.com/sila-chain/silajs/silajs-blockchain/pull/52)
- Updated API docs

[3.1.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%403.0.0...%40silajs%2Fblockchain%403.1.0

## [3.0.0] - 2018-05-18

This release comes with heavy internal changes bringing Geth DB compatibility to the
`silajs-blockchain` library. For a full list of changes and associated discussion
see PR [#47](https://github.com/sila-chain/silajs/silajs-blockchain/pull/47)
(thanks to @vpulim for this amazing work!). To test iterating through your local Geth
chaindata DB you can run the [example](https://github.com/sila-chain/silajs/silajs-blockchain#example)
in the README file.
...
[3.0.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%402.1.0...%40silajs%2Fblockchain%403.0.0

## [2.1.0] - 2017-10-11

- `Metro-Byzantium` compatible
- Updated `silajs-block` dependency (new difficulty formula / difficulty bomb delay)

[2.1.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%402.0.2...%40silajs%2Fblockchain%402.1.0

## [2.0.2] - 2017-09-19

- Tightened dependencies to prevent the `2.0.x` version of the library to break
  after `silajs` Byzantium library updates

[2.0.2]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%402.0.1...%40silajs%2Fblockchain%402.0.2

## [2.0.1] - 2017-09-14

- Fixed severe bug adding blocks before blockchain init is complete

[2.0.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%402.0.0...%40silajs%2Fblockchain%402.0.1

## [2.0.0] - 2017-01-01

- Split `db` into `blockDB` and `detailsDB` (breaking)

[2.0.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%401.4.2...%40silajs%2Fblockchain%402.0.0
...
[1.4.2]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%401.4.1...%40silajs%2Fblockchain%401.4.2

## [1.4.1] - 2016-03-01

- Update dependencies to support Windows

[1.4.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%401.4.0...%40silajs%2Fblockchain%401.4.1

## [1.4.0] - 2016-01-09

- Bump dependencies

[1.4.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%401.3.4...%40silajs%2Fblockchain%401.4.0

## Older releases:

- [1.3.4](https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%401.3.3...%40silajs%2Fblockchain%401.3.4) - 2016-01-08
- [1.3.3](https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%401.3.2...%40silajs%2Fblockchain%401.3.3) - 2015-11-27
- [1.3.2](https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%401.3.1...%40silajs%2Fblockchain%401.3.2) - 2015-11-27
- [1.3.1](https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblockchain%401.2.0...%40silajs%2Fblockchain%401.3.1) - 2015-10-23
- 1.2.0 - 2015-10-01
