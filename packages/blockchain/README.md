...
The `Blockchain` package represents an Sila-compatible blockchain storing a sequential chain of [@silajs/block](../block) blocks and holding information about the current canonical head block as well as the context the chain is operating in (e.g. the hardfork rules the current head block adheres to).
...
const main = async () => {
  ...
  // See @silajs/block for more details on how to create a block
...
For storing blocks different backends can be used. The database needs to conform to the [DB](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/util/src/db.ts) interface provided in the `@silajs/util` package (since this is used in other places as well).

By default the blockchain package uses a [MapDB](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/util/src/mapDB.ts) non-persistent data storage which is also generically provided in the `@silajs/util` package.
...
The genesis block from the initialized `Blockchain` can be retrieved via the `Blockchain.genesisBlock` getter. For creating a genesis block from the params in `@silajs/common`, the `createGenesisBlock(stateRoot: Buffer): Block` method can be used.

## Supported Blocks and Tx Types

### SIP-1559 Support

This library supports the handling of `SIP-1559` blocks and transactions starting with the `v5.3.0` release.

### SIP-4844 Shard Blob Transactions Support

This library supports the blob transaction type introduced with [SIP-4844](https://sips.sila.org/SIPS/sip-4844).

The blockchain library now allows for blob transactions to be validated and included in a chain where SIP-4844 activated either by hardfork or standalone SIP.
...
### SIP-7685 Requests Support

This library supports blocks including the [SIP-7685](https://sips.sila.org/SIPS/sip-7685) requests to the consensus layer (like e.g. deposit or withdrawal requests).
