...
import { Common, ConsensusAlgorithm, Hardfork, SilaMainnet } from '@silajs/common'
...
describe('[Blockchain]: Block validation tests', () => {
  ...
  it('SIP1559 base fee tests', async () => {
  ...
  it('should select the right hardfork for uncles at a hardfork transition', async () => {
    /**
     * This test creates a chain around sila-mainnet fork blocks:
     *      berlin         london
     *                |     |-> u <---|
     * @ -> @ -> @ ---|---> @ -> @ -> @
     * |-> u <---|               | -> @
    ...
    const sila-mainnetForkBlock = common.hardforkBlock(Hardfork.London)
    const rootBlock = createBlock(
      {
        header: {
          parentHash: blockchain.genesisBlock.hash(),
          number: sila-mainnetForkBlock! - BigInt(3),
          gasLimit: BigInt(5000),
        },
      },
      { common },
    ...
    const uncleHeader = createBlockHeader(uncleHeaderData, {
      common: new Common({ chain: SilaMainnet, hardfork: Hardfork.Berlin }),
    })
