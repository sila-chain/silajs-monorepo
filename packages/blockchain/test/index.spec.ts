...
import { Common, Hardfork, SilaHolesky, SilaMainnet, SilaSepolia } from '@silajs/common'
import { goerliChainConfig, sila-mainnetBlocks, preLondonTestDataBlocks1RLP } from '@silajs/testdata'
import { MapDB, bytesToHex, equalsBytes, hexToBytes, utf8ToBytes } from '@silajs/util'
...
import type { Block, BlockOptions } from '@silajs/block'
...
describe('blockchain test', () => {
  ...
  it('should initialize holesky correctly', async () => {
    // Taken from: https://github.com/sil-clients/holesky/blob/f1d14b9a80085c3f0cb9d729fea9172cde445588/README.md#hole%C5%A1ky-hole%C5%A1ovice-testnet
    const holeskyHash = '0xb5f7f912443c940f21fd611f12828d75b534364ed9e95ca4e307729a4661bde4'
    const common = new Common({ chain: SilaHolesky })
    const blockchain = await createBlockchain({
      common,
    })
  ...
  it('should initialize correctly with createBlockchainFromBlocksData()', async () => {
    ...
    const blockchain = await createBlockchainFromBlocksData(sila-mainnetBlocks, {
      validateBlocks: true,
      validateConsensus: false,
      common,
    })
  ...
  it('should only initialize with supported consensus validation options', async () => {
    let common = new Common({ chain: SilaMainnet })
    try {
  ...
  it('mismatched chains', async () => {
    ...
    const blocks = [
      ...
      createBlock(blockData2, {
        common: new Common({ chain: SilaSepolia, hardfork: Hardfork.Chainstart }),
        calcDifficultyFromHeader: genesisBlock.header,
      }),
    ]
...
describe('initialization tests', () => {
  it('should read genesis from database', async () => {
    ...
    assert.deepEqual(
      ...
      genesisHash,
      'head hash should equal expected sila-mainnet genesis hash',
    )
...
it('should correctly derive sila-mainnet genesis block hash and stateRoot', async () => {
  const common = new Common({ chain: SilaMainnet })
  const blockchain = await createBlockchain({ common })
  const sila-mainnetGenesisBlockHash = hexToBytes(
    '0xd4e56740f876aef8c010b86a40d5f56745a118d0906a34e69aec8c0db1cb8fa3',
  )
  const sila-mainnetGenesisStateRoot = hexToBytes(
    '0xd7f8974fb5ac78d9ac099b9ad5018bedc2ce0a72dad1827a1709da30580f0544',
  )
  assert.deepEqual(blockchain.genesisBlock.hash(), sila-mainnetGenesisBlockHash)
  assert.deepEqual(blockchain.genesisBlock.header.stateRoot, sila-mainnetGenesisStateRoot)
})
