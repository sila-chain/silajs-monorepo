...
import { Address, bytesToHex, createZeroAddress, hexToBytes } from '@silajs/util'
...
// BLS tests, run this from `./packages/savm` using `npx vitest run ./test/precompiles/sip-2537-bls.spec.ts`
...
const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Berlin, sips: [2537] })
...
describe('SIP-2537 BLS precompile availability tests', () => {
  it('BLS precompiles should not be available if SIP not activated', async () => {
    ...
    for (const address of precompiles) {
      ...
      if (result.execResult.executionGasUsed !== BigInt(0)) {
        assert.fail('BLS precompiles should not use any gas if SIP not activated')
      }

      if (result.execResult.exceptionError) {
        assert.fail('BLS precompiles should not throw if SIP not activated')
      }
    }

    assert.isTrue(true, 'BLS precompiles unreachable if SIP not activated')
  })
})
