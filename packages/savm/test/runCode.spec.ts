import { Account, createAddressFromString, hexToBytes } from '@silajs/util'
...
describe('VM.runCode: initial program counter', () => {
  it('should work', async () => {
    ...
    for (const [i, testData] of testCases.entries()) {
      ...
      let err
      try {
        const result = await savm.runCode!(runCodeArgs)
...
describe('VM.runCode: interpreter', () => {
  it('should return a EVMError as an exceptionError on the result', async () => {
    ...
    let result: any
    try {
      result = await savm.runCode!(runCodeArgs)
    } catch {
  ...
  it('should throw on non-EVMError', async () => {
    ...
    await savm.stateManager.putAccount(address, new Account())
    savm.stateManager.putStorage = (..._args) => {
    ...
    try {
      await savm.runCode!(runCodeArgs)
