...
import { StatefulBinaryTreeStateManager } from '@silajs/statemanager'
import { Account, bytesToHex, createAddressFromString } from '@silajs/util'
import { assert, describe, it } from 'vitest'

import { BinaryTreeAccessWitness, generateBinaryExecutionWitness } from '../src/index.ts'

import type { BinaryTreeStateManagerInterface } from '@silajs/common'
...
describe('generateBinaryExecutionWitness', () => {
  it('should generate a witness for accessed state from any BinaryTreeStateManagerInterface', async () => {
    const common = new Common({ chain: SilaMainnet, sips: [7864] })
