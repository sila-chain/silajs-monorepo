...
describe('Stack', () => {
  ...
  it('stack items should not change if they are DUPed', async () => {
    ...
    */
    await savm.stateManager.putAccount(addr, account)
    await savm.stateManager.putCode(addr, hexToBytes(code))
    await savm.stateManager.putAccount(caller, new Account(BigInt(0), BigInt(0x11)))
    const runCallArgs = {
      caller,
  ...
  it('stack should return the padded value', async () => {
    ...
    for (let pushN = 0x60; pushN <= 0x7f; pushN++) {
      ...
      const resWithoutJumps = await savm.runCall({
        // PUSHx 01
      ...
      const resWithJumps = await savm.runCall({
