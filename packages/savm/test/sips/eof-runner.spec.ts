import { Account, Address, type PrefixedHexString, hexToBytes } from '@silajs/util'
...
describe('EOF: should run a simple contract', async () => {
  it('should run without failing', async () => {
    ...
    await savm.stateManager.putCode(contractAddress, code)
    await savm.stateManager.putAccount(caller, new Account(BigInt(0), BigInt(0x11111111)))
