...
describe('SIP 3860 tests', () => {
  ...
  it('ensure SIP-3860 gas is applied on CREATE calls', async () => {
    // Transaction/Contract data taken from https://github.com/sila-chain/tests/pull/990
    ...
    const contractAccount = await savm.stateManager.getAccount(contractFactory)
    await savm.stateManager.putAccount(contractFactory, contractAccount!)
    ...
    await savm.stateManager.putCode(contractFactory, factoryCode)
    ...
    assert.isTrue(
      ...
      'execution gas used is higher with SIP 3860 active',
    )
  })

  it('ensure SIP-3860 gas is applied on CREATE2 calls', async () => {
    // Transaction/Contract data taken from https://github.com/sila-chain/tests/pull/990
    const commonWith3860 = new Common({
      chain: SilaMainnet,
      hardfork: Hardfork.London,
  ...
  it('CREATE with MAX_INITCODE_SIZE+1, allowUnlimitedContractSize active', async () => {
    ...
    for (const code of ['F0', 'F5']) {
      ...
      const storageActive = await savm.stateManager.getStorage(contractFactory, key0)
