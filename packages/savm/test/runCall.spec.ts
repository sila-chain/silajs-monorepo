import { Common, Hardfork, SilaMainnet, createCommonFromGethGenesis } from '@silajs/common'
import { SIGNER_G, sip4844GethGenesis } from '@silajs/testdata'
import {
  Account,
  Address,
  MAX_UINT64,
...
import { defaultBlock } from '../src/savm.ts'
...
describe('RunCall tests', () => {
  it('Create where FROM account nonce is 0', async () => {
    ...
    const res = await savm.runCall({ to: undefined })
    assert.strictEqual(
  ...
  /*
    This test:
        ...
        Instantiates the SAVM at the Constantinople fork
...
*/
  ...
  it('Constantinople: SIP-1014 CREATE2 creates the right contract address', async () => {
    ...
    */
    ...
    await savm.stateManager.putCode(contractAddress, hexToBytes(code)) // setup the contract code
    await savm.stateManager.putAccount(caller, new Account(BigInt(0), BigInt(0x11111111))) // give the calling account a big balance so we don't run out of funds
    ...
    for (let value = 0; value <= 1000; value += 20) {
      ...
      // run the actual call
      const res = await savm.runCall(runCallArgs)
  ...
  it('Byzantium cannot access Constantinople opcodes', async () => {
    ...
    const evmConstantinople = await createEVM({
      common: new Common({ chain: SilaMainnet, hardfork: Hardfork.Constantinople }),
    })
  ...
  it('Ensure that Istanbul sstoreCleanRefundEIP2200 gas is applied correctly', async () => {
    ...
    */
    ...
    await savm.stateManager.putCode(address, hexToBytes(code))
    await savm.stateManager.putStorage(
      address,
      new Uint8Array(32),
    ...
    const result = await savm.runCall(runCallArgs)
  ...
  it('ensure correct gas for calling non-existent accounts in homestead', async () => {
    ...
    const address = new Address(hexToBytes('0x00000000000000000000000000000000000000ff'))
    // setup the vm
    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Homestead })
    const savm = await createEVM({ common })
  ...
  it('ensure selfdestruct pays for creating new accounts', async () => {
    ...
    const address = new Address(hexToBytes('0x00000000000000000000000000000000000000ff'))
    // setup the vm
    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.TangerineWhistle })
    const savm = await createEVM({ common })
  ...
  it('ensure that sstores pay for the right gas costs pre-byzantium', async () => {
    ...
    await savm.stateManager.putAccount(caller, new Account())
    await savm.stateManager.putCode(address, hexToBytes(code))

    const account = await savm.stateManager.getAccount(caller)
    account!.balance = BigInt(100)
    await savm.stateManager.putAccount(caller, account!)
  ...
  */
  ...
  it('Ensure that contracts cannot exceed nonce of MAX_UINT64 when creating new contracts (SIP-2681)', async () => {
    // setup the accounts for this test
    const caller = new Address(hexToBytes('0x00000000000000000000000000000000000000ee')) // caller address
    const address = new Address(hexToBytes('0x00000000000000000000000000000000000000ff'))
    ...
    */
    ...
    const account = await savm.stateManager.getAccount(address)
    account!.nonce = MAX_UINT64 - BigInt(1)
    await savm.stateManager.putAccount(address, account!)
    ...
    let storage = await savm.stateManager.getStorage(address, slot)
    ...
    storage = await savm.stateManager.getStorage(address, slot)
    assert.deepEqual(
      storage,
      emptyBytes,
  ...
  it('Ensure that IDENTITY precompile copies the memory', async () => {
    // This test replays the Geth chain split; https://silascan.io/tx/0x1cb6fb36633d270edefc04d048145b4298e67b8aa82a9e5ec4aa1435dd770ce4
    // Exploit post-mortem: https://github.com/sila-chain/go-sila/blob/master/docs/postmortems/2021-08-22-split-postmortem.md
    // Permalink: https://github.com/sila-chain/go-sila/blob/90987db7334c1d10eb866ca550efedb66dea8a20/docs/postmortems/2021-08-22-split-postmortem.md
    // setup the accounts for this test
    ...
    const deployedCode = await savm.stateManager.getCode(result.createdAddress!)
  ...
  it('runCall() -> skipBalance behavior', async () => {
    ...
    await savm.stateManager.putCode(contractAddress, contractCode)
    ...
    for (const balance of [undefined, BigInt(5)]) {
      await savm.stateManager.modifyAccountFields(sender, { nonce: BigInt(0), balance })
      const res = await savm.runCall(runCallArgs)
      assert.isTrue(true, 'runCall should not throw with no balance and skipBalance')
      const senderBalance = (await savm.stateManager.getAccount(sender))!.balance
      assert.strictEqual(
        senderBalance,
        balance ?? BigInt(0),
    ...
    const res2 = await savm.runCall({ ...runCallArgs, skipBalance: false })
    assert.include(
  ...
  it('runCall() => use BLOBHASH opcode from SIP 4844', async () => {
    // setup the savm
    ...
    const res2 = await savm.runCall(runCall2Args)
    assert.strictEqual(
  ...
  it('runCall() => use BLOBBASEFEE opcode from SIP 7516', async () => {
    // setup the savm
    const common = createCommonFromGethGenesis(sip4844GethGenesis, {
      chain: 'custom',
      hardfork: Hardfork.SilaCancun,
    ...
    assert.strictEqual(
      savm.getActiveOpcodes().get(BLOBBASEFEE_OPCODE)!.name,
      'BLOBBASEFEE',
  ...
  it('step event: ensure SAVM memory and not internal memory gets reported', async () => {
    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Berlin })
    const savm = await createEVM({ common })
  ...
  it('ensure code deposit errors are logged correctly (>= Homestead)', async () => {
    ...
    const res2 = await savm.runCall(runCallArgs2)
  ...
  it('ensure call and callcode handle gas stipend correctly', async () => {
    // See: https://github.com/sila-chain/silajs-monorepo/issues/3194
    ...
    for (const [opcode, gas, expectedOutput] of [
      ...
      await savm.stateManager.putAccount(callCodeAddress, new Account())
      await savm.stateManager.putCode(callCodeAddress, callCode)

      await savm.stateManager.putAccount(callerAddress, new Account(undefined, BigInt(1)))
      await savm.stateManager.putCode(callerAddress, callerCode)
      ...
      const callResult = bytesToHex(
        await savm.stateManager.getStorage(callerAddress, new Uint8Array(32)),
      )
