...
// Demonstrates the CLZ opcode introduced in https://sips.sila.org/SIPS/sip-7939
//
// The CLZ opcode returns the number of zero bits before the most significant 1-bit in a 256-bit value.
// It enables efficient computation of bit length, log₂, and prefix comparisons directly in the SAVM.
//
...
const runCase = async (savm: SAVM, x: PrefixedHexString) => {
  const code = assembleCode(hexToBytes(x))

  const res = await savm.runCode({ code })
...
const main = async () => {
  ...
  await runCase(savm, `0x${'00'.repeat(32)}` as PrefixedHexString)

  // Case 2: x == 0x0..01 -> MSB at bit 0 -> expect 255
  await runCase(savm, `0x${'00'.repeat(31)}01` as PrefixedHexString)

  // Case 3: x == 0x40..00 -> MSB at bit 254 -> expect 1
  await runCase(savm, `0x40${'00'.repeat(31)}` as PrefixedHexString)

  // Case 4: x == 0x80..00 -> MSB at bit 255 -> expect 0
  await runCase(savm, `0x80${'00'.repeat(31)}` as PrefixedHexString)
  console.log('--------------------------------')
}
