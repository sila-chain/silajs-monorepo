...
import { type PrefixedHexString, bytesToHex, randomBytes, utf8ToBytes } from '@silajs/util'
...
const main = async () => {
  ...
  // Demonstrates signing SHA-256("silajs-savm-p256verify-example") with a private key
  //
  // Input format (see SIP-7951):
  // - 32 bytes: msgHash
  // - 32 bytes: r
  // - 32 bytes: s
  ...
  const message = 'silajs-savm-p256verify-example'
  ...
  await runPrecompile('P256VERIFY', '0x100', data, Hardfork.SilaOsaka)
  ...
  await runPrecompile('P256VERIFY', '0x100', badData, Hardfork.SilaOsaka)
}
