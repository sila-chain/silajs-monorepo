[**@silajs/savm**](../README.md)

***

[@silajs/savm](../README.md) / EVMMockBlockchain

# Class: EVMMockBlockchain

Defined in: [types.ts:560](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/savm/src/types.ts#L560)

## Implements

- [`EVMMockBlockchainInterface`](../interfaces/EVMMockBlockchainInterface.md)

## Constructors

### Constructor

> **new EVMMockBlockchain**(): `EVMMockBlockchain`

#### Returns

`EVMMockBlockchain`

## Methods

### getBlock()

> **getBlock**(): `Promise`\<\{ `hash`: `Uint8Array`\<`ArrayBuffer`\>; \}\>

Defined in: [types.ts:561](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/savm/src/types.ts#L561)

#### Returns

`Promise`\<\{ `hash`: `Uint8Array`\<`ArrayBuffer`\>; \}\>

#### Implementation of

[`EVMMockBlockchainInterface`](../interfaces/EVMMockBlockchainInterface.md).[`getBlock`](../interfaces/EVMMockBlockchainInterface.md#getblock)

***

### putBlock()

> **putBlock**(): `Promise`\<`void`\>

Defined in: [types.ts:568](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/savm/src/types.ts#L568)

#### Returns

`Promise`\<`void`\>

#### Implementation of

[`EVMMockBlockchainInterface`](../interfaces/EVMMockBlockchainInterface.md).[`putBlock`](../interfaces/EVMMockBlockchainInterface.md#putblock)

***

### shallowCopy()

> **shallowCopy**(): `EVMMockBlockchain`

Defined in: [types.ts:569](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/savm/src/types.ts#L569)

#### Returns

`EVMMockBlockchain`

#### Implementation of

[`EVMMockBlockchainInterface`](../interfaces/EVMMockBlockchainInterface.md).[`shallowCopy`](../interfaces/EVMMockBlockchainInterface.md#shallowcopy)
