...
import type { BlockLevelAccessList, PrefixedHexString } from '@silajs/util'
import {
  Account,
  Address,
  BIGINT_0,
...
import { createEIP7708TransferLog } from './sip7708.ts'
...
import type { Common, StateManagerInterface } from '@silajs/common'
...
const debug = debugDefault('savm:savm')
const debugGas = debugDefault('savm:gas')
const debugPrecompiles = debugDefault('savm:precompiles')
...
/**
 * Creates an ExecResult for code-deposit out-of-gas errors (SIP-3541).
 ...
 * - {@link createEVM}
 */
export class SAVM implements EVMInterface {
   ...
   * @remarks Experimental (SilaAmsterdam): may change on patch releases.
   */
  public sip7928CallPostTargetOog = false
  /**
   * SIP-8037 per-frame state-gas snapshots. Pushed on each message-level
  ...
  /**
   * SAVM is run in DEBUG mode (default: false)
  ...
  constructor(opts: EVMOpts) {
      ...
      for (const m of mandatory) {
        if (!(m in this.stateManager)) {
          throw SilaJSErrorWithoutCode(
            `State manager used must implement ${m} if Binary Trees (SIP-7864) is activated`,
          )
        }
      }
    }
    ...
    // Supported SIPs (sorted by SIP number; keep in sync with README / types.ts)
    const supportedEIPs = [
    ...
    for (const sip of this.common.sips()) {
      if (!supportedEIPs.includes(sip)) {
        throw SilaJSErrorWithoutCode(`SIP-${sip} is not supported by the SAVM`)
      }
    }

    if (!SAVM.supportedHardforks.includes(this.common.hardfork() as Hardfork)) {
      throw SilaJSErrorWithoutCode(
  ...
  protected async _executeCall(message: MessageWithTo): Promise<EVMResult> {
    ...
    if (this.common.isActivatedEIP(7864)) {
      if (message.accessWitness === undefined) {
        throw SilaJSErrorWithoutCode('accessWitness is required for SIP-7864')
      }
    ...
    // SIP-7928: Add codeAddress to BAL for DELEGATECALL/CALLCODE
    ...
    // SIP-7708: Create SIL transfer log for non-zero value transfers to a different account.
    ...
    if (
      ...
      sip7708Log = createEIP7708TransferLog(message.caller, message.to, message.value)
      if (this.DEBUG) {
        debug(
          `SIP-7708: Created SIL transfer log from ${message.caller} to ${message.to} value=${message.value}`,
        )
      }
    }

    if (exit) {
      // Even on early exit, we may need to return the SIP-7708 log if value
      // was transferred. SIP-8037: still charge state-gas if this empty-code
      ...
      const earlyResult: ExecResult = {
        ...
        returnValue: new Uint8Array(0),
        logs: sip7708Log ? [sip7708Log] : undefined,
      }
      // SIP-8037: new-account state-gas for inner CALLs is now pre-charged at
    ...
    if (message.isCompiled) {
      ...
      if (sip7708Log !== undefined) {
        result.logs = result.logs !== undefined ? [sip7708Log, ...result.logs] : [sip7708Log]
      }
      ...
      result = await this.runInterpreter(
        ...
        { initialLogs: sip7708Log ? [sip7708Log] : undefined },
      )
    }
    ...
    // SIP-8037: the new-account state-gas charge is now pre-charged at the
  ...
  protected async _executeCreate(message: Message): Promise<EVMResult> {
    ...
    if (
      ...
      // See SIP 7610 and the discussion `https://sila-magicians.org/t/sip-7610-revert-creation-in-case-of-non-empty-storage`
    ...
    // SIP-161 on account creation and CREATE execution
    ...
    // SIP-7708: Create SIL transfer log for contract creation with value
    let sip7708CreateLog: Log | undefined
    if (
      this.common.isActivatedEIP(7708) &&
      message.value > BIGINT_0 &&
      ...
      sip7708CreateLog = createEIP7708TransferLog(message.caller, message.to, message.value)
      if (this.DEBUG) {
        debug(
          `SIP-7708: Created SIL transfer log for CREATE from ${message.caller} to ${message.to} value=${message.value}`,
        )
      }
    }
    ...
    if (exit) {
      ...
      return {
        ...
        execResult: {
          ...
          returnValue: new Uint8Array(0),
          logs: sip7708CreateLog ? [sip7708CreateLog] : undefined,
        },
      }
    }
    ...
    let result = await this.runInterpreter({ ...message, gasLimit, isCreate: true } as Message, {
      initialLogs: sip7708CreateLog ? [sip7708CreateLog] : undefined,
    })
    result.executionGasUsed += message.gasLimit - gasLimit
    ...
    // SIP-8037 state-gas charge for the new account + code deposit. Computed
    ...
    if (!result.exceptionError && !this.common.isActivatedEIP(6800)) {
      if (this.common.isActivatedEIP(8037)) {
        ...
        // remaining gas in this CREATE frame. Don't mutate savm.* yet — we
    ...
    // Check for SpuriousDragon SIP-170 code size limit
    ...
    // SIP-8037: commit state-gas accounting now that the success / failure
    ...
    if (this.common.isActivatedEIP(8037) && createSucceeded) {
      ...
      // tx (per SIP-8037 SELFDESTRUCT deferred-refund rules).
      //
  ...
  async runCall(opts: EVMRunCallOpts): Promise<EVMResult> {
    ...
    if (this.common.isActivatedEIP(8037)) {
      // SIP-8037: snapshot reservoir + cumulative state-gas used at frame
    ...
    if (
      ...
      if (this.common.isActivatedEIP(8037)) {
        // SIP-8037 §"Gas accounting for halts and reverts": restore the
        ...
        // SIP-8037: on ANY creation-frame failure (revert / halt / OOG /
      ...
      if (this.common.isActivatedEIP(8037)) {
        // SIP-8037: drop the frame's snapshot; current reservoir / used
  ...
  protected runPrecompile(
    ...
    if (typeof code !== 'function') {
      throw SilaJSErrorWithoutCode('Invalid precompile')
    }
    ...
    if (!message.code) {
      ...
      if (precompile) {
        ...
        if (
          ...
          // SIP-7928: Track delegation target access in BAL
          if (this.common.isActivatedEIP(7928)) {
  ...
  protected async _reduceSenderBalance(account: Account, message: Message): Promise<void> {
    ...
    // SIP-7928: Record the sender's reduced balance in BAL
