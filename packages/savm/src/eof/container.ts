...
/*
  ...
  EOF Containers are described in SIP-3540.
...
class EOFHeader {
  ...
  constructor(input: Uint8Array) {
    if (input.length > MAX_HEADER_SIZE) {
      throw SilaJSErrorWithoutCode('err: container size more than maximum valid size')
    }
    ...
    if (input.length < 15) {
      throw SilaJSErrorWithoutCode('err: container size less than minimum valid size')
    }
    ...
    // Note: in SAVM, if one would set the Program Counter to this byte, it would start executing the bytecode of the first code section
  ...
  // Setting the Program Counter in the SAVM to a number of this array would start executing the bytecode of the indexed section
  ...
  getSectionFromProgramCounter(programCounter: number) {
    ...
    if (programCounter < 0 || programCounter > lastEnd) {
      ...
      throw SilaJSErrorWithoutCode('program counter out of bounds')
    }
    ...
    throw SilaJSErrorWithoutCode(`Invalid program counter value: ${programCounter}`)
  }
}
...
class EOFBody {
  ...
  // and these are used for the CALLDATA in the SAVM when trying to create a contract via a transaction, and the deployment code is an EOF container
  ...
  constructor(
    ...
    // For details, see "Data section lifecycle" of SIP 7620
    ...
    if (eofMode !== EOFContainerMode.Initmode && !dataSectionAllowedSmaller) {
      ...
      if (eofMode === EOFContainerMode.Default) {
        ...
        // Tx init mode: the remaining bytes (if any) are used as CALLDATA in the SAVM, in case of a Tx init
 ...
 * @param savm SAVM, to read opcodes from
...
export function validateEOF(
  ...
  const containerMap = verifyCode(container, savm, containerMode)
  ...
  for (let i = 0; i < container.body.containerSections.length; i++) {
    ...
    validateEOF(subContainer, savm, mode)
  }
  return container
}
