 ...
 * This is a compilation of all the extra validation rules introduced by the various SIPs
 * In particular, the stack validation SIP https://sips.sila.org/SIPS/sip-5450 is a big part here
 * @param container EOFContainer to verify
 * @param savm The SAVM to run in (pulls opcodes from here)
...
export function verifyCode(
  ...
  return validateOpcodes(container, savm, mode)
}
...
function validateOpcodes(
  ...
  const opcodes = savm.getActiveOpcodes()
  ...
  for (const code of container.body.codeSections) {
    ...
    // it rather exits the current SAVM call frame
    ...
    // Implement the SIP 5450 stack validation algorithm
    ...
    // For all validation rules per opcode, find the corresponding SIP, the rules are there
