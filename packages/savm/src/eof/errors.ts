  ...
  switch (type) {
    case EOFErrorMessage.OUT_OF_BOUNDS: {
      ...
      if (pos === 0 || pos === 2 || pos === 3 || pos === 6) {
        throw SilaJSErrorWithoutCode(args[1])
      }
      throw SilaJSErrorWithoutCode(EOFErrorMessage.OUT_OF_BOUNDS + ` `)
    }
    case EOFErrorMessage.VERIFY_BYTES: {
      const pos = args[0]
      if (pos === 0 || pos === 2 || pos === 3 || pos === 6) {
    ...
    case EOFErrorMessage.TYPE_SIZE: {
      throw SilaJSErrorWithoutCode(EOFErrorMessage.TYPE_SIZE + args[0])
    }
    ...
    case EOFErrorMessage.INVALID_TYPE_SIZE: {
      throw SilaJSErrorWithoutCode(EOFErrorMessage.INVALID_TYPE_SIZE)
    }
    case EOFErrorMessage.INVALID_CODE_SIZE: {
      throw SilaJSErrorWithoutCode(EOFErrorMessage.INVALID_CODE_SIZE + args[0])
    }
    case EOFErrorMessage.INPUTS: {
      throw SilaJSErrorWithoutCode(`${EOFErrorMessage.INPUTS} - typeSection ${args[0]}`)
    }
    case EOFErrorMessage.OUTPUTS: {
      throw SilaJSErrorWithoutCode(`${EOFErrorMessage.OUTPUTS} - typeSection ${args[0]}`)
    }
    case EOFErrorMessage.CODE0_INPUTS: {
      throw SilaJSErrorWithoutCode(`first code section should have 0 inputs`)
    }
    case EOFErrorMessage.CODE0_OUTPUTS: {
      throw SilaJSErrorWithoutCode(`first code section should have 0 outputs`)
    }
    case EOFErrorMessage.MAX_INPUTS: {
      throw SilaJSErrorWithoutCode(
    ...
    case EOFErrorMessage.CODE_SECTION: {
      throw SilaJSErrorWithoutCode(`expected code: codeSection ${args[0]}: `)
    }
    case EOFErrorMessage.DATA_SECTION: {
      throw SilaJSErrorWithoutCode(EOFErrorMessage.DATA_SECTION)
    }
    case EOFErrorMessage.MAX_STACK_HEIGHT: {
      throw SilaJSErrorWithoutCode(
    ...
    case EOFErrorMessage.DANGLING_BYTES: {
      throw SilaJSErrorWithoutCode(EOFErrorMessage.DANGLING_BYTES)
    }
    default: {
      throw SilaJSErrorWithoutCode(type)
    }
  }
}
