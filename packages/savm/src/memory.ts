import { SilaJSErrorWithoutCode, concatBytes } from '@silajs/util'
...
export class Memory {
  ...
  write(offset: number, size: number, value: Uint8Array) {
    ...
    if (value.length !== size) throw SilaJSErrorWithoutCode('Invalid value size')
    if (offset + size > this._store.length)
      throw SilaJSErrorWithoutCode('Value exceeds memory capacity')
