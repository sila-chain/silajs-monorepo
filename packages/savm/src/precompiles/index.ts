...
import { type Address, type PrefixedHexString, bytesToUnprefixedHex } from '@silajs/util'
...
interface PrecompileAvailabilityCheckTypeEIP {
  type: typeof PrecompileAvailabilityCheck.SIP
  param: number
}
...
const precompileEntries: PrecompileEntry[] = [
  ...
  },
  {
    address: BYTES_19 + '0a',
    check: {
      type: PrecompileAvailabilityCheck.SIP,
      param: 4844,
    },
...
/**
 * Specifies a precompile to add to (or override in) the SAVM.
 * The address can be an `Address` instance or a `0x`-prefixed hex string.
 */
type AddPrecompile = {
  address: Address | PrefixedHexString
...
function getActivePrecompiles(
  ...
  for (const entry of precompileEntries) {
    ...
    if (
      ...
      (entry.check.type === PrecompileAvailabilityCheck.SIP &&
