import { ConsensusAlgorithm } from '@silajs/common'
import {
  Account,
  BIGINT_0,
  BIGINT_1,
...
import { SIP7708_BURN_TOPIC, SIP7708_SYSTEM_ADDRESS, SIP7708_TRANSFER_TOPIC } from './sip7708.ts'
import { activeCostPerStateByte } from './sip8037.ts'
...
import {
  type Sip7928PostTargetCallOog,
  consumeEip7928PostTargetCallOog,
} from './opcodes/SIP7928.ts'
import { isEIP8024PairImmediateValid, isEIP8024SingleImmediateValid } from './opcodes/SIP8024.ts'
...
const debugGas = debugDefault('savm:gas')

export interface InterpreterOpts {
  pc?: number
  /** Logs to prepend to the result (e.g. SIP-7708 SIL transfer log from message-level value transfer) */
  initialLogs?: Log[]
}

/**
 * Immediate (unprocessed) result of running an SAVM bytecode.
 */
export interface RunResult {
  ...
  /**
   * A map which tracks which addresses were created (used in SIP 6780)
   */
...
export interface Env {
  ...
  /** Logs to prepend (e.g. SIP-7708 SIL transfer log from message-level value transfer) */
  initialLogs?: Log[]
}
...
export interface RunState {
  ...
  /** SIP-7928 CALL post-target OOG: cleared in runStep after output is zeroed. */
  sip7928PostTargetCallOog?: Sip7928PostTargetCallOog
  /** SIP-7928 CREATE post-target OOG: signals the CREATE/CREATE2 handler to push 0 without executing. */
  sip7928PostTargetCreateOog?: boolean
...
/**
 * Parses and executes SAVM bytecode.
 */
export class Interpreter {
  ...
  async run(code: Uint8Array, opts: InterpreterOpts = {}): Promise<InterpreterResult> {
    if (!this.common.isActivatedEIP(3540) || code[0] !== FORMAT) {
      // SIP-3540 isn't active and first byte is not 0xEF - treat as legacy bytecode
    ...
    if (pc !== 0 && (pc < 0 || pc >= this._runState.code.length)) {
      throw SilaJSErrorWithoutCode('Internal error: program counter not in range')
    }
    ...
    while (this._runState.programCounter < this._runState.code.length) {
      ...
      try {
        ...
        // Revert access witness changes if we revert - per SIP-4762
  ...
  async runStep(opcodeObj?: OpcodeMapEntry): Promise<void> {
    ...
    // Cache pre-gas memory size if doing tracing (SIP-7756)
  ...
  /**
   * Get info for an opcode from SAVM's list of opcodes.
   */
  ...
  async _runStepHook(dynamicFee: bigint, gasLeft: bigint, memorySize: bigint): Promise<void> {
    ...
    if (this._evm.DEBUG) {
      ...
      if (!(name in this.opDebuggers)) {
        this.opDebuggers[name] = debugDefault(`savm:ops:${name}`)
      }
     ...
     * @property {Uint8Array} memory the memory of the SAVM as a `Uint8Array`
  ...
  /**
   * SIP-8037: Charge state-gas. Draws from the per-tx state-gas reservoir
  ...
  chargeStateGas(amount: bigint, context?: string): void {
    ...
    const savm = this._evm
    let remaining = amount
    if (savm.stateGasReservoir > BIGINT_0) {
      const fromReservoir = remaining < savm.stateGasReservoir ? remaining : savm.stateGasReservoir
      savm.stateGasReservoir -= fromReservoir
    ...
    savm.executionStateGasUsed += amount
    if (savm.DEBUG) {
      debugGas(
        `${context !== undefined ? context + ': ' : ''}charged ${amount} state gas (reservoir=${savm.stateGasReservoir}, executionStateGasUsed=${savm.executionStateGasUsed}, gasLeft=${this._runState.gasLeft})`,
      )
    }
  }

  /**
   * SIP-8037: Refill the state-gas reservoir, e.g. on revert / exceptional
  ...
  refillStateGasReservoir(amount: bigint, context?: string): void {
    if (amount === BIGINT_0) return
    const savm = this._evm
    savm.stateGasReservoir += amount
    savm.executionStateGasUsed -= amount
    if (savm.DEBUG) {
      debugGas(
        `${context !== undefined ? context + ': ' : ''}refilled ${amount} state gas (reservoir=${savm.stateGasReservoir}, executionStateGasUsed=${savm.executionStateGasUsed})`,
      )
    }
  }
  ...
  async getExternalBalance(address: Address): Promise<bigint> {
    // Track address access for SIP-7928 BAL
  ...
  async storageStore(key: Uint8Array, value: Uint8Array): Promise<void> {
    // SIP-7928: Get the original (pre-transaction) value BEFORE storing
    ...
    if (!account) {
      throw SilaJSErrorWithoutCode('could not read account while persisting memory')
    }
  ...
  /**
   * Returns the block's slot number (SIP-7843).
   */
  getBlockSlotNumber(): bigint {
    if (this._env.block.header.slotNumber === undefined) {
      throw SilaJSErrorWithoutCode('slotNumber is not available on this block')
    }
    return this._env.block.header.slotNumber
  }

  /**
   * Returns the Base Fee of the block as proposed in [SIP-3198](https://sips.sila.org/SIPS/sip-3198)
   */
  getBlockBaseFee(): bigint {
    const baseFee = this._env.block.header.baseFeePerGas
    if (baseFee === undefined) {
      // Sanity check
      throw SilaJSErrorWithoutCode('Block has no Base Fee')
    }
    return baseFee
  }

  /**
   * Returns the Blob Base Fee of the block as proposed in [SIP-7516](https://sips.sila.org/SIPS/sip-7516)
   */
  getBlobBaseFee(): bigint {
    const blobBaseFee = this._env.block.header.getBlobGasPrice()
    if (blobBaseFee === undefined) {
      // Sanity check
      throw SilaJSErrorWithoutCode('Block has no Blob Base Fee')
    }
    return blobBaseFee
  }
   ...
   * CHAINID opcode proposed in [SIP-1344](https://sips.sila.org/SIPS/sip-1344).
   */
  getChainId(): bigint {
  ...
  async _baseCall(msg: Message): Promise<bigint> {
    ...
    // Check if account has enough sila and max depth not exceeded
    if (
    ...
    if (!results.execResult.exceptionError) {
      ...
      const account = await this._stateManager.getAccount(this._env.address)
      if (!account) {
        throw SilaJSErrorWithoutCode('could not read contract account')
      }
      this._env.contract = account
  ...
  async create(
    ...
    // SIP-8037: helper to refund the pre-charged NEW_ACCOUNT state-gas
    // when the CREATE short-circuits BEFORE a child frame is spawned
    // (depth limit, insufficient balance, SIP-2681 nonce overflow,
    // SIP-3860 oversized initcode). The pre-charge happened in
  ...
  async _selfDestruct(toAddress: Address): Promise<void> {
    ...
    // SIP-7708: Emit SIL transfer log for SELFDESTRUCT with value to a different account
    if (this.common.isActivatedEIP(7708) && contractBalance > BIGINT_0 && !toSelf) {
      ...
      const transferLog: Log = [
        SIP7708_SYSTEM_ADDRESS,
        [SIP7708_TRANSFER_TOPIC, fromTopic, toTopic],
        data,
      ]
    ...
    // SIP-8037: SELFDESTRUCT that transfers value to a non-existent beneficiary
    ...
    if (!doModify) {
      ...
      // (i.e. burn the SIL in current account)
      doModify = this._env.createdAddresses!.has(this._env.address.toString())
      // If contract is not being created in this tx...
      if (!doModify) {
        // Check if SIL being sent to another account (thus set balance to 0)
        doModify = !toSelf
      }
    }

    // SIP-7708: Emit a Burn log (LOG2) for SELFDESTRUCT to self only when the balance
    // is actually zeroed (doModify=true, i.e. same-tx contract creation). Pre-existing
    // contracts where SIP-6780 prevents the burn should not emit a log.
    if (this.common.isActivatedEIP(7708) && contractBalance > BIGINT_0 && toSelf && doModify) {
      ...
      const data = setLengthLeft(bigIntToBytes(contractBalance), 32)
      const burnLog: Log = [SIP7708_SYSTEM_ADDRESS, [SIP7708_BURN_TOPIC, contractTopic], data]
