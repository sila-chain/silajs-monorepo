...
const hardforkOpcodes: { hardfork: Hardfork; opcodes: OpcodeEntry }[] = [
  {
    ...
    opcodes: {
      0xf4: asyncAndDynamicGasOp('DELEGATECALL'), // SIP-7
    },
  },
  {
  ...
  },
  {
    ...
    opcodes: {
      0xfd: dynamicGasOp('REVERT'), // SIP-140
      0xfa: asyncAndDynamicGasOp('STATICCALL'), // SIP-214
      0x3d: asyncOp('RETURNDATASIZE'), // SIP-211
      0x3e: asyncAndDynamicGasOp('RETURNDATACOPY'), // SIP-211
    },
  },
  {
    hardfork: Hardfork.Constantinople,
    opcodes: {
      0x1b: defaultOp('SHL'), // SIP-145
      0x1c: defaultOp('SHR'), // SIP-145
      0x1d: defaultOp('SAR'), // SIP-145
      0x3f: asyncAndDynamicGasOp('EXTCODEHASH'), // SIP-1052
      0xf5: asyncAndDynamicGasOp('CREATE2'), // SIP-1014
    },
  },
  {
    hardfork: Hardfork.Istanbul,
    opcodes: {
      0x46: defaultOp('CHAINID'), // SIP-1344
      0x47: defaultOp('SELFBALANCE'), // SIP-1884
    },
  },
  {
    hardfork: Hardfork.SilaParis,
    opcodes: {
      0x44: asyncOp('PREVRANDAO'), // SIP-4399
    },
  },
]

const eipOpcodes: { sip: number; opcodes: OpcodeEntry }[] = [
  {
    sip: 8024,
    opcodes: {
      0xe6: defaultOp('DUPN'),
 ...
 *
 * @param common {Common} Silajs Common metadata object.
 * @param customOpcodes List with custom opcodes (see SAVM `customOpcodes` option description).
...
export function getOpcodesForHF(common: Common, customOpcodes?: CustomOpcode[]): OpcodeContext {
  ...
  for (const eipOps of eipOpcodes) {
    if (common.isActivatedEIP(eipOps.sip)) {
  ...
  for (const key in opcodeBuilder) {
    ...
    if (baseFee === undefined) {
      throw SilaJSErrorWithoutCode(`base fee not defined for: ${opcodeBuilder[key].name}`)
    }
