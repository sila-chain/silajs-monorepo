...
/**
 * Adjusts gas usage and refunds of SStore ops per SIP-1283 (Constantinople)
 *
  ...
  if (equalsBytes(originalStorage, value)) {
    ...
    if (originalStorage.length === 0) {
      ...
      runState.interpreter.refundGas(
        ...
        'SIP-1283 -> netSstoreResetClearRefund',
      )
    } else {
      // Otherwise, add 4800 gas to refund counter.
      runState.interpreter.refundGas(
        common.param('netSstoreResetRefundGas'),
        'SIP-1283 -> netSstoreResetRefund',
      )
    }
  }
