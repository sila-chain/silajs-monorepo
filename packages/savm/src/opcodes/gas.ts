...
import { updateSstoreGasEIP1283 } from './SIP1283.ts'
import { updateSstoreGasEIP2200 } from './SIP2200.ts'
import {
...
import { callFamilyGas, create7928Gas } from './SIP7928.ts'
import {
...
/**
 * Warms the delegation address for SIP-7702 (call after verifying sufficient gas).
 */
function sip7702WarmAddress(runState: RunState, delegationAddress: Uint8Array): void {
  ...
  new Map<number, AsyncDynamicGasHandler>([
    ...
    [
      ...
      async function (runState, gas, common): Promise<bigint> {
        ...
        if (returnDataOffset + dataLength > runState.interpreter.getReturnDataSize()) {
          // For an EOF contract, the behavior is changed (see SIP 7069)
    ...
    [
      ...
      async function (runState, gas, common): Promise<bigint> {
        ...
        // but ONLY if the SIP-2200 sentry check passes (per SIP-7928).
        ...
        // If we reach here, the SIP-2200 sentry check passed (didn't trap).
        // Per SIP-7928, now track the storage read for BAL. If the SSTORE
        ...
        if (common.isActivatedEIP(2929)) {
          // We have to do this after the Istanbul (SIP2200) checks.
    ...
    [
      ...
      async function (runState, gas, common): Promise<bigint> {
        ...
        // --test=tests/prague/sip7692_eof_v1/sip7620_eof_create/test_eofcreate.py::test_eofcreate_then_call[fork_SilaCancunEIP7692-blockchain_test]
    ...
    [
      ...
      async function (runState, gas, common): Promise<bigint> {
        ...
        if (common.isActivatedEIP(7928)) {
          // SIP-7928/SIP-8037: the static-context check is deferred into
          // create7928Gas so it fires AFTER the new-account state-gas
          // pre-charge (see SIP7928.ts).
    ...
    [
      ...
      async function (runState, gas, common): Promise<bigint> {
        ...
        if (common.isActivatedEIP(7928)) {
          // SIP-7928/SIP-8037: static-context check deferred into
    ...
    [
      ...
      async function (runState, gas, common): Promise<bigint> {
        ...
        if (common.gteHardfork(Hardfork.SpuriousDragon)) {
          ...
          // SIP-150 (Tangerine Whistle) gas semantics
          const exists =
        ...
        // SIP-2929/7928: Get cold access cost first (no side effects)
        ...
        // SIP-7928: Check if we have enough gas for the cold access (checkpoint 1)
        ...
        if (common.isActivatedEIP(7928)) {
          ...
          // per SIP-7928, if we pass the cold access check but fail at new account
        ...
        // Now commit the address warming (SIP-2929)
        if (common.isActivatedEIP(2929)) {
