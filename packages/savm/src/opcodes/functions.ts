...
import { decodeEIP8024PairImmediate, decodeEIP8024SingleImmediate } from './SIP8024.ts'
import {
...
export const handlers: Map<number, OpHandler> = new Map([
  ...
  [
    ...
    async function (runState) {
      const addressBigInt = runState.stack.pop()
      const address = createAddressFromStackBigInt(addressBigInt)
      // SIP-7928: Track address access in BAL
  ...
  // 0x44: DIFFICULTY (SIP-4399: supplanted as PREVRANDAO)
  [
    0x44,
    function (runState, common) {
  ...
  [
    ...
    async function (runState) {
      ...
      // SIP-8037: state-gas accounting at the end of SSTORE (per the spec
      ...
      if (common.isActivatedEIP(8037) && originalIsZero) {
        ...
        if (currentIsZero && !newIsZero) {
          ...
          const savm = runState.interpreter._evm
          const prior = savm.createdAccountStateGas.get(addrKey) ?? BIGINT_0
          savm.createdAccountStateGas.set(addrKey, prior + amount)
          ...
          savm.createdAccountStateGas.set(addrKey, next)
        }
      }
    },
  ],
  ...
  [
    ...
    async function (runState, common) {
      ...
      if (runState.sip7928PostTargetCreateOog === true) {
        runState.sip7928PostTargetCreateOog = false
        runState.stack.push(BIGINT_0)
        return
      }
