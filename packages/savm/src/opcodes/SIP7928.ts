...
import { BIGINT_0, BIGINT_31, BIGINT_32 } from '@silajs/util'
...
export type Sip7928PostTargetCallOog = {
  outOffset: bigint
  outLength: bigint
}

/** True when `regularGas + stateGas` fits in `gas_left + state_gas_reservoir` (SIP-8037). */
...
export function sip7928TrapPreTarget(runState: RunState, common: Common, gas: bigint): void {
...
export function sip7928SchedulePostTargetCallOog(
  runState: RunState,
  outOffset: bigint,
  outLength: bigint,
): bigint {
  runState.sip7928PostTargetCallOog = { outOffset, outLength }
  runState.interpreter._evm.sip7928CallPostTargetOog = true
  return runState.interpreter.getGasLeft()
}

/** Post-target CREATE OOG: charge accrued gas only; opcode pushes 0. */
export function sip7928PostTargetCreateOog(
  runState: RunState,
  common: Common,
  gas: bigint,
): bigint {
  ...
  runState.sip7928PostTargetCreateOog = true
  return gas
}
...
function postTargetCallOogOrTrap(
  ...
  return sip7928SchedulePostTargetCallOog(runState, outOffset, outLength)
}

export function consumeEip7928PostTargetCallOog(runState: RunState): void {
  const pending = runState.sip7928PostTargetCallOog
  if (pending === undefined) {
    return
  }
  runState.sip7928PostTargetCallOog = undefined
...
function finalizeCallMessageGas(
  ...
  // SIP-8037: pre-charge the new-account state-gas BEFORE the inner call
  ...
  // The post-frame charge in savm.ts is skipped under SIP-8037 to avoid
...
export type CallFamilyGasOpts = {
  ...
  sip7702GetAccessCost: (
    runState: RunState,
    common: Common,
    address: Address,
    charge2929Gas: boolean,
  ) => Promise<{ gas: bigint; delegationAddress: Uint8Array | null }>
  sip7702WarmAddress: (runState: RunState, delegationAddress: Uint8Array) => void
}

/**
 * Shared SIP-2929 / SIP-7928 / SIP-8037 gas path for CALL, CALLCODE, DELEGATECALL, STATICCALL.
 */
export async function callFamilyGas(
  ...
  sip7928TrapPreTarget(runState, common, gas)
  ...
  sip7928TrapPreTarget(runState, common, gas + valueTransferGas)
  ...
  if (common.isActivatedEIP(7702)) {
    const { gas: delegationGas, delegationAddress } = await sip7702GetAccessCost(
      runState,
      common,
      toAddress,
      charge2929Gas,
    ...
    if (delegationAddress !== null) {
      if (common.isActivatedEIP(7928)) {
        ...
        sip7702WarmAddress(runState, delegationAddress)
...
/**
 * Shared SIP-7928 / SIP-8037 gas path for CREATE and CREATE2.
 */
export async function create7928Gas(
  ...
  // SIP-3860: reject oversized initcode BEFORE any state-gas pre-charge so
  // that SIP-8037's reservoir is preserved when the size check fails. This
  ...
  if (common.isActivatedEIP(8037)) {
    if (!canAfford(runState, gas, newAccountStateGas)) {
      ...
      return sip7928PostTargetCreateOog(runState, common, gas)
    }
    if (gas > runState.interpreter.getGasLeft()) {
      addAddressToBAL(runState, targetAddress, common)
      return sip7928PostTargetCreateOog(runState, common, gas)
  ...
  // SIP-8037 / SIP-7928: the static-context check is performed AFTER the
  ...
  // restore (savm.ts) then refills the pre-charged `STATE_BYTES_PER_NEW_ACCOUNT *
