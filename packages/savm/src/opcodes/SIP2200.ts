...
import { adjustSstoreGasEIP2929 } from './SIP2929.ts'
...
/**
 * Adjusts gas usage and refunds of SStore ops per SIP-2200 (Istanbul)
 *
  ...
  if (equalsBytes(originalStorage, currentStorage)) {
    ...
    if (value.length === 0) {
      runState.interpreter.refundGas(
        ...
        'SIP-2200 -> sstoreClearRefundEIP2200',
      )
    }
    // Write existing slot
