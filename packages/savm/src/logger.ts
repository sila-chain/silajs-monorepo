...
export class EVMPerformanceLogger {
  ...
  startTimer(tag: string) {
    if (this.currentTimer !== undefined) {
      throw SilaJSErrorWithoutCode('Cannot have two timers running at the same time')
    }
  ...
  pauseTimer() {
    ...
    if (timer === undefined) {
      throw SilaJSErrorWithoutCode('No timer to pause')
    }
    timer.pause()
  ...
  unpauseTimer(timer: Timer) {
    if (this.currentTimer !== undefined) {
      throw SilaJSErrorWithoutCode('Cannot unpause timer: another timer is already running')
    }
    timer.unpause()
  ...
  stopTimer(
    ...
    if (this.currentTimer === undefined || this.currentTimer !== timer) {
      throw SilaJSErrorWithoutCode('Cannot stop timer: another timer is already running')
    }
