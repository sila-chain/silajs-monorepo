import { BIGINT_0, SilaJSErrorWithoutCode, createZeroAddress } from '@silajs/util'

import type { BinaryTreeAccessWitnessInterface } from '@silajs/common'
...
export class Message {
  ...
  constructor(opts: MessageOpts) {
    ...
    if (this.value < 0) {
      throw SilaJSErrorWithoutCode(`value field cannot be negative, received ${this.value}`)
    }
  }
  ...
  get codeAddress(): Address {
    ...
    if (!codeAddress) {
      throw SilaJSErrorWithoutCode('Missing codeAddress')
    }
    return codeAddress
  }
}
