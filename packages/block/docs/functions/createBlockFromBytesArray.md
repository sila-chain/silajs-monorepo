[**@silajs/block**](../README.md)

***

[@silajs/block](../README.md) / createBlockFromBytesArray

# Function: createBlockFromBytesArray()

> **createBlockFromBytesArray**(`values`, `opts?`): [`Block`](../classes/Block.md)

Defined in: [block/constructors.ts:116](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/block/src/block/constructors.ts#L116)

Static constructor to create a block from an array of Bytes values

## Parameters

### values

[`BlockBytes`](../type-aliases/BlockBytes.md)

### opts?

[`BlockOptions`](../interfaces/BlockOptions.md)

## Returns

[`Block`](../classes/Block.md)

a new [Block](../classes/Block.md) object
