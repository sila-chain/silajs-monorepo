...
import { SilaJSErrorWithoutCode, bigIntToBytes, equalsBytes } from '@silajs/util'
...
export function createBlockHeaderFromBytesArray(values: BlockHeaderBytes, opts: BlockOptions = {}) {
  ...
  if (header.common.isActivatedEIP(1559) && baseFeePerGas === undefined) {
    const sip1559ActivationBlock = bigIntToBytes(header.common.eipBlock(1559)!)
    if (
      sip1559ActivationBlock !== undefined &&
      equalsBytes(sip1559ActivationBlock, number as Uint8Array)
    ) {
      throw SilaJSErrorWithoutCode('invalid header. baseFeePerGas should be provided')
    }
  }
  if (header.common.isActivatedEIP(4844)) {
    if (excessBlobGas === undefined) {
      throw SilaJSErrorWithoutCode('invalid header. excessBlobGas should be provided')
    } else if (blobGasUsed === undefined) {
      throw SilaJSErrorWithoutCode('invalid header. blobGasUsed should be provided')
    }
  }
  if (header.common.isActivatedEIP(4788) && parentBeaconBlockRoot === undefined) {
    throw SilaJSErrorWithoutCode('invalid header. parentBeaconBlockRoot should be provided')
  }

  if (header.common.isActivatedEIP(7685) && requestsHash === undefined) {
    throw SilaJSErrorWithoutCode('invalid header. requestsHash should be provided')
  }
  if (header.common.isActivatedEIP(7928) && header.blockAccessListHash === undefined) {
    throw SilaJSErrorWithoutCode('invalid header. blockAccessListHash should be provided')
  }
  if (header.common.isActivatedEIP(7843) && header.slotNumber === undefined) {
    throw SilaJSErrorWithoutCode('invalid header. slotNumber should be provided')
  }
  return header
}
...
export function createBlockHeaderFromRLP(
  ...
  if (!Array.isArray(values)) {
    throw SilaJSErrorWithoutCode('Invalid serialized header input. Must be array')
  }
 ...
 *
 * @param blockParams - Sila JSON RPC of block (eth_getBlockByNumber)
