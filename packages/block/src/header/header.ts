import { Common, ConsensusAlgorithm, ConsensusType, Hardfork, SilaMainnet } from '@silajs/common'
...
export class BlockHeader {
  ...
  get prevRandao() {
    if (!this.common.isActivatedEIP(4399)) {
      const msg = this._errorMsg(
        'The prevRandao parameter can only be accessed when SIP-4399 is activated',
      )
  ...
  constructor(headerData: HeaderData, opts: BlockOptions = {}) {
    ...
    if (!this.common.isActivatedEIP(1559) && baseFeePerGas !== undefined) {
      throw SilaJSErrorWithoutCode(
        'A base fee for a block can only be set with SIP1559 being activated',
      )
    }

    if (!this.common.isActivatedEIP(4895) && withdrawalsRoot !== undefined) {
      throw SilaJSErrorWithoutCode(
        'A withdrawalsRoot for a header can only be provided with SIP4895 being activated',
      )
    }
    ...
    if (!this.common.isActivatedEIP(4844)) {
      ...
      if (excessBlobGas !== undefined) {
        throw SilaJSErrorWithoutCode(
          'excess blob gas can only be provided with SIP4844 activated',
        )
      }
    }
    ...
    if (!this.common.isActivatedEIP(7685) && requestsHash !== undefined) {
      throw SilaJSErrorWithoutCode('requestsHash can only be provided with SIP 7685 activated')
    }

    if (!this.common.isActivatedEIP(7928) && blockAccessListHash !== undefined) {
      throw SilaJSErrorWithoutCode(
        'blockAccessListHash can only be provided with SIP 7928 activated',
      )
    }

    if (!this.common.isActivatedEIP(7843) && slotNumber !== undefined) {
      throw SilaJSErrorWithoutCode('slotNumber can only be provided with SIP 7843 activated')
    }
  ...
  protected _genericFormatValidation() {
    ...
    // Validation for SIP-1559 blocks
    if (this.common.isActivatedEIP(1559)) {
      if (typeof this.baseFeePerGas !== 'bigint') {
        const msg = this._errorMsg('SIP1559 block has no base fee field')
        throw SilaJSErrorWithoutCode(msg)
      }
      ...
      if (
        ...
        if (this.baseFeePerGas !== initialBaseFee) {
          const msg = this._errorMsg('Initial SIP1559 block does not have initial base fee')
          throw SilaJSErrorWithoutCode(msg)
        }
      }
    }
    ...
    if (this.common.isActivatedEIP(4895)) {
      if (this.withdrawalsRoot === undefined) {
        const msg = this._errorMsg('SIP4895 block has no withdrawalsRoot field')
        throw SilaJSErrorWithoutCode(msg)
      }
    ...
    if (this.common.isActivatedEIP(4788)) {
      if (this.parentBeaconBlockRoot === undefined) {
        const msg = this._errorMsg('SIP4788 block has no parentBeaconBlockRoot field')
        throw SilaJSErrorWithoutCode(msg)
      }
    ...
    if (this.common.isActivatedEIP(7685)) {
      if (this.requestsHash === undefined) {
        const msg = this._errorMsg('SIP7685 block has no requestsHash field')
        throw SilaJSErrorWithoutCode(msg)
      }
    }
    ...
    if (this.common.isActivatedEIP(7928)) {
      if (this.blockAccessListHash === undefined) {
        const msg = this._errorMsg('SIP7928 block has no blockAccessListHash field')
        throw SilaJSErrorWithoutCode(msg)
      }
    ...
    if (this.common.isActivatedEIP(7843)) {
      if (this.slotNumber === undefined) {
        const msg = this._errorMsg('SIP7843 block has no slotNumber field')
        throw SilaJSErrorWithoutCode(msg)
      }
    }
  }
  ...
  protected _consensusFormatValidation() {
    ...
    // Validation for PoS blocks (SIP-3675)
  ...
  validateGasLimit(parentBlockHeader: BlockHeader) {
    ...
    // SIP-1559: assume double the parent gas limit on fork block
  ...
  public calcNextBaseFee(): bigint {
    if (!this.common.isActivatedEIP(1559)) {
      const msg = this._errorMsg(
        'calcNextBaseFee() can only be called with SIP1559 being activated',
      )
      throw SilaJSErrorWithoutCode(msg)
    }
  ...
  getBlobGasPrice(): bigint {
    if (this.excessBlobGas === undefined) {
      throw SilaJSErrorWithoutCode('header must have excessBlobGas field populated')
    }
