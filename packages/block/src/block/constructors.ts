...
import type { TxOptions, TypedTransaction } from '@silajs/tx'
import {
  createTx,
...
import type { EthersProvider, WithdrawalBytes } from '@silajs/util'
...
export function createBlockFromBytesArray(values: BlockBytes, opts?: BlockOptions): Block {
  ...
  if (
    ...
    throw SilaJSErrorWithoutCode(
      'Invalid serialized block input: SIP-4895 is active, and no withdrawals were provided as array',
    )
  }
...
export function createBlockFromRLP(serialized: Uint8Array, opts?: BlockOptions): Block {
  ...
  if (!Array.isArray(values)) {
    throw SilaJSErrorWithoutCode('Invalid serialized block input. Must be array')
  }
 ...
 * @param uncles - Optional list of Sila JSON RPC of uncles (eth_getUncleByBlockHashAndIndex)
...
export const createBlockFromJSONRPCProvider = async (
  ...
  if (blockData === null) {
    throw SilaJSErrorWithoutCode('No block data returned from provider')
  }
