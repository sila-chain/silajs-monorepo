import { ConsensusType } from '@silajs/common'
...
/**
 * Class representing a block in the Sila network. The {@link BlockHeader} has its own
...
export class Block {
  ...
  constructor(
    ...
    if (!this.common.isActivatedEIP(4895) && withdrawals !== undefined) {
      throw SilaJSErrorWithoutCode('Cannot have a withdrawals field if SIP 4895 is not active')
    }
  ...
  raw(): BlockBytes {
    const bytesArray: BlockBytes = [
      ...
      this.transactions.map((tx) =>
        tx.supports(Capability.SIP2718TypedTransaction) ? tx.serialize() : tx.raw(),
      ) as Uint8Array[],
      ...
      if (this.common.isActivatedEIP(1559)) {
        if (tx.supports(Capability.SIP1559FeeMarket)) {
          ...
          if (tx.maxFeePerGas < this.header.baseFeePerGas!) {
            errs.push('tx unable to pay base fee (SIP-1559 tx)')
          }
        } else {
          tx = tx as LegacyTx
          if (tx.gasPrice < this.header.baseFeePerGas!) {
            errs.push('tx unable to pay base fee (non SIP-1559 tx)')
          }
        }
      }
   ...
   * @param validateBlockSize if set to `true`, will check for block size limit (SIP-7934) (default: false)
   */
  async validateData(
    ...
    // SIP-7934: RLP Execution Block Size Limit validation
  ...
  async withdrawalsTrieIsValid(): Promise<boolean> {
    if (!this.common.isActivatedEIP(4895)) {
      throw SilaJSErrorWithoutCode('SIP 4895 is not activated')
    }
