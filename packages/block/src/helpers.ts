...
import { isBlob4844Tx } from '@silajs/tx'
import {
  BIGINT_0,
  BIGINT_1,
...
import type { CLRequest, CLRequestType, PrefixedHexString, Withdrawal } from '@silajs/util'
  ...
  if (requests.length > 1) {
    for (let x = 1; x < requests.length; x++) {
      if (requests[x].type < requests[x - 1].type)
        throw SilaJSErrorWithoutCode('requests are not sorted in ascending order')
    }
  }
