import type { Common, ParamsDict } from '@silajs/common'
import type { JSONRPCTx, JSONTx, TransactionType, TxData } from '@silajs/tx'
import type {
  AddressLike,
  BigIntLike,
  BytesLike,
 ...
 * using a {@link Common} object, or `chain` and `hardfork`. Defaults to sila-mainnet without specifying a
 * hardfork.
 */
export interface BlockOptions {
   ...
   *
   * Default: {@link Common} object set to `sila-mainnet` and the HF currently defined as the default
  ...
  setHardfork?: boolean
  /**
   * Block parameters sorted by SIP can be found in the exported `paramsBlock` dictionary,
...
export interface JSONRPCBlock {
  ...
  baseFeePerGas?: PrefixedHexString // If SIP-1559 is enabled for this block, returns the base fee per gas
  withdrawals?: Array<JSONRPCWithdrawal> // If SIP-4895 is enabled for this block, array of withdrawals
  withdrawalsRoot?: PrefixedHexString // If SIP-4895 is enabled for this block, the root of the withdrawal trie of the block.
  blobGasUsed?: PrefixedHexString // If SIP-4844 is enabled for this block, returns the blob gas used for the block
  excessBlobGas?: PrefixedHexString // If SIP-4844 is enabled for this block, returns the excess blob gas for the block
  parentBeaconBlockRoot?: PrefixedHexString // If SIP-4788 is enabled for this block, returns parent beacon block root
  requestsHash?: PrefixedHexString // If SIP-7685 is enabled for this block, returns the requests root
  blockAccessListHash?: PrefixedHexString // SIP-7928 (SilaAmsterdam, experimental): DATA, 32 Bytes
  slotNumber?: PrefixedHexString // SIP-7843 (SilaAmsterdam, experimental): QUANTITY, 64 Bits
}
...
export type ExecutionPayload = {
  ...
  requestsHash?: PrefixedHexString
  blockAccessListHash?: PrefixedHexString // DATA, 32 Bytes (SIP-7928, SilaAmsterdam experimental)
  slotNumber?: PrefixedHexString // QUANTITY, 64 Bits (SIP-7843, SilaAmsterdam experimental)
}
