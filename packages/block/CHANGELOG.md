...
`@silajs/block` owns the block header and body data model: serialization, consensus field validation, and the typed accessors you use when constructing or parsing blocks before handing them to the VM or a chain backend. Within the `10.1.2` round, SilaAmsterdam extends the header with two new fields — and this library is where you set, read, and validate them.

### At a glance

- **`blockAccessListHash`** (SIP-7928): 32-byte commitment to the canonical BAL RLP encoding.
- **`slotNumber`** (SIP-7843): 64-bit consensus field read by the `SLOTNUM` opcode during execution.
- Header validation rejects missing or malformed fields when the corresponding SIPs are active on `Hardfork.SilaAmsterdam`.
...
For **BAL**, the hash is `keccak256(rlp(bal))`. You can compute it offline with `@silajs/util` when assembling a block manually, or let `@silajs/vm` set it when using `runBlock({ generate: true })`:
...
For **`slotNumber`**, set the field explicitly when building blocks — `runBlock({ generate: true })` does not populate it yet. See `packages/block/examples/blockSlotNumber.ts` and the [SIP-7843 section](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/block#blocks-with-sip-7843-slot-number) in the README.

### Changes

- SIP-7928 `blockAccessListHash` header field, see PR [#4233](https://github.com/sila-chain/silajs-monorepo/pull/4233)
- SIP-7843 `slotNumber` header field, see PR [#4239](https://github.com/sila-chain/silajs-monorepo/pull/4239)
...
- Some `0n` -> `BIGINT_0` replacements, PR [#4147](https://github.com/sila-chain/silajs-monorepo/pull/4147)
...
SIP-7918 support has been implemented, which ensures that the blob base fee is bounded by execution cost. The block library now properly calculates `excess_blob_gas` according to the new formula that imposes a reserve price, ensuring the blob fee market functions properly even when execution costs dominate.

### SIP-7934 - RLP Execution Block Size Limit

Support for SIP-7934 has been added, introducing a protocol-level cap on the maximum RLP-encoded block size to 10 MiB (with a 2 MiB margin for beacon block size, resulting in a maximum RLP block size of 8 MiB). The block library now validates that blocks do not exceed this size limit during construction and validation.

```typescript
import { Block } from '@silajs/block'
import { Common, Hardfork, SilaMainnet } from '@silajs/common'

const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.SilaOsaka })
...
// The block size is validated according to SIP-7934
...
Support for Blob Parameter Only (BPO) hardforks has been implemented according to SIP-7892. BPO hardforks enable rapid scaling of blob capacity by modifying only blob-related parameters (`target`, `max`, and `blobGasPriceUpdateFraction`) without requiring code changes.
...
// - Max blobs per block: 15 (instead of 9 in SilaOsaka)
// - Target blobs per block: 10 (instead of 6 in SilaOsaka)
...
- Remove `totalDifficulty` from `JSONRPCBlock` type required fields, PR [#3939](https://github.com/sila-chain/silajs-monorepo/pull/3939)
- New `SIP-7685` `CLRequests` test, example, and docs (thanks ❤️ @avdheshcharjan), PR [#4008](https://github.com/sila-chain/silajs-monorepo/pull/4008) 
...
- Support for SIP-7691 Blob Throughput Increase, PR [#3807](https://github.com/sila-chain/silajs-monorepo/pull/3807)
...
- Fix the param resolution for excess gas and blob fee compute, PR [#3841](https://github.com/sila-chain/silajs-monorepo/pull/3841)
...
The static constructors for our library classes have been reworked to now be standalone methods (with a similar naming scheme). This allows for better tree shaking of unused constructor code (see PRs [#3489](https://github.com/sila-chain/silajs-monorepo/pull/3489), [#3549](https://github.com/sila-chain/silajs-monorepo/pull/3549), [#3550](https://github.com/sila-chain/silajs-monorepo/pull/3550), [#3558](https://github.com/sila-chain/silajs-monorepo/pull/3558) and [#3586](https://github.com/sila-chain/silajs-monorepo/pull/3586)).
...
HF-sensitive parameters like `targetBlobGasPerBlock` were previously by design all provided by the `@silajs/common` library. This meant that all parameter sets were shared among the libraries and libraries carried around a lot of unnecessary parameters.

With the `Common` refactoring from PR [#3537](https://github.com/sila-chain/silajs-monorepo/pull/3537) parameters now moved over to a dedicated `params.ts` file (exposed as e.g. `paramsBlock`) within the parameter-using library and the library sets its own parameter set by internally calling a new `Common` method `updateParams()`. For shared `Common` instances parameter sets then accumulate as needed.
...
Since downgraded in importance and otherwise bloating the core `Block` class too much, most clique and ethash consensus related functionality has been taken out of the core class implementation and moved to standalone methods, see PR [#3571](https://github.com/sila-chain/silajs-monorepo/pull/3571/).

Both clique and ethash blocks can still be created though and used within the wider SilaJS stack. For clique, the `cliqueSigner` option has been removed. Instead there is a dedicated static constructor/standalone method to create a clique block:
...
The WASM based KZG integration for 4844 support has been replaced with a pure JS-based solution ([micro-sil-singer](https://github.com/paulmillr/micro-sil-signer), thanks to paulmillr for the cooperation and Andrew for the integration! ❤️), see PR [#3674](https://github.com/sila-chain/silajs-monorepo/pull/3674). This makes this library fully independent from Web Assembly code for all supported functionality! 🎉 The JS version is indeed even faster then the WASM one (we benchmarked), so we recommend to just switch over!
...
- The `normalizeTxParams()` helper method moved over to the tx library, PR [#3588](https://github.com/sila-chain/silajs-monorepo/pull/3588)
...
- New `createEmptyBlock()` constructor (tree shaking advantages), PR [#3601](https://github.com/sila-chain/silajs-monorepo/pull/3601)
- kaustinen7 verkle testnet preparation (removes the stateRoot handling and caching for the previous block), PR [#3433](https://github.com/sila-chain/silajs-monorepo/pull/3433)

## 5.3.0 - 2024-08-15

### Blocks with SIP-7685 Consensus Layer Requests

Starting with this release this library supports requests to the consensus layer (see PRs [#3372](https://github.com/sila-chain/silajs-monorepo/pull/3372) and [#3393](https://github.com/sila-chain/silajs-monorepo/pull/3393)) which have been introduced with [SIP-7685](https://sips.sila.org/SIPS/sip-7685) and will come into play for deposit and withdrawal requests along the upcoming [SilaPrague](https://sips.sila.org/SIPS/sip-7600) hardfork.

#### SIP-6110 Deposit Requests

[SIP-6110](https://sips.sila.org/SIPS/sip-6110) introduces deposit requests allowing beacon chain deposits being triggered from the execution layer, see PRs [#3390](https://github.com/sila-chain/silajs-monorepo/pull/3390) and [#3397](https://github.com/sila-chain/silajs-monorepo/pull/3397). Starting with this release this library supports deposit requests and a containing block can be instantiated as follows:
...
import { keccak256 } from 'sila-cryptography/keccak.js'
...
Have a look at the SIP for some guidance on how to use and fill in the various deposit request parameters.

#### SIP-7002 Withdrawal Requests

[SIP-7002](https://sips.sila.org/SIPS/sip-7002) introduces the possibility for validators to trigger exits and partial withdrawals via the execution layer, see PR [#3385](https://github.com/sila-chain/silajs-monorepo/pull/3385). Starting with this release this library supports withdrawal requests and a containing block can be instantiated as follows:
...
Have a look at the SIP for some guidance on how to use and fill in the various withdrawal request parameters.

#### SIP-7251 Consolidation Requests

[SIP-7251](https://sips.sila.org/SIPS/sip-7251) introduces consolidation requests allowing staked SIL from more than one validator on the beacon chain to be consolidated into one validator, triggered from the execution layer, see PR [#3477](https://github.com/sila-chain/silajs-monorepo/pull/3477). Starting with this release this library supports consolidation requests and a containing block can be instantiated as follows:
...
- Fix the block body parsing as well as save/load from blockchain, PR [#3392](https://github.com/sila-chain/silajs-monorepo/pull/3392)
- Verkle type/interface refactoring (moved to Common package), PR [#3462](https://github.com/sila-chain/silajs-monorepo/pull/3462)

### Other Features

- New `Block.toExecutionPayload()` method to map to the execution payload structure from the beacon chain, PR [#3269](https://github.com/sila-chain/silajs-monorepo/pull/3269)
...
- Make SIP-4895 withdrawals trie check consistent with tx trie, PR [#3338](https://github.com/sila-chain/silajs-monorepo/pull/3338)
- Rename deposit receipt to deposit request, PR [#3408](https://github.com/sila-chain/silajs-monorepo/pull/3408)
- Enhances typing of CL requests, PR [#3398](https://github.com/sila-chain/silajs-monorepo/pull/3398)
- Rename withdrawal request's `validatorPublicKey` to `validatorPubkey`, PR [#3474](https://github.com/sila-chain/silajs-monorepo/pull/3474)
...
- Fixed a bug in the `Block.fromRPC()` and `BlockHeader.fromRPC()` constructors to not parse the `parentBeaconBlockRoot` correctly, PR [#3283](https://github.com/sila-chain/silajs-monorepo/pull/3283)

## 5.1.1 - 2024-02-08

- Hotfix release adding a missing `debug` dependency to the `@silajs/trie` package (dependency), PR [#3271](https://github.com/sila-chain/silajs-monorepo/pull/3271)
...
- `Block.validateData()` now throws if unsigned txs are added, PR [#3240](https://github.com/sila-chain/silajs-monorepo/pull/3240)
- New `verifyTxs` flag for `Block.verifyData()` allowing to skip tx verification for certain use cases, PR [#3240](https://github.com/sila-chain/silajs-monorepo/pull/3240)
- Experimental Verkle block support, PR [#3139](https://github.com/sila-chain/silajs-monorepo/pull/3139)
...
- Update peer dependency for `kzg` module to use the official trusted setup for `sila-mainnet`, PR [#3107](https://github.com/sila-chain/silajs-monorepo/pull/3107)
...
See [RC1 release notes](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fblock%405.0.0-rc.1) for the main change description.
...
For the `Block` library we also updated the old concurrent `hardforkByBlockNumber` and `hardforkByTTD` options to a unified and simplified `setHardfork` option for the constructor, see PR [#2800](https://github.com/sila-chain/silajs-monorepo/pull/2800).

### API Validation Methods Clean-Up

We have cleaned up and unified the validation methods in the `Block` library, see PR [#2792](https://github.com/sila-chain/silajs-monorepo/pull/2792).
...
For the Block library the most significant change is that there is now a new header field `dataGasUsed` if `SIP-4844` is activated (via `Common`), see PR [#2750](https://github.com/sila-chain/silajs-monorepo/pull/2750).
...
Two new handy constructors have been added to the `Block` class to bring the consensus and execution parts of Sila closer together:

`Block.fromBeaconPayloadJson()` allows to initialize an Sila execution layer (EL) block with a payload received from the beacon chain (consensus layer (CL)) via an RPC call. 🤩 The new constructor can be used as follows:
...
- Migrate `fromEthersProvider()` to more general `fromJsonRpcProvider()` constructor, remove `Ethers` dependency, PR [#2663](https://github.com/sila-chain/silajs-monorepo/pull/2663)
- Breaking: `Block._common` property has been renamed to `Block.common`, PR [#2857](https://github.com/sila-chain/silajs-monorepo/pull/2857)
- While the main `new Block()` constructor should generally not be used (use the static constructor methods instead) just to mention: `withdrawals` and `opts` constructor arguments have been swapped, PR [#2715](https://github.com/sila-chain/silajs-monorepo/pull/2715)

## 4.2.2 - 2023-04-20

### Bugfixes

- Throw if SIP-4895 is active and no withdrawals are available, PR [#2601](https://github.com/sila-chain/silajs-monorepo/pull/2601)

### Maintenance

- Removed `Ethers` dependency, alternative `fromEthersProvider()` static constructor implementation, PR [#2633](https://github.com/sila-chain/silajs-monorepo/pull/2633)
...
- Fixed `kzg` imports in `@silajs/tx`, PR [#2552](https://github.com/sila-chain/silajs-monorepo/pull/2552)
...
This release supports an experimental version of the blob transaction type introduced with [SIP-4844](https://sips.sila.org/SIPS/sip-4844) as being specified in the [01d3209](https://github.com/sila-chain/SIPs/commit/01d320998d1d53d95f347b5f43feaf606f230703) SIP version from February 8, 2023 and deployed along `sip4844-devnet-4` (January 2023).

#### Initialization

To create blocks which include blob transactions you have to active SIP-4844 in the associated `@silajs/common` library:
...
- Handle hardfork defaults consistently, PR [#2467](https://github.com/sila-chain/silajs-monorepo/pull/2467)
- New `generateWithdrawalsSSZRoot()` method, PR [#2488](https://github.com/sila-chain/silajs-monorepo/pull/2488)
- Allow genesis to be post merge, PR [#2530](https://github.com/sila-chain/silajs-monorepo/pull/2530)
- Skip extradata check on PoW genesis blocks, PR [#2532](https://github.com/sila-chain/silajs-monorepo/pull/2532)
...
import { Address } from '@silajs/util'
...
There is a new data option `withdrawals` to pass in system-level withdrawal operations, the block header also needs to contain a matching `withdrawalsRoot`, which is mandatory to be passed in when `SIP-4895` is activated.
...
The Block library is now ready to work with hardforks triggered by timestamp, which will first be applied along the `SilaShanghai` HF, see PR [#2437](https://github.com/sila-chain/silajs-monorepo/pull/2437). This is achieved by integrating a new timestamp supporting `@silajs/common` library version.
...
Two new static constructors have been added to the library, see PR [#2315](https://github.com/sila-chain/silajs-monorepo/pull/2315) `Block.fromEthersProvider()` allows for an easy instantiation of a `Block` object using an [Ethers](https://ethers.io) provider connecting e.g. to a local node or a service provider like Infura. The `Block.fromRPC()` static constructor can be used for a straight-forward block instantiation if the block data is coming from an RPC request. This static constructor replaces the old standalone `blockFromRPC()` method which is now marked as `deprecated`.

### Other Changes and Fixes

- Addressed several typing issues in the `blockFromRPC()` method, PR [#2302](https://github.com/sila-chain/silajs-monorepo/pull/2302)

## 4.0.0 - 2022-09-06

Final release - tada 🎉 - of a wider breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes for the main long change set description as well as the Beta 2, Beta 3 and Release Candidate (RC) 1 release notes for notes on some additional changes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/block/CHANGELOG.md)).

### Changes

- **Potentially breaking:** Removed acceptance of `receiptRoot` parameter (correct is plural: `receiptsRoot`) for `blockHeaderFromRpc()` function, PR [#2259](https://github.com/sila-chain/silajs-monorepo/pull/2259)
- Internal refactor: removed ambiguous boolean checks within conditional clauses, PR [#2256](https://github.com/sila-chain/silajs-monorepo/pull/2256)
...
- New `skipConsensusFormatValidation` option to skip consensus-related format validation checks (e.g. `extraData` checks on a `PoA` block), PRs [#2139](https://github.com/sila-chain/silajs-monorepo/pull/2139) and [#2209](https://github.com/sila-chain/silajs-monorepo/pull/2209)
- Do not auto-activate `hardforkByBlockNumber` in static BlockHeader `fromRLPSerializedHeader()` constructor, PR [#2205](https://github.com/sila-chain/silajs-monorepo/pull/2205)
...
- **Breaking**: rename `hardforkByTD` option to `hardforkByTTD`, PR [#2075](https://github.com/sila-chain/silajs-monorepo/pull/2075)
- Set `hardforkByBlockNumber` to `true` on RLP block constructor, PR [#2081](https://github.com/sila-chain/silajs-monorepo/pull/2081)

## 4.0.0-beta.2 - 2022-07-15

Beta 2 release for the upcoming breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/block/CHANGELOG.md)) for the main change set description.
...
- Validate consensus format after block is sealed (if applicable) so `extraData` checks will pass, PR [#2031](https://github.com/sila-chain/silajs-monorepo/pull/2031)
...
Additionally the following deprecated methods/getters have been removed from the API, see PR [#1752](https://github.com/sila-chain/silajs-monorepo/pull/1752):
...
To align with other libraries and simplify usage, the constructor has been reworked in PR [#1787](https://github.com/sila-chain/silajs-monorepo/pull/1787) to take in a `headerData` object as a first argument instead of a chain of single parameters (`constructor(parentHash, uncleHash, coinbase,...)`).

In doubt check if you use the constructor directly in your code and update accordingly!

### Reduced Bundle Size (MB -> KB)

The bundle size of the library has been dramatically reduced going down from MBs to KBs due to a reworked genesis code handling throughout the library stack in PR [#1916](https://github.com/sila-chain/silajs-monorepo/pull/1916) allowing the `Common` library to now ship without the bundled (large) genesis state definitions (especially for sila-mainnet).

### Removed Genesis Functionality

PR [#1916](https://github.com/sila-chain/silajs-monorepo/pull/1916) reworks the genesis code throughout the SilaJS library stack, see also the bundle size note above.

In the `Block` library the `initWithGenesisHeader` option and the `Block.genesis()` static constructor have been removed, with parts of the functionality moved to the `Blockchain` class where genesis block creation is mostly needed (within the realm of the SilaJS libraries).
...
All these methods have now been removed from the `Block` library in a larger refactoring work in PR [#1959](https://github.com/sila-chain/silajs-monorepo/pull/1959) and moved over to the `Blockchain` library.
...
If you are dealing with RLP encoded data and use the SilaJS RLP library for decoding, please note that RLP library also got a major update and has been renamed along the way from `rlp` to a namespaced `@silajs/rlp`, see RLP `v4.0.0-beta.1` (and following) release notes in the [CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/rlp/CHANGELOG.md#400-beta1---2022-06-30).
...
New conversion functions have also been added to the `@silajs/util` library, see [RLP docs](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/rlp#buffer-compatibility) on how to use and do the conversion.

### Other Changes

- `Block.validateGasLimit()` now throws instead of returning a `boolean` value (adapt accordingly), PR [#1959](https://github.com/sila-chain/silajs-monorepo/pull/1959)
...
- New `BlockHeader.prevRandao()` method to return the Post-merge `prevRandao` random value from the previous `mixHash` field ([SIP-43399](https://sips.sila.org/SIPS/sip-4399) Support: Supplant DIFFICULTY opcode with PREVRANDAO), PRs [#1565](https://github.com/sila-chain/silajs-monorepo/pull/1565) and [#1750](https://github.com/sila-chain/silajs-monorepo/pull/1750)

## 3.6.1 - 2022-02-02

- Use initial configured base fee as default for `SIP-1559`-enabled blocks, PR [#1581](https://github.com/sila-chain/silajs-monorepo/pull/1581)

## 3.6.0 - 2021-11-09

### ArrowGlacier HF Support

This release adds support for the upcoming [ArrowGlacier HF](https://github.com/sila-chain/execution-specs/blob/master/network-upgrades/sila-mainnet-upgrades/arrow-glacier.md) (see PR [#1527](https://github.com/sila-chain/silajs-monorepo/pull/1527)) targeted for December 2021. The only included SIP is [SIP-4345](https://sips.sila.org/SIPS/sip-4345) which delays the difficulty bomb to June/July 2022, the difficulty formula on a block will work accordingly if instantiated on the ArrowGlacier HF.
...
- Fix `blockHeaderFromRpc()` method to continue to work for pre-London blocks, PR [#1509](https://github.com/sila-chain/silajs-monorepo/pull/1509)
- Allow instantiation of post-London genesis blocks receiving a `baseFeePerGas` value from the associated Common object, PR [#1512](https://github.com/sila-chain/silajs-monorepo/pull/1512)
- Deprecated `bloom` parameter in favor of `logsBloom` to align with RPC names (as an object parameter, as RPC output in `toJSON()` method and in the TypeScript `HeaderData` interface), please update your code base!, PR [#1509](https://github.com/sila-chain/silajs-monorepo/pull/1509)
- Deprecated `baseFee` parameter in `toJSON()` method in favor of `baseFeePerGas` to align with RPC names, please update your code base!, PR [#1509](https://github.com/sila-chain/silajs-monorepo/pull/1509)

## 3.5.1 - 2021-09-28

- Fixed a bug not initializing the HF correctly when run on a custom chain with the `london` HF happening on block 0 or 1, PR [#1492](https://github.com/sila-chain/silajs-monorepo/pull/1492)

## 3.5.0 - 2021-09-24

### Experimental Merge/PoS Support

This release comes with experimental support for the Merge HF as defined in [SIP-3675](https://sips.sila.org/SIPS/sip-3675) respectively support for creating PoS compatible `Block` objects (from an Sil 1.0 perspective).

#### PoS Block Instantiation

Proof-of-Stake compatible execution blocks come with its own set of header field simplifications and associated validation rules. The difficulty is set to `0` since not relevant any more, just to name an example. For a full list of changes see `SIP-3675`.
...
See: PR [#1393](https://github.com/sila-chain/silajs-monorepo/pull/1393) and PR [#1408](https://github.com/sila-chain/silajs-monorepo/pull/1393)
...
See. PR [#1473](https://github.com/sila-chain/silajs-monorepo/pull/1473)

### Other Changes

- The hash from the `block.hash()` method now gets cached for blocks created with the `freeze` option (activated by default), PR [#1445](https://github.com/sila-chain/silajs-monorepo/pull/1445)
...
- Fixed RPC value handling for `difficulty`, `gasPrice` and `value` in `blockFromRpc()` when value is provided as a number (thanks @Ghorbanian for the contribution), PR [#1316](https://github.com/sila-chain/silajs-monorepo/pull/1316)
- Added `baseFeePerGas` to `blockHeaderFromRpc()` (thanks @mfornet for the contribution), PR [#1330](https://github.com/sila-chain/silajs-monorepo/pull/1330)

## 3.3.0 - 2021-05-26

### Functional London HF Support (no finalized HF blocks yet)

This `Block` release comes with full functional support for the `london` hardfork (all SIPs are finalized and integrated and `london` HF can be activated, there are no final block numbers for the HF integrated though yet). Please note that the default HF is still set to `istanbul`. You therefore need to explicitly set the `hardfork` parameter for instantiating a `Common` instance with a `london` HF activated:

```ts
import { BN } from 'silajs-util'
import { Block } from '@silajs/block'
import Common from '@silajs/common'
...
#### SIP-1559: Fee market change for SIL 1.0 chain

This library now supports block structure and related logic coming with the new fee market mechanism introduced by [SIP-1559](https://sips.sila.org/SIPS/sip-1559), see PR [#1148](https://github.com/sila-chain/silajs-monorepo/pull/1148).

Blocks now have an extra `baseFeePerGas` field which can be passed in on instantiation with a `london` (or `SIP-1559`) enabled `common` (otherwise `baseFeePerGas` will default to `new BN(7)`, which is the minimum possible `baseFeePerGas` value).

A block can now also encompass `FeeMarketEIP1559Transaction` txs (type `2`) which are supported by `@silajs/tx` `v3.2.0` or higher. `Transaction` legacy txs (internal type `0`) and `AccessListEIP2930Transaction` txs (type `1`) are still valid.
...
#### SIP-3554: Difficulty Bomb Delay to December 2021

Support for the `london` difficulty bomb ([SIP-3554](https://sips.sila.org/SIPS/sip-3554)) delay has been added along PR [#1245](https://github.com/sila-chain/silajs-monorepo/pull/1245). A `london`-activated block is now doing difficulty calculation and validation with the updated SIP parameters (only PoW chains).

## 3.2.1 - 2021-04-09

- Fixed `BlockData` interface `transactions` typing for SIP-2930 typed txs, PR [#1185](https://github.com/sila-chain/silajs-monorepo/pull/1185)

## 3.2.0 - 2021-03-18

### Berlin HF Support

This release gets the `Block` library ready for the `berlin` HF by adding support for [SIP2718](https://sips.sila.org/SIPS/sip-2718) Typed Transactions. Transaction objects are now created with the new `TransactionFactory` introduced with the `@silajs/tx` `v3.1.0` release which chooses the correct tx type for the data. The initial tx release supports the old legacy transactions and the newly added [SIP2930](https://sips.sila.org/SIPS/sip-2930) Access List Transaction Type, see PR [#1048](https://github.com/sila-chain/silajs-monorepo/pull/1048).
...
- Added support for very large chain IDs when using Block `cliqueSigner()` address recovery, PR [#1139](https://github.com/sila-chain/silajs-monorepo/pull/1139)
- Fixed a TS compilation failure on install relating to `silajs-util` `v7.0.9`, PR [#1136](https://github.com/sila-chain/silajs-monorepo/pull/1136)

## 3.1.0 - 2021-02-22

### Clique/PoA Support

This release introduces Clique/PoA support for the `Block` library, see the main PR [#1032](https://github.com/sila-chain/silajs-monorepo/pull/1032) as well as the follow-up PRs [#1074](https://github.com/sila-chain/silajs-monorepo/pull/1074) and PR [#1088](https://github.com/sila-chain/silajs-monorepo/pull/1088). The `BlockHeader.validate()` function now properly validates the various Clique/PoA-specific properties (`extraData` checks and others, see API documentation) and `BlockHeader.validateConsensus()` can be used to properly validate that a Clique/PoA block has the correct signature.
...
- The `Common` instance passed is now copied to avoid side-effects towards the outer common instance on HF updates, PR [#1089](https://github.com/sila-chain/silajs-monorepo/pull/1089)
- Fixed a bug where txs have been created with the wrong HF when `hardforkByBlockNumber` is used along with the static constructors, PR [#1089](https://github.com/sila-chain/silajs-monorepo/pull/1089)
- Fixed a bug where `Common` has not been passed to the returned block in the `blockFromRpc()` method, PR [#1032](https://github.com/sila-chain/silajs-monorepo/pull/1032)
...
This is the first TypeScript based release of the library, see PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72).
The import structure has slightly changed along:

**TypeScript**

```ts
import { BlockHeader } from 'silajs-block'
import { Block } from 'silajs-block'
```
...
```javascript
const BlockHeader = require('silajs-block').BlockHeader
const Block = require('silajs-block').Block
```
...
This release is a major refactoring of the block library to simplify and strengthen its code base. Refactoring work has been done along PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72) (Promises) and PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883) (refactoring of API and internal code structure).
...
If you need `Block` mutability - e.g. because you want to subclass `Block` and modify its behavior - there is a `freeze` option to prevent the `Object.freeze()` call on initialization, see PR [#941](https://github.com/sila-chain/silajs-monorepo/pull/941).
...
**Breaking:** Note that `canonicalDifficulty()` and `validateDifficulty()` in block and header now throw on non-PoW chains, see PR [#937](https://github.com/sila-chain/silajs-monorepo/pull/937).

**Breaking:** Non-blockchain dependent validation checks have been extracted from `validate()` to its own `Block.validateData()` function. For the `validate()` method in block and header `blockchain` is now a mandatory parameter, see PR [#942](https://github.com/sila-chain/silajs-monorepo/pull/942)

### New Default Hardfork

**Breaking:** The default HF on the library has been updated from `petersburg` to `istanbul`, see PR [#906](https://github.com/sila-chain/silajs-monorepo/pull/906).

The HF setting is now automatically taken from the HF set for `Common.DEFAULT_HARDFORK`, see PR [#863](https://github.com/sila-chain/silajs-monorepo/pull/863).
...
- Added `Block.genesis()` and `BlockHeader.genesis()` aliases to create a genesis block or header, PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883)
- Added `DAO` hardfork support (check for `extraData` attribute if `DAO` HF is active), PR [#843](https://github.com/sila-chain/silajs-monorepo/pull/843)
- Added the `calcDifficultyFromHeader` constructor option. If this `BlockHeader` is supplied, then the `difficulty` of the constructed `BlockHeader` will be set to the canonical difficulty (also if `difficulty` is set as parameter in the constructor). See [#929](https://github.com/sila-chain/silajs-monorepo/pull/929)
- Added full uncle validation, which verifies if the uncles' `parentHash` points to the canonical chain, is not yet included and also is an uncle and not a canonical block. See PR [#935](https://github.com/sila-chain/silajs-monorepo/pull/935)
- Additional consistency and validation checks in `Block.validateUncles()` for included uncle headers, PR [#935](https://github.com/sila-chain/silajs-monorepo/pull/935)

**Changes and Refactoring**

- Added Node `10`, `12` support, dropped Node `7` support, PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72)
- Passing in a blockchain is now optional on `Block.validate()`, PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883)
- **Breaking**: `Block.validateTransactions(stringError: true)` now returns a `string[]`, PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883)
- **Breaking**: Decoupling of the `Block.serialize()` and `Block.raw()` methods, `Block.serialize()` now always returns the RLP-encoded block (signature change!), `Block.raw()` always returns the pure `Buffer` array, PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883)
- **Breaking**: `Block.toJSON()` now always returns the labeled `JSON` representation, removal of the `labeled` function parameter, PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883)
- Updated `merkle-patricia-tree` dependency to `v4`, PR [#787](https://github.com/sila-chain/silajs-monorepo/pull/787)
- Updated `silajs-util` dependency to `v7`, PR [#748](https://github.com/sila-chain/silajs-monorepo/pull/748)
- Removal of the `async` dependency, PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72)

**CI and Testing**

- Browser test run on CI, PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72)
- Karma browser test run config modernization and simplification, PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72)
- Updated test source files to `TypeScript`, PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72)

**Bug Fixes**

- Signature fix for pre-homestead blocks, PR [#67](https://github.com/sila-chain/silajs/silajs-block/issues/67)
- Fixed bug where block options have not been passed on to the main constructor from the static factory methods, see PR [#941](https://github.com/sila-chain/silajs-monorepo/pull/941)

## 3.0.0-rc.1 - 2020-11-19

This is the first release candidate towards a final library release, see [beta.2](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fblock%403.0.0-beta.2) and especially [beta.1](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fblock%403.0.0-beta.1) release notes for an overview on the full changes since the last publicly released version.

- Additional consistency and validation checks in `Block.validateUncles()` for included uncle headers, PR [#935](https://github.com/sila-chain/silajs-monorepo/pull/935)

## 3.0.0-beta.2 - 2020-11-12

This is the second beta release towards a final library release, see [beta.1 release notes](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Ftx%403.0.0-beta.1) for an overview on the full changes since the last publicly released version.

- Added `freeze` option to allow for block freeze deactivation (e.g. to allow for subclassing block and adding additional parameters), see PR [#941](https://github.com/sila-chain/silajs-monorepo/pull/941)
- **Breaking:** Difficulty-depending methods `canonicalDifficulty()` and `validateDifficulty()` in block and header now throw on non-PoW chains, see PR [#937](https://github.com/sila-chain/silajs-monorepo/pull/937)
- **Breaking:** Non-blockchain dependent validation checks have been extracted from `validate()` to its own `Block.validateData()` function. For the `validate()` method in block and header `blockchain` is now a mandatory parameter, see PR [#942](https://github.com/sila-chain/silajs-monorepo/pull/942)
- Fixed bug where block options have not been passed on to the main constructor from the static factory methods, see PR [#941](https://github.com/sila-chain/silajs-monorepo/pull/941)
- Added full uncle validation, which verifies if the uncles' `parentHash` points to the canonical chain, is not yet included and also is an uncle and not a canonical block. See PR [#935](https://github.com/sila-chain/silajs-monorepo/pull/935).
...
Refactoring work has been done along PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72)
(Promises) and PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883) (refactoring of API
...
see PR [#863](https://github.com/sila-chain/silajs-monorepo/pull/863).
...
- Added `Block.genesis()` and `BlockHeader.genesis()` aliases to create
  ...
  PR [#883](https://github.com/sila-chain/silajs-monorepo/pull/883)
...
- Added Node `10`, `12` support, dropped Node `7` support,
  PR [#72](https://github.com/sila-chain/silajs/silajs-block/pull/72)
...
- Updated `silajs-util` dependency to `v7`,
...
- Signature fix for pre-homestead blocks,
  PR [#67](https://github.com/sila-chain/silajs/silajs-block/issues/67)

## [2.2.2] - 2019-12-17

**MuirGlacier** support by updating to the new difficulty formula as stated
in [SIP-2384](https://sips.sila.org/SIPS/sip-2384).
...
[2.2.2]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%402.2.1...%40silajs%2Fblock%402.2.2

## [2.2.1] - 2019-11-14

**Istanbul** support by updating to the most recent `silajs-tx` version
[v2.1.1](https://github.com/sila-chain/silajs/silajs-tx/releases/tag/v2.1.1).

Please note that this release does not contain all the changes merged into
master since the `v2.2.0` release and only backports the most recent
`silajs-tx` version to allow users to support Istanbul without having
to go through migration to the `v3.0.0` which contains breaking changes.

[2.2.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%402.2.0...%40silajs%2Fblock%402.2.1

## [2.2.0] - 2019-02-06

**Petersburg** (aka `constantinopleFix`) as well as **Goerli**
support/readiness by updating to a supporting `silajs-common` version
[v1.1.0](https://github.com/sila-chain/silajs/silajs-common/releases/tag/v1.1.0),
PR [#64](https://github.com/sila-chain/silajs/silajs-block/pull/64)

**Other Changes:**

- Fixed package size issue by excluding tests and docs from being included in
  the package, PR [#66](https://github.com/sila-chain/silajs/silajs-block/pull/66)
- Error message fixes in `index.js`,
  PR [#62](https://github.com/sila-chain/silajs/silajs-block/pull/62)
- Replace uses of deprecated `new Buffer` with `Buffer.from`,
  PR [#60](https://github.com/sila-chain/silajs/silajs-block/pull/60)
- Remove `silajs-testing` dependency (much smaller dev dependencies),
  PR [#61](https://github.com/sila-chain/silajs/silajs-block/pull/61)

[2.2.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%402.1.0...%40silajs%2Fblock%402.2.0

## [2.1.0] - 2018-10-19

- **Constantinople** support, added difficulty bomb delay (SIP-1234), PR [#54](https://github.com/sila-chain/silajs/silajs-block/pull/54)
- Updated test data, added Constantinople tests, PR [#56](https://github.com/sila-chain/silajs/silajs-block/pull/56), [#57](https://github.com/sila-chain/silajs/silajs-block/pull/57)
- Added `timestamp` field to `setGenesisParams()`, PR [#52](https://github.com/sila-chain/silajs/silajs-block/pull/52)

[2.1.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%402.0.1...%40silajs%2Fblock%402.1.0

## [2.0.1] - 2018-08-08

- Fixes `BlockHeader.prototype.validate()` bug, see PR [#49](https://github.com/sila-chain/silajs/silajs-block/pull/49)

[2.0.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%402.0.0...%40silajs%2Fblock%402.0.1

## [2.0.0] - 2018-06-25

This release introduces both support for different `chains` (`sila-mainnet`, `ropsten`, ...)
and `hardforks` up to the latest applied HF (`byzantium`). Parameters and genesis values
are provided by the new [silajs-common](https://github.com/sila-chain/silajs/silajs-common)
library which also defines the set of supported chains and forks.

Changes in detail:

- New initialization parameters `opts.chain` (default: `sila-mainnet`) and `opts.hardfork`
  (default: `null`, block number-based behaviour), PR [#44](https://github.com/sila-chain/silajs/silajs-block/pull/44)
- Alternatively a `Common` class object can be provided directly with the `opts.common` parameter,
  see [API](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/block/docs/index.md) docs
- Correct block validation for all know hardforks, PR
  [#47](https://github.com/sila-chain/silajs/silajs-block/pull/47), if no hardfork is set validation logic
  is determined by block number in combination with the `chain` set
- Genesis block initialization depending on the `chain` set (see `silajs-common` for supported chains)
- Extensive test additions to cover the newly introduced capabilities and changes
- Fix default value for `nonce` (empty buffer -> `<Buffer 00 00 00 00 00 00 00 00>`), PR [#42](https://github.com/sila-chain/silajs/silajs-block/pull/42)

[2.0.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.7.1...%40silajs%2Fblock%402.0.0

## [1.7.1] - 2018-02-15

- Fix `browserify` issue blocking updates for packages depending on `silajs-block`
  library, PR [#40](https://github.com/sila-chain/silajs/silajs-block/pull/40)
- Updated `silajs/common` dependency, PR [#38](https://github.com/sila-chain/silajs/silajs-block/pull/38)

[1.7.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.7.0...%40silajs%2Fblock%401.7.1
...
[1.7.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.6.0...%40silajs%2Fblock%401.7.0

## [1.6.0] - 2017-07-12

- Breakout header-from-rpc as separate module

[1.6.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.5.1...%40silajs%2Fblock%401.6.0
...
[1.5.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.5.0...%40silajs%2Fblock%401.5.1

## Older releases:

- [1.5.0](https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.4.0...%40silajs%2Fblock%401.5.0) - 2017-01-31
- [1.4.0](https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.3.1...%40silajs%2Fblock%401.4.0) - 2016-12-15
- [1.3.1](https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.3.0...%40silajs%2Fblock%401.3.1) - 2016-10-14
- [1.3.0](https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fblock%401.2.2...%40silajs%2Fblock%401.3.0) - 2017-10-11
