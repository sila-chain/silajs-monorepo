...
} from '@silajs/testdata'
import { createLegacyTx, paramsTx } from '@silajs/tx'
import {
  KECCAK256_RLP_ARRAY,
  MAX_RLP_BLOCK_SIZE,
  bytesToHex,
...
import type { NestedUint8Array } from '@silajs/util'
...
describe('[Block]: block functions', () => {
  ...
  it('should test transaction validation - transaction not signed', async () => {
    ...
    // Create tx with gas limit over max (but not yet on SilaOsaka)
    ...
    }, 'exceeds the maximum allowed by SIP-7825')
  })
  ...
  it('should test transaction validation with legacy tx in london', async () => {
    ...
    assert.isTrue(
      result[0].includes('tx unable to pay base fee (non SIP-1559 tx)'),
  ...
  it('should test data integrity', async () => {
    ...
    block = createBlock(
      ...
      { common: new Common({ chain: SilaMainnet, hardfork: Hardfork.SilaShanghai }) },
    )
    ...
    block = createBlock(
      ...
      { common: new Common({ chain: SilaMainnet, hardfork: Hardfork.Chainstart }) },
    )
    await checkThrowsAsync(block.validateData(false, false), 'invalid uncle hash')
  })

  it('should test isGenesis (sila-mainnet default)', () => {
  ...
  it('should test genesis hashes (sila-mainnet default)', () => {
    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Chainstart })
  ...
  it('should test hash() method (sila-mainnet default)', () => {
    let common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Chainstart })
    const rlp = hexToBytes(`0x${genesisHashesTestData.test.genesis_rlp_hex}`)
    const hash = hexToBytes(`0x${genesisHashesTestData.test.genesis_hash}`)
  ...
  it('DAO hardfork', () => {
    ...
    // Set block number from test block to sila-mainnet DAO fork block 1920000
    blockData[0][8] = hexToBytes('0x1D4C00')

    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Dao })
    assert.throws(
      function () {
  ...
  it('should set canonical difficulty if I provide a calcDifficultyFromHeader header', () => {
    ...
    common = new Common({ chain: SilaMainnet, hardfork: Hardfork.London })
...
describe('[Block]: SIP-7934 RLP Execution Block Size Limit', () => {
  ...
  it('should not throw when size exceeds but SIP-7934 is not activated', async () => {
    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Chainstart })
    const block = createLargeBlock(common)

    // This should not throw since SIP-7934 is not activated
    await block.validateData(false, false, true)
  })

  it('should not throw when size exceeds, SIP-7934 is activated, but validateBlockSize is default (false)', async () => {
    const params = {
      7934: {
  ...
  it('should throw when SIP-7934 is activated and validateBlockSize is explicitly set to true', async () => {
    const params = {
      7934: {
        maxRlpBlockSize: 8_388_608, // 8 MiB (MAX_BLOCK_SIZE - SAFETY_MARGIN)
      },
  ...
  it('should not throw when SIP-7934 is activated, validateBlockSize is true, but size does not exceed', async () => {
    const params = {
      7934: {
        maxRlpBlockSize: 8_388_608, // 8 MiB (MAX_BLOCK_SIZE - SAFETY_MARGIN)
      },
  ...
  it('should throw when creating block from RLP when size exceeds limit', async () => {
    ...
    // Create a block without SIP-7934 active first to avoid size check during creation
    ...
    // createBlockFromRLP should throw when SIP-7934 is active
    assert.throws(() => {
