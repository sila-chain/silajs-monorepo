...
// Older version at https://github.com/abdelhamidbakhta/besu/blob/bf54b6c0b40d3015fc85ff9b078fbc26592d80c0/sila/core/src/test/resources/org/hyperledger/besu/sila/core/fees/basefee-test.json
import { paramsBlock } from '../src/params.ts'

import { sip1559baseFeeData } from './testdata/sip1559baseFee.ts'
...
describe('SIP1559 tests', () => {
  it('Header -> Initialization', () => {
    ...
    assert.throws(
        ...
        createBlockHeader(
          {
            ...
            timestamp: BigInt(1),
            gasLimit: genesis.header.gasLimit * BigInt(2), // Special case on SIP-1559 transition block
      ...
      undefined,
      undefined,
      'should throw when setting baseFeePerGas with SIP1559 not being activated',
    )
  })
  ...
  it('Header -> genericFormatValidation checks', async () => {
    try {
      ...
      const expectedError = 'Initial SIP1559 block does not have initial base fee'
      assert.isTrue(
    ...
    try {
      ...
      const expectedError = 'SIP1559 block has no base fee field'
      assert.isTrue(
        e.message.includes(expectedError),
        'should throw with no base fee field when SIP1559 is activated',
      )
    }
  })
  ...
  it('Header -> _genericFormValidation -> success case', async () => {
    ...
    assert.isTrue(true, 'Valid initial SIP1559 header should be valid')
  })
  ...
  it('Header -> validate() -> success cases', async () => {
    ...
    assert.isTrue(true, 'should correctly validate subsequent SIP-1559 blocks')
  })
  ...
  it('Header -> calcNextBaseFee()', () => {
    for (let index = 0; index < sip1559baseFeeData.length; index++) {
      const item = sip1559baseFeeData[index]
