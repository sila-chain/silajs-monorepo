...
import type { BatchDBOp, DB } from '@silajs/util'
...
/**
 * The basic trie interface, use with `import { MerklePatriciaTrie } from '@silajs/mpt'`.
 *
...
export class MerklePatriciaTrie {
  ...
  constructor(opts?: MPTOpts) {
    if (opts?.valueEncoding !== undefined && opts.db === undefined) {
      throw SilaJSErrorWithoutCode('`valueEncoding` can only be set if a `db` is provided')
    }
    if (opts !== undefined) {
  ...
  database(db?: DB<string, string | Uint8Array>, valueEncoding?: ValueEncoding) {
    if (db !== undefined) {
      if (db instanceof CheckpointDB) {
        throw SilaJSErrorWithoutCode('Cannot pass in an instance of CheckpointDB')
      }
  ...
  async lookupNode(node: Uint8Array | Uint8Array[]): Promise<MPTNode> {
    ...
    if (value === null) {
      ...
      throw SilaJSErrorWithoutCode('Missing node in DB')
    }
  ...
  protected async _updateNode(
    ...
    if (!lastNode) {
      throw SilaJSErrorWithoutCode('Stack underflow')
    }
  ...
  protected async _deleteNode(keyBytes: Uint8Array, stack: MPTNode[]): Promise<void> {
    ...
    const collapseBranchWithOneChild = (
      ...
      if (!(parentNode instanceof ExtensionMPTNode)) {
        throw SilaJSErrorWithoutCode('Expected extension node')
      }
    ...
    if (lastNode === undefined) throw SilaJSErrorWithoutCode('missing last node')
    ...
    if (lastNode instanceof BranchMPTNode) {
      ...
      if (!(parentNode instanceof BranchMPTNode)) {
        throw SilaJSErrorWithoutCode('Expected branch node')
      }
  ...
  async saveStack(pathNibbles: Nibbles, stack: MPTNode[], opStack: BatchDBOp[]): Promise<void> {
    ...
    while (stack.length > 0) {
      ...
      if (node === undefined) {
        throw SilaJSErrorWithoutCode('saveStack: missing node')
      }
    ...
    for (const op of ops) {
      if (op.type === 'put') {
        if (op.value === null || op.value === undefined) {
          throw SilaJSErrorWithoutCode('Invalid batch db operation')
        }
  ...
  /** Applies key hashing (keccak) when useKeyHashing is enabled (Sila-style). */
  ...
  async commit(): Promise<void> {
    if (!this.hasCheckpoints()) {
      throw SilaJSErrorWithoutCode('trying to commit when not checkpointed')
    }
  ...
  async revert(): Promise<void> {
    if (!this.hasCheckpoints()) {
      throw SilaJSErrorWithoutCode('trying to revert when not checkpointed')
    }
