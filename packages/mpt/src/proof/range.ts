import { SilaJSErrorWithoutCode, equalsBytes } from '@silajs/util'
...
// reference: https://github.com/sila-chain/go-sila/blob/20356e57b119b4e70ce47665a71964434e15200d/trie/proof.go
...
async function unset(
  ...
  if (child instanceof BranchMPTNode) {
    ...
    throw SilaJSErrorWithoutCode('invalid node')
  }
}
...
async function unsetInternal(
  ...
  if (node instanceof ExtensionMPTNode || node instanceof LeafMPTNode) {
    ...
    if (shortForkLeft === -1 && shortForkRight === -1) {
      throw SilaJSErrorWithoutCode('invalid range')
    }
...
export async function verifyMerkleRangeProof(
  ...
  if (keys.length !== values.length) {
    throw SilaJSErrorWithoutCode('invalid keys length or values length')
  }

  // Make sure the keys are in order
  for (let i = 0; i < keys.length - 1; i++) {
    if (nibblesCompare(keys[i], keys[i + 1]) >= 0) {
      throw SilaJSErrorWithoutCode('invalid keys order')
    }
  }
  // Make sure all values are present
  for (const value of values) {
    if (value.length === 0) {
      throw SilaJSErrorWithoutCode('invalid values')
    }
  }
  ...
  if (proof === null && firstKey === null && lastKey === null) {
    ...
    if (!equalsBytes(rootHash, trie.root())) {
      throw SilaJSErrorWithoutCode('invalid all elements proof: root mismatch')
    }
    return false
  }
  ...
  if (proof !== null && firstKey !== null && lastKey === null) {
    ...
    if (keys.length === 0) {
      ...
      if (value !== null || (await hasRightElement(trie, firstKey))) {
        throw SilaJSErrorWithoutCode('invalid zero element proof: value mismatch')
      }
  ...
  if (keys.length === 1 && nibblesCompare(firstKey, lastKey) === 0) {
    ...
    if (value === null || !equalsBytes(value, values[0])) {
      throw SilaJSErrorWithoutCode('invalid one element proof: value mismatch')
    }
  ...
  if (!equalsBytes(trie.root(), rootHash)) {
    throw SilaJSErrorWithoutCode('invalid two edge elements proof: root mismatch')
  }
