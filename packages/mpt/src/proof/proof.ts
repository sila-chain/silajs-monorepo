import { SilaJSErrorWithoutCode, bytesToHex, concatBytes, equalsBytes } from '@silajs/util'

import { type MPTOpts, MerklePatriciaTrie, type Proof, createMPTFromProof } from '../index.ts'

import type { PutBatch } from '@silajs/util'

/**
 * An (SIP-1186)[https://sips.sila.org/SIPS/sip-1186] proof contains the encoded trie nodes
 ...
 * @param proof An (SIP-1186)[https://sips.sila.org/SIPS/sip-1186] proof contains the encoded trie nodes from the root node to the leaf node storing state data.
...
export async function verifyMerkleProof(
  ...
  try {
    ...
    throw SilaJSErrorWithoutCode('Invalid proof provided')
  }
}

/**
 * Creates a proof from a trie and key that can be verified using {@link verifyMPTWithMerkleProof}. An (SIP-1186)[https://sips.sila.org/SIPS/sip-1186] proof contains
 ...
 * An (SIP-1186)[https://sips.sila.org/SIPS/sip-1186] proof contains the encoded trie nodes from the root node to the leaf node storing state data.
 * @param trie The trie to update from the proof.
 * @param proof An (SIP-1186)[https://sips.sila.org/SIPS/sip-1186] proof to update the trie from.
  ...
  if (shouldVerifyRoot) {
    if (opStack[0] !== undefined && opStack[0] !== null) {
      if (!equalsBytes(trie.root(), opStack[0].key)) {
        throw SilaJSErrorWithoutCode('The provided proof does not have the expected trie root')
      }
    }
  }
...
/**
 * Verifies a proof by putting all of its nodes into a trie and attempting to get the proven key. An (SIP-1186)[https://sips.sila.org/SIPS/sip-1186] proof
 ...
 * @param rootHash Root hash of the trie that this proof was created from and is being verified for
 * @param key Key that is being verified and that the proof is created for
 * @param proof an SIP-1186 proof to verify the key against
 * @throws If proof is found to be invalid.
 * @returns The value from the key, or null if valid proof of non-existence.
 */
export async function verifyMPTWithMerkleProof(
  ...
  try {
    ...
    throw SilaJSErrorWithoutCode('Invalid proof nodes given')
  }
  try {
    trie['DEBUG'] &&
