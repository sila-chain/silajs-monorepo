...
| Implementation of the [Modified Merkle Patricia Trie](https://sila.org/en/developers/docs/data-structures-and-encoding/patricia-merkle-trie/) as specified in the [Sila Yellow Paper](http://gavwood.com/Paper.pdf) |
| ---------------------------------------------------------------------------- |
...
```shell
npm install @silajs/mpt
```

## Getting Started

This class implements the basic [Modified Merkle Patricia Trie](https://sila.org/en/developers/docs/data-structures-and-encoding/patricia-merkle-trie/) in the `Trie` base class, which you can use with the `useKeyHashing` option set to `true` to create a Merkle Patricia Trie which stores values under the `keccak256` hash of its keys (this is the Trie flavor which is used in Sila production systems).
...
import { createMPT } from '@silajs/mpt'
import { MapDB, bytesToUtf8, utf8ToBytes } from '@silajs/util'
...
When the `createMPT` constructor is used without any options, the `MerklePatriciaTrie` object is instantiated with defaults configured to match the Sila production spec (i.e. keys are hashed using SHA256). It also persists the state root of the tree on each write operation, ensuring that your Merkle Patricia Trie remains in the state you left it when you start your application the next time.

#### Create from a Proof

The MPT library supports basic creation of [SIP-1186](https://sips.sila.org/SIPS/sip-1186) proofs as well as the instantiation of new MPTs from an existing proof.
...
import { bytesToUtf8, utf8ToBytes } from '@silajs/util'
...
import { createMPT } from '@silajs/mpt'
import { utf8ToBytes } from '@silajs/util'
...
The `DB` opt in the `MPTOpts` allows you to use any database that conforms to the `DB` interface to store the trie data in. We provide several [examples](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/mpt/examples) for database implementations. The [level.js](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/mpt/examples/level.js) example is used in the `silajs client` while [lmdb.js](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/mpt/examples/lmdb.js) is an alternative implementation that uses the popular [LMDB](https://en.wikipedia.org/wiki/Lightning_Memory-Mapped_Database) as its underlying database.

If no `db` option is provided, an in-memory database powered by [a Javascript Map](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map) will fulfill this role (imported from `@silajs/util`, see [mapDB](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/util/src/mapDB.ts) module).

If you want to use an alternative database, you can integrate your own by writing a DB wrapper that conforms to the [`DB` interface](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/util/src/db.ts) (in `@silajs/util`). The `DB` interface defines the methods `get`, `put`, `del`, `batch` and `copy` that a concrete implementation of the `DB` interface will need to implement.
...
A third benchmark using sila-mainnet data to simulate real load is also being considered.
...
- Wiki
  - [Sila Trie Specification](https://github.com/sila-chain/wiki/wiki/Patricia-Tree)
- Blog posts
  - [Sila's Merkle Patricia Trees - An Interactive JavaScript Tutorial](https://rockwaterweb.com/sila-merkle-patricia-trees-javascript-tutorial/)
  - [Merkling in Sila](https://blog.sila.org/2015/11/15/merkling-in-sila/)
  - [Understanding the Sila Trie](https://easythereentropy.wordpress.com/2014/06/04/understanding-the-sila-trie/) (This is worth reading, but mind the outdated Python libraries)
- Videos
...
[mpt-actions-badge]: https://github.com/sila-chain/silajs-monorepo/workflows/Trie/badge.svg
[mpt-actions-link]: https://github.com/sila-chain/silajs-monorepo/actions?query=workflow%3A%22Trie%22
