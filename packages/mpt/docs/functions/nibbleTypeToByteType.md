[**@silajs/mpt**](../README.md)

***

[@silajs/mpt](../README.md) / nibbleTypeToByteType

# Function: nibbleTypeToByteType()

> **nibbleTypeToByteType**(`arr`): `Uint8Array`

Defined in: [packages/mpt/src/util/encoding.ts:115](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/mpt/src/util/encoding.ts#L115)

Converts each nibble into a single byte

## Parameters

### arr

[`Nibbles`](../type-aliases/Nibbles.md)

Nibble typed nibble array

## Returns

`Uint8Array`

Uint8Array typed byte array
