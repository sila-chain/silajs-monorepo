...
`@silajs/mpt` implements the Merkle Patricia Trie used by `@silajs/statemanager` and `@silajs/vm` for account and storage state. SilaAmsterdam execution touches many more state paths (BAL recording, state-gas accounting), but those semantics live in the SAVM/VM layers — within the `10.1.2` round this package is primarily a **version sync** so consumers can depend on a consistent `@silajs/*` set when experimenting with SilaAmsterdam.

### At a glance

- Monorepo version sync for `@silajs/*` `10.1.2` — no public API changes.
- Internal MPT class refactor and simplification, see PR [#4241](https://github.com/sila-chain/silajs-monorepo/pull/4241).

### SilaAmsterdam (experimental)

State reads and writes during SilaAmsterdam block execution flow through the same MPT interface as before. For BAL and state-gas behaviour see [@silajs/vm](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/vm#amsterdam-hardfork-experimental).
...
- Simplify rangeProof handling & flatten mpt file structure, PR [#3740](https://github.com/sila-chain/silajs-monorepo/pull/3740)
- Update level dependency to v9, PR [#3904](https://github.com/sila-chain/silajs-monorepo/pull/3904)
...
The trie package itself has been renamed, see PR [#3719](https://github.com/sila-chain/silajs-monorepo/pull/3719):

- `@silajs/trie` -> `@silajs/mpt`
...
The main class has been renamed to avoid confusion with Verkle, see PR [#3717](https://github.com/sila-chain/silajs-monorepo/pull/3717):

- `Trie` -> `MerklePatriciaTrie`

#### Static Constructors

The static constructors for our library classes have been reworked to now be standalone methods (with a similar naming scheme). This allows for better tree shaking of unused constructor code (see PRs [#3515](https://github.com/sila-chain/silajs-monorepo/pull/3515):

- `Trie.create()` -> `createMPT()`

#### Proof Functionality

Proof functionality also has been extracted from the trie class to make the core code base smaller, see PR [#3551](https://github.com/sila-chain/silajs-monorepo/pull/3551), also the names have been made more expressive to better distinguish between Merkle/Verkle, see PRs [#3557](https://github.com/sila-chain/silajs-monorepo/pull/3557), [#3718](https://github.com/sila-chain/silajs-monorepo/pull/3718) and [#3730](https://github.com/sila-chain/silajs-monorepo/pull/3730):
...
We have now removed the stream functionality and the associated dependency and replaced with a simpler async value map retrieval by using `trie.getValueMap()`, see PR [#3519](https://github.com/sila-chain/silajs-monorepo/pull/3519). For an example see the MPT [examples](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/mpt/examples) folder.

### Other Breaking Changes

- Refactor trie util helpers, PR [#3534](https://github.com/sila-chain/silajs-monorepo/pull/3534)
- Improve node typings and class architecture, PR [#3708](https://github.com/sila-chain/silajs-monorepo/pull/3708)
- Rename trie helpers to mpt, PR [#3718](https://github.com/sila-chain/silajs-monorepo/pull/3718)
...
- Address type issues and type-related improvements, PR [#3624](https://github.com/sila-chain/silajs-monorepo/pull/3624)
- Debug logger namespace standardization (use with `#` for the core logger, so e.g. `mpt:#`), PR [#3692](https://github.com/sila-chain/silajs-monorepo/pull/3692)
...
- Fixes an issue in the delete operation used for unhashed tries and pruning activated which resulted in a wrong state root (bad!), PR [#3333](https://github.com/sila-chain/silajs-monorepo/issues/3333)

## 6.2.0 - 2024-03-18

In the hope that you do not have yet integrated: we needed to remove the new more modern async trie iteration with web streams functionality (new `createAsyncReadStream()` method) introduced with the `v6.1.0` release - see PR [#3231](https://github.com/sila-chain/silajs-monorepo/pull/3231) for context - since the related Node.js web streams API import caused relatively severe problems for all upstream libraries when being used in the browser.

So functionality has been removed in PR [#3280](https://github.com/sila-chain/silajs-monorepo/pull/3280). We hope we find a good way to re-introduce along with the next breaking release round without limiting browser functionality.

### Other Changes

- Add optional `partialPath` parameter to `trie.findPath()`, PR [#3305](https://github.com/sila-chain/silajs-monorepo/pull/3305)
- Add export for `Path` interface for better TypeScript integration when using the `findPath()` method, PR [#3292](https://github.com/sila-chain/silajs-monorepo/pull/3292)
- Improved zero-element proof behavior in `verifyProof()` method, PR [#3047](https://github.com/sila-chain/silajs-monorepo/pull/3047)
- Fix a type error related to the `lru-cache` dependency, PR [#3285](https://github.com/sila-chain/silajs-monorepo/pull/3285)

## 6.1.1 - 2024-02-08

- Hotfix release adding a missing `debug` dependency, PR [#3271](https://github.com/sila-chain/silajs-monorepo/pull/3271)

## 6.1.0 - 2024-02-08

### Extended SIP-1186 Proof Functionality

This release complements on [SIP-1186](https://sips.sila.org/SIPS/sip-1186) proof functionality, see PR [#2949](https://github.com/sila-chain/silajs-monorepo/pull/2949) and [#3267](https://github.com/sila-chain/silajs-monorepo/pull/3267).

A new static `Trie.createFromProof()` constructor now allows for an easier instantiation of a trie given an SIP-1186 conformant proof, which can be created from an existing trie using the `trie.createProof()` method.
...
Also note that along with this release round there is additional higher level proof functionality available shipped within the `@silajs/statemanager` package.
...
The trie library now allows for using debug logging with the `DEBUG=ethjs,trie:*` flag on the command line as already being implemented in other SilaJS libraries, see PR [#3019](https://github.com/sila-chain/silajs-monorepo/pull/3019).

See [Debugging](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/trie#debugging) README section for usage instructions. This comes in pretty handy if in-depth trie analysis with step-by-step following of path reads is needed.

### Bugfixes

- Fix empty-root check, PR [#3001](https://github.com/sila-chain/silajs-monorepo/pull/3001)

### Other Changes

- New parameter `skipKeyTransform` (default: `false`) for Trie `put()`, `del()` and `batch()` method to allow to pass in already hashed keys, PR [#2950](https://github.com/sila-chain/silajs-monorepo/pull/2950)
- New `keyPrefix` option tries to store node keys with a static prefix (used upstream in the `statemanager` package to speed to storage trie reads), PR [#3023](https://github.com/sila-chain/silajs-monorepo/pull/3023)
- Performance: `findPath()` optimizations, PR [#3066](https://github.com/sila-chain/silajs-monorepo/pull/3066)
- Make `null` available as type option for `put()` method value, PR [#3020](https://github.com/sila-chain/silajs-monorepo/pull/3020)
- Allow partial trie options for `shallowCopy()` (e.g. for a more flexible cache configuration for the trie copy), PR [#3063](https://github.com/sila-chain/silajs-monorepo/pull/3063)
- Use `lock` class from `@silajs/util`, PR [#3109](https://github.com/sila-chain/silajs-monorepo/pull/3109)
- Improve util types and handling, PR [#2951](https://github.com/sila-chain/silajs-monorepo/pull/2951)
...
There is a new permanent trie node cache which can be leveraged to make Trie operations significantly faster, see PR [#2667](https://github.com/sila-chain/silajs-monorepo/pull/2667). Since this also increases base-memory usage of a trie instantiation, this new cache is mainly intended to be used in rather long-lived trie scenarios.
...
- Breaking: `DB` interface and `MapDB` implementation have been moved to [@silajs/util](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/util/src/db.ts) (for re-usage), PR [#2669](https://github.com/sila-chain/silajs-monorepo/pull/2669)
...
Maintenance release with dependency updates, PR [#2521](https://github.com/sila-chain/silajs-monorepo/pull/2521)
...
- Fixed a dependency issue when using `TrieReadStream`, PR [#2318](https://github.com/sila-chain/silajs-monorepo/pull/2318)
- Fixed a pruning verification issue for a case where only the root key is present as a key but not the root value, PR [#2296](https://github.com/sila-chain/silajs-monorepo/pull/2296)

## 5.0.0 - 2022-09-06

Final release - tada 🎉 - of a wider breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes for the main long change set description as well as the Beta 2, Beta 3 and Release Candidate (RC) 1 release notes for notes on some additional changes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/trie/CHANGELOG.md)).

### Trie Pruning

Some great last minute feature (thanks @faustbrian and @jochem-brouwer on this!) allowing to prune a trie on state root updates with a new `useNodePruning` option (default: `false`), see PR [#2203](https://github.com/sila-chain/silajs-monorepo/pull/2203). This allows for a much smaller DB footprint for the trie for use cases with frequent state root updates.

### Other Changes

- New `setCheckpoints(checkpoints: Checkpoint[])` method to support the manual setting of checkpoints for advanced use cases, PR [#2240](https://github.com/sila-chain/silajs-monorepo/pull/2240)
- Internal refactor: removed ambiguous boolean checks within conditional clauses, PR [#2249](https://github.com/sila-chain/silajs-monorepo/pull/2249)
...
For this reason, a single `root(hash?: Buffer): Buffer` function serves as a replacement for the previous `root` getter and setter and can effectively work to get and set properties, see PR [#2219](https://github.com/sila-chain/silajs-monorepo/pull/2219). This makes it obvious that you intend to modify an internal property of the trie that is neither accessible or mutable via any other means other than this particular function.
...
The `isCheckpoint` getter function has been removed, see PR [#2218](https://github.com/sila-chain/silajs-monorepo/pull/2218) The `hasCheckpoints()` function serves as its replacement and offers the same behaviour.
...
Prior to v5, this package shipped with a LevelDB integration out of the box. Finalized within this RC release round we have introduced a database abstraction and therefore no longer ship with the aforementioned LevelDB implementation, see previous Beta CHANGELOGs as well as PR [#2167](https://github.com/sila-chain/silajs-monorepo/pull/2167). However, for your convenience, we provide all of the necessary steps so that you can integrate it accordingly.

See our [Upgrade Guide](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/trie/UPGRADING.md) for more instructions on how to use the `Trie` library with LevelDB now.

### Other Changes

- Store `opts` in private property with defaults, PR [#2224](https://github.com/sila-chain/silajs-monorepo/pull/2224)
- **Potentially breaking:** Mark `db` as protected and rename to `_db` to avoid leaky properties, PR [#2221](https://github.com/sila-chain/silajs-monorepo/pull/2221)
- Replace `get/set` for `key/value` for nodes with function, PR [#2220](https://github.com/sila-chain/silajs-monorepo/pull/2220)
- Rename `persistRoot` to `useRootPersistence`, PR [#2223](https://github.com/sila-chain/silajs-monorepo/pull/2223)
...
- Use `micro-bmark` for benchmarks, PR [#2128](https://github.com/sila-chain/silajs-monorepo/pull/2128)
- Move `Trie#_findValueNodes()` function to `TrieReadStream`, PR [#2186](https://github.com/sila-chain/silajs-monorepo/pull/2186)
- Replaced `semaphore-async-await` with simpler implementation, PR [#2187](https://github.com/sila-chain/silajs-monorepo/pull/2187)
- Renamed `Semaphore` to `Lock`, PR [#2234](https://github.com/sila-chain/silajs-monorepo/pull/2234)

## 5.0.0-beta.3 - 2022-08-10

Beta 3 release for the upcoming breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes for the main long change set description as well as the Beta 2 release notes for notes on some additional changes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/devp2p/CHANGELOG.md)).

### Root Hash Persistence

The trie library now comes with a new constructor option `useRootPersistence` (note that the option has been called `persistRoot` up to Beta 3) which is disabled by default but allows to persist state root updates along write operations directly in the DB and therefore omits the need to manually set to a new state root, see PR [#2071](https://github.com/sila-chain/silajs-monorepo/pull/2071) and PR [#2123](https://github.com/sila-chain/silajs-monorepo/pull/2123), thanks to @faustbrian for the contribution! ❤️

To activate root hash persistence you can set the `useRootPersistence` option on instantiation:

```ts
import { Trie, LevelDB } from '@silajs/trie'
...
- Fix: Pass down a custom hash function for hashing on trie copies, PR [#2068](https://github.com/sila-chain/silajs-monorepo/pull/2068)

# 5.0.0-beta.2 - 2022-07-15

Beta 2 release for the upcoming breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/trie/CHANGELOG.md)) for the main change set description.
...
So if you use the Trie library together with other SilaJS libraries check if the respective imports need an update.

## Custom Hash Function

There is a new constructor option `hash` which allows to customize the hash function used for secure trie key hashing - see PR [#2043](https://github.com/sila-chain/silajs-monorepo/pull/2043) - thanks to @libotony for the great contribution on this! ❤️
...
**Attention!** This library release aligns (and therefore: changes!) the library name with the other SilaJS libraries and switches to the new scoped package name format, see PR [#1953](https://github.com/sila-chain/silajs-monorepo/pull/1953). In this case the library is renamed as follows:

- `merkle-patricia-tree` -> `@silajs/trie`
...
Options for the Trie constructor are now also taken in as an options dict like in the other SilaJS libraries. This makes it easier to add additional options in the future, see PR [#1874](https://github.com/sila-chain/silajs-monorepo/pull/1874).
...
The following deprecated or semi-private methods have been removed, see PR [#1874](https://github.com/sila-chain/silajs-monorepo/pull/1874) and PR [#1834](https://github.com/sila-chain/silajs-monorepo/pull/1834).
...
The trie source files have been reorganized to provide a more consistent and clean file and folder layout, see PR [#1972](https://github.com/sila-chain/silajs-monorepo/pull/1972). This might or might not affect you, depending if you use direct file references for importing (if you do you might want to generally switch to use root-level exports from the main `index.ts` file).
...
- New `Trie.verifyRangeProof()` function to check whether the given leaf nodes and edge proof can prove the given trie leaves range is matched with the specific root (useful for snapsync, thanks to @samlior for this generous code contribution ❤️), PR [#1731](https://github.com/sila-chain/silajs-monorepo/pull/1731)
...
- Adds try-catch for "Missing node in DB" in ReadStream, PR [#1515](https://github.com/sila-chain/silajs-monorepo/pull/1515)

## 4.2.1 - 2021-08-17

**Bug Fixes**

- Better error checking for invalid proofs with a differentiation on proofs of non-existence (`Trie.verifyProof()` returns `null`) and invalid proofs where `Trie.verifyProof()` will throw, see [README section](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/trie#merkle-proofs) for further details, PR [#1373](https://github.com/sila-chain/silajs-monorepo/pull/1373)

**Maintenance**

- Remove use of deprecated setRoot, PR [#1376](https://github.com/sila-chain/silajs-monorepo/pull/1376)
...
See: PR [#1219](https://github.com/sila-chain/silajs-monorepo/pull/1219)

## 4.1.0 - 2021-02-16

This release comes with a reworked checkpointing mechanism (PR [#1030](https://github.com/sila-chain/silajs-monorepo/pull/1030) and subsequently PR [#1035](https://github.com/sila-chain/silajs-monorepo/pull/1035)). Instead of copying over the whole DB on checkpoints the operations in between checkpoints are now recorded in memory and either applied in batch on a `Trie.checkpoint()` call or discarded along a `Trie.revert()`. This more fine-grained operational mode leads to a substantial performance gain (up to 50x) when working with larger tries.

Another performance related bug has been fixed along PR [#127](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/127) removing an unnecessary double-serialization call on nodes. This gives a general performance gain of 10-20% on putting new values in a trie.

Other changes:

## New Features

A new exported `WalkController` class has been added and `trie.walkTrie()` has been made a public method along. This allows for creating own custom ways to traverse a trie. PR [#135](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/135)

### Refactoring, Development and Documentation

- Better `Trie` code documentation, PR [#125](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/125)
- Internal `Trie` function reordering & partial retwrite, PR [#125](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/125)
- Added simple integrated profiling, PR [#128](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/128)
- Reworked benchmarking to be based on `benchmark.js`, basic CI integration, PR [#130](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/130)
- Upgrade to `silajs-config` `2.0` libs for linting and formatting, PR [#133](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/133)
- Switched coverage from `coverall` to `codecov`, PR [#137](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/137)
...
See the [docs](https://github.com/sila-chain/silajs/merkle-patricia-tree/tree/master/docs) for the latest Promise-based method signatures.
...
- Convert trieNode to ES6 class ([#71](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/71))
- Merge checkpoint and secure interface with their ES6 classes ([#73](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/73))
- Extract db-related methods from baseTrie ([#74](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/74))
- \_lookupNode callback to use standard error, response pattern ([#83](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/83))
- Accept leveldb in constructor, minor fixes ([#92](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/92))
- Refactor TrieNode, add levelup types ([#98](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/98))
- Promisify rest of library ([#107](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/107))
- Use `Nibbles` type for `number[]` ([#115](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/115))
- Upgrade silajs-util to 7.0.0 / Upgrade level-mem to 5.0.1 ([#116](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/116))
- Create dual ES5 and ES2017 builds ([#117](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/117))
- Include checkpoints by default in SecureTrie.copy ([#119](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/119))
- Rename Trie.prove to Trie.createProof ([#122](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/122))

### Added

- Support for proofs of null/absence. Dried up prove/verify. ([#82](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/82))
- Add more Sila state DB focused example accessing account values ([#89](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/89))

### Fixed

- Drop silajs-testing dep and fix bug in branch value update ([#69](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/69))
- Fix prove and verifyProof in SecureTrie ([#79](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/79))
- Fixed src code links in docs ([#93](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/93))

### Dev / Testing / CI

- Update tape to v4.10.1 ([#81](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/81))
- Org links and git hooks ([#87](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/87))
- Use module.exports syntax in util files ([#90](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/90))
- Rename deprecated sha3 consts and func to keccak256 ([#91](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/91))
- Migrate to Typescript ([#96](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/96))
- Fix Travis's xvfb service ([#97](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/97))
- Fix test cases and docs ([#104](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/104))
- Upgrade CI Provider from Travis to GH Actions ([#105](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/105))
- Upgrade test suite to TS ([#106](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/106))
- Better document `_formatNode` ([#109](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/109))
- Move `failingRefactorTests` to `secure.spec.ts` ([#110](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/110))
- Fix test suite typos ([#114](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/110))

[4.0.0]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/v3.0.0...v4.0.0
...
- Replace `levelup` 1.2.1 + `memdown` 1.0.0 with `level-mem` 3.0.1 and upgrade `level-ws` to 1.0.0, PR [#56](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/56)
- Support for `ES6` classes, PRs [#57](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/57), [#61](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/61)
- Updated `async` and `readable-stream` dependencies (resulting in smaller browser builds), PR [#60](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/60)
- Updated, automated and cleaned up [API documentation](https://github.com/sila-chain/silajs/merkle-patricia-tree/blob/master/docs/index.md) build, PR [#63](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/63)

[3.0.0]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/v2.3.2...v3.0.0

## [2.3.2] - 2018-09-24

- Fixed a bug in verify proof if the tree contains an extension node with an embedded branch node, PR [#51](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/51)
- Fixed `_scratch` 'leak' to global/window, PR [#42](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/42)
- Fixed coverage report leaving certain tests, PR [#53](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/53)

[2.3.2]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/v2.3.1...v2.3.2

## [2.3.1] - 2018-03-14

- Fix OutOfMemory bug when trying to create a read stream on large trie structures
  (e.g. a current state DB from a Geth node), PR [#38](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/38)
- Fix race condition due to mutated `_getDBs`/`_putDBs`, PR [#28](https://github.com/sila-chain/silajs/merkle-patricia-tree/pull/28)

[2.3.1]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/v2.3.0...v2.3.1

## [2.3.0] - 2017-11-30

- Methods for merkle proof generation `Trie.prove()` and verification `Trie.verifyProof()` (see [./proof.js](./proof.js))

[2.3.0]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/v2.2.0...v2.3.0
...
[2.2.0]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/v2.1.2...v2.2.0
...
[2.1.2]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/v2.1.1...v2.1.2
...
[2.1.1]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/2.0.3...v2.1.1

## [2.0.3] - 2015-09-24

- Initial, first of the currently released version on npm

[2.0.3]: https://github.com/sila-chain/silajs/merkle-patricia-tree/compare/1.1.x...2.0.3
