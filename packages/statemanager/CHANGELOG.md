...
`@silajs/statemanager` is the state persistence abstraction the VM uses for account/storage reads and writes — whether backed by an in-memory trie (`MerkleStateManager`), a cached RPC view, or other implementations. SilaAmsterdam increases how much state metadata is tracked during execution (BAL footprints, state-gas touches), but that tracking happens inside the SAVM/VM; the state manager interface itself is unchanged in the `10.1.2` round. Debug logging across packages now uses `@silajs/util`'s `isDebugEnabled()` internally, see PR [#4265](https://github.com/sila-chain/silajs-monorepo/pull/4265).
...
- Migrate `AccessWitness` from StateManager to SAVM, PR [#3770](https://github.com/sila-chain/silajs-monorepo/pull/3770)
- Small adjustments, PR [#3775](https://github.com/sila-chain/silajs-monorepo/pull/3775)
- Fix handling of storage values, PR [#3778](https://github.com/sila-chain/silajs-monorepo/pull/3778)
- Verkle SM storage fixes, PR [#3780](https://github.com/sila-chain/silajs-monorepo/pull/3780)

### Other Changes

- Experimental binary tree state manager, PR [#3885](https://github.com/sila-chain/silajs-monorepo/pull/3885)
...
We have renamed the previously called `DefaultStateManager` to a more neutral `MerkleStateManager`, see PR [#3641](https://github.com/sila-chain/silajs-monorepo/pull/3641), to reflect the rising importance of other state managers and align with future (at least) dual Merkle/Verkle state world:

- `DefaultStateManager` -> `MerkleStateManager`

#### Core State get/put Methods

The names for the core state manager methods to access and write state have been simplified, see PR [#3541](https://github.com/sila-chain/silajs-monorepo/pull/3541):
...
The following proof methods have been taken out of the core classes and made standalone-methods (tree shaking + keep core classes limited to core state functionality), see PR [#3672](https://github.com/sila-chain/silajs-monorepo/pull/3672):
...
There is a new abstraction layer for the account, code and storage caches for all state managers, called `Caches`, see PRs [#3554](https://github.com/sila-chain/silajs-monorepo/pull/3554), [#3569](https://github.com/sila-chain/silajs-monorepo/pull/3569) and [#3596](https://github.com/sila-chain/silajs-monorepo/pull/3596).
...
```ts
import { Caches, MerkleStateManager } from '@silajs/statemanager'
...
The [StateManagerInterface](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/common/src/interfaces.ts), which all state managers implement, is located in the `@silajs/common` package for re-usability reasons. Along the breaking release work, this interface as been strongly simplified, see PRs [#3543](https://github.com/sila-chain/silajs-monorepo/pull/3543) and [#3541](https://github.com/sila-chain/silajs-monorepo/pull/3541). A dedicated `EVMStateManagerInterface` has been removed, which allows for easier state manager usage within the SAVM package.
...
- Do not throw calling `getContractStorage()` on non-existing accounts, PR [#3536](https://github.com/sila-chain/silajs-monorepo/pull/3536)
...
- Switch `js-sdsl` to `js-sdsl/orderedMap` sub package, PR [#3528](https://github.com/sila-chain/silajs-monorepo/pull/3528)

## 2.4.0 - 2024-08-15

### Verkle Updates

- Various fixes for Kaustinen4 support (partial account integration, `getContractCodeSize()`, other), PR [#3269](https://github.com/sila-chain/silajs-monorepo/pull/3269)
...
- Modify RPCStateManager `getAccount()`, PR [#3345](https://github.com/sila-chain/silajs-monorepo/pull/3345)

### Bugfixes

- Fixes an issue where under certain deployment conditions wrong storage values could be provided, PR [#3434](https://github.com/sila-chain/silajs-monorepo/pull/3434)
- Fixes statemanager empty code bug, PR [#3483](https://github.com/sila-chain/silajs-monorepo/pull/3483)
...
- Properly apply statemanager `opts` in `fromProof()`, PR [#3276](https://github.com/sila-chain/silajs-monorepo/pull/3276)
- New optional `getAppliedKey()` method for the interface (see interface definition in `@silajs/common`), PR [#3143](https://github.com/sila-chain/silajs-monorepo/pull/3143)
- Fix inconsistency between the normal and the RPC statemanager regarding empty account return values, PR [#3323](https://github.com/sila-chain/silajs-monorepo/pull/3323)
- Fix a type error related to the `lru-cache` dependency, PR [#3285](https://github.com/sila-chain/silajs-monorepo/pull/3285)
- Add tests for verkle statemanager, PR [#3257](https://github.com/sila-chain/silajs-monorepo/pull/3257)

## 2.2.2 - 2024-02-08

- Hotfix release moving the `@silajs/verkle` dependency from a peer dependency to the main dependencies (note that this decision might be temporary)
...
Coming with the work from PR [#3186](https://github.com/sila-chain/silajs-monorepo/pull/3186) it is now possible to instantiate a new state manager from an [SIP-1186](https://sips.sila.org/SIPS/sip-1186) conformant proof with the new `DefaultStateManager.fromProof()` static constructor.
...
See trie [README](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/statemanager#instantiating-from-a-proof) for a comprehensive example on this.

### EthersStateManager -> RPCStateManager

This release replaces the specific `EthersStateManager`, which can be used to RPC-retrieve state data for (still somewhat experimental) on-chain block execution, with a more generic `RPCStateManager` (which still can be used well in conjunction with Ethers, dependency has been removed though), see PR [#3167](https://github.com/sila-chain/silajs-monorepo/pull/3167).

This new `RPCStateManager` can now be used with any type of JSON-RPC provider that supports the `sil` namespace (e.g. an Infura endpoint). See [README](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/statemanager#rpcstatemanager) for an example on how to use the extended provider capabilities.
...
- Export `originalStorageCache` to ease implementation of own state managers, PR [#3161](https://github.com/sila-chain/silajs-monorepo/pull/3161)
- This release integrates a new `StatelessVerkleStateManager`. This code is still very experimental and so do not tell anyone 😋, but if you dug so deep and found this note here you are likely eligible for early testing and experimentation, PRs [#3139](https://github.com/sila-chain/silajs-monorepo/pull/3139) and [#3179](https://github.com/sila-chain/silajs-monorepo/pull/3179)

## 2.1.0 - 2023-10-23

### New Diff-Based Code Cache

This release introduces a new code cache implementation, see PR [#3022](https://github.com/sila-chain/silajs-monorepo/pull/3022) and [#3080](https://github.com/sila-chain/silajs-monorepo/pull/3080). The new cache complements the expanded account and storage caches and now also tracks stored/deployed-code-changes along commits and reverts and so only keeps code in the cache which made it to the final state change.
...
This release introduces a new option `prefixStorageTrieKeys` which triggers the underlying trie to store storage key values with a prefix based on the account address, see PR [#3023](https://github.com/sila-chain/silajs-monorepo/pull/3023). This significantly increases performance for consecutive storage accesses for the same account on especially larger tries, since trie node accesses get noticeably faster when performed by the underlying key-value store since values are stored close to each other.
...
- Fix for `dumpStorage()` for `EthersStateManager`, PR [#3009](https://github.com/sila-chain/silajs-monorepo/pull/3009)

### Other Changes

- Allow for users to decide if to either downlevel (so: adopt them for a short-lived scenario) caches or not on `shallowCopy()` by adding a new `downlevelCaches` parameter (default: `true`), PR [#3063](https://github.com/sila-chain/silajs-monorepo/pull/3063)
- Return zero values for `getProof()` as `0x0`, PR [#3038](https://github.com/sila-chain/silajs-monorepo/pull/3038)
...
See [RC1 release notes](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fstatemanager%402.0.0-rc.1) for the main change description.

Following additional changes since RC1:

- `Breaking`: new `dumpStorageRangeAt()` implementation + EVMStateManager interface addition (in @silajs/common), PR [#2922](https://github.com/sila-chain/silajs-monorepo/pull/2922)
...
This release comes with a significantly more elaborate caching mechanism for account and storage caches, see PR [#2630](https://github.com/sila-chain/silajs-monorepo/pull/2634), [#2634](https://github.com/sila-chain/silajs-monorepo/pull/2634) and [#2618](https://github.com/sila-chain/silajs-monorepo/pull/2618).
...
Also along PR [#2630](https://github.com/sila-chain/silajs-monorepo/pull/2630) and [#2634](https://github.com/sila-chain/silajs-monorepo/pull/2634): the StateManager API has been significantly cleaned up with one of the major changes being `getAccount()` not returning an empty account any more if no result was found. While this needs some adoption this one single change makes state handling a lot cleaner.
...
The `StateManagerInterface` has now been moved to the `@sila/common` package for more universal access and should be loaded from there with:

```ts
import type { StateManagerInterface } from '@silajs/common'
```
...
```ts
// statemanager / StateManagerInterface (in @silajs/common)
...
- `EthersStateManager` now uses `Ethers` `v6`, PR [#2720](https://github.com/sila-chain/silajs-monorepo/pull/2720)
- Integrate an `OriginalStorage` cache, other VM/SAVM EEI refactoring related changes, PR [#2702](https://github.com/sila-chain/silajs-monorepo/pull/2702)
...
- Breaking: `StateManager._common` property has been renamed to `StateManager.common` and made public, PR [#2857](https://github.com/sila-chain/silajs-monorepo/pull/2857)
...
Added `EthersStateManager` to direct exports (if you use please fix our deep imports), see PR [#2419](https://github.com/sila-chain/silajs-monorepo/pull/2419).

Import is now simplified to:

```ts
import { EthersStateManager } from '@silajs/statemanager'
```
...
There is a new dedicated state manager `EthersStateManager` added to the library. This new state manager gets its state via Ethers RPC calls and allows e.g. for a stateless execution of selected sila-mainnet (or other Sila network) blocks and transactions, see PR [#2315](https://github.com/sila-chain/silajs-monorepo/pull/2315).
...
- Performance is rather slow (when using a remote provider for state data), particularly for more recent `sila-mainnet` blocks
- The API of this new state manager might change in future (bugfix or minor) releases

This should nevertheless be useful for a certain number of use cases, if there is e.g. the need for some quick analysis of certain sila-mainnet (SAVM) behavior e.g.. The `StateManager` package [README](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/statemanager#ethersstatemanager) contains a new dedicated section on how to use the new state manager.
...
- \*\*Attention:" Removed unused `common` option from `StateManager`, PR [#2197](https://github.com/sila-chain/silajs-monorepo/pull/2197)
- Added `prefixCodeHashes` flag defaulting to `true` which allows to deactivate prefix code hashes saved in the DB (see PR [#1438](https://github.com/sila-chain/silajs-monorepo/pull/1438) for context), PR [#2179](https://github.com/sila-chain/silajs-monorepo/pull/2179)
...
- Upgrades the `@silajs/trie` library to Beta 3 which allows to pass in a custom hash function/library (for performance), PR [#2043](https://github.com/sila-chain/silajs-monorepo/pull/2043)

## 1.0.0-beta.2 - 2022-07-15

Beta 2 release for the upcoming breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/statemanager/CHANGELOG.md)) for the main change set description.
...
```ts
import DefaultStateManager from '@silajs/statemanager'
```
...
The `StateManager` has been extracted from the `VM` and is now a separate package, see PR [#1817](https://github.com/sila-chain/silajs-monorepo/pull/1817). The new package can be installed separately with:

```shell
npm i @silajs/statemanager
```

The `@silajs/vm` package still has this package added as a dependency and it is automatically integrated. The `StateManager` provides a high-level interface to an underlying state storage solution. This is classically a `Trie` (in our case: an `@silajs/trie`) instance, but can also be something else, e.g. a plain database, an underlying RPC connection or a Verkle Tree in the future.
...
A `StateManager` must adhere to a predefined interface `StateManager` and implement a certain set of state access methods like `getAccount()`, `putContractCode()`,... Such an implementation is then guaranteed to work e.g. in the `@silajs/vm` implementation.
...
- `touchAccount()` (SAVM-specific, remained in `EVMStateAccess` interface in SAVM)
- All methods from `SIP2929StateManager` (removed as separate interface) (SAVM-specific, remained in `EVMStateAccess` interface in SAVM)
- `getOriginalContractStorage()` (SAVM-specific, remained in `EVMStateAccess` interface in SAVM)
- `hasGenesisState()` (removed)
- `generateGenesis()` (removed)
- `generateCanonicalGenesis()` (SAVM-specific, remained in `EVMStateAccess` interface in SAVM)
- `cleanupTouchedAccounts()` (SAVM-specific, remained in `EVMStateAccess` interface in SAVM)
- `clearOriginalStorageCache()` (SAVM-specific, remained in `EVMStateAccess` interface in SAVM)
