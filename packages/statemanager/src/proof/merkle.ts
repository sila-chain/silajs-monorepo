...
import type { Proof, StorageProof } from '@silajs/common'
...
export async function verifyMerkleStateProof(
  ...
  if (value === null) {
    ...
    if (!equalsBytes(nonce, emptyBytes)) {
      throw SilaJSErrorWithoutCode(`${notEmptyErrorMsg} (nonce is not zero)`)
    }
    const balance = unpadBytes(hexToBytes(proof.balance))
    if (!equalsBytes(balance, emptyBytes)) {
      throw SilaJSErrorWithoutCode(`${notEmptyErrorMsg} (balance is not zero)`)
    }
    ...
    if (nonce !== BigInt(proof.nonce)) {
      throw SilaJSErrorWithoutCode(`${invalidErrorMsg} nonce does not match`)
    }
    if (balance !== BigInt(proof.balance)) {
      throw SilaJSErrorWithoutCode(`${invalidErrorMsg} balance does not match`)
    }
    if (!equalsBytes(storageRoot, hexToBytes(proof.storageHash))) {
      throw SilaJSErrorWithoutCode(`${invalidErrorMsg} storageHash does not match`)
    }
    if (!equalsBytes(codeHash, hexToBytes(proof.codeHash))) {
      throw SilaJSErrorWithoutCode(`${invalidErrorMsg} codeHash does not match`)
    }
  }
