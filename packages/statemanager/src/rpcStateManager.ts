...
import type { AccountFields, StateManagerInterface, StorageDump } from '@silajs/common'
...
export class RPCStateManager implements StateManagerInterface {
  ...
  constructor(opts: RPCStateManagerOpts) {
    ...
    if (typeof opts.provider === 'string' && opts.provider.startsWith('http')) {
      ...
      throw SilaJSErrorWithoutCode(`valid RPC provider url required; got ${opts.provider}`)
    }
   ...
   */
  hasStateRoot = () => {
    throw SilaJSErrorWithoutCode('function not implemented')
  }
}
...
export class RPCBlockChain {
  ...
  constructor(provider: string) {
    if (provider === undefined || provider === '')
      throw SilaJSErrorWithoutCode('provider URL is required')
