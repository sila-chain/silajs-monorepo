...
import type { Address, DB } from '@silajs/util'
...
export class MerkleStateManager implements StateManagerInterface {
  ...
  constructor(opts: MerkleStateManagerOpts = {}) {
    ...
    this.common = opts.common ?? new Common({ chain: SilaMainnet })
  ...
  /**
   * Gets the storage trie for the SAVM-internal account identified by the provided address/hash.
  ...
  async getStorage(address: Address, key: Uint8Array): Promise<Uint8Array> {
    if (key.length !== 32) {
      throw SilaJSErrorWithoutCode('Storage key must be 32 bytes long')
    }
  ...
  async putStorage(address: Address, key: Uint8Array, value: Uint8Array): Promise<void> {
    ...
    if (value.length > 32) {
      throw SilaJSErrorWithoutCode('Storage value cannot be longer than 32 bytes')
    }

    const account = await this.getAccount(address)
    if (!account) {
      throw SilaJSErrorWithoutCode('putStorage() called on non-existing account')
    }
  ...
  async setStateRoot(stateRoot: Uint8Array, clearCache: boolean = true): Promise<void> {
    ...
    if (!equalsBytes(stateRoot, this._trie.EMPTY_TRIE_ROOT)) {
      ...
      if (!hasRoot) {
        throw SilaJSErrorWithoutCode('State trie does not contain state root')
      }
    }
  ...
  async dumpStorage(address: Address): Promise<StorageDump> {
    ...
    const account = await this.getAccount(address)
    if (!account) {
      throw SilaJSErrorWithoutCode(`dumpStorage f() can only be called for an existing account`)
    }
    const trie = this._getStorageTrie(address, account)
  ...
  async dumpStorageRange(address: Address, startKey: bigint, limit: number): Promise<StorageRange> {
    if (!Number.isSafeInteger(limit) || limit < 0) {
      throw SilaJSErrorWithoutCode(`Limit is not a proper uint.`)
    }

    await this.flush()
    const account = await this.getAccount(address)
    if (!account) {
      throw SilaJSErrorWithoutCode(`Account does not exist.`)
    }
  ...
  async generateCanonicalGenesis(initState: any): Promise<void> {
    if (this._checkpointCount !== 0) {
      throw SilaJSErrorWithoutCode('Cannot create genesis state with uncommitted checkpoints')
    }
    if (this.DEBUG) {
