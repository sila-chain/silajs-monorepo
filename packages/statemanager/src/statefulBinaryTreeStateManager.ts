import { BinaryTree } from '@silajs/binarytree'
import { BinaryTreeAccessedStateType } from '@silajs/common'
import { RLP } from '@silajs/rlp'
import type { Address, BinaryTreeExecutionWitness, PrefixedHexString } from '@silajs/util'
import {
  Account,
  ...
  implements StateManagerInterface, BinaryTreeStateManagerInterface
{
  ...
  constructor(opts: StatefulBinaryTreeStateManagerOpts) {
    ...
    if (opts.common?.isActivatedEIP(7864) === false) {
      throw SilaJSErrorWithoutCode('SIP-7864 required for binary tree state management')
    }
  ...
  getCode = async (address: Address): Promise<Uint8Array> => {
    ...
    for (let x = 0; x < chunks.length; x++) {
      if (chunks[x] === undefined)
        throw SilaJSErrorWithoutCode(`expected code chunk at index ${x}, got undefined`)
   ...
   * (e.g. execution witness generation in `@silajs/savm`).
   */
  get tree(): BinaryTree {
    return this._tree
  }
  ...
  dumpStorage?(_address: Address): Promise<StorageDump> {
    throw SilaJSErrorWithoutCode('Method not implemented.')
  }
