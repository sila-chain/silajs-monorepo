import { Chain, ChainNameFromNumber } from '@silajs/common'
import { getGenesis } from '@silajs/genesis'
import { Account, Address, hexToBytes, utf8ToBytes } from '@silajs/util'
...
describe('stateManager', () => {
  it(`should generate the genesis state root correctly for sila-mainnet from common`, async () => {
    if (isBrowser() === true) {
      return
    }
    ...
    await stateManager.generateCanonicalGenesis(getGenesis(Chain.SilaMainnet))
    const stateRoot = await stateManager.getStateRoot()

    assert.deepEqual(
      stateRoot,
      expectedStateRoot,
      `generateCanonicalGenesis should produce correct state root for sila-mainnet from common`,
    )
  }, 30_000)
