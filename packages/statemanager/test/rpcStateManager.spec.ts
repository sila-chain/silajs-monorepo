import { createBlockFromJSONRPCProvider, createBlockFromRPC } from '@silajs/block'
import { Common, Hardfork, SilaMainnet } from '@silajs/common'
import { createEVM } from '@silajs/savm'
import type { EVMMockBlockchainInterface, EVMRunCallOpts } from '@silajs/savm'
import { verifyMerkleProof } from '@silajs/mpt'
import { createFeeMarket1559Tx, createTxFromRPC } from '@silajs/tx'
import {
  Address,
  bigIntToBytes,
  bytesToHex,
...
import { createVM, runBlock, runTx } from '@silajs/vm'
...
// `PROVIDER=https://sila-mainnet.infura.io/v3/[mySuperS3cretproviderKey] npx vitest run test/rpcStateManager.spec.ts

describe('RPC State Manager initialization tests', async () => {
  vi.mock('@silajs/util', async () => {
    const util = await vi.importActual('@silajs/util')
    return {
      ...util,
...
describe('RPC State Manager API tests', () => {
  it('should work', async () => {
    ...
    assert.isTrue(account!.nonce > 0n, 'Vitalik.sil returned a valid nonce')
    ...
    assert.isTrue(retrievedVitalikAccount.nonce > 0n, 'Vitalik.sil is stored in cache')
    ...
    assert.isDefined(state.getAccount(vitalikDotEth), 'vitalik.sil does exist')
    ...
    assert.isDefined(
      ...
      'UNI SRC20 contract code was found in cache',
    )
...
describe('runTx custom transaction test', () => {
  it('should work', async () => {
    ...
    assert.strictEqual(result.totalGasSpent, 21000n, 'sent some SIL to vitalik.sil')
  })
})

describe('runTx test: replay sila-mainnet transactions', () => {
  it('should work', async () => {
    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.London })
...
describe('blockchain', () =>
  it('uses blockhash', async () => {
    ...
    const savm = await createEVM({ blockchain, stateManager: state })
    ...
    await savm.stateManager.setStateRoot(
    ...
    await savm.stateManager.putCode(contractAddress, hexToBytes(code))
