...
function evp_kdf(data: Uint8Array, salt: Uint8Array, opts?: Partial<EvpKdfOpts>) {
  ...
  function iter(block: Uint8Array) {
    if (params.digest !== 'md5')
      throw SilaJSErrorWithoutCode('Only md5 is supported in evp_kdf')
    let hash = md5.create()
    hash.update(block)
    hash.update(data)
    hash.update(salt)
...
export interface SilaWalletOptions {
  address: string
  encrypted: boolean
  locked: boolean
  hash: string
...
/*
 * Third Party API: Import a wallet generated by SilaWallet
 * This wallet format is created by https://github.com/SilentCicero/silajs-accounts
 * and used on https://www.myetherwallet.com/
 */
export async function fromEtherWallet(
  input: string | SilaWalletOptions,
  password: string,
): Promise<Wallet> {
  const json: SilaWalletOptions = typeof input === 'object' ? input : JSON.parse(input)

  let privateKey: Uint8Array
  if (!json.locked) {
    if (json.private.length !== 64) {
      throw SilaJSErrorWithoutCode('Invalid private key length')
    }
    privateKey = unprefixedHexToBytes(json.private)
  } else {
    if (typeof password !== 'string') {
      throw SilaJSErrorWithoutCode('Password required')
    }

    if (password.length < 7) {
      throw SilaJSErrorWithoutCode('Password must be at least 7 characters')
    }
    ...
    if (!cipher.salt) {
      throw SilaJSErrorWithoutCode('Unsupported SilaWallet key format')
    }
  ...
  if (wallet.getAddressString() !== json.address) {
    throw SilaJSErrorWithoutCode('Invalid private key or address')
  }
  return wallet
}
...
export function fromQuorumWallet(passphrase: string, userid: string): Wallet {
  if (passphrase.length < 10) {
    throw SilaJSErrorWithoutCode('Passphrase must be at least 10 characters')
  }
  if (userid.length < 10) {
    throw SilaJSErrorWithoutCode('User id must be at least 10 characters')
  }
