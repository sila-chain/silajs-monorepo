...
function validateHexString(paramName: string, str: string, length?: number) {
  ...
  if ((length as number) % 2) {
    throw SilaJSErrorWithoutCode(`Invalid length argument, must be an even number`)
  }
...
function validateBytes(paramName: string, bytes: Uint8Array, length?: number) {
  ...
  if (typeof length === 'number' && bytes.length !== length) {
    throw SilaJSErrorWithoutCode(`Invalid ${paramName}, Uint8Array must be ${length} bytes`)
  }
  return bytes
}
...
// https://github.com/sila-chain/homestead-guide/blob/master/old-docs-for-reference/go-sila-wiki.rst/Passphrase-protected-key-store-spec.rst
...
// https://github.com/sila-chain/wiki/wiki/Web3-Secret-Storage-Definition
...
export class Wallet {
  ...
  constructor(privateKey?: Uint8Array, publicKey?: Uint8Array) {
    ...
    if (publicKey && !isValidPublic(publicKey)) {
      throw SilaJSErrorWithoutCode('Invalid public key')
    }
  }
  ...
  public static fromExtendedPublicKey(extendedPublicKey: string): Wallet {
    if (extendedPublicKey.slice(0, 4) !== 'xpub') {
      throw SilaJSErrorWithoutCode('Not an extended public key')
    }
    const publicKey: Uint8Array = bs58check.decode(extendedPublicKey).subarray(45)
    // Convert to an Sila public key
  ...
  public static fromExtendedPrivateKey(extendedPrivateKey: string): Wallet {
    if (extendedPrivateKey.slice(0, 4) !== 'xprv') {
      throw SilaJSErrorWithoutCode('Not an extended private key')
    }
    const tmp: Uint8Array = bs58check.decode(extendedPrivateKey)
    if (tmp[45] !== 0) {
      throw SilaJSErrorWithoutCode('Invalid extended private key')
    }
  ...
  public static async fromV1(input: string | V1Keystore, password: string): Promise<Wallet> {
    ...
    if (json.Version !== '1') {
      throw SilaJSErrorWithoutCode('Not a V1 Wallet')
    }
    if (json.Crypto.KeyHeader.Kdf !== 'scrypt') {
      throw SilaJSErrorWithoutCode('Unsupported key derivation scheme')
    }
    ...
    if (bytesToUnprefixedHex(mac) !== json.Crypto.MAC) {
      throw SilaJSErrorWithoutCode('Key derivation failed - possibly wrong passphrase')
    }
  ...
  public static async fromV3(
    ...
    if (json.version !== 3) {
      throw SilaJSErrorWithoutCode('Not a V3 wallet')
    }
    ...
    if (json.crypto.kdf === 'scrypt') {
      ...
      if (kdfparams.prf !== 'hmac-sha256') {
        throw SilaJSErrorWithoutCode('Unsupported parameters to PBKDF2')
      }
  ...
  /*
   * Import an Sila Pre Sale wallet.
   * Based on https://github.com/sila-chain/pyethsaletool/blob/master/pyethsaletool.py
   * JSON fields: encseed, ethaddr, btcaddr, email
   *
   * @param input A JSON serialized string, or an object representing SilSale Keystore.
   * @param password The keystore password.
   */
  public static async fromEthSale(
    input: string | SilSaleKeystore,
    password: string,
  ): Promise<Wallet> {
    const json: SilSaleKeystore = typeof input === 'object' ? input : JSON.parse(input)
    ...
    if (bytesToUnprefixedHex(wallet.getAddress()) !== json.ethaddr) {
      throw SilaJSErrorWithoutCode('Decoded key mismatch - possibly wrong passphrase')
    }
    return wallet
  }
   ...
   * @param opts The options for the keystore. See [its spec](https://github.com/sila-chain/wiki/wiki/Web3-Secret-Storage-Definition) for more info.
   */
  public async toV3(password: string, opts?: Partial<V3Params>): Promise<V3Keystore> {
    ...
    switch (v3Params.kdf) {
      ...
      case KDFFunctions.Scrypt:
        ...
        break
      default:
        throw SilaJSErrorWithoutCode('Unsupported kdf')
    }
