...
import { SilaHDKey } from '../src/hdkey.ts'
...
const fixturehd = SilaHDKey.fromMasterSeed(fixtureseed)
...
describe('HD Key tests', () => {
  it('.fromMasterSeed()', () => {
    assert.doesNotThrow(function () {
      SilaHDKey.fromMasterSeed(fixtureseed)
    })
  }, 30000)

  it('.fromMnemonic()', () => {
    const path = "m/44'/60'/0'/0/0"
    const hdWallet = SilaHDKey.fromMnemonic(fixtureMnemonic)
  ...
  it('.fromExtendedKey()', () => {
    const onlyPublicExtendedKey = SilaHDKey.fromExtendedKey(
      'xpub661MyMwAqRbcGout4B6s29b6gGQsowyoiF6UgXBEr7eFCWYfXuZDvRxP9zEh1Kwq3TLqDQMbkbaRpSnoC28oWvjLeshoQz1StZ9YHM1EpcJ',
    )
    assert.deepEqual(
    ...
    const fullExtendedKey = SilaHDKey.fromExtendedKey(
      'xprv9s21ZrQH143K4KqQx9Zrf1eN8EaPQVFxM2Ast8mdHn7GKiDWzNEyNdduJhWXToy8MpkGcKjxeFWd8oBSvsz4PCYamxR7TX49pSpp3bmHVAY',
    )
    assert.deepEqual(
  ...
  it('.getWallet()', () => {
    ...
    const hdnode = SilaHDKey.fromExtendedKey(
      'xpub661MyMwAqRbcGout4B6s29b6gGQsowyoiF6UgXBEr7eFCWYfXuZDvRxP9zEh1Kwq3TLqDQMbkbaRpSnoC28oWvjLeshoQz1StZ9YHM1EpcJ',
    )
