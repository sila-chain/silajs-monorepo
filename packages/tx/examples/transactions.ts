...
import { createLegacyTx, createLegacyTxFromBytesArray } from '@silajs/tx'
import type { PrefixedHexString } from '@silajs/util'
import { bytesToHex, hexToBytes, randomBytes } from '@silajs/util'

// We create an unsigned transaction.
// Notice we don't set the `to` field because we are creating a new contract.
// This transaction's chain is set to sila-mainnet.
...
const tx2 = createLegacyTxFromBytesArray(rawTx.map(hexToBytes)) // This is also a sila-mainnet transaction
