import { Common, Hardfork, SilaSepolia } from '@silajs/common'
import { createBlob4844Tx } from '@silajs/tx'
import type { PrefixedHexString } from '@silajs/util'
import { Units, bytesToHex, getBlobs, hexToBytes, randomBytes } from '@silajs/util'
import { trustedSetup } from '@paulmillr/trusted-setups/fast-peerdas.js'
import { KZG as microEthKZG } from 'micro-sil-signer/kzg.js'

/**
 * This example is optimized to send a raw SilaJS tx to the SilaSepolia network.
 ...
 * (Ethers or viem e.g.).
 *
 * Usage   : node examples/sendRawSilaSepoliaTx.ts <PRIVATE_KEY>
 * Example : node examples/sendRawSilaSepoliaTx.ts 0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef
 *
 * Full curl command to send the tx:
 * node examples/sendRawSilaSepoliaTx.ts <PRIVATE_KEY> | curl -X POST -H "Content-Type: application/json" -d @- https://sila-sepolia-rpc.publicnode.com
 */
...
const common = new Common({ chain: SilaSepolia, hardfork: Hardfork.SilaOsaka, customCrypto: { kzg } })
...
const txData = {
  ...
  value: 0,
  to,
  blobs: getBlobs('This is a beautiful SilaJS blob ❤️'),
}
