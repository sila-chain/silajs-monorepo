...
beforeAll(async () => {
  ...
  const jsKzgSetup = {
    lib: jsKzg,
    label: 'JS',
    common: createCommonFromGethGenesis(sip4844GethGenesis, {
      chain: 'customChain',
...
describe('SIP4844 addSignature tests', () => {
...
describe('SIP4844 constructor tests - valid scenarios', () => {
  it('should work', () => {
    for (const kzg of kzgs) {
      const txData = {
        type: 0x03,
...
describe('SIP4844 constructor tests - invalid scenarios', () => {
  it('should work', () => {
    for (const kzg of kzgs) {
      const baseTxData = {
        type: 0x03,
      ...
      assert.isTrue(commonWithEIP7594.isActivatedEIP(7594), 'SIP-7594 should be activated')
      try {
        createBlob4844Tx({ ...baseTxData, ...tooManyBlobs7 }, { common: commonWithEIP7594 })
      } catch (err: any) {
        assert.isTrue(
          err.message.includes('7 blobs exceeds max 6 blobs per tx (SIP-7594)'),
