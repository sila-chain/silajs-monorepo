import { Common, Hardfork, SilaMainnet, SilaSepolia, createCustomCommon } from '@silajs/common'
...
describe('[Transaction]', () => {
  ...
  it('Initialization', () => {
    const nonEIP2930Common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Istanbul })
    assert.isDefined(
      createLegacyTx({}, { common: nonEIP2930Common }),
      'should initialize on a pre-Berlin Hardfork (SIP-2930 not activated)',
    )
    ...
    common = createCustomCommon({ chainId: 999 }, SilaMainnet)
  ...
  it('Initialization -> throws when creating a a transaction with incompatible chainid and v value', () => {
    ...
    common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Petersburg })
    assert.throws(() => createLegacyTxFromRLP(serialized, { common }))
  })

  it('Initialization -> throws if v is set to an SIP155-encoded value incompatible with the chain id', () => {
    assert.throws(() => {
      const common = new Common({ chain: SilaSepolia, hardfork: Hardfork.Petersburg })
  ...
  it('addSignature() -> correctly adds correct signature values from ecrecover with ChainID protection enabled', () => {
    const privKey = hexToBytes(`0x${txsData[0].privateKey}`)
    const tx = createLegacyTx({}, { common: new Common({ chain: SilaSepolia }) })
    const signedTx = tx.sign(privKey)
  ...
  it('addSignature() -> throws when adding the wrong v value', () => {
    ...
    const tx = createLegacyTx({}, { common: new Common({ chain: SilaSepolia }) })
    const signedTx = tx.sign(privKey)
    // `convertV` set to true: this will apply SIP-155 replay transaction twice, so it should throw!
    assert.throws(() => {
  ...
  it("getHashedMessageToSign(), getSenderPublicKey() (implicit call) -> verify SIP155 signature based on Vitalik's tests", () => {
  ...
  it('getHashedMessageToSign(), sign(), getSenderPublicKey() (implicit call) -> verify SIP155 signature before and after signing', () => {
    // Inputs and expected results for this test are taken directly from the example in https://sips.sila.org/SIPS/sip-155
    const txRaw = [
      '0x09',
      '0x4a817c800',
      '0x5208',
  ...
  it('sign(), getSenderPublicKey() (implicit call) -> SIP155 hashing when singing', () => {
  ...
  it('sign(), serialize(): serialize correctly after being signed with SIP155 Signature for tx created on sila-mainnet', () => {
    const txRaw = [
      '0x1',
      '0x02540be400',
      '0x5208',
    ...
    const common = createCustomCommon({ chainId: 3 }, SilaMainnet)
    const tx = createLegacyTxFromBytesArray(
      txRaw.map((rawTxData) => hexToBytes(rawTxData as PrefixedHexString)),
      { common },
    )
  ...
  it('sign(), verifySignature(): should ignore any previous signature when decided if SIP155 should be used in a new one', () => {
  ...
  it('common propagates from the common of tx, not the common in TxOptions', () => {
    ...
    const newCommon = new Common({ chain: SilaMainnet, hardfork: Hardfork.SilaParis })
