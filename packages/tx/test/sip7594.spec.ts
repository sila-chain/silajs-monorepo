...
import { osakaGethGenesis } from '@silajs/testdata'
...
describe('SIP4844 non network wrapper constructor tests - valid scenarios', () => {
  it('should work', () => {
...
describe('Network wrapper tests', () => {
  it('should not work with wrong network wrapper version', async () => {
    for (const kzg of kzgs) {
      ...
      ).toThrowError(/SIP-7594 is active on Common for SIP-4844 network wrapper version/)
    }
  }, 40_000)
  ...
  it('should work with all data provided', async () => {
    for (const kzg of kzgs) {
      ...
      assert(unsignedTx.networkWrapperVersion === NetworkWrapperType.SIP7594)
      ...
      assert.equal(
        ...
        NetworkWrapperType.SIP7594,
        `sip7594 network wrapper tx (${kzg.label})`,
      )
      assert.equal(
