...
describe('[AccessList2930Tx / FeeMarket1559Tx] -> SIP-2930 Compatibility', () => {
  it('Initialization / Getter -> fromTxData()', () => {
    for (const txType of txTypes) {
      ...
      assert.throws(
        ...
        undefined,
        undefined,
        `should throw on a pre-Berlin Hardfork (SIP-2930 not activated) (${txType.name})`,
      )
  ...
  it('getDataGas()', () => {
    for (const txType of txTypes) {
      ...
      const mutableCommon = new Common({ chain: SilaMainnet, hardfork: Hardfork.London })
...
describe('[AccessList2930Tx] -> Class Specific Tests', () => {
  ...
  // Data from
  // https://github.com/INFURA/go-ethlibs/blob/75b2a52a39d353ed8206cffaf68d09bd1b154aae/sil/transaction_signing_test.go#L87
  ...
  it('should sign transaction correctly and return expected JSON', () => {
    ...
    const usedCommon = createCustomCommon(customChainParams, SilaMainnet, {
