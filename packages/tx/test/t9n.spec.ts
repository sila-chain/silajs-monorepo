...
import { bytesToHex, hexToBigInt, hexToBytes } from '@silajs/util'
...
const forkMap: { [key: string]: string } = {
  ...
  SIP150: Hardfork.TangerineWhistle,
  SIP158: Hardfork.SpuriousDragon,
...
const args: T9NArgs = yargs
  ...
  .option('dir', {
    ...
    default: '../sila-tests/TransactionTests',
    string: true,
  }).argv as T9NArgs
      ...
      for (const testName in tests) {
        ...
        for (const fork in test.result) {
          it(`${testName} [${getFork(fork)}]`, () => {
            let common: Common
            try {
              common = new Common({ chain: SilaMainnet, hardfork: getFork(fork) })
            } catch {
