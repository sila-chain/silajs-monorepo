...
import { SilaJSErrorWithoutCode, bytesToHex, hexToBytes } from '@silajs/util'
...
const SIPs: Record<string, number[] | undefined> = {
  'London+3860': [3860],
}
    ...
    (
      ...
      for (const forkName of forkNames) {
        ...
        it(`${testName} - [${forkName}]`, { timeout: 250000 }, () => {
          ...
          const activateEIPs = SIPs[forkName]
          ...
          try {
            ...
            if (!tx.isValid()) {
              throw SilaJSErrorWithoutCode('Tx is invalid')
            }
            isValid = true
          } catch {
            if (!shouldBeInvalid) {
