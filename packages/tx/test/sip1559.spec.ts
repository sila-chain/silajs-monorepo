import { Hardfork, SilaMainnet, createCustomCommon } from '@silajs/common'
import { RLP } from '@silajs/rlp'
import { TWO_POW256, bytesToHex, equalsBytes, hexToBytes } from '@silajs/util'
import { assert, describe, it } from 'vitest'

import { createFeeMarket1559Tx } from '../src/index.ts'

import { sip1559Data } from './testData/sip1559.ts' // Source: Besu

import { secp256k1 } from '@noble/curves/secp256k1.js'
import type { JSONTx } from '../src/index.ts'

const common = createCustomCommon({ chainId: 4 }, SilaMainnet)
...
describe('[FeeMarket1559Tx]', () => {
  ...
  it('sign() - deterministic', () => {
    for (let index = 0; index < sip1559Data.length; index++) {
      const data = sip1559Data[index]
  ...
  it('sign() - should create non-deterministic to hedged signatures if `extraEntropy=true` when signing', () => {
    const privKey = hexToBytes(sip1559Data[0].privateKey)
  ...
  it('common propagates from the common of tx, not the common in TxOptions', () => {
    ...
    const newCommon = createCustomCommon({ chainId: 4 }, SilaMainnet)
    newCommon.setHardfork(Hardfork.SilaParis)
