 ...
 * @returns Common instance (copied if provided, new SilaMainnet instance if not)
 */
export function getCommon(common?: Common): Common {
  return common?.copy() ?? new Common({ chain: SilaMainnet })
}
 ...
 * @throws SilaJSErrorWithoutCode if any transaction field is an array
 */
  ...
  for (const [key, value] of Object.entries(values)) {
    if (txDataKeys.includes(key)) {
      if (Array.isArray(value)) {
        throw SilaJSErrorWithoutCode(`${key} cannot be an array`)
      }
    }
  }
}
  ...
  for (const [key, value] of Object.entries(values)) {
    switch (bits) {
      ...
      case 256:
        ...
        break
      default: {
        throw SilaJSErrorWithoutCode('unimplemented bits value')
      }
    }
  }
}
...
export function sharedConstructor(
  ...
  // SIP-2681 limits nonce to 2^64-1 (cannot equal 2^64-1)
  valueOverflowCheck({ nonce: tx.nonce }, 64, true)

  // SIP-7825: Transaction Gas Limit Cap.
  // Under SIP-8037 the cap applies to the regular-gas dimension only (state-gas
