...
function validateVAndExtractChainID(common: Common, _v?: bigint): bigint | undefined {
  ...
  if (v !== undefined) {
    // v is 1. not matching the SIP-155 chainId included case and...
    // v is 2. not matching the classic v=27 or v=28 case
    if (v < 37 && v !== 27 && v !== 28) {
      throw SilaJSErrorWithoutCode(
        `Legacy txs need either v = 27/28 or v >= 37 (SIP-155 replay protection), got v = ${v}`,
      )
    }
  }

  // No unsigned tx and SIP-155 activated and chain ID included
  if (v !== undefined && v !== 0 && common.gteHardfork('spuriousDragon') && v !== 27 && v !== 28) {
    if (!meetsEIP155(BigInt(v), common.chainId())) {
      throw SilaJSErrorWithoutCode(
        `Incompatible SIP155-based V ${v} and chain id ${common.chainId()}. See the Common parameter of the Transaction constructor to set the chain id.`,
      )
    }
...
export class LegacyTx implements TransactionInterface<typeof TransactionType.Legacy> {
  ...
  public constructor(txData: TxData, opts: TxOptions = {}) {
    ...
    if (this.gasPrice * this.gasLimit > MAX_INTEGER) {
      throw SilaJSErrorWithoutCode('gas limit * gasPrice cannot exceed MAX_INTEGER (2^256-1)')
    }

    if (this.common.gteHardfork('spuriousDragon')) {
      if (!this.isSigned()) {
        this.activeCapabilities.push(Capability.SIP155ReplayProtection)
      } else {
        // SIP155 spec:
        ...
        // v and chain ID meet SIP-155 conditions
  ...
  getMessageToSign(): Uint8Array[] {
    ...
    if (this.supports(Capability.SIP155ReplayProtection)) {
  ...
  addSignature(
    ...
    // 27 or 28 for non-SIP-155 protected txs
    // 35 or 36 + chainId * 2 for SIP-155 protected txs
    // See: https://sips.sila.org/SIPS/sip-155
    convertV: boolean = false,
  ): LegacyTx {
    r = toBytes(r)
    s = toBytes(s)
    if (convertV && this.supports(Capability.SIP155ReplayProtection)) {
