...
export function createEOACode7702TxFromBytesArray(values: TxValuesArray, opts: TxOptions = {}) {
  if (values.length !== 10 && values.length !== 13) {
    throw SilaJSErrorWithoutCode(
      'Invalid SIP-7702 transaction. Only expecting 10 values (for unsigned tx) or 13 values (for signed tx).',
    )
  }
...
export function createEOACode7702TxFromRLP(serialized: Uint8Array, opts: TxOptions = {}) {
  if (
    ...
    throw SilaJSErrorWithoutCode(
      `Invalid serialized tx input: not an SIP-7702 transaction (wrong tx type, expected: ${
