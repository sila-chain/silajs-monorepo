...
export function create1559FeeMarketTxFromBytesArray(values: TxValuesArray, opts: TxOptions = {}) {
  if (values.length !== 9 && values.length !== 12) {
    throw SilaJSErrorWithoutCode(
      'Invalid SIP-1559 transaction. Only expecting 9 values (for unsigned tx) or 12 values (for signed tx).',
    )
  }
...
export function createFeeMarket1559TxFromRLP(serialized: Uint8Array, opts: TxOptions = {}) {
  if (
    ...
    throw SilaJSErrorWithoutCode(
      `Invalid serialized tx input: not an SIP-1559 transaction (wrong tx type, expected: ${
