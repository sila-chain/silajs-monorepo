...
export function createAccessList2930TxFromBytesArray(values: TxValuesArray, opts: TxOptions = {}) {
  if (values.length !== 8 && values.length !== 11) {
    throw SilaJSErrorWithoutCode(
      'Invalid SIP-2930 transaction. Only expecting 8 values (for unsigned tx) or 11 values (for signed tx).',
    )
  }
...
export function createAccessList2930TxFromRLP(serialized: Uint8Array, opts: TxOptions = {}) {
  if (
    ...
    throw SilaJSErrorWithoutCode(
      `Invalid serialized tx input: not an SIP-2930 transaction (wrong tx type, expected: ${
