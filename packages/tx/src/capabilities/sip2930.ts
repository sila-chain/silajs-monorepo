...
import type { SIP2930CompatibleTx } from '../types.ts'

/**
 * The amount of gas paid for the data in this tx
 */
export function getDataGas(tx: SIP2930CompatibleTx): bigint {
 ...
 */
function getAccessListDataGas(tx: SIP2930CompatibleTx): bigint {
  ...
  // SIP-7981: add floor cost for access list bytes (20 bytes/address + 32 bytes/slot, 4 tokens/byte)
 ...
 */
export function verifyAccessList(tx: SIP2930CompatibleTx) {
  ...
  for (const accessListItem of accessList) {
    if (accessListItem.length !== 2) {
      throw SilaJSErrorWithoutCode(
        'Invalid SIP-2930 transaction: access list item should have 2 elements',
      )
    }
    const [address, storageSlots] = accessListItem
    if (address.length !== 20) {
      throw SilaJSErrorWithoutCode(
        'Invalid SIP-2930 transaction: address length should be 20 bytes',
      )
    }
    for (const slot of storageSlots) {
      if (slot.length !== 32) {
        throw SilaJSErrorWithoutCode(
          'Invalid SIP-2930 transaction: storage slot length should be 32 bytes',
        )
      }
    }
  }
