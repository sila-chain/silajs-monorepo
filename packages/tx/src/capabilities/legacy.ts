 ...
 * @throws SilaJSErrorWithoutCode if the transaction is unsigned
 */
...
/**
 * SIP-2: All transaction signatures whose s-value is greater than secp256k1n/2are considered invalid.
 ...
 * @throws SilaJSErrorWithoutCode if the signature is invalid
 */
export function getSenderPublicKey(tx: LegacyTxInterface): Uint8Array {
  ...
  try {
    ...
    const sender = ecrecoverFunction(
      ...
      tx.supports(Capability.SIP155ReplayProtection) ? tx.common.chainId() : undefined,
    )
    if (Object.isFrozen(tx)) {
 ...
 * @throws SilaJSErrorWithoutCode if the base fee exceeds the gas price
 */
...
export function sign(
  ...
  // Hack for the constellation that we have got a legacy tx after spuriousDragon with a non-SIP155 conforming signature
  // and want to recreate a signature (where SIP155 should be applied)
  ...
  if (
    ...
    !tx.supports(Capability.SIP155ReplayProtection)
  ) {
    ;(tx as LegacyTx)['activeCapabilities'].push(Capability.SIP155ReplayProtection)
    hackApplied = true
  }
  ...
  if (recovery === undefined) {
    throw SilaJSErrorWithoutCode('Invalid signature recovery')
  }
  const signedTx = tx.addSignature(BigInt(recovery), r, s, true)

  // Hack part 2
  if (hackApplied) {
    const index = (tx as LegacyTx)['activeCapabilities'].indexOf(Capability.SIP155ReplayProtection)
    if (index > -1) {
