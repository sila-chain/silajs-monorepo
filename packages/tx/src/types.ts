import { bytesToBigInt, toBytes } from '@silajs/util'

import type { Common, Hardfork, ParamsDict } from '@silajs/common'
import type {
  Address,
  AddressLike,
  BigIntLike,
...
export interface TxOptions {
   ...
   *
   * Default: {@link Common} object set to `sila-mainnet` and the default hardfork as defined in the {@link Common} class.
   *
   * Current default hardfork: `istanbul`
   */
  common?: Common
  /**
   * Tx parameters sorted by SIP can be found in the exported `paramsTx` dictionary,
...
export interface TransactionCache {
  ...
  // See: https://github.com/sila-chain/silajs-monorepo/issues/3932
...
/**
 * Type guard to check if transaction is an AccessList SIP-2930 transaction
 * @param tx - The transaction to check
 * @returns true if transaction is AccessList SIP-2930 type
 */
...
/**
 * Type guard to check if transaction is a Fee Market SIP-1559 transaction
 * @param tx - The transaction to check
 * @returns true if transaction is Fee Market SIP-1559 type
 */
...
/**
 * Type guard to check if transaction is a Blob SIP-4844 transaction
 * @param tx - The transaction to check
 * @returns true if transaction is Blob SIP-4844 type
 */
...
/**
 * Type guard to check if transaction is an EOA Code SIP-7702 transaction
 * @param tx - The transaction to check
 * @returns true if transaction is EOA Code SIP-7702 type
 */
...
export interface SIP2718CompatibleTx<T extends TransactionType = TransactionType>
  extends TransactionInterface<T> {
  readonly chainId: bigint
  getMessageToSign(): Uint8Array
}

export interface SIP2930CompatibleTx<T extends TransactionType = TransactionType>
  extends SIP2718CompatibleTx<T> {
  readonly accessList: AccessListBytes
}

export interface SIP1559CompatibleTx<T extends TransactionType = TransactionType>
  extends SIP2930CompatibleTx<T> {
  readonly maxPriorityFeePerGas: bigint
  readonly maxFeePerGas: bigint
}

export interface SIP4844CompatibleTx<T extends TransactionType = TransactionType>
  extends SIP1559CompatibleTx<T> {
...
export interface SIP7702CompatibleTx<T extends TransactionType = TransactionType>
  extends SIP1559CompatibleTx<T> {
...
/**
 * Type guard to check if transaction data is AccessList SIP-2930 transaction data
 * @param txData - The transaction data to check
 * @returns true if transaction data is AccessList SIP-2930 type
 */
...
/**
 * Type guard to check if transaction data is Fee Market SIP-1559 transaction data
 * @param txData - The transaction data to check
 * @returns true if transaction data is Fee Market SIP-1559 type
 */
...
/**
 * Type guard to check if transaction data is Blob SIP-4844 transaction data
 * @param txData - The transaction data to check
 * @returns true if transaction data is Blob SIP-4844 type
 */
...
/**
 * Type guard to check if transaction data is EOA Code SIP-7702 transaction data
 * @param txData - The transaction data to check
 * @returns true if transaction data is EOA Code SIP-7702 type
 */
...
export interface FeeMarketEIP1559TxData extends AccessList2930TxData {
  /**
   * The transaction's gas price, inherited from {@link Transaction}.  This property is not used for SIP1559
 ...
 * (an SIP1559 tx e.g. lacks a `gasPrice`).
 */
...
/*
 * Based on https://sila.org/en/developers/docs/apis/json-rpc/
 */
export interface JSONRPCTx {
  ...
  gasPrice: string // QUANTITY - gas price provided by the sender in wei. If SIP-1559 tx, defaults to maxFeePerGas.
  ...
  type: string // QUANTITY - SIP-2718 Typed Transaction type
  accessList?: JSONTx['accessList'] // SIP-2930 access list
