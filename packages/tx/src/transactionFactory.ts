import { SilaJSErrorWithoutCode, fetchFromProvider, getProvider } from '@silajs/util'
...
import type { EthersProvider } from '@silajs/util'
...
export function createTxFromRLP<T extends TransactionType>(
  ...
  if (data[0] <= 0x7f) {
    ...
    switch (data[0]) {
      ...
      default:
        throw SilaJSErrorWithoutCode(`TypedTransaction with ID ${data[0]} unknown`)
    }
  } else {
...
export function createTxFromBlockBodyData(
  ...
  if (data instanceof Uint8Array) {
    ...
    throw SilaJSErrorWithoutCode('Cannot decode transaction: unknown type input')
  }
}
...
export async function createTxFromJSONRPCProvider(
  ...
  if (txData === null) {
    throw SilaJSErrorWithoutCode('No data returned from provider')
  }
