...
import type { KZG, PrefixedHexString } from '@silajs/util'
import type {
...
const validateBlobTransactionNetworkWrapper = (
  ...
  if (blobVersionedHashes.length === 0) {
    throw SilaJSErrorWithoutCode('Invalid transaction with empty blobs')
  }

  let isValid
  try {
    if (networkWrapperVersion === NetworkWrapperType.SIP4844) {
    ...
    throw SilaJSErrorWithoutCode(`KZG verification of blobs fail with error=${error}`)
  }
  if (!isValid) {
    throw SilaJSErrorWithoutCode('KZG proof cannot be verified from blobs/commitments')
  }
 ...
 *   - `maxPriorityFeePerGas` - Maximum priority fee per gas (SIP-1559)
 *   - `maxFeePerGas` - Maximum fee per gas (SIP-1559)
 *   - `gasLimit` - Gas limit for the transaction
 *   - `to` - Recipient address (optional for contract creation)
 *   - `value` - Value to transfer in wei
 *   - `data` - Transaction data
 *   - `accessList` - Access list for SIP-2930 (optional)
 *   - `maxFeePerBlobGas` - Maximum fee per blob gas (SIP-4844)
 ...
 *   - `networkWrapperVersion` - Network wrapper version (0=SIP-4844, 1=SIP-7594)
 * @param opts - Transaction options including Common instance with KZG initialized
 * @returns A new Blob4844Tx instance
 *
 * @throws {SilaJSErrorWithoutCode} If KZG is not initialized in Common
 * @throws {SilaJSErrorWithoutCode} If both blobsData and blobs are provided
 *
 * Notes:
 ...
 * - KZG proof type depends on SIP-7594 activation: per-Blob proofs (SIP-4844) or per-Cell proofs (SIP-7594)
 */
 ...
 *   - `maxPriorityFeePerGas` - Maximum priority fee per gas (SIP-1559) as Uint8Array
 *   - `maxFeePerGas` - Maximum fee per gas (SIP-1559) as Uint8Array
 ...
 *   - `accessList` - Access list for SIP-2930 as Uint8Array (optional)
 *   - `maxFeePerBlobGas` - Maximum fee per blob gas (SIP-4844) as Uint8Array
 ...
 *
 * @throws {SilaJSErrorWithoutCode} If KZG is not initialized in Common
 * @throws {SilaJSErrorWithoutCode} If values array length is not 11 (unsigned) or 14 (signed)
 *
...
export function createBlob4844TxFromBytesArray(values: TxValuesArray, opts: TxOptions = {}) {
  ...
  if (values.length !== 11 && values.length !== 14) {
    throw SilaJSErrorWithoutCode(
      'Invalid SIP-4844 transaction. Only expecting 11 values (for unsigned tx) or 14 values (for signed tx).',
    )
  }
  ...
  return new Blob4844Tx(
    {
      ...
      maxFeePerBlobGas,
      blobVersionedHashes,
      v: v !== undefined ? bytesToBigInt(v) : undefined, // SIP2930 supports v's with value 0 (empty Uint8Array)
      r,
      s,
    },
    opts,
 ...
 *
 * @throws {SilaJSErrorWithoutCode} If KZG is not initialized in Common
 * @throws {SilaJSErrorWithoutCode} If serialized data is not a valid SIP-4844 transaction
 * @throws {SilaJSErrorWithoutCode} If RLP decoded data is not an array
 *
...
export function createBlob4844TxFromRLP(serialized: Uint8Array, opts: TxOptions = {}) {
  ...
  if (equalsBytes(serialized.subarray(0, 1), txTypeBytes(TransactionType.BlobEIP4844)) === false) {
    throw SilaJSErrorWithoutCode(
      `Invalid serialized tx input: not an SIP-4844 transaction (wrong tx type, expected: ${
  ...
  if (!Array.isArray(values)) {
    throw SilaJSErrorWithoutCode('Invalid serialized tx input: must be array')
  }
 ...
 *
 * @throws {SilaJSErrorWithoutCode} If Common instance is not provided
 * @throws {SilaJSErrorWithoutCode} If KZG is not initialized in Common
 * @throws {SilaJSErrorWithoutCode} If serialized data is not a valid SIP-4844 transaction
 ...
 * @throws {SilaJSErrorWithoutCode} If KZG verification fails
 * @throws {SilaJSErrorWithoutCode} If versioned hashes don't match commitments
 *
 * Network Wrapper Formats:
 * - SIP-4844: `0x03 || rlp([tx_values, blobs, kzg_commitments, kzg_proofs])` (4 values)
 * - SIP-7594: `0x03 || rlp([tx_values, network_wrapper_version, blobs, kzg_commitments, kzg_proofs])` (5 values)
 *
 * Notes:
 * - Requires a Common instance with `customCrypto.kzg` initialized
 * - Validates KZG proofs against blobs and commitments
 * - Verifies versioned hashes match computed commitments
 * - Supports both SIP-4844 and SIP-7594 network wrapper formats
...
export function createBlob4844TxFromSerializedNetworkWrapper(
  ...
  if (!opts || !opts.common) {
    throw SilaJSErrorWithoutCode('common instance required to validate versioned hashes')
  }
  ...
  if (networkTxValues.length === 4) {
    ...
    networkWrapperVersion = Uint8Array.from([NetworkWrapperType.SIP4844])
  ...
  if (
    networkWrapperVersionInt !== NetworkWrapperType.SIP4844 &&
    networkWrapperVersionInt !== NetworkWrapperType.SIP7594
  ) {
...
/**
 * Returns the SIP 4844 transaction network wrapper in JSON format similar to toJSON, including
