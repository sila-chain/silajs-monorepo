[**@silajs/tx**](../README.md)

***

[@silajs/tx](../README.md) / JSONBlobTxNetworkWrapper

# Type Alias: JSONBlobTxNetworkWrapper

> **JSONBlobTxNetworkWrapper** = [`JSONTx`](../interfaces/JSONTx.md) & `object`

Defined in: [types.ts:614](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/tx/src/types.ts#L614)

## Type Declaration

### blobs

> **blobs**: `PrefixedHexString`[]

### kzgCommitments

> **kzgCommitments**: `PrefixedHexString`[]

### kzgProofs

> **kzgProofs**: `PrefixedHexString`[]

### networkWrapperVersion

> **networkWrapperVersion**: `PrefixedHexString`
