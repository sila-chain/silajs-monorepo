import { Block, BlockHeader, createBlock, createBlockHeader } from '@silajs/block'
...
import type { BlockData, HeaderData } from '@silajs/block'
import type { DB, DBObject, PrefixedHexString } from '@silajs/util'
...
export class Miner {
  ...
  constructor(mineObject: BlockHeader | Block, ethash: Ethash) {
    if (mineObject instanceof BlockHeader) {
      ...
      throw SilaJSErrorWithoutCode('unsupported mineObject')
    }
...
export class Ethash {
  ...
  mkcache(cacheSize: number, seed: Uint8Array) {
    ...
    const o = [keccak_512(seed)]

    let i
    for (i = 1; i < n; i++) {
      o.push(keccak_512(o[o.length - 1]))
    }

    for (let _ = 0; _ < params.CACHE_ROUNDS; _++) {
      for (i = 0; i < n; i++) {
        const v = new DataView(o[i].buffer).getUint32(0, true) % n
        o[i] = keccak_512(xor(o[(i - 1 + n) % n], o[v]))
      }
    }
  ...
  calcDatasetItem(i: number): Uint8Array {
    ...
    mix = keccak_512(mix) as Uint8Array<ArrayBuffer>
  ...
  run(val: Uint8Array, nonce: Uint8Array, fullSize?: number) {
    if (fullSize === undefined) {
      if (this.fullSize === undefined) {
        throw SilaJSErrorWithoutCode('fullSize needed')
      } else {
    ...
    const s = keccak_512(concatBytes(val, bytesReverse(nonce)))
    ...
    return {
      mix: cmix,
      hash: keccak_256(concatBytes(s, cmix)),
    }
  }
  ...
  headerHash(rawHeader: Uint8Array[]) {
    return keccak_256(RLP.encode(rawHeader.slice(0, -2)))
  }
  ...
  async loadEpoc(number: bigint) {
    ...
    if (!this.cacheDB) {
      throw SilaJSErrorWithoutCode('cacheDB needed')
    }
