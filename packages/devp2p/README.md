...
> **\[DEPRECATED\]** This library has been deprecated (decreased importance of devp2p as a networking layer for L1 + deprecation of SilaJS Client).
...
- [Sila Wire Protocol (SIL)](#sila-wire-protocol-sil)
- [Debugging](#debugging)
- [Developer](#developer)
...
// connect to local sila node (debug)
```
...
- `options.shouldGetDnsPeers` - Toggles whether or not peers should be discovered by querying SIP-1459 DNS lists (default: `false`)
...
- `options.dnsNetworks` - SIP-1459 ENR tree urls to query for peer discovery (default: network dependent)
...
- [Node discovery protocol](https://sila.org/en/developers/docs/networking-layer/#discovery)
- [RLPx - Node Discovery Protocol](https://github.com/sila-chain/devp2p/blob/master/rlpx.md#node-discovery)
- [Kademlia Peer Selection](https://github.com/sila-chain/wiki/wiki/Kademlia-Peer-Selection)
...
Instantiate an [@silajs/common](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/common)
...
- `options.clientId` - Client ID string (default example: `silajs-devp2p/v2.1.3/darwin-x64/nodejs`).
- `options.remoteClientIdFilter` - Optional list of client ID filter strings (e.g. `['go1.5', 'quorum']`).
- `options.capabilities` - Upper layer protocol capabilities, e.g. `[devp2p.SIL.eth63, devp2p.SIL.eth62]`.
...
- `options.common` - An instance of [`@silajs/common`](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/common).
...
- [RLPx: Cryptographic Network & Transport Protocol](https://github.com/sila-chain/devp2p/blob/master/rlpx.md)
- [devp2p wire protocol](https://github.com/sila-chain/wiki/wiki/%C3%90%CE%9EVp2p-Wire-Protocol)

## Sila Wire Protocol (SIL)

Upper layer protocol for exchanging Sila network data like block headers or transactions with a node, see [./src/protocol/sil/](./src/protocol/sil/).
...
sil.events.once('status', () => {
  sil.sendMessage(devp2p.SilMessageCodes.GET_BLOCK_HEADERS, [
    Uint8Array.from([1]),
...
- [Sila wire protocol](https://github.com/sila-chain/wiki/wiki/Sila-Wire-Protocol)
...
```shell
DEBUG=ethjs,devp2p:dpt:\*,devp2p:sil node -r tsx/register [YOUR_SCRIPT_TO_RUN.ts]
```
...
| `devp2p:sil`          | SIL protocol message logging (`STATUS`, `GET_BLOCK_HEADER`, `TRANSACTIONS`,... messages) |
...
DEBUG=ethjs,devp2p:dpt:\*,devp2p:sil,verbose node -r tsx/register [YOUR_SCRIPT_TO_RUN.ts]
...
| Logger                        | Usage                                                                                                |
| ----------------------------- | ---------------------------------------------------------------------------------------------------- |
| `devp2p:sil`                  | e.g. `devp2p:sil:GET_BLOCK_HEADERS`                                                                  |
...
on two message names along `SIL` protocol debugging:

```shell
DEBUG=ethjs,devp2p:sil:GET_BLOCK_HEADERS,devp2p:sil:BLOCK_HEADERS -r tsx/register [YOUR_SCRIPT_TO_RUN.ts]
```

Example logging output:

```shell
devp2p:sil:GET_BLOCK_HEADERS Received GET_BLOCK_HEADERS message from 207.154.201.177:30303: d188659b37d8e321bc52c782198181c08080 +50ms
devp2p:sil:GET_BLOCK_HEADERS Send GET_BLOCK_HEADERS message to 159.65.72.121:30303: c81bc682ded8328080 +431ms
devp2p:sil:BLOCK_HEADERS Received BLOCK_HEADERS message from 159.65.72.121:30303: c21bc0 +417ms
devp2p:sil:GET_BLOCK_HEADERS Send GET_BLOCK_HEADERS message to 162.55.98.224:30303: c81dc682df0a328080 +339ms
devp2p:sil:BLOCK_HEADERS Received BLOCK_HEADERS message from 162.55.98.224:30303: f968dd1df968d9f90217a0af80dab03492dfc689936dc9ff272ed3743da1... +72ms
```
...
Logging can be limited to the peer with which the first successful subprotocol (e.g. `SIL`) connection could be established:
...
- Python: [pydevp2p](https://github.com/sila-chain/pydevp2p)
- Go: [Go Sila](https://github.com/sila-chain/go-sila/tree/master/p2p)
...
[devp2p-npm-link]: https://www.npmjs.org/package/silajs-devp2p
