import { unprefixedHexToBytes, utf8ToBytes } from '@silajs/util'
...
import { devp2pTestData } from '@silajs/testdata'
...
function testdataBefore(fn: Function) {
  return (t: EciesTestContext) => {
    const v = devp2pTestData.sip8Values
    ...
    const h = devp2pTestData.sip8Handshakes
...
it(
  'Random: auth -> ack -> header -> body (old format/no SIP8)',
  randomBefore((t: EciesTestContext) => {
    assert.doesNotThrow(() => {
...
it(
  'Testdata: auth -> ack (old format/no SIP8)',
