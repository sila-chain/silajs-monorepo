import { Common, Hardfork, SilaMainnet, SilaSepolia } from '@silajs/common'
import { hexToBytes, intToBytes } from '@silajs/util'
import { assert, describe, it } from 'vitest'

import * as devp2p from '../../src/index.ts'
import { SIL, SilMessageCodes } from '../../src/index.ts'
...
const capabilities = [devp2p.SIL.eth68]
...
describe('SIL simulator tests', () => {
  it('SIL: send status message (successful)', async () => {
    await new Promise((resolve) => {
      const rlpxs = util.getTestRLPXs(2, 2, 21762, capabilities)
      rlpxs[0].events.once('peer:added', (peer) => {
        const protocol = peer.getProtocols()[0] as SIL
  ...
  it('SIL: send status message (NetworkId mismatch)', async () => {
    const c1 = new Common({ chain: SilaMainnet, hardfork: Hardfork.London })
    const c2 = new Common({ chain: SilaSepolia, hardfork: Hardfork.London })
  ...
  it('SIL: send status message (Genesis block mismatch)', async () => {
  ...
  it('SIL: should use latest protocol version on default', async () => {
    await new Promise((resolve) => {
      const rlpxs = util.getTestRLPXs(2, 2, 21762, capabilities)
      rlpxs[0].events.once('peer:added', (peer: Peer) => {
        const protocol = peer.getProtocols()[0] as SIL
  ...
  it('SIL -> Sil65 -> sendStatus(): should throw on non-matching latest block provided', async () => {
    const status0: any = Object.assign({}, status)
    status0['latestBlock'] = intToBytes(100000)
    const cap = [devp2p.SIL.eth65]
  ...
  it('SIL: send not-allowed eth68', async () => {
    await new Promise((resolve) => {
      const rlpxs = util.getTestRLPXs(2, 2, 21762, capabilities)
      rlpxs[0].events.once('peer:added', (peer: Peer) => {
        const protocol = peer.getProtocols()[0] as SIL
        ...
        protocol.events.on('message', (code) => {
          switch (code) {
            case SilMessageCodes.STATUS:
              protocol.sendStatus(status)
              assert.throws(
                () => protocol.sendMessage(SilMessageCodes.GET_NODE_DATA, []),
                /Code 13 not allowed with version 68/,
              )
              assert.throws(
                () => protocol.sendMessage(SilMessageCodes.NODE_DATA, []),
  ...
  it('SIL -> Sil65 -> ForkId validation 1a)', async () => {
    const status0: any = Object.assign({}, status)
  ...
  it('SIL: send unknown message code', async () => {
    await new Promise((resolve) => {
  ...
  it('SIL: invalid status send', async () => {
    const status1 = Object.assign({}, status)
    status1.genesisHash = new Uint8Array(32)
    await new Promise((resolve) => {
      ...
      rlpxs[0].events.once('peer:added', (peer: Peer) => {
        const protocol = peer.getProtocols()[0] as SIL
        assert.throws(
          () => protocol.sendMessage(devp2p.SilMessageCodes.STATUS, []),
