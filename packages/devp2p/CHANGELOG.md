...
- Outdated `LES` support has been removed from the package, PR [#3759](https://github.com/sila-chain/silajs-monorepo/pull/3759)
...
- "Guard" logging to only run with `ethjs` guard, PR [#3654](https://github.com/sila-chain/silajs-monorepo/pull/3654/)
- Debug logger namespace standardization (use with `#` for the core logger, so e.g. `devp2p:#`), PR [#3692](https://github.com/sila-chain/silajs-monorepo/pull/3692)
...
- Downstream dependency updates, see PR [#3297](https://github.com/sila-chain/silajs-monorepo/pull/3297)
...
- Debug log optimizations, PR [#3165](https://github.com/sila-chain/silajs-monorepo/pull/3165)
- Dependency updates, PR [#3212](https://github.com/sila-chain/silajs-monorepo/pull/3212)

## 6.0.1 - 2023-10-26

- Kademlia bucket add fix, PR [#2957](https://github.com/sila-chain/silajs-monorepo/pull/2957)
- Performance: only create `DEBUG` msgs if debugging, PR [#2958](https://github.com/sila-chain/silajs-monorepo/pull/2958)
- Pin `scanf` dependency (fixes broken types), PR [#3060](https://github.com/sila-chain/silajs-monorepo/pull/3060)
- Minimal `RLPx` test suite, PR [#3126](https://github.com/sila-chain/silajs-monorepo/pull/3126)
...
### SIL/67 and SIL/68 Support

Support for both `SIL/67` (see PR [#2263](https://github.com/sila-chain/silajs-monorepo/pull/2263)) and `SIL/68` (see PR [#2828](https://github.com/sila-chain/silajs-monorepo/pull/2828)) have been added to the library.

The [SIL/67](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#eth67-sip-4938-march-2022) protocol version removes `GetNodeData` and `NodeData` messages, now largely substituted by the `SNAP` protocol.

The [SIL/68](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#eth68-sip-5793-october-2022) protocol version "changes the `NewPooledTransactionHashes` message to include types and sizes of the announced transactions".

### Improved Typing / API Changes

Typing for the `devp2p` library has been significantly improved with PR [#2863](https://github.com/sila-chain/silajs-monorepo/pull/2863) and the API has been consolidated along PR [#2889(https://github.com/sila-chain/silajs-monorepo/pull/2889).

There are the following distinct API changes:

- Types and interfaces generally have been consolidated in a `types.ts` file (as in the other SilaJS libraries)
- Rename: `devp2p.DISCONNECT_REASONS` -> `devp2p.DISCONNECT_REASON`
- Rename: `SilProtocol` -> `ProtocolType` (e.g. `ProtocolType.SIL`)
...
This release updates the underlying `@silajs/common` dependency version to make the library ready for the upcoming `SilaShanghai` hardfork (scheduled for early 2023) regarding the `forkHash` related fork switch logic, see PR [#2521](https://github.com/sila-chain/silajs-monorepo/pull/2521). Note that a `timestamp` to trigger the `SilaShanghai` fork update is only added to Common for the `sepolia` testnet and not yet for `goerli` or `sila-mainnet`.
...
- Fixed DNS Discovery ENR record decoding (better connectivity, avoids loosing DNS peer suggestions), PR [#2546](https://github.com/sila-chain/silajs-monorepo/pull/2546)
- Removed outdated Parity DPT ping/pong hack, PR [#2538](https://github.com/sila-chain/silajs-monorepo/pull/2538)
- Improved devp2p HELLO logging message (added protocol version and client ID), PR [#2538](https://github.com/sila-chain/silajs-monorepo/pull/2538)

## 5.0.2 - 2022-12-09

### Hardfork-By-Time Support

The devp2p library is now ready to work with hardforks triggered by timestamp, which will first be applied along the `SilaShanghai` HF, see PR [#2437](https://github.com/sila-chain/silajs-monorepo/pull/2437). This is achieved by integrating a new timestamp supporting `@silajs/common` library version.

One specific devp2p change is that the forkid is now calculated based on timestamps for timestamp-based HFs, see [SIP-6122](https://github.com/sila-chain/SIPs/pull/6122).
...
- Added env check (performance optimization) for DEBUG mode using `debug` package, PR [#2311](https://github.com/sila-chain/silajs-monorepo/pull/2311)
...
- Internal refactor: removed ambiguous boolean checks within conditional clauses, PR [#2254](https://github.com/sila-chain/silajs-monorepo/pull/2254)
...
This change should not directly affect this library but might be relevant since it is not recommended to use different Common library versions between the different SilaJS libraries.

## 5.0.0-beta.2 - 2022-07-15

Beta 2 release for the upcoming breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/devp2p/CHANGELOG.md)) for the main change set description.
...
[Sila Snapshot Protocol](https://github.com/sila-chain/devp2p/blob/master/caps/snap.md) (SNAP) support has been added in PR [#1883](https://github.com/sila-chain/silajs-monorepo/pull/1883). This allows to run the SNAP protocol as a side-protocol to the SIL protocol for exchanging state snapshots between peers.
...
- Removed Node.js specific `assert` usage, PR [#1924](https://github.com/sila-chain/silajs-monorepo/pull/1924)
- Deduplicated `keccak` and `secp256k1` library usage in favor of `sila-cryptography`, Noble crypto library uses, PR [#1947](https://github.com/sila-chain/silajs-monorepo/pull/1947)
- Replaced `hi-base32` dependency with `@scure/base` from [@paulmillr](https://github.com/paulmillr) (Noble crypto library author), PR [#1947](https://github.com/sila-chain/silajs-monorepo/pull/1947)

## 4.2.2 - 2022-04-29

- Solved memory leak "DPT discovers nodes when open_slots = 0", PR [#1816](https://github.com/sila-chain/silajs-monorepo/pull/1816)
- Fixed per-message debug logging, PR [#1776](https://github.com/sila-chain/silajs-monorepo/pull/1776)
- SIL-LES class refactor, PR [#1600](https://github.com/sila-chain/silajs-monorepo/pull/1600)

## 4.2.1 - 2022-02-01

- Dependencies: deduplicated RLP import, PR [#1549](https://github.com/sila-chain/silajs-monorepo/pull/1549)
- Fixed duplicated debug messages (`DEBUG` logger, see `README`), PR [#1643](https://github.com/sila-chain/silajs-monorepo/pull/1643)

## 4.2.0 - 2021-09-24

### SIP-706 Snappy Compression (RLPx v5)

This release adds support for RLPx v5 allowing for the compression of RLPx messages with the Snappy compression algorithm as defined in [SIP-706](https://sips.sila.org/SIPS/sip-706). If the connecting peer doesn't support v5, the connection falls back to v4 and does the communication without compressing the payload.

See: PRs [#1399](https://github.com/sila-chain/silajs-monorepo/pull/1399), [#1442](https://github.com/sila-chain/silajs-monorepo/pull/1442) and [#1484](https://github.com/sila-chain/silajs-monorepo/pull/1484)
...
- Debug per specific `SIL` or `LES` message (e.g. `devp2p:sil:GET_BLOCK_HEADERS`)
...
See: PR [#1449](https://github.com/sila-chain/silajs-monorepo/pull/1449)

## 4.1.0 - 2021-07-15

### Finalized London HF Support

This release integrates a `Common` library version which provides the `london` HF blocks for all networks including `sila-mainnet` and is therefore the first release with finalized London HF support. For the `devp2p` library this particularly means that the fork hashes for the `london` HF will be correct when using sil/64 or higher.

### Support for sil/66 and les/4

PR [#1331](https://github.com/sila-chain/silajs-monorepo/pull/1331) added support for sil/66 and [#1324](https://github.com/sila-chain/silajs-monorepo/pull/1324) for les/4. Be sure to check out the updated peer communication [examples](./examples).
...
- Fixed zero Buffer forkhash bug in case no future fork known, PR #1148 commit [`afd00a8`](https://github.com/sila-chain/silajs-monorepo/pull/1148/commits/afd00a8bfee1b524352a0f6c79f3bcfe43debe4c)
...
This is the first-production ready release of this library. During our work on the [SilaJS Client](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/client) we were finally able to battle-test this library in a real-world environment (so: towards `sila-mainnet`, the main official testnets like `goerli` or `rinkeby` as well as ephemeral testnets like `yolov3`). We fixed a myriad of partly critical bugs along the way (which are extremely hard to reproduce just in a test environment) and can now fully recommend to use this library for `SIL` protocol integrations up to version `SIL/65` in a production setup. Note that the `LES` support in the library is still outdated (but working), an update is planned (let us know if you have demand).

### SIL/64 and SIL/65 Support

The `SIL` protocol support has been updated to now also support versions `64` and `65`. Biggest protocol update here is `SIL/64` introduced with PR [#82](https://github.com/sila-chain/silajs/silajs-devp2p/pull/82) which adds support for selecting peers by fork ID (see associated [SIP-2124](https://sips.sila.org/SIPS/sip-2124)). This allows for a much more differentiated chain selection and avoids connecting to peers which are on a different chain but having a shared chain history with the same blocks and the same block hashes.

`SIL/65` implemented in PR [#1159](https://github.com/sila-chain/silajs-monorepo/pull/1159) adds three new message types `NewPooledTransactionHashes (0x08)`, `GetPooledTransactions (0x09)` and `PooledTransactions (0x0a)` for a more efficient exchange on txs from the tx pool ([SIP-2464](https://sips.sila.org/SIPS/sip-2464)).

### DNS Discovery Support

Node discovery via DNS has been added to quickly acquire testnet (or sila-mainnet) peers from the DNS ENR tree per [SIP-1459](https://sips.sila.org/SIPS/sip-1459), see PRs [#1070](https://github.com/sila-chain/silajs-monorepo/pull/1070), [#1097](https://github.com/sila-chain/silajs-monorepo/pull/1097) and [#1149](https://github.com/sila-chain/silajs-monorepo/pull/1149). This allows for easier peer discovery especially on the testnets. Peer search is randomized as being recommended in the SIP and the implementation avoids to download the entire DNS tree at once.

DNS discovery can be activated in the `DPT` module with the `shouldGetDnsPeers` option, in addition there is a new `shouldFindNeighbours` option allowing to deactivate the classical v4 discovery process. Both discovery methods can be used in conjunction though. DNS Peer discovery can be customized/configured with additional constructor options `dnsRefreshQuantity`, `dnsNetworks` and `dnsAddress`. See [API section](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/devp2p#api) in the README for a description.

### Other Features / Changes

- Updated `goerli` bootnodes, PR [#1031](https://github.com/sila-chain/silajs-monorepo/pull/1031)
- `maxPeers`, `dpt`, and `listenPort` are now optional in `RLPxOptions`, PR [#1019](https://github.com/sila-chain/silajs-monorepo/pull/1019)
- New `DPTOptions` interface, `DPT` type improvements, PR [#1029](https://github.com/sila-chain/silajs-monorepo/pull/1029)
- Improved `RLPx` disconnect reason debug output, PR [#1031](https://github.com/sila-chain/silajs-monorepo/pull/1031)
- `LES`: unify `SIL` and `LES` `sendMessage()` signature by somewhat change payload semantics and pass in `reqId` along, PR [#1087](https://github.com/sila-chain/silajs-monorepo/pull/1087)
- `RLPx`: limit connection refill debug logging to a restarted interval log message to not bloat logging too much, PR [#1087](https://github.com/sila-chain/silajs-monorepo/pull/1087)

### Connection Reliability / Bug Fixes

- Subdivided interval calls to refill `RLPx` peer connections to improve networking distribution and connection reliability, PR [#1036](https://github.com/sila-chain/silajs-monorepo/pull/1036)
- Fixed an error in `DPT` not properly banning old peers and replacing with a new peer on `KBucket` ping, PR [#1036](https://github.com/sila-chain/silajs-monorepo/pull/1036)
- Connection reliability: distribute network traffic on `DPT` additions of new neighbour peers, PR [#1036](https://github.com/sila-chain/silajs-monorepo/pull/1036)
- Fixed a critical peer data processing bug, PR [#1064](https://github.com/sila-chain/silajs-monorepo/pull/1064)
- Added socket destroyed checks on peer message sending to safeguard against stream-was-destroyed error, PR [#1075](https://github.com/sila-chain/silajs-monorepo/pull/1075)
- `DPT`: fixed undefined array access in SIL.\_getStatusString() on malformed SIL/64 status msgs, PR [#1029](https://github.com/sila-chain/silajs-monorepo/pull/1029)

### Maintenance / Testing / CI

- Added dedicated browser build published to `dist.browser` to `package.json`, PR [#1184](https://github.com/sila-chain/silajs-monorepo/pull/1184)
- Updated `rlp-encoding` dependency to the SilaJS `rlp` library, PR [#94](https://github.com/sila-chain/silajs/silajs-devp2p/pull/94)
- `RLPx` type improvements, PR [#1036](https://github.com/sila-chain/silajs-monorepo/pull/1036)
- Switched to `Codecov`, PR [#92](https://github.com/sila-chain/silajs/silajs-devp2p/pull/92)
- Upgraded dev deps (config 2.0, monorepo betas, typedoc), PR [#93](https://github.com/sila-chain/silajs/silajs-devp2p/pull/93)

## [3.0.3] - 2020-09-29

- Moved `TypeScript` type packages for `lru-cache` and `bl` from `devDependencies` to
  `dependencies`, PR [#90](https://github.com/sila-chain/silajs/silajs-devp2p/pull/90)

[3.0.3]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v3.0.2...v3.0.3

## [3.0.2] - 2020-09-28

- Fixed `TypeScript` import issue causing problems when integrating the library in a
  `TypeScript` project, PR [#88](https://github.com/sila-chain/silajs/silajs-devp2p/pull/88)
- Updated `k-bucket` library to `v5`, added types from new `@types/k-bucket` package from
  @tomonari-t, PR [#88](https://github.com/sila-chain/silajs/silajs-devp2p/pull/88)

[3.0.2]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v3.0.1...v3.0.2

## [3.0.1] - 2020-06-10

This release focuses on improving the [debugging](https://github.com/sila-chain/silajs/silajs-devp2p#debugging)
capabilities of the library. PR [#72](https://github.com/sila-chain/silajs/silajs-devp2p/pull/72)
...
Relevant PRs [#75](https://github.com/sila-chain/silajs/silajs-devp2p/pull/75) and
[#73](https://github.com/sila-chain/silajs/silajs-devp2p/pull/73):

- Added number of peers to `refillConnections()` debug message
- Replaced try/catch logic for SIP-8 auth check to avoid side-effects and get rid of misleading _wrong-ecies-header_ debug output
...
- Refactored `Peer` class for better code readability, PR [#77](https://github.com/sila-chain/silajs/silajs-devp2p/pull/77)

There has also been a new [high-level diagram](https://github.com/sila-chain/silajs/silajs-devp2p#api) added to the `README` which can be used to get an overview on the structure, available loggers and the event flow of the library (PR [#76](https://github.com/sila-chain/silajs/silajs-devp2p/pull/76)).

[3.0.1]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v3.0.0...v3.0.1

## [3.0.0] - 2020-05-25

First `TypeScript` release of the library, see PR [#56](https://github.com/sila-chain/silajs/silajs-devp2p/pull/56) for all the changes and associated discussion.
...
**Noteworthy Changes from PR [#56](https://github.com/sila-chain/silajs/silajs-devp2p/pull/56):**

- Type additions for all method signatures and class members of all protocol components (`dpt`, `sil`, `les`, `rlpx`)
- Addition of various structuring interfaces (like [PeerInfo](https://github.com/sila-chain/silajs/silajs-devp2p/blob/master/src/dpt/message.ts#L10) for `DPT` message input) and `enum` constructs (like [MESSAGE_CODES](https://github.com/sila-chain/silajs/silajs-devp2p/blob/master/src/sil/index.ts#L186) from the `SIL` protocol)
- Port of the [examples](https://github.com/sila-chain/silajs/silajs-devp2p/tree/master/examples) to `TypeScript`
- Port of all the [test cases](https://github.com/sila-chain/silajs/silajs-devp2p/tree/master/test) to `TypeScript`
- Integration of the library into the common [silajs-config](https://github.com/sila-chain/silajs/silajs-config) SilaJS configuration setup (`standard` -> `TSLint` linting, docs with `TypeDoc`, `TypeScript` compilation, `Prettier` formatting rules)
...
- Added Node 12,13 support, upgrade from Travis to GitHub actions, PR [#57](https://github.com/sila-chain/silajs/silajs-devp2p/pull/57)
- Updated `silajs-common` dependency to `v1.5.1` for a bootnode update, PR [#67](https://github.com/sila-chain/silajs/silajs-devp2p/pull/67)
- Removed Node 6, 8 support, updated `secp256k1` dependency to from `v3.1.0` to `v4.0.1`, PR [#68](https://github.com/sila-chain/silajs/silajs-devp2p/pull/68)
- Updated `keccak` dependency to `v3.0.0`, PR [#64](https://github.com/sila-chain/silajs/silajs-devp2p/pull/64)
- Some dependency cleanup, PRs [#62](https://github.com/sila-chain/silajs/silajs-devp2p/pull/62), [#65](https://github.com/sila-chain/silajs/silajs-devp2p/pull/65), [#58](https://github.com/sila-chain/silajs/silajs-devp2p/pull/58)

[3.0.0]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.5.1...v3.0.0

## [2.5.1] - 2018-12-12

- Fix connection error by ignoring `RLPX` peers with missing tcp port, PR [#45](https://github.com/sila-chain/silajs/silajs-devp2p/pull/45)

[2.5.1]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.5.0...v2.5.1

## [2.5.0] - 2018-03-22

- Light client protocol (`LES/2`) implementation, PR [#21](https://github.com/sila-chain/silajs/silajs-devp2p/pull/21)
- `LES/2` usage example, see: `examples/peer-communication-les.js`
- Better test coverage for upper-layer protocols (`SIL`, `LES/2`), PR [#34](https://github.com/sila-chain/silajs/silajs-devp2p/pull/34)

[2.5.0]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.4.0...v2.5.0

## [2.4.0] - 2018-02-28

- First release providing a reliable `SIL` connection
- Fix Parity `DPT` ping echo hash bug preventing the library to connect
  to Parity clients, PR [#32](https://github.com/sila-chain/silajs/silajs-devp2p/pull/32)
- Fixed a bug not setting weHello in peer after sent `HELLO` msg

[2.4.0]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.3.0...v2.4.0

## [2.3.0] - 2018-02-27

- Fix critical `RLPX` bug leading to not processing incoming `SIP-8` `Auth` or `Ack` messages, PR [#26](https://github.com/sila-chain/silajs/silajs-devp2p/pull/26)
- Fix bug not forwarding `k-bucket` remove event through `DPT` (so `peer:removed` from
  `DPT` was not working), PR [#27](https://github.com/sila-chain/silajs/silajs-devp2p/pull/27)
- Fix updating `ingressMac` with wrong `Auth` msg leading to diverging `Mac` hashes, PR [#29](https://github.com/sila-chain/silajs/silajs-devp2p/pull/29)
- Fix bug not let first `SIL` `status` message emit a `message` event, PR [#30](https://github.com/sila-chain/silajs/silajs-devp2p/pull/30)
- Large rework of the test setup, additional `DPT`, `RLPX` and `SIL` simulator tests,
  improving test coverage from 48% to 84%, PR [#25](https://github.com/sila-chain/silajs/silajs-devp2p/pull/25)

[2.3.0]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.2.0...v2.3.0
...
[2.2.0]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.1.3...v2.2.0
...
[2.1.3]: https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.1.2...v2.1.3

## Older releases:

- [2.1.2](https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.1.1...v2.1.2) - 2017-05-16
- [2.1.1](https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.1.0...v2.1.1) - 2017-04-27
- [2.1.0](https://github.com/sila-chain/silajs/silajs-devp2p/compare/v2.0.0...v2.1.0) - 2016-12-11
- [2.0.0](https://github.com/sila-chain/silajs/silajs-devp2p/compare/v1.0.0...v2.0.0) - 2016-11-14
- 1.0.0 - 2016-10-18
