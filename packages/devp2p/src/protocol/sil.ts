...
export interface SilStatusMsg extends Array<Uint8Array | Uint8Array[]> {}

export type SilStatusOpts = {
  td: Uint8Array
  bestHash: Uint8Array
...
export type SilMessageCodes = (typeof SilMessageCodes)[keyof typeof SilMessageCodes]

// Create a reverse mapping: from numeric value back to the key name
export const SilMessageCodeNames: { [key in SilMessageCodes]: string } = Object.entries(
  SilMessageCodes,
).reduce(
  (acc, [key, value]) => {
    acc[value] = key
  ...
  {} as { [key in SilMessageCodes]: string },
)

export class SIL extends Protocol {
  protected _status: SilStatusMsg | null = null
  protected _peerStatus: SilStatusMsg | null = null
  ...
  constructor(version: number, peer: Peer, send: SendMethod) {
    super(peer, send, ProtocolType.SIL, version, SilMessageCodes)
  ...
  static eth62 = { name: 'sil', version: 62, length: 8, constructor: SIL }
  static eth63 = { name: 'sil', version: 63, length: 17, constructor: SIL }
  static eth64 = { name: 'sil', version: 64, length: 17, constructor: SIL }
  static eth65 = { name: 'sil', version: 65, length: 17, constructor: SIL }
  static eth66 = { name: 'sil', version: 66, length: 17, constructor: SIL }
  static eth67 = { name: 'sil', version: 67, length: 17, constructor: SIL }
  static eth68 = { name: 'sil', version: 68, length: 17, constructor: SIL }

  _handleMessage(code: SilMessageCodes, data: Uint8Array) {
    const payload = RLP.decode(data)

    if (code !== SilMessageCodes.STATUS && this.DEBUG) {
    ...
    switch (code) {
      case SilMessageCodes.STATUS: {
        ...
        this._peerStatus = payload as SilStatusMsg
        const peerStatusMsg = `${
      ...
      case SilMessageCodes.NEW_BLOCK_HASHES:
      case SilMessageCodes.TX:
      case SilMessageCodes.GET_BLOCK_HEADERS:
      case SilMessageCodes.BLOCK_HEADERS:
      case SilMessageCodes.GET_BLOCK_BODIES:
      case SilMessageCodes.BLOCK_BODIES:
      case SilMessageCodes.NEW_BLOCK:
        if (this._version >= SIL.eth62.version) break
        return

      case SilMessageCodes.GET_RECEIPTS:
      case SilMessageCodes.RECEIPTS:
        if (this._version >= SIL.eth63.version) break
        return

      case SilMessageCodes.NEW_POOLED_TRANSACTION_HASHES:
      case SilMessageCodes.GET_POOLED_TRANSACTIONS:
      case SilMessageCodes.POOLED_TRANSACTIONS:
        if (this._version >= SIL.eth65.version) break
        return

      case SilMessageCodes.GET_NODE_DATA:
      case SilMessageCodes.NODE_DATA:
        if (this._version >= SIL.eth63.version && this._version <= SIL.eth66.version) break
        return
  ...
  _getStatusString(status: SilStatusMsg) {
  ...
  sendStatus(status: SilStatusOpts) {
    ...
    if (this.DEBUG) {
      this.debug(
        ...
        `Send STATUS message to ${this._peer['_socket'].remoteAddress}:${
          this._peer['_socket'].remotePort
        } (sil${this._version}): ${this._getStatusString(this._status)}`,
      )
    }
    ...
    this._send(SilMessageCodes.STATUS, payload)
    this._handleStatus()
  }

  sendMessage(code: SilMessageCodes, payload: Input) {
    if (this.DEBUG) {
    ...
    switch (code) {
      case SilMessageCodes.STATUS:
        throw SilaJSErrorWithoutCode('Please send status message through .sendStatus')
      ...
      case SilMessageCodes.NEW_BLOCK:
        if (this._version >= SIL.eth62.version) break
        throw SilaJSErrorWithoutCode(`Code ${code} not allowed with version ${this._version}`)
      ...
      default:
        throw SilaJSErrorWithoutCode(`Unknown code ${code}`)
    }
  ...
  getMsgPrefix(msgCode: SilMessageCodes): string {
    return SilMessageCodeNames[msgCode]
  }
}
