...
export class RLPx {
  ...
  constructor(privateKey: Uint8Array, options: RLPxOptions) {
    ...
    this.clientId =
      options.clientId ?? utf8ToBytes(`silajs-devp2p/${os.platform()}-${os.arch()}/nodejs`)
  ...
  async connect(peer: PeerInfo) {
    ...
    if (this._peers.has(peerKey)) throw SilaJSErrorWithoutCode('Already connected')
    if (this._getOpenSlots() === 0)
      throw SilaJSErrorWithoutCode('Too many peers already connected')
    ...
    socket.setTimeout(this._timeout, () =>
      deferred.reject(SilaJSErrorWithoutCode('Connection timeout')),
    )
  ...
  _isAliveCheck() {
    if (!this._isAlive()) throw SilaJSErrorWithoutCode('Server already destroyed')
  }
