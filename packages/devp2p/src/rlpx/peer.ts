...
export class Peer {
  ...
  _sendAuth() {
    ...
    this.DEBUG &&
      this._logger(
        `Send auth (SIP8: ${this._EIP8}) to ${this._socket.remoteAddress}:${this._socket.remotePort}`,
      )
    if (this._EIP8 === true) {
  ...
  _sendAck() {
    ...
    this.DEBUG &&
      this._logger(
        `Send ack (SIP8: ${this._eciesSession['_gotEIP8Auth']}) to ${this._socket.remoteAddress}:${this._socket.remotePort}`,
      )
  ...
  _handleAck() {
    ...
    if (!this._eciesSession['_gotEIP8Ack']) {
      ...
      this.DEBUG &&
        this._logger(
          `Received ack (SIP8) from ${this._socket.remoteAddress}:${this._socket.remotePort}`,
        )
    }
    this._state = 'Header'
    this._nextPacketSize = 32
  ...
  _handleHello(payload: any) {
    ...
    this._protocols = Object.keys(shared)
      ...
      .map((obj) => {
        ...
        // The send method handed over to the subprotocol object (e.g. an `SIL` instance).
        // The subprotocol is then calling into the lower level method
        // (e.g. `SIL` calling into `Peer._sendMessage()`).
        const sendMethod = (code: number, data: Uint8Array) => {
          if (code > obj.length) throw SilaJSErrorWithoutCode('Code out of range')
  ...
  _handleBody() {
    ...
    // underlying subprotocol (e.g. `SIL`)
      ...
      if (protocolName === 'Peer') {
        try {
          ...
          if (msgCode === PREFIXES.DISCONNECT) {
            ...
            throw SilaJSErrorWithoutCode(e)
          }
        }
      }
