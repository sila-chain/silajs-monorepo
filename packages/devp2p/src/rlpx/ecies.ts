...
import type { Common, CustomCrypto } from '@silajs/common'
...
// https://github.com/sila-chain/pydevp2p/blob/master/devp2p/crypto.py#L295
// https://github.com/sila-chain/go-sila/blob/fe532a98f9f32bb81ef0d8d013cf44327830d11e/crypto/ecies/ecies.go#L165
// https://github.com/sila-chain/cpp-sila/blob/develop/libdevcrypto/CryptoPP.cpp#L36
...
export class ECIES {
  ...
  _decryptMessage(data: Uint8Array, sharedMacData: Uint8Array | null = null): Uint8Array {
    assertEq(
      data.subarray(0, 1),
      hexToBytes('0x04'),
      'wrong ecies header (possible cause: SIP8 upgrade)',
      debug,
    )
  ...
  parseAuthEIP8(data: Uint8Array): void {
    ...
    assertEq(data.length, size, 'message length different from specified size (SIP8)', debug)
  ...
  parseBody(data: Uint8Array): Uint8Array | undefined {
    if (this._bodySize === null) throw SilaJSErrorWithoutCode('need to parse header first')
