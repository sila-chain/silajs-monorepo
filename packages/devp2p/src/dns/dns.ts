...
export class DNS {
   ...
   * @param {string[]}        dnsNetworks enrTree strings (See SIP-1459 for format)
   * @return {PeerInfo}
   */
  ...
  private _selectRandomPath(branches: string[], context: SearchContext): string {
    ...
    if (Object.keys(circularRefs).length === branches.length) {
      throw SilaJSErrorWithoutCode('Unresolvable circular path detected')
    }
  ...
  private async _getTXTRecord(subdomain: string, context: SearchContext): Promise<string> {
    ...
    if (response.length === 0)
      throw SilaJSErrorWithoutCode('Received empty result array while fetching TXT record')
    if (response[0].length === 0) throw SilaJSErrorWithoutCode('Received empty TXT record')
