...
import { SilaJSErrorWithoutCode, bytesToUtf8, utf8ToBytes } from '@silajs/util'
...
export class ENR {
  ...
  static parseAndVerifyRecord(enr: string, common?: Common): PeerInfo {
    if (!enr.startsWith(this.RECORD_PREFIX))
      throw SilaJSErrorWithoutCode(`String encoded ENR must start with '${this.RECORD_PREFIX}'`)
    ...
    if (!isVerified) throw SilaJSErrorWithoutCode('Unable to verify ENR signature')
   ...
   *
   * @param  {string} root  (See SIP-1459 for encoding details)
  ...
  static parseAndVerifyRoot(root: string, publicKey: string, common?: Common): string {
    if (!root.startsWith(this.ROOT_PREFIX))
      throw SilaJSErrorWithoutCode(`ENR root entry must start with '${this.ROOT_PREFIX}'`)
    ...
    if (!rootValues.eRoot)
      throw SilaJSErrorWithoutCode("Could not parse 'e' value from ENR root entry")
    if (!rootValues.lRoot)
      throw SilaJSErrorWithoutCode("Could not parse 'l' value from ENR root entry")
    if (!rootValues.seq)
      throw SilaJSErrorWithoutCode("Could not parse 'seq' value from ENR root entry")
    if (!rootValues.signature)
      throw SilaJSErrorWithoutCode("Could not parse 'sig' value from ENR root entry")
    ...
    if (!isVerified) throw SilaJSErrorWithoutCode('Unable to verify ENR root signature')
   ...
   *
   * @param  {string}        tree (See SIP-1459 )
   * @return {ENRTreeValues}
   */
  static parseTree(tree: string): ENRTreeValues {
    if (!tree.startsWith(this.TREE_PREFIX))
      throw SilaJSErrorWithoutCode(`ENR tree entry must start with '${this.TREE_PREFIX}'`)
    ...
    if (!treeValues.publicKey)
      throw SilaJSErrorWithoutCode('Could not parse public key from ENR tree entry')
    if (!treeValues.domain)
      throw SilaJSErrorWithoutCode('Could not parse domain from ENR tree entry')
  ...
  static parseBranch(branch: string): string[] {
    if (!branch.startsWith(this.BRANCH_PREFIX))
      throw SilaJSErrorWithoutCode(`ENR branch entry must start with '${this.BRANCH_PREFIX}'`)
