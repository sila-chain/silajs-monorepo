...
import { SilaJSErrorWithoutCode, bytesToHex, bytesToUnprefixedHex } from '@silajs/util'
...
export class Server {
    ...
    this._requests.set(rKey, {
      ...
      timeoutId: setTimeout(() => {
        if (this._requests.get(rKey) !== undefined) {
          ...
          deferred.reject(
            SilaJSErrorWithoutCode(`Timeout error: ping ${peer.address}:${peer.udpPort}`),
          )
        } else {
  ...
  _isAliveCheck() {
    if (this._socket === null) throw SilaJSErrorWithoutCode('Server already destroyed')
  }
