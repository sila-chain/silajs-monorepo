...
const address = {
  encode(value: string) {
    ...
    throw SilaJSErrorWithoutCode(`Invalid address: ${value}`)
  },
  decode(bytes: Uint8Array) {
    ...
    throw SilaJSErrorWithoutCode(`Invalid address bytes: ${bytesToHex(bytes)}`)
  },
}
...
export function encode<T>(typename: string, data: T, privateKey: Uint8Array, common?: Common) {
  ...
  if (type === undefined) throw SilaJSErrorWithoutCode(`Invalid typename: ${typename}`)
...
export function decode(bytes: Uint8Array, common?: Common) {
  ...
  if (typename === undefined) throw SilaJSErrorWithoutCode(`Invalid type: ${type}`)
