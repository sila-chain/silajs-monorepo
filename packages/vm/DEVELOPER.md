...
Currently there is a hybrid situation where tests from old forks are still run against the now-deprecated [Sila tests](https://github.com/sila-chain/tests) repository, while we started to run news tests (SilaOsaka+) against the new [execution-spec-tests](https://github.com/sila-chain/execution-spec-tests) repository (which now again transitioned to [execution-specs](https://github.com/sila-chain/execution-specs)).

There are currently two submodules integrated along:

- [packages/sila-tests](./../sila-tests/) pointing towards a specific commit or tag from the `sila/tests` `develop` branch.
...
located called [execution-spec-tests-fixtures](https://github.com/sila-chain/silajs/execution-spec-tests-fixtures) (maintained by the SilaJS team)
...
```bash
# Run with default fork (SilaPrague)
npm run test:state
...
```bash
// Old runner
VITE_FORK=SilaPrague npx vitest test/tester/state.spec.ts
...
# Direct command
// Old runner
VITE_FORK=SilaPrague npx vitest test/tester/blockchain.spec.ts
...
By default state tests use the latest hardfork (for the old runner: `DEFAULT_FORK_CONFIG` in `test/tester/config.ts`, currently `SilaPrague`).

#### Running with specific forks and SIPs

NOT FULLY IMPLEMENTED IN NEW RUNNER YET.

The `--fork` parameter can also be used to activate SIPs. This is done by first entering the hardfork, and then add the SIPs separated with the `+` sign. For instance:

`npm run test:state -- --fork='London+3855'`

To activate multiple SIPs:
...
[attribute description](http://sila-tests.readthedocs.io/en/latest/test_types/state_tests.html) in
...
```bash
DEBUG=ethjs,dummy:* VITE_CUSTOM_TESTS_PATH=../execution-spec-tests/fusaka-devnet-5/state_tests/osaka/sip7951_p256verify_precompiles  VITE_FORK=SilaOsaka npx vitest watch --coverage --coverage.reporter=html --ui --coverage.allowExternal --coverage.include=savm/src/precompiles/0c-bls12-g1msm.ts [--coverage.include=savm/src/precompiles/index.ts] test/tester/state.spec.ts
```

This can be useful to identify gaps in the coverage of the official spec tests and see where additional tests are needed.

The specific command stays in "watch" mode and opens a specific UI to see coverage also for SAVM files (this needs the `coverage.allowExternal` flag to be set). The `DEBUG` part can be helpful to not have coverage being distorted by non-debug calls artificially lowering the coverage.
...
Sila's official test suite can be found in the [execution-specs](https://github.com/sila-chain/execution-specs) repository.

### Test Generation with EELS

To use the built-in (so: on the test repo side) EELS SAVM implementation, follow the installation instructions in the [execution-specs](https://github.com/sila-chain/execution-specs) repository and e.g. fill tests with:

```bash
uv run fill -v tests/prague/sip2537_bls_12_381_precompiles/test_bls12_g1msm.py --fork SilaOsaka --clean -m state_test
```

This will generate fixtures in the following directory:

```text
fixtures/state_tests/prague/sip2537_bls_12_381_precompiles/bls12_g1msm/
```
...
```bash
VITE_CUSTOM_TESTS_PATH=../../../execution-specs/fixtures/state_tests/prague/sip2537_bls_12_381_precompiles/bls12_g1msm/
```

### Test Generation with SilaJS/T8NTool

The VM has t8ntool (transition-tool) support, see: <https://github.com/sila-chain/silajs-monorepo/blob/master/packages/vm/test/t8n/README.md>. This tool can be used to create fixtures from the `execution-spec-tests` repo. These fixtures can be consumed by other clients in their test runner (similar to running `npm run test:blockchain` or `npm run test:state` in the VM package). The t8ntool readme also links to a guide on how to write tests to contribute to `execution-spec-tests`.
...
All/most State tests are replicated as Blockchain tests in a `GeneralStateTests` [sub directory](https://github.com/sila-chain/tests/tree/develop/docs/test_types/TestStructures/GeneralStateTests) in the Sila tests repo, so for debugging single test cases the Blockchain test version of the State test can be used.

### Comparing Stack Traces

Other client implementations often also provide functionality for output trace information.

A convenient way is to use a local `geth` installation (can be the binary installation and doesn't has to be build from source or something) and then use the included `savm` tool like:

```shell
savm --json --nomemory statetest node_modules/silajs-testing/tests/GeneralStateTests/stCreate2/create2collisionCode2.json
```
...
For comparing `SAVM` traces [here](https://gist.github.com/cdetrio/41172f374ae32047a6c9e97fa9d09ad0) are some instructions for setting up `pysila` to generate corresponding traces for state tests.
...
Test runs can be profiled using the new SAVM/VM profiling functionality by using the `--profile` option for test runs:
...
This helps us see how the VM performs when running sila-mainnet blocks.

View the historical benchmark data for the master branch on the [github page](http://silajs.github.io/silajs-monorepo/dev/bench/vm).
...
`npm run benchmarks -- sila-mainnetBlocks`

To define the number of samples to be run pass in a number like so: `npm run benchmarks -- sila-mainnetBlocks:10`

If you want to get a more detailed look to find bottlenecks we can use [0x](https://github.com/davidmarkclements/0x):

```
npm run profiling -- sila-mainnetBlocks:10
```
