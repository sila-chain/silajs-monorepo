...
/**
 * Data collection script for offline sila-mainnet block replay.
 * Runs blocks via RPC while intercepting state accesses to capture the pre-state,
 * then saves block JSON + pre-state for offline replay with runSilaMainnetBlock.ts.
 *
...
import { bytesToHex, fetchFromProvider, intToHex } from '@silajs/util'
