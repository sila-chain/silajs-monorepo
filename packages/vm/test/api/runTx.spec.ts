...
import { Blockchain, createBlockchain } from '@silajs/blockchain'
...
import { SIGNER_A, goerliChainConfig } from '@silajs/testdata'
import { createAccountWithDefaults, getTransaction, setBalance } from './utils.ts'

import type { FeeMarketEIP1559TxData, LegacyTx, TypedTxData } from '@silajs/tx'
...
describe('runTx() -> successful API parameter usage', async () => {
  ...
  it('simple run (unmodified options)', async () => {
    let common = new Common({ chain: SilaMainnet, hardfork: Hardfork.London })
    let vm = await createVM({ common })
    await simpleRun(vm, 'sila-mainnet (PoW), london HF, default SM - should run without errors')
  ...
  it('test hardfork mismatch', async () => {
    ...
    block.common.setHardfork(Hardfork.SilaParis)
    try {
      await runTx(vm, { tx, block })
  ...
  it('Legacy Transaction with HF set to pre-Berlin', async () => {
    ...
    assert.isTrue(
      ...
      `sila-mainnet (PoW), istanbul HF, default SM - should run without errors (${TRANSACTION_TYPES[0].name})`,
    )
  })
...
describe('runTx() -> API parameter usage/data errors', () => {
  it('Typed Transaction with HF set to pre-Berlin', async () => {
    ...
    const tx = getTransaction(new Common({ chain: SilaMainnet, hardfork: Hardfork.Berlin }), 1, true)
    ...
    try {
      ...
      assert.isTrue(
        e.message.includes('(SIP-2718) not activated'),
  ...
  it('run with insufficient sip1559 funds', async () => {
    const vm = await createVM({ common })
...
describe('runTx() -> consensus bugs', () => {
  it('validate out-of-gas does not give any refunds', async () => {
    // There was a consensus bug in the following sila-mainnet tx:
    ...
    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.SpuriousDragon })
...
it('Validate CALL does not charge new account gas when calling CALLER and caller is non-empty', async () => {
  ...
  // Calls CALLER and sends back the SIL just sent with the transaction
...
describe('SIP 4844 transaction tests', () => {
  ...
  it('should work', async () => {
    ...
    // Stub getBlock to produce a valid parent header under SIP 4844
