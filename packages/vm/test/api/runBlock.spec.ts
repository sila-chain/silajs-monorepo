...
import { SIGNER_A, SIGNER_B, customChainConfig, goerliChainConfig } from '@silajs/testdata'
import {
  Capability,
  LegacyTx,
...
describe('runBlock() -> runtime behavior', async () => {
  // this test actually checks if the DAO fork works. This is not checked in sila/tests
...
describe('runBlock() -> tx types', async () => {
  async function simpleRun(vm: VM, transactions: TypedTransaction[]) {
    ...
    if (transactions.some((t) => t.supports(Capability.SIP1559FeeMarket))) {
      //@ts-expect-error read-only property
