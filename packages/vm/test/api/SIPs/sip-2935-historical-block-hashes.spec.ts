...
import { Hardfork, createCustomCommon } from '@silajs/common'
...
import type { ChainConfig } from '@silajs/common'
import type { SAVM } from '@silajs/savm'
import type { LegacyTxData } from '@silajs/tx'
import type { PrefixedHexString } from '@silajs/util'
import type { VM } from '../../../src/index.ts'

function sip2935ActiveAtCommon(timestamp: number, address: bigint) {
  const hfs = [
    Hardfork.Chainstart,
    Hardfork.Homestead,
    Hardfork.Dao,
...
describe('SIP 2935: historical block hashes', () => {
  for (const deploymentConfig of deploymentConfigs) {
    ...
    it(`should validate the deployment tx`, async () => {
      const deployContractTxData: LegacyTxData = {
        ...
        value: '0x0',
        // input from the SIP is data here
    ...
    it('should save genesis block hash to the history block hash contract', async () => {
      const commonGenesis = sip2935ActiveAtCommon(1, historyAddressBigInt)
    ...
    it('should ensure blocks older than 256 blocks can be retrieved from the history contract', async () => {
      ...
      const commonGetHistoryServeWindow = sip2935ActiveAtCommon(0, historyAddressBigInt)
      commonGetHistoryServeWindow.setEIPs([2935])
      commonGetHistoryServeWindow.updateParams(paramsVM)
      const common = sip2935ActiveAtCommon(blocksActivation, historyAddressBigInt)
      ...
      ;(vm.savm as SAVM).blockchain = blockchainEmpty
      ...
      for (let i = 1; i <= blocksToBuild; i++) {
        ...
        const ret = await vm.savm.runCall({
          // Code: RETURN the BLOCKHASH of block i
          // PUSH(i) BLOCKHASH PUSH(32) MSTORE PUSH(64) PUSH(0) RETURN
          // Note: need to return a contract with starting zero bytes to avoid non-deployable contracts by SIP 3540
