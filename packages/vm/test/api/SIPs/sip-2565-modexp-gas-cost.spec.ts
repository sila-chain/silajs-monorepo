...
import { Address, bytesToHex, createZeroAddress, equalsBytes, hexToBytes } from '@silajs/util'
import { assert, describe, it } from 'vitest'

import { createVM } from '../../../src/index.ts'

// See https://github.com/holiman/go-sila/blob/2c99023b68c573ba24a5b01db13e000bd9b82417/core/vm/testdata/precompiles/modexp_eip2565.json
import { sip2565Data } from '../testdata/sip-2565.ts'

describe('SIP-2565 ModExp gas cost tests', () => {
  it('Test return data, gas cost and execution status against testdata', async () => {
    const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Byzantium, sips: [2565] })
    const vm = await createVM({ common })

    for (const test of sip2565Data) {
      const testName = test.Name
      const to = new Address(hexToBytes('0x0000000000000000000000000000000000000005'))
      const result = await vm.savm.runCall({
