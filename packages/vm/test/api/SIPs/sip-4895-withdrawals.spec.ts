import { createBlock, genWithdrawalsTrieRoot } from '@silajs/block'
...
import { decode } from '@silajs/rlp'
import { withdrawalsGethGenesis } from '@silajs/testdata'
...
describe('SIP4895 tests', () => {
  it('SIP4895: withdrawals execute as expected', async () => {
  ...
  it('SIP4895: state update should exclude 0 amount updates', async () => {
    const blockchain = await createBlockchain()
    const vm = await createVM({ common, blockchain })
  ...
  it('should build a block correctly with withdrawals', async () => {
    ...
    common.setHardfork(Hardfork.SilaShanghai)
