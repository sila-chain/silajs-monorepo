...
import { Account, Address, bytesToHex, hexToBytes } from '@silajs/util'
...
describe('SIP-3529 tests', () => {
  const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Berlin, sips: [3529] })

  it('should verify SIP test cases', async () => {
    ...
    for (const testCase of testCases) {
      ...
      vm.savm.journal.addAlwaysWarmSlot(bytesToHex(address.bytes), bytesToHex(key))

      await vm.savm.runCode!({
        code,
        to: address,
        gasLimit,
      })
