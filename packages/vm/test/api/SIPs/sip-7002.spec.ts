...
const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.SilaCancun, sips: [7685, 7002] })

// Note: this deployment tx data is the deployment tx in order to setup the SIP-7002 contract
// It is taken from the SIP
...
describe('SIP-7002 tests', () => {
  ...
  it('should throw when contract is not deployed', async () => {
    ...
    try {
      ...
      assert.isTrue(e.message.includes('Attempt to accumulate SIP-7002 requests failed') === true)
    }
  })
})
