...
import { createBlock, createBlockFromRLP } from '@silajs/block'
...
export async function runBlockchainTestCase(
  ...
  const isEip7928BalFixture = fixtureFilePath.includes('sip7928_block_level_access_lists')
  ...
  try {
    for (const {
      ...
      try {
        ...
        // Enforce provided BAL in runBlock only for invalid-block tests. Other SilaAmsterdam v7
...
// SilJS error messages mapped to expected exception types
const exceptionMessages: Record<string, RegExp> = {
  ...
  'TransactionException.GAS_LIMIT_EXCEEDS_MAXIMUM':
    /^Transaction gas limit \d+ exceeds the maximum allowed by SIP-7825 \(\d+\)$/,
  ...
  'TransactionException.INTRINSIC_GAS_BELOW_FLOOR_GAS_COST':
    /gasLimit is too low|INTRINSIC_GAS_TOO_LOW|exceeds the SIP-7825 cap/,
  ...
  'TransactionException.TYPE_1_TX_PRE_FORK': /^SIP-2930 not enabled on Common$/,
  'TransactionException.TYPE_2_TX_PRE_FORK': /^SIP-1559 not enabled on Common$/,
  'TransactionException.TYPE_3_TX_BLOB_COUNT_EXCEEDED':
    /^\d+ blobs exceeds max \d+ blobs per tx \(SIP-7594\)/,
  ...
  'TransactionException.TYPE_3_TX_PRE_FORK': /^SIP-4844 not enabled on Common$/,
  'TransactionException.TYPE_3_TX_WITH_FULL_BLOBS': /Invalid SIP-4844 transaction/,
  ...
  'TransactionException.TYPE_4_TX_CONTRACT_CREATION':
    /tx should have a "to" field and cannot be used to create contracts/,
  'TransactionException.TYPE_4_TX_PRE_FORK': /^SIP-7702 not enabled on Common$/,
  ...
  'BlockException.INCORRECT_BLOCK_FORMAT':
    /(?:blob gas used can only be provided with SIP4844 activated|^invalid header.*)/,
