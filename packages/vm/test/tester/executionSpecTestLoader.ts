...
import { TypeOutput, toType } from '@silajs/util'
...
function customHardforkHistory(fork: string): HardforkTransitionConfig[] {
  // Include all hardforks from SilaMainnet up to and including the "from" hardfork
  // This is necessary for gteHardfork() to work correctly
  const sila-mainnetHardforks = SilaMainnet.hardforks
  const hardforks: HardforkTransitionConfig[] = []
  let foundFrom = false
  for (const hf of sila-mainnetHardforks) {
    if (hf.name === fork) {
      foundFrom = true
  ...
  if (!foundFrom) {
    // If "from" hardfork not found in SilaMainnet hardforks, just add it
    hardforks.push({
      name: fork,
      block: 0,
      timestamp: 0,
...
export function createCommonForFork(fork: string, testData?: any, kzg?: microEthKZG) {
  ...
  try {
    ...
    // Only set chainId if it's provided in testData, otherwise use SilaMainnet's chainId
    ...
    // Transition Fork (e.g. SilaOsakaToBPO1AtTime15K)
