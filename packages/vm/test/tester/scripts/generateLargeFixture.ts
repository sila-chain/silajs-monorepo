...
import { Common, ConsensusType, Hardfork, SilaMainnet } from '@silajs/common'
...
import { buildBlock, createVM, runBlock } from '@silajs/vm'
...
const activatedFromGenesisHardforks = SilaMainnet.hardforks.map((hardfork) => ({
  ...hardfork,
...
const referenceFixturePath = path.join(
  repoRoot,
  'packages/execution-spec-tests/dev/blockchain_tests/amsterdam/glamsterdam_devnet_v611_mixed_with_other_eips/sip7928_block_level_access_lists/block_access_lists/bal_all_transaction_types.json',
)
const defaultOutputPath = path.join(
  repoRoot,
  'packages/execution-spec-tests/dev/blockchain_tests/amsterdam/glamsterdam_devnet_v611_mixed_with_other_eips/sip7928_block_level_access_lists/block_access_lists/bal_large_generated_fixture.json',
)
...
async function main() {
  ...
  const common = new Common({
    chain: {
      ...SilaMainnet,
      defaultHardfork: Hardfork.SilaAmsterdam,
      consensus: {
  ...
  const fixtureId =
    'tests/amsterdam/sip7928_block_level_access_lists/test_block_access_lists_large_generated_fixture[fork_SilaAmsterdam-blockchain_test]'
  const description = [
    `Synthetic SilaAmsterdam BAL stress fixture with ${blocks} blocks.`,
    `Each block targets a mean of ${txsPerBlock} transactions, 200 internal contract calls, and ${withdrawalsPerBlock} withdrawals.`,
    'Scenarios include legacy, SIP-2930, SIP-1559, SIP-4844, and SIP-7702 transactions; access-list warming; contract storage writes; internal CALL/DELEGATECALL/STATICCALL/precompile fan out; CREATE/CREATE2; self-destruct calls; history/beacon-root queries; and SIP-7002 withdrawal requests.',
  ].join('\n')
  ...
  const fixtureInfo = {
    ...
    comment: 'silajs generated stress fixture',
    'filling-transition-tool': '@silajs/vm buildBlock',
    description,
