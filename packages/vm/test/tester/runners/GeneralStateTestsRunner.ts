...
import { Account, bytesToHex, createAddressFromString, toBytes } from '@silajs/util'
...
async function runTestCase(options: any, testData: any, t: typeof assert) {
  ...
  // Otherwise sila-mainnet genesis will throw since this has difficulty nonzero
  ...
  await vm.savm.journal.putAccount(coinbaseAddress, account ?? new Account())
  ...
  if (tx) {
    if (tx.isValid()) {
      ...
      if (options.jsontrace === true) {
        vm.savm.events!.on('step', stepHandler)
  ...
  vm.savm.events!.removeListener('step', stepHandler)
