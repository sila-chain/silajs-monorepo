...
export function getArguments() {
  const argsParsed = yargs(hideBin(process.argv))
    ...
    .option('trace', {
      describe: 'Write SAVM traces as JSON to trace-<tx-number>-<tx-hash>.json',
      type: 'boolean',
      default: false,
    })
    .strict()
...
/**
 * Formats an individual SAVM step trace as a JSON object
 * @param step an {@link InterpreterStep} emitted by the SAVM `step` event
 * @param memory whether to include the memory in the trace
 * @returns a JSON object that matches the SIP-7756 trace format
 */
export const stepTraceJSON = (step: InterpreterStep, memory: boolean = false) => {
  ...
  const opTrace = {
    ...
    memSize,
    stack: hexStack,
    depth: step.depth + 1, // Depth starts at 1 - SIP-7756
...
/**
 * Formats an individual SAVM summary trace as a JSON object
 * @param event an {@link AfterTxEvent} emitted by the vm `afterTx` event
 * @param vm a {@link VM} instance
 * @returns a JSON object that matches the SIP-7756 summary object format
 */
