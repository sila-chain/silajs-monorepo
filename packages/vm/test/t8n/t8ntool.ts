...
import { EVMMockBlockchain, NobleBLS } from '@silajs/savm'
import { RLP } from '@silajs/rlp'
import { createTx } from '@silajs/tx'
import { bigIntToHex, bytesToHex, hexToBytes, toBytes } from '@silajs/util'
...
import type { TypedTxData } from '@silajs/tx'
import type { CLRequest, CLRequestType, PrefixedHexString } from '@silajs/util'
...
/**
 * Normalizes txData to use with SilaJS keywords. For instance, 1559-txs have `v` fields on the inputs, where SilaJS expects `yParity`
  ...
  private async run(args: T8NOptions) {
    ...
    if (args.trace === true) {
      ...
      if (args.state.reward !== BigInt(-1)) {
        await rewardAccount(this.vm.savm, block.header.coinbase, args.state.reward, this.vm.common)
        await this.vm.savm.journal.cleanup()
      }
