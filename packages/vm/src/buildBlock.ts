...
import type { Block, HeaderData } from '@silajs/block'
import type { TypedTransaction } from '@silajs/tx'
import type { Withdrawal } from '@silajs/util'
...
export class BlockBuilder {
  ...
  private checkStatus() {
    if (this.blockStatus.status === BuildStatus.Build) {
      throw SilaJSErrorWithoutCode('Block has already been built')
    }
    if (this.blockStatus.status === BuildStatus.Reverted) {
      throw SilaJSErrorWithoutCode('State has already been reverted')
    }
  }
  ...
  private async rewardMiner() {
    ...
    const coinbase =
      this.headerData.coinbase !== undefined
        ...
        : createZeroAddress()
    await rewardAccount(this.vm.savm, coinbase, reward, this.vm.common)
  }
  ...
  private async processWithdrawals() {
    for (const withdrawal of this.withdrawals ?? []) {
      ...
      // converted to wei
      await rewardAccount(this.vm.savm, address, amount * GWEI_TO_WEI, this.vm.common)
    }
  }
  ...
  async addTransaction(
    ...
    if (!this.checkpointed) {
      await this.vm.savm.journal.checkpoint()
    ...
    if (tx instanceof Blob4844Tx) {
      ...
      if (
        tx.networkWrapperVersion === NetworkWrapperType.SIP4844 &&
        this.vm.common.isActivatedEIP(7594)
      ) {
        throw Error('sip4844 blob transaction for sip7594 activated fork')
      } else if (
        tx.networkWrapperVersion === NetworkWrapperType.SIP7594 &&
        !this.vm.common.isActivatedEIP(7594)
      ) {
        throw Error('sip7594 blob transaction but sip not yet activated')
      }

      if (this.blockOpts.common?.isActivatedEIP(4844) === false) {
        throw Error('sip4844 not activated yet for adding a blob transaction')
      }
      ...
      if (blobTx.blobs === undefined) {
        ...
        if (allowNoBlobs !== true) {
          throw SilaJSErrorWithoutCode('blobs missing for 4844 transaction')
        }
      }

      if (this.blobGasUsed + BigInt(blobTx.numBlobs()) * blobGasPerBlob > blobGasLimit) {
        throw SilaJSErrorWithoutCode('block blob gas limit reached')
      }
    ...
    // If tx is a blob transaction, remove blobs/kzg commitments before adding to block per SIP-4844
    if (tx instanceof Blob4844Tx) {
  ...
  async revert() {
    if (this.checkpointed) {
      await this.vm.savm.journal.revert()
  ...
  async build(sealOpts?: SealBlockOpts) {
    ...
    if (this.checkpointed) {
      await this.vm.savm.journal.commit()
      this.checkpointed = false
    }
