import { createBlock, genRequestsRoot } from '@silajs/block'
import { ConsensusType, Hardfork } from '@silajs/common'
import { type SAVM, type EVMInterface, computeIntrinsicGasDimensions8037 } from '@silajs/savm'
import { MerklePatriciaTrie } from '@silajs/mpt'
import { RLP } from '@silajs/rlp'
import { TransactionType } from '@silajs/tx'
import {
  Account,
  Address,
...
const withdrawalsRewardsCommitLabel = 'Withdrawals, Rewards, SAVM journal commit'
...
export async function runBlock(vm: VM, opts: RunBlockOpts): Promise<RunBlockResult> {
  ...
  if (vm.common.isActivatedEIP(7928)) {
    vm.savm.blockLevelAccessList = createBlockLevelAccessList()
  }
  ...
  if (
    ...
    await _applyDAOHardfork(vm.savm)
  ...
  if (vm.common.isActivatedEIP(7928) && vm.savm.blockLevelAccessList !== undefined) {
    try {
      validateBlockAccessListGasLimit(vm.savm.blockLevelAccessList, block.header.gasLimit)
    } catch {
      await vm.savm.journal.revert()
      if (vm.DEBUG) {
        debug(`block checkpoint reverted`)
  ...
  if (generateFields) {
    ...
    if (vm.common.isActivatedEIP(7928) && vm.savm.blockLevelAccessList !== undefined) {
      generatedFields.blockAccessListHash = vm.savm.blockLevelAccessList.hash()
    }
    const blockData = {
      ...block,
    ...
    try {
      ...
      if (vm.common.isActivatedEIP(7928) && providedBlockAccessList !== undefined) {
        const generated = vm.savm.blockLevelAccessList
        if (
          generated === undefined ||
      ...
      if (vm.common.isActivatedEIP(7864)) {
        if (vm.savm.binaryTreeAccessWitness === undefined) {
          throw Error(`binaryTreeAccessWitness required if binary tree (SIP-7864) is activated`)
        }
        ...
        if (
          (await vm['_opts'].stateManager!.verifyBinaryTreePostState!(
            vm.savm.binaryTreeAccessWitness,
          )) === false
        ) {
  ...
  const results: RunBlockResult = {
    ...
    requestsHash,
    requests,
    blockLevelAccessList: vm.savm.blockLevelAccessList,
  }
...
async function applyBlock(vm: VM, block: Block, opts: RunBlockOpts): Promise<ApplyBlockResult> {
  ...
  if (opts.reportPreimages === true) {
    if (vm.savm.stateManager.getAppliedKey === undefined) {
      throw SilaJSErrorWithoutCode(
        'applyBlock: savm.stateManager.getAppliedKey can not be undefined if reportPreimages is true',
      )
    }
    blockResults.preimages.set(
      bytesToHex(vm.savm.stateManager.getAppliedKey(block.header.coinbase.toBytes())),
  ...
  if (vm.common.isActivatedEIP(4895)) {
    if (opts.reportPreimages === true) vm.savm.journal.startReportingPreimages!()
    await assignWithdrawals(vm, block)
    if (opts.reportPreimages === true && vm.savm.journal.preimages !== undefined) {
      for (const [key, preimage] of vm.savm.journal.preimages) {
        blockResults.preimages.set(key, preimage)
      }
    }
  ...
  if (vm.common.isActivatedEIP(7864) && vm.savm.systemBinaryTreeAccessWitness !== undefined) {
    vm.savm.systemBinaryTreeAccessWitness?.commit()
    if (vm.DEBUG) {
      debug('Binary tree access witness aggregate costs:')
      vm.savm.binaryTreeAccessWitness?.debugWitnessCost()
      debug('System binary tree access witness aggregate costs:')
      vm.savm.systemBinaryTreeAccessWitness?.debugWitnessCost()
    }
    vm.savm.binaryTreeAccessWitness?.merge(vm.savm.systemBinaryTreeAccessWitness)
  }
...
/**
 * vm method runs the logic of SIP 2935 (save blockhashes to state)
 * It will put the `parentHash` of the block to the storage slot of `block.number - 1` of the history storage contract.
 * vm contract is used to retrieve BLOCKHASHes in SAVM if SIP 2935 is activated.
 * In case that the previous block of `block` is pre-SIP-2935 (so we are on the SIP 2935 fork block), additionally
...
export async function accumulateParentBlockHash(
  ...
  if (!vm.common.isActivatedEIP(2935)) {
    throw SilaJSErrorWithoutCode(
      'Cannot call `accumulateParentBlockHash`: SIP 2935 is not active',
    )
  }
  ...
  async function putBlockHash(vm: VM, hash: Uint8Array, number: bigint) {
    ...
    if (vm.common.isActivatedEIP(7864)) {
      if (vm.savm.systemBinaryTreeAccessWitness === undefined) {
        throw Error(`systemBinaryTreeAccessWitness required if binary tree (SIP-7864) is activated`)
      }
      // Add to system binary tree access witness so that it doesn't warm up tx accesses
      vm.savm.systemBinaryTreeAccessWitness.writeAccountStorage(historyAddress, ringKey)
    }
    const key = setLengthLeft(bigIntToBytes(ringKey), 32)
    if (vm.common.isActivatedEIP(7928)) {
      vm.savm.blockLevelAccessList!.addStorageWrite(
...
export async function accumulateParentBeaconBlockRoot(vm: VM, root: Uint8Array, timestamp: bigint) {
  if (!vm.common.isActivatedEIP(4788)) {
    throw SilaJSErrorWithoutCode(
      'Cannot call `accumulateParentBeaconBlockRoot`: SIP 4788 is not active',
    )
  }
...
async function applyTransactions(vm: VM, block: Block, opts: RunBlockOpts) {
  ...
  // Block header gas accounting (SIP-7778: no refund subtraction).
  // Under SIP-8037 this is the bottleneck of two dimensions:
  ...
  // SIP-8037 per-dimension accumulators
  ...
  for (let txIdx = 0; txIdx < block.transactions.length; txIdx++) {
    if (vm.common.isActivatedEIP(7928)) {
      vm.savm.blockLevelAccessList!.blockAccessIndex = txIdx + 1
    }
    ...
    // SIP-8037 pre-execution check (spec):
    ...
    // execution-specs/tests/amsterdam/sip8037.../test_state_gas_reservoir.py
    // (`test_block_state_gas_limit_boundary` and
    // `test_creation_tx_regular_check_subtracts_intrinsic_state`).
    // Pre-SIP-8037 keeps the original check (`tx.gasLimit + gasUsed <= block.gasLimit`).
    ...
    // Pre-SIP-8037: single-dimension accumulator (blockGasSpent already
    // applies the SIP-7623 calldata floor and respects SIP-7778's no-refund
    // semantics).
    // SIP-8037: track regular and state dimensions independently and use
    ...
    if (vm.common.isActivatedEIP(8037) && txRes.txRegularGas !== undefined) {
      ...
      // the SIP-7623 calldata floor (max(intrinsic+exec_regular, floorCost))
...
async function assignWithdrawals(vm: VM, block: Block): Promise<void> {
  if (vm.common.isActivatedEIP(7928)) {
    vm.savm.blockLevelAccessList!.blockAccessIndex = block.transactions.length + 1
  }
  ...
  for (const withdrawal of withdrawals) {
    ...
    await rewardAccount(vm.savm, address, amount * GWEI_TO_WEI, vm.common)
  }
}
...
async function assignBlockRewards(vm: VM, block: Block): Promise<void> {
  ...
  for (const ommer of ommers) {
    ...
    const account = await rewardAccount(vm.savm, ommer.coinbase, reward, vm.common)
    if (vm.DEBUG) {
  ...
  const account = await rewardAccount(vm.savm, block.header.coinbase, reward, vm.common)
  if (vm.DEBUG) {
...
export async function rewardAccount(
  ...
  let account = await savm.stateManager.getAccount(address)
  if (account === undefined) {
    if (common.isActivatedEIP(7864) === true && reward !== BIGINT_0) {
      if (savm.systemBinaryTreeAccessWitness === undefined) {
        throw Error(`systemBinaryTreeAccessWitness required if binary tree (SIP-7864) is activated`)
      }
      savm.systemBinaryTreeAccessWitness.writeAccountHeader(address)
    }
    account = new Account()
  }
  ...
  if (common.isActivatedEIP(7928)) {
    if (reward === BIGINT_0) {
      savm.blockLevelAccessList?.addAddress(address.toString())
    } else {
      savm.blockLevelAccessList!.addBalanceChange(
        address.toString(),
        account.balance,
        savm.blockLevelAccessList!.blockAccessIndex,
        originalBalance,
      )
    }
  }
  await savm.journal.putAccount(address, account)

  if (common.isActivatedEIP(7864) === true && reward !== BIGINT_0) {
    if (savm.systemBinaryTreeAccessWitness === undefined) {
      throw Error(`systemBinaryTreeAccessWitness required if binary tree (SIP-7864) is activated`)
    }
    savm.systemBinaryTreeAccessWitness.writeAccountBasicData(address)
    savm.systemBinaryTreeAccessWitness.readAccountCodeHash(address)
  }
  return account
}
...
export function encodeReceipt(
  ...
  // Serialize receipt according to SIP-2718:
 ...
 */
async function _applyDAOHardfork(savm: EVMInterface) {
  const state = savm.stateManager
  ...
  if ((await state.getAccount(DAORefundContractAddress)) === undefined) {
    await savm.journal.putAccount(DAORefundContractAddress, new Account())
  }
  ...
  for (const addr of DAOAccountList) {
    ...
    await savm.journal.putAccount(address, account)
    if (savm.common.isActivatedEIP(7928)) {
      savm.blockLevelAccessList!.addBalanceChange(
        address.toString(),
        account.balance,
        savm.blockLevelAccessList!.blockAccessIndex,
  ...
  await savm.journal.putAccount(DAORefundContractAddress, DAORefundAccount)
  if (savm.common.isActivatedEIP(7928)) {
    savm.blockLevelAccessList!.addBalanceChange(
...
function parseProvidedBlockAccessList(
  ...
  if (isAccountOrderOnlyViolation(json)) {
    throw SilaJSErrorWithoutCode('invalid header: block access list accounts are not sorted')
  }
  if (blockAccessListHash !== undefined && generateFields === false) {
