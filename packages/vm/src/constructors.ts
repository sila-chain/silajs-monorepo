...
import { EVMMockBlockchain, createEVM, getActivePrecompiles } from '@silajs/savm'
...
export async function createVM(opts: VMOpts = {}): Promise<VM> {
  ...
  // Add common, SM, blockchain, SAVM here
  ...
  if (opts.savm !== undefined && opts.evmOpts !== undefined) {
    throw SilaJSErrorWithoutCode('the savm and evmOpts options cannot be used in conjunction')
  }

  if (opts.savm === undefined) {
    let enableProfiler = false
    if (opts.profilerOpts?.reportAfterBlock === true || opts.profilerOpts?.reportAfterTx === true) {
      enableProfiler = true
    }
    const evmOpts = opts.evmOpts ?? {}
    opts.savm = await createEVM({
      common: opts.common,
  ...
  if (opts.activatePrecompiles === true && !didPassStateManager) {
    await opts.savm.journal.checkpoint()
    ...
    for (const [addressStr] of getActivePrecompiles(opts.common)) {
      ...
      let account = await opts.savm.stateManager.getAccount(address)
      ...
      if (account === undefined) {
        ...
        await opts.savm.stateManager.putAccount(address, newAccount)
      }
    }
    await opts.savm.journal.commit()
  }
