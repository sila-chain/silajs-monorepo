import { SilaMainnet } from '@silajs/common'
import { activeCostPerStateByte } from '@silajs/savm'
...
/**
 * SIP-8037 system-call gas split.
 ...
 * portion (set on vm.savm before the call). The block argument is required
...
/**
 * Snapshot/restore the per-SAVM 8037 reservoir state around a system call
...
function setupSystemCallReservoir(vm: VM, reservoir: bigint) {
  ...
  const savm = vm.savm
  const prior = {
    reservoir: savm.stateGasReservoir,
    used: savm.executionStateGasUsed,
    snapshots: (savm as unknown as { _stateGasSnapshots: unknown[] })._stateGasSnapshots,
  }
  savm.stateGasReservoir = reservoir
  savm.executionStateGasUsed = BIGINT_0
  ;(savm as unknown as { _stateGasSnapshots: unknown[] })._stateGasSnapshots = []
  return prior
}
...
function teardownSystemCallReservoir(
  ...
  const savm = vm.savm
  savm.stateGasReservoir = prior.reservoir
  savm.executionStateGasUsed = prior.used
  ;(savm as unknown as { _stateGasSnapshots: unknown[] })._stateGasSnapshots = prior.snapshots
}
...
function cloneSystemAccessEntry(vm: VM, systemAddressHex: PrefixedHexString) {
  const access = vm.savm.blockLevelAccessList?.accesses[systemAddressHex]
  if (access === undefined) {
    return undefined
  }
...
function restoreSystemAccessEntry(
  ...
  const bal = vm.savm.blockLevelAccessList
  if (bal === undefined) {
    return
  }
...
export const accumulateRequests = async (
  ...
  if (common.isActivatedEIP(6110)) {
    const depositContractAddress =
      vm.common['_chainParams'].depositContractAddress ?? SilaMainnet.depositContractAddress
    if (depositContractAddress === undefined)
      throw SilaJSErrorWithoutCode('deposit contract address required with SIP 6110')
...
function parseDepositLog(requestData: Uint8Array) {
  if (requestData.length !== LOG_SIZE) {
    throw SilaJSErrorWithoutCode(LOG_LAYOUT_MISMATCH)
  }
