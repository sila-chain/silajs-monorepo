...
import type { EVMInterface, EVMMockBlockchainInterface } from '@silajs/savm'
import { isDebugEnabled } from '@silajs/util'
import type { BigIntLike } from '@silajs/util'
...
export class VM {
  ...
  async shallowCopy(downlevelCaches = true): Promise<VM> {
    ...
    const evmCopy = await createEVM(evmOpts) // TODO fixme (should copy the EVMInterface, not default SAVM)
    return createVM({
      stateManager,
