import type { Block, BlockOptions, HeaderData } from '@silajs/block'
import type { Common, ParamsDict, StateManagerInterface } from '@silajs/common'
import type {
  EVMInterface,
...
import type { AccessList, TypedTransaction } from '@silajs/tx'
import type {
...
export type TxReceipt = PreByzantiumTxReceipt | PostByzantiumTxReceipt | SIP4844BlobTxReceipt
 ...
 * state root field with a status code field (SIP-658)
 */
...
export interface SIP4844BlobTxReceipt extends PostByzantiumTxReceipt {
  /**
...
export interface RunBlockOpts {
   ...
   * When set and SIP-7928 is active, {@link runBlock} validates structure and header hash
   * before execution and RLP equality against the generated list after execution.
   *
   * @remarks Experimental (SilaAmsterdam): may change on patch releases. See `@silajs/vm`
...
export interface RunTxOpts {
  /**
   * The `@silajs/block` the `tx` belongs to.
  ...
  /**
   * If true, adds a generated SIP-2930 access list
   * to the `RunTxResult` returned.
   *
   * Option works with all tx types. SIP-2929 needs to
