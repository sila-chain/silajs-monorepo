...
`@silajs/vm` is the block-and-transaction execution orchestrator: it wires state, the SAVM, consensus checks, and receipts into the high-level APIs most integrators call (`runBlock()`, `runTx()`, `createVM()`). Within the `10.1.2` round, this package carries the bulk of SilaAmsterdam execution logic — BAL builder/validator flows, two-dimensional block gas, and receipt-level fork behaviour — along with spec-alignment fixes and an internal `runTx()` refactor since `10.1.1`.

### At a glance

- End-to-end **SilaAmsterdam** block execution on `Hardfork.SilaAmsterdam`: nine SIPs active together, matching how execution-spec-tests and devnets exercise the fork.
- **SIP-7928** Block Level Access Lists (BAL): generate during `runBlock({ generate: true })`, validate with `RunBlockOpts.blockAccessList`, read back via `RunBlockResult.blockLevelAccessList`.
- **SIP-8037 / SIP-7778** two-dimensional block gas and refund-aware header accounting — surfaced on `RunTxResult` without extra VM flags.
- Passes the v700 mixed EST slice for [tests-bal@v7.1.0](https://github.com/sila-chain/execution-specs/releases/tag/tests-bal@v7.1.0).
...
| | |
| --- | --- |
| **Spec snapshot** | [tests-bal@v7.1.0](https://github.com/sila-chain/execution-specs/releases/tag/tests-bal@v7.1.0) |
| **Testnet** | [BAL devnet-7](https://notes.sila.org/@ethpandaops/bal-devnet-7) |
| **Full overview** | [SilaAmsterdam hardfork (experimental)](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/vm#amsterdam-hardfork-experimental) |

When SIP-7928 is active the VM accumulates state accesses automatically during execution — no separate opt-in flag. For **block building**, run the block with `generate: true` and read the BAL from the result; the `afterBlock` event delivers a block whose header already carries the matching `blockAccessListHash`. For **validation**, pass a known BAL (JSON, RLP, or `BlockLevelAccessList`) via `blockAccessList`; the VM checks structure and hash before execution and compares against the list produced afterward.
...
import { Account, createAddressFromPrivateKey, createZeroAddress, hexToBytes } from '@silajs/util'
...
Under **SIP-8037**, each transaction contributes to separate regular and state gas dimensions (`RunTxResult.txRegularGas`, `txStateGas`); block header gas used becomes `max(block_regular_gas, block_state_gas)`. Under **SIP-7778**, `RunTxResult.blockGasSpent` is what counts toward the header (refunds excluded), while `totalGasSpent` is what the sender pays. **SIP-7708** transfer/burn logs appear in receipts like ordinary logs.

Offline BAL parsing and hash checks live in [@silajs/util](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/util#module-bal); opcode-level details in [@silajs/savm](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/savm#supported-sips).

### Changes

- SIP-7928 BAL generate/validate integration in `runBlock()`, see PR [#4233](https://github.com/sila-chain/silajs-monorepo/pull/4233), [#4264](https://github.com/sila-chain/silajs-monorepo/pull/4264), [#4303](https://github.com/sila-chain/silajs-monorepo/pull/4303), [#4304](https://github.com/sila-chain/silajs-monorepo/pull/4304), [#4306](https://github.com/sila-chain/silajs-monorepo/pull/4306)
- SIP-8037 two-dimensional block gas and state-gas reservoir, see PR [#4285](https://github.com/sila-chain/silajs-monorepo/pull/4285), [#4293](https://github.com/sila-chain/silajs-monorepo/pull/4293), [#4301](https://github.com/sila-chain/silajs-monorepo/pull/4301), [#4307](https://github.com/sila-chain/silajs-monorepo/pull/4307)
- SIP-7976 calldata floor enforcement during block execution, see PR [#4280](https://github.com/sila-chain/silajs-monorepo/pull/4280)
- SIP-7778 block gas accounting, see PR [#4248](https://github.com/sila-chain/silajs-monorepo/pull/4248), [#4307](https://github.com/sila-chain/silajs-monorepo/pull/4307)
- SIP-7708 / SIP-7843 execution-layer behaviour, see PR [#4239](https://github.com/sila-chain/silajs-monorepo/pull/4239), [#4263](https://github.com/sila-chain/silajs-monorepo/pull/4263), [#4301](https://github.com/sila-chain/silajs-monorepo/pull/4301)
- Structural refactor and cleanup of `runTx()`, see PR [#4240](https://github.com/sila-chain/silajs-monorepo/pull/4240)
- Execution-spec-tests runner and fixture updates through v7.1.0, see PR [#4207](https://github.com/sila-chain/silajs-monorepo/pull/4207), [#4298](https://github.com/sila-chain/silajs-monorepo/pull/4298)
- SilaAmsterdam documentation and release ↔ spec tracking table, see PR [#4308](https://github.com/sila-chain/silajs-monorepo/pull/4308)

## 10.1.1 - 2025-01-28

- Revert state changes if header validation fails in `runBlock`, see PR [#4227](https://github.com/sila-chain/silajs-monorepo/pull/4227)
- Use SIP-defined `systemCallGasLimit` for system contract calls (SIP-7251/SIP-7002), see PR [#4223](https://github.com/sila-chain/silajs-monorepo/pull/4223)
- SIP-3607 (reject transactions from senders with deployed code) now treated as always active, see PR [#4210](https://github.com/sila-chain/silajs-monorepo/pull/4210)
...
- `SIP-6110` log layout check fix, PR [#3977](https://github.com/sila-chain/silajs-monorepo/pull/3977)
- Convert `StatelessVerkleStateManager` usage to type (bundle optimizations) and small Verkle interface adjustment, PRs [#4021](https://github.com/sila-chain/silajs-monorepo/pull/4021) and [#4022](https://github.com/sila-chain/silajs-monorepo/pull/4022)
...
- Additional SIP-7685 adjustments/updates, PR [#3807](https://github.com/sila-chain/silajs-monorepo/pull/3807)
...
- Adjust `BlockBuilder.build()` return value format to return a dictionary containing the block built together with the requests generated, PR [#3706](https://github.com/sila-chain/silajs-monorepo/pull/3706)
- SIP-7702 related updates (devnet-4), PR [#3737](https://github.com/sila-chain/silajs-monorepo/pull/3737)
- More SIP-7702 updates (devnet-5), PR [#3807](https://github.com/sila-chain/silajs-monorepo/pull/3807)
- Fix requests contract address (devnet-5), PR [#3818](https://github.com/sila-chain/silajs-monorepo/pull/3818)
...
- Verkle execution witness support, PR [#3731](https://github.com/sila-chain/silajs-monorepo/pull/3731)
- Small adjustments, PR [#3775](https://github.com/sila-chain/silajs-monorepo/pull/3775)
- Add simple cache to SAVM access witness, PR [#3810](https://github.com/sila-chain/silajs-monorepo/pull/3810)

### Other Changes
- Support for `t8ntool` for `execution-spec-tests` test filling, PR [#3603](https://github.com/sila-chain/silajs-monorepo/pull/3603)
- Block builder adjustments, PR [#3603](https://github.com/sila-chain/silajs-monorepo/pull/3603)
- Filter SIP-6110 logs based on topic, PR [#3901](https://github.com/sila-chain/silajs-monorepo/pull/3901)
...
The static constructors for our library classes have been reworked to now be standalone methods (with a similar naming scheme). This allows for better tree shaking of unused constructor code (see PR [#3618](https://github.com/sila-chain/silajs-monorepo/pull/3618)):

- `VM.create()` -> `createVM`

##### Core Procedural Methods

See: PR [#3530](https://github.com/sila-chain/silajs-monorepo/pull/3530)
...
HF-sensitive parameters like `historyStorageAddress` were previously by design all provided by the `@silajs/common` library. This meant that all parameter sets were shared among the libraries and libraries carried around a lot of unnecessary parameters.

With the `Common` refactoring from PR [#3537](https://github.com/sila-chain/silajs-monorepo/pull/3537) parameters now moved over to a dedicated `params.ts` file (exposed as e.g. `paramsVM`) within the parameter-using library and the library sets its own parameter set by internally calling a new `Common` method `updateParams()`. For shared `Common` instances parameter sets then accumulate as needed.
...
This is the first SilaJS VM release where we could realize a fully WASM-free version of the underlying SAVM by default! 🤩 We were finally able to replace all crypto primitives which still relied on Web Assembly code with pure JavaScript/TypeScript pendants, thanks a lot to @paulmillr from Noble for the cooperation on this! ❤️

Together with a strong dependency reduction being accomplished along this release this opens up for new use cases for the JavaScript VM/SAVM in more security sensitive contexts. The code of the SAVM is now compact enough that it gets fully auditable (and we plan an SAVM audit for 2025), see e.g. [here](https://gist.github.com/holgerd77/2c032488196b4afee5d976dc85ee70eb) for an SAVM bundle snapshot including _all_ dependencies!

See SAVM release notes for a detailed breakdown of the changes!

### Mega-EOF Support (experimental)

The underlying SAVM now supports Mega-EOF in an experimental form! 🎉 See SAVM release notes for more details.

### SIP-7702 EOA Account Abstraction Support (experimental)

The VM now experimentally supports running [SIP-7702](https://sips.sila.org/SIPS/sip-7702) txs with `runTx()` (or `runBlock()`) and along transform an EOA into a smart contract for the period of one transaction and execute the respective bytecode, see PRs [#3470](https://github.com/sila-chain/silajs-monorepo/pull/3470), [#3577](https://github.com/sila-chain/silajs-monorepo/pull/3577) and [#3581](https://github.com/sila-chain/silajs-monorepo/pull/3581).
...
- VM is now by default initialized without state manager caches (tree shaking), PR [#3601](https://github.com/sila-chain/silajs-monorepo/pull/3601)
- VM uses `EVMMockBlockchain` as blockchain default (tree shaking), PR [#3601](https://github.com/sila-chain/silajs-monorepo/pull/3601)
...
- Implement `t8ntool` to use for `execution-spec-tests`, PR [#3603](https://github.com/sila-chain/silajs-monorepo/pull/3603)
- Upgrade `sila-cryptography` to v3, PR [#3668](https://github.com/sila-chain/silajs-monorepo/pull/3668)
- Exit early on non-existing system contracts, PR [#3614](https://github.com/sila-chain/silajs-monorepo/pull/3614)
- Add `allowNoBlobs` option to `BlockBuilder.addTransaction()`, PR [#3603](https://github.com/sila-chain/silajs-monorepo/pull/3603)
- Exit early in `runBlock()` if system contract has no code, PR [#3603](https://github.com/sila-chain/silajs-monorepo/pull/3603)
...
Starting with this release the VM supports [SIP-2935](https://sips.sila.org/SIPS/sip-2935) which stores the latest 8192 block hashes in the storage of a system contract, see PR [#3475](https://github.com/sila-chain/silajs-monorepo/pull/3475) as the major integration PR (while work on this has already been done in previous PRs).

This SIP will be activated along the SilaPrague hardfork. Note that this SIP has no effect on the resolution of the `BLOCKHASH` opcode, which will be a separate activation taking place by the integration of [SIP-7709](https://sips.sila.org/SIPS/sip-7709) in the following SilaOsaka hardfork.
...
- Missing beaconroot account verkle fix, PR [#3421](https://github.com/sila-chain/silajs-monorepo/pull/3421)
...
- Updates SIP-2935 tests with the new proposed bytecode and corresponding config, PR [#3438](https://github.com/sila-chain/silajs-monorepo/pull/3438)
- Fix SIP-2935 address conversion issues, PR [#3447](https://github.com/sila-chain/silajs-monorepo/pull/3447)
- Remove backfill of block hashes on SIP-2935 activation, PR [#3478](https://github.com/sila-chain/silajs-monorepo/pull/3478)

### Other Features

- Add `evmOpts` to the VM opts to allow for options chaining to the underlying SAVM, PR [#3481](https://github.com/sila-chain/silajs-monorepo/pull/3481)
...
- Small clean-up to `VM._emit()`, PR [#3396](https://github.com/sila-chain/silajs-monorepo/pull/3396)
- Update `mcl-wasm` Dependency (Esbuild Issue), PR [#3461](https://github.com/sila-chain/silajs-monorepo/pull/3461)

### Bugfixes

- Fix block building with blocks including CL requests, PR [#3413](https://github.com/sila-chain/silajs-monorepo/pull/3413)
- Ensure system address is not created if it is empty, PR [#3400](https://github.com/sila-chain/silajs-monorepo/pull/3400)

## 8.0.0 - 2024-03-18

### New SAVM.create() Async Static Constructor / Mandatory VM.create() Constructor
...
- Support for Preimage generation (verkle-related, experimental), new `reportPreimages` `VM.runTx()` option, PR [#3143](https://github.com/sila-chain/silajs-monorepo/pull/3143) and [#3298](https://github.com/sila-chain/silajs-monorepo/pull/3298)
...
### New SAVM/VM Profiler

This releases ships with a completely new dedicated SAVM/VM profiler (❤️ to Jochem for the integration) to measure how the different opcode implementations are doing, see PR [#2988](https://github.com/sila-chain/silajs-monorepo/pull/2988), [#3011](https://github.com/sila-chain/silajs-monorepo/pull/3011), [#3013](https://github.com/sila-chain/silajs-monorepo/pull/3013) and [#3041](https://github.com/sila-chain/silajs-monorepo/pull/3041).

Most of profiling is taking place in the SAVM (so: the dedicated opcode profiling), see the respective [README section](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/savm/README.md#profiling-the-savm) for usage instructions and the SAVM v2.1.0 [CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/savm/CHANGELOG.md) for latest performance gains.
...
The VM profiler addition now also allows for running blockchain or state tests with the profiler activated, e.g. to benchmark certain extreme-case or attack scenarios, see [DEVELOPER](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/vm/DEVELOPER.md) docs for usage instructions (see PR [#3115](https://github.com/sila-chain/silajs-monorepo/pull/3115)).
...
- Fix block builder `london` HF transition, PR [#3039](https://github.com/sila-chain/silajs-monorepo/pull/3039)

## Other Changes

- Allow for users to decide if to either downlevel (so: adopt them for a short-lived scenario) state caches or not on `shallowCopy()` by adding a new `downlevelCaches` parameter (default: `true`), PR [#3063](https://github.com/sila-chain/silajs-monorepo/pull/3063)
- Update sila tests to 12.3, PR [#2971](https://github.com/sila-chain/silajs-monorepo/pull/2971)
- Update sila tests to 12.4, PR [#3052](https://github.com/sila-chain/silajs-monorepo/pull/3052)
- Reactivate selected slow tests, PR [#2991](https://github.com/sila-chain/silajs-monorepo/pull/2991)
- Better error message for `runTx()` `gasLimit` check to avoid confusion with SIP1559 base fee, PR [#3118](https://github.com/sila-chain/silajs-monorepo/pull/3118)
...
See [RC1 release notes](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fvm%407.0.0-rc.1) for the main change description.
...
For the `VM` library we also updated the old concurrent `hardforkByBlockNumber` and `hardforkByTTD` options to a unified and simplified `setHardfork` option for both the constructor and within the `VM.runBlock()` method, see PR [#2800](https://github.com/sila-chain/silajs-monorepo/pull/2800).

### SIP-5656: MCOPY - Memory copying instruction

This release adds support in the underlying SAVM for [SIP-5656](https://sips.sila.org/SIPS/sip-5656) "MCOPY - Memory copying instruction" - which is scheduled to be activated along the [SilaCancun](https://github.com/sila-chain/execution-specs/blob/master/network-upgrades/sila-mainnet-upgrades/cancun.md) HF.
...
## Opcode Renamings (SAVM): SHA3 -> KECCAK, DIFFICULTY -> PREVRANDAO (post Merge)

In this release two opcodes (in the SAVM) have been renamed to more adequately match their functionality, see PR [#2706](https://github.com/sila-chain/silajs-monorepo/pull/2706).
...
- Integrate `dataGasUsed` block information for `VM.buildBlock()` and `VM.runTx()`, PR [#2636](https://github.com/sila-chain/silajs-monorepo/pull/2636)
...
- Fix `dataGasPrice` calculation in `VM.runTx()`, PR [#2779](https://github.com/sila-chain/silajs-monorepo/pull/2779)
- Update c-kzg to big endian implementation (`0x0a` KZG point evaluation precompile in SAVM), PR [#2746](https://github.com/sila-chain/silajs-monorepo/pull/2746)
...
- Replace `rustbn.js` with wasm-compiled `rustbn-wasm` module (in SAVM), PR [#2834](https://github.com/sila-chain/silajs-monorepo/pull/2834)
...
- Move KZG precompile address (in SAVM) from `0x14` to `0x0a`, PR [#2811](https://github.com/sila-chain/silajs-monorepo/pull/2811)
...
- Breaking: `VM._common` property has been renamed to `VM.common`, PR [#2857](https://github.com/sila-chain/silajs-monorepo/pull/2857)
- SIP-1153 TLOAD TSTORE update opcode byte (SAVM), PR [#2884](https://github.com/sila-chain/silajs-monorepo/pull/2884)

## 6.4.2 - 2023-04-20

### Features

- Diff-based Touched Accounts Checkpointing (fixes performance/memory bottlenecks), PR [#2581](https://github.com/sila-chain/silajs-monorepo/pull/2581)

### Bugfixes

- Cleanup touched accounts after withdrawals, PR [#2601](https://github.com/sila-chain/silajs-monorepo/pull/2601)
...
To run VM/SAVM related SIP-4844 functionality you have to active the SIP in the associated `@silajs/common` library:
...
SIP-4844 comes with a new opcode `DATAHASH` and adds a new point evaluation precompile at address `0x14` in the underlying `@silajs/savm` package.
...
The VM is now capable of running blob-including txs and blocks with `VM.runTx()` and `VM.runBlock()` taking the new gas costs for blob transactions into account. The underlying SAVM `v1.3.0` now supports the new `DATAHASH` opcode and the new point evaluation precompile.

The Block Builder API (see [README](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/vm)) has been expanded to now also allow for building blocks including shard blob transactions and calculate the correct values for data gas usage.

### Other Changes

- Added `minerValue` as a getter to the `BlockBuilder` and a result value for `RunTxResult`, PR [#2457](https://github.com/sila-chain/silajs-monorepo/pull/2457)
- New option `skipHardforkValidation` for `VM.runTx()` and `VM.runBlock()`, PR [#2486](https://github.com/sila-chain/silajs-monorepo/pull/2486)
- **Breaking** (for experimental feature): Changes withdrawal amount representation from WEI to GWEI, see PR [#2483](https://github.com/sila-chain/silajs-monorepo/pull/2483)
- More flexible logic to execute pre-/post Merge txs with `VM.runTx()` when exact HF is not known, see PR [#2505](https://github.com/sila-chain/silajs-monorepo/pull/2505)
- VM/EEI copy fixes, PR [#2529](https://github.com/sila-chain/silajs-monorepo/pull/2529)
- Block builder related logic updates in `VM.buildBlock()` and `VM.runTx()`, PR [#2533](https://github.com/sila-chain/silajs-monorepo/pull/2533)
...
Withdrawals support can be activated by initializing a respective `Common` object, see [@silajs/block](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/block) library README for an example on how to instantiate a withdrawals block:
...
In the VM withdrawals blocks can now both be executed with `VM.runBlock()` and build with `VM.buildBlock()` (for a more complex example you can have a look at the SIP tests in `test/api/SIPs/sip-4895-withdrawals.spec.ts`).

### Hardfork-By-Time Support

The VM library is now ready to work with hardforks triggered by timestamp, which will first be applied along the `SilaShanghai` HF, see PR [#2437](https://github.com/sila-chain/silajs-monorepo/pull/2437). This is achieved by integrating a new timestamp supporting `@silajs/common` library version.

### Bug Fixes and Other Changes

- More correctly timed `nonce` updates in `VM.runTx()` to avoid certain consensus-critical `nonce`/`account` update constellations. PR [#2404](https://github.com/sila-chain/silajs-monorepo/pull/2404)

A reminder: This package does not contain the core SAVM code any more. For SAVM related bugfixes see the associated [@silajs/savm](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/savm) `v1.2.3` release.
...
- Migrated from `rbtree` to [js-sdsl](https://github.com/js-sdsl/js-sdsl) package for caching functionality (maintained, better performance), PR [#2285](https://github.com/sila-chain/silajs-monorepo/pull/2285)
- Added [retesteth](https://github.com/sila-chain/retesteth) support for transition-tool protocol, PR [#2088](https://github.com/sila-chain/silajs-monorepo/pull/2088)

## 6.0.0 - 2022-09-06

Final release - tada 🎉 - of a wider breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes for the main long change set description as well as the Beta 2, Beta 3 and Release Candidate (RC) 1 release notes for notes on some additional changes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/vm/CHANGELOG.md)).

### Changes

- **Breaking:** Renamed `receiptRoot` parameter to `receiptsRoot` (plural for `receipts`) for `VM.runBlock()` result and `AfterBlock` event, PR [#2259](https://github.com/sila-chain/silajs-monorepo/pull/2259)
- Internal refactor: removed ambiguous boolean checks within conditional clauses, PR [#2248](https://github.com/sila-chain/silajs-monorepo/pull/2248)
...
This is the biggest VM change in this release. The inheritance structure of both the VM and the underlying SAVM has been reworked and the `VM` and `SAVM` classes have been freed from being a child class of `AsyncEventEmitter` and inheriting all its properties and methods in favor of a new `events` property cleanly separating all events logic from the core `VM`/`SAVM`, see PR [#2235](https://github.com/sila-chain/silajs-monorepo/pull/2235).

This allows for an easier typing of the inner `SAVM` and makes the core VM/SAVM classes leaner and not overloaded with various other partly unused properties. The new `events` property is optional.
...
- Made `touchAccount` of `VMState` public, PR [#2183](https://github.com/sila-chain/silajs-monorepo/pull/2183)
- **Potentially breaking:** Removed `common` option from underlying `StateManager`, PR [#2197](https://github.com/sila-chain/silajs-monorepo/pull/2197)
- Reworked/adjusted underlying SAVM `skipBalance` option semantics, PR [#2138](https://github.com/sila-chain/silajs-monorepo/pull/2138)
- Fixed an underlying SAVM event signature typing bug, PR [#2184](https://github.com/sila-chain/silajs-monorepo/pull/2184)
...
The VM now aligns with the updated `Blockchain` interface from PR [#2069](https://github.com/sila-chain/silajs-monorepo/pull/2069) and now takes in any blockchain implementations adhering to the interface for the `blockchain` option. This makes it easier to do customization on the Blockchain class or pass in an alternative implementation.

## Other Changes

- **Breaking:** Renamed `hardforkByTD` option for the constructor and `VM.runBlock()` to `hardforkByTTD`, PR [#2075](https://github.com/sila-chain/silajs-monorepo/pull/2075)
- Fixed a bug in the underlying SAVM when nonce is 0, PR [#2054](https://github.com/sila-chain/silajs-monorepo/pull/2054)
...
- Added additional exports for the underlying SAVM `SavmErrorMessage`, `ExecResult`, `InterpreterStep`, `Message`, PR [#2063](https://github.com/sila-chain/silajs-monorepo/pull/2063)

## 6.0.0-beta.2 - 2022-07-15

Beta 2 release for the upcoming breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/vm/CHANGELOG.md)) for the main change set description.
...
### SAVM and StateManager Extraction

This breaking release round comes with some broader changes to the VM package. The code base has been substantially modularized and two new packages, `@silajs/savm` and `@silajs/statemanager` have been created, also see the CHANGELOGs from both new packages for additional guidance.

The SAVM package extracts the inner core, the Sila Virtual Machine (SAVM) respectively the bytecode engine, see PRs [#1892](https://github.com/sila-chain/silajs-monorepo/pull/1892), [#1955](https://github.com/sila-chain/silajs-monorepo/pull/1955) and [#1977](https://github.com/sila-chain/silajs-monorepo/pull/1977) for the main implementation work and PR [#1974](https://github.com/sila-chain/silajs-monorepo/pull/1974) for the package extraction work.

The StateManager extracts the high-level state access interface, see PR [#1817](https://github.com/sila-chain/silajs-monorepo/pull/1817). Both can now directly be passed in to the `VM` package along instantiation or are otherwise instantiated as default versions with default values being set.

The SAVM related options now moved over to the SAVM package directly, namely:

- `allowUnlimitedContractSize`
- `customOpcodes`
- `customPrecompiles`

Both the `runCall()` and `runCode()` API methods also moved over to the `savm` package and are now called through the contained SAVM:

- `vm.savm.runCall()`
- `vm.savm.runCode()`

The VM provides an implementation for the `EEI` interface from the `SAVM` package which can be used by the SAVM to request environmental data for bytecode processing. It is now also possible to provide a custom EEI by a corresponding constructor option.
...
- Removed deprecated `SIP2930Receipt` and `SIP1559Receipt` types
...
- The `vm.runBlockchain()` API method has been removed, PR [#1916](https://github.com/sila-chain/silajs-monorepo/pull/1916)
- Improved `skipBalance` logic, PR [#1849](https://github.com/sila-chain/silajs-monorepo/pull/1849)
