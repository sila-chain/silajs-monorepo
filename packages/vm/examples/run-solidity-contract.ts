...
import { bytesToHex, createAddressFromPrivateKey, hexToBytes } from '@silajs/util'
 ...
 *
 * Note: this example uses the `solc` and `viem` devDependencies from the `@silajs/vm` package.
...
function getGreeterDeploymentBytecode(solcOutput: any): any {
  return solcOutput.contracts['helpers/Greeter.sol'].Greeter.savm.bytecode.object
}
...
async function getGreeting(vm: VM, contractAddress: Address, caller: Address) {
  ...
  const greetResult = await vm.savm.runCall({
    to: contractAddress,
    caller,
