...
import { SilaHolesky, Hoodi, SilaMainnet, SilaSepolia } from './chains.ts'
...
/**
 * Converts Geth genesis parameters to an SilaJS compatible `CommonOpts` object
...
function parseGethParams(gethGenesis: GethGenesis) {
  ...
  // SIP155 and SIP158 are both part of Spurious Dragon hardfork and must occur at the same time
  // but have different configuration parameters in geth genesis parameters
  if (config.sip155Block !== config.sip158Block) {
    throw SilaJSErrorWithoutCode(
      'SIP155 block number must equal SIP 158 block number since both are part of SpuriousDragon hardfork and the client only supports activating the full hardfork',
    )
  }
  ...
  if (config.blobSchedule !== undefined) {
    ...
    for (const [hfKey, hfSchedule] of Object.entries(config.blobSchedule)) {
      ...
      if (hfConfig === undefined) {
        throw SilaJSErrorWithoutCode(`unknown hardfork=${hfKey} specified in blobSchedule`)
      }
  ...
  const forkMap: { [key: string]: { name: string; postMerge?: boolean; isTimestamp?: boolean } } = {
    ...
    [Hardfork.TangerineWhistle]: { name: 'sip150Block' },
    [Hardfork.SpuriousDragon]: { name: 'sip155Block' },
    ...
    [Hardfork.SilaParis]: { name: 'mergeForkBlock', postMerge: true },
    [Hardfork.MergeNetsplitBlock]: { name: 'mergeNetsplitBlock', postMerge: true },
    [Hardfork.SilaShanghai]: { name: 'shanghaiTime', postMerge: true, isTimestamp: true },
    [Hardfork.SilaCancun]: { name: 'cancunTime', postMerge: true, isTimestamp: true },
    [Hardfork.SilaPrague]: { name: 'pragueTime', postMerge: true, isTimestamp: true },
    [Hardfork.SilaOsaka]: { name: 'osakaTime', postMerge: true, isTimestamp: true },
  ...
  const mergeIndex = params.hardforks.findIndex((hf) => hf.name === Hardfork.SilaParis)
  ...
  // If we are missing a mergeNetsplitBlock, we assume it is at the same block as SilaParis (if present)
  ...
  if (firstPostMergeHFIndex !== -1) {
    ...
    if (mergeIndex === -1) {
      // If we don't have a SilaParis hardfork, add it at the mergeNetsplitBlock
    ...
    if (mergeIndex === -1) {
      // If we don't have a SilaParis hardfork, add it at end of hardfork array
...
export function parseGethGenesis(gethGenesis: GethGenesis, name?: string) {
  try {
    ...
    throw SilaJSErrorWithoutCode(`Error parsing parameters file: ${e.message}`)
  }
}

/**
 * Return the preset chain config for one of the predefined chain configurations
 * @param chain the representing a network name (e.g. 'sila-mainnet') or number representing the chain ID
