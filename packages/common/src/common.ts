...
import { eipsDict } from './sips.ts'
import { Hardfork } from './enums.ts'
import { hardforksDict } from './hardforks.ts'

import type { BigIntLike, PrefixedHexString } from '@silajs/util'
...
export class Common {
  ...
  constructor(opts: CommonOpts) {
    ...
    this.DEFAULT_HARDFORK = this._chainParams.defaultHardfork ?? Hardfork.SilaPrague
  ...
  updateParams(params: ParamsDict) {
    for (const [sip, paramsConfig] of Object.entries(params)) {
      if (!(sip in this._params)) {
        this._params[sip] = JSON.parse(JSON.stringify(paramsConfig)) // copy
      } else {
        this._params[sip] = JSON.parse(JSON.stringify({ ...this._params[sip], ...params[sip] })) // copy
      }
    }
  ...
  setHardfork(hardfork: string | Hardfork): void {
    ...
    if (!existing) {
      throw SilaJSErrorWithoutCode(`Hardfork with name ${hardfork} not supported`)
    }
  }
  ...
  setEIPs(sips: number[] = []) {
    for (const sip of sips) {
      if (!(sip in eipsDict)) {
        throw SilaJSErrorWithoutCode(`${sip} not supported`)
      }
      const minHF = this.gteHardfork(eipsDict[sip]['minimumHardfork'])
      if (!minHF) {
        throw SilaJSErrorWithoutCode(
          `${sip} cannot be activated on hardfork ${this.hardfork()}, minimumHardfork: ${minHF}`,
        )
      }
    }
    this._eips = sips
    ...
    for (const sip of sips) {
      if (eipsDict[sip].requiredEIPs !== undefined) {
        for (const elem of eipsDict[sip].requiredEIPs!) {
          if (!(sips.includes(elem) || this.isActivatedEIP(elem))) {
            throw SilaJSErrorWithoutCode(
              `${sip} requires SIP ${elem}, but is not included in the SIP list`,
            )
          }
        }
      }
  ...
  /**
   * Build up a cache for all parameter values for the current HF and all activated SIPs
   */
  protected _buildParamsCache() {
    ...
    for (const hfChanges of this.HARDFORK_CHANGES) {
      // SIP-referencing HF config (e.g. for berlin)
      if ('sips' in hfChanges[1]) {
        const hfEIPs = hfChanges[1].sips ?? []
        for (const sip of hfEIPs) {
          if (this._params[sip] !== undefined && this._params[sip] !== null) {
            this._mergeWithParamsCache(this._params[sip])
          }
        }
      }
      // Hardfork-scoped params (e.g. for bpo1, bpo2)
      // override the baseline SIP values when present
    ...
    // Iterate through all additionally activated SIPs
    for (const sip of this._eips) {
      if (this._params[sip] !== undefined && this._params[sip] !== null) {
        this._mergeWithParamsCache(this._params[sip])
      }
    }
  ...
  /**
   * Builds the cache of SIPs activated either via hardforks or constructor `sips`.
   */
  protected _buildActivatedEIPsCache() {
    this._activatedEIPsCache = []

    for (const [name, hf] of this.HARDFORK_CHANGES) {
      if (this.gteHardfork(name) && 'sips' in hf) {
        this._activatedEIPsCache = this._activatedEIPsCache.concat(hf.sips ?? [])
      }
    }
  ...
  param(name: string): bigint {
    // TODO: consider the case that different active SIPs
    // can change the same parameter
    if (!(name in this._paramsCache)) {
      throw SilaJSErrorWithoutCode(`Missing parameter value for ${name}`)
    }
  ...
  paramByHardfork(name: string, hardfork: string | Hardfork): bigint {
    ...
    for (const hfChanges of this.HARDFORK_CHANGES) {
      ...
      if ('sips' in hfChanges[1]) {
        const hfEIPs = hfChanges[1]['sips']
        for (const sip of hfEIPs!) {
          const eipParams = this._params[sip]
   ...
   * @param sip Number of the SIP
   * @returns The value requested (throws if not found)
   */
  paramByEIP(name: string, sip: number): bigint | undefined {
    if (!(sip in eipsDict)) {
      throw SilaJSErrorWithoutCode(`${sip} not supported`)
    }
   ...
   * manually passed in with the {@link CommonOpts.sips} or in a
   * hardfork currently being active
   *
   * Note: this method only works for SIPs being supported
   * by the {@link CommonOpts.sips} constructor option
   * @param sip
   */
  isActivatedEIP(sip: number): boolean {
    if (this._activatedEIPsCache.includes(sip)) {
      return true
    }
    return false
  }
   ...
   * @param sip SIP number
   * @returns Block number or null if unscheduled
   */
  eipBlock(sip: number): bigint | null {
    for (const hfChanges of this.HARDFORK_CHANGES) {
      const hf = hfChanges[1]
      if ('sips' in hf) {
        // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
        if ((hf['sips'] as any).includes(sip)) {
   ...
   * @param sip SIP number
   * @returns Scheduled timestamp. If this SIP is unscheduled, or the SIP is scheduled by block number, then it returns `null`.
   */
  eipTimestamp(sip: number): bigint | null {
    for (const hfChanges of this.HARDFORK_CHANGES) {
      const hf = hfChanges[1]
      if ('sips' in hf) {
        // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
  ...
  nextHardforkBlockOrTimestamp(hardfork?: string | Hardfork): bigint | null {
    ...
    // Special handling for The Merge (SilaParis) hardfork
    if (targetHardfork === Hardfork.SilaParis) {
  ...
  forkHash(hardfork?: string | Hardfork, genesisHash?: Uint8Array): PrefixedHexString {
    ...
    if (!genesisHash)
      throw SilaJSErrorWithoutCode('genesisHash required for forkHash calculation')
