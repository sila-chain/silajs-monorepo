...
import { Common, Hardfork, SilaMainnet, createCustomCommon } from '../src/index.ts'
...
describe('[Common]: Custom chains', () => {
  ...
  it('custom() -> base functionality', () => {
    const sila-mainnetCommon = new Common({ chain: SilaMainnet })

    const customChainParams = { name: 'custom', chainId: 123 }
    const customChainCommon = createCustomCommon(customChainParams, SilaMainnet, {
    ...
    // Fallback params from sila-mainnet
    assert.deepEqual(customChainCommon.genesis(), sila-mainnetCommon.genesis())
    assert.deepEqual(customChainCommon.bootstrapNodes(), sila-mainnetCommon.bootstrapNodes())
    assert.deepEqual(customChainCommon.hardforks(), sila-mainnetCommon.hardforks())
  ...
  it('custom() -> behavior', () => {
    let common = createCustomCommon({ chainId: 123 }, SilaMainnet)
    assert.strictEqual(
      common.consensusAlgorithm(),
      'casper',
      'should default to sila-mainnet base chain',
    )
    assert.strictEqual(common.chainName(), 'sila-mainnet', 'should set default custom chain name')

    common = createCustomCommon({ chainId: 123 }, SilaMainnet)
    assert.strictEqual(
      common.hardfork(),
  ...
  it('customHardforks parameter: initialization and transition tests', () => {
    ...
    // Note: default HF of Common is currently SilaPrague
    // Did not pass any "hardfork" param
    assert.strictEqual(c.hardfork(), Hardfork.SilaPrague)
    c.setHardforkBy({
      blockNumber: 0,
    })
...
describe('custom chain setup with hardforks with undefined/null block numbers', () => {
  it('Should work', () => {
    ...
    const common = createCustomCommon({ hardforks: nullHardforks }, SilaMainnet)
