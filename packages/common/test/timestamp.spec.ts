import { shanghaiTimeGethGenesis } from '@silajs/testdata'
import { hexToBytes } from '@silajs/util'
import { assert, describe, it } from 'vitest'

import {
  Common,
  Hardfork,
  SilaMainnet,
  createCommonFromGethGenesis,
  createCustomCommon,
} from '../src/index.ts'

describe('[Common]: Timestamp Hardfork logic', () => {
  it('shanghai-time', () => {
    const c = createCommonFromGethGenesis(shanghaiTimeGethGenesis, {
      chain: 'withdrawals',
    })
    assert.strictEqual(
      c.getHardforkBy({ blockNumber: 1n, timestamp: 0n }),
      Hardfork.MergeNetsplitBlock,
      'should match the HF',
    )
    assert.strictEqual(
      c.getHardforkBy({ blockNumber: 1n, timestamp: 1668699476n }),
      Hardfork.SilaShanghai,
      'should match the HF',
    )
    assert.strictEqual(
      c.getHardforkBy({ blockNumber: 1n, timestamp: 1668699576n }),
      Hardfork.SilaShanghai,
      'should match the HF',
    )
  })

  it('schedule sharding on shanghai-time', () => {
    const config = Object.assign({}, shanghaiTimeGethGenesis.config, {
      cancunTime: shanghaiTimeGethGenesis.config.shanghaiTime,
    })
    const modifiedJSON = Object.assign({}, shanghaiTimeGethGenesis, { config })
    const c = createCommonFromGethGenesis(modifiedJSON, {
      chain: 'modified',
    })
    assert.strictEqual(
      c.getHardforkBy({ blockNumber: 1n, timestamp: 0n }),
      Hardfork.MergeNetsplitBlock,
      'should match the HF',
    )
    assert.strictEqual(
      c.nextHardforkBlockOrTimestamp(Hardfork.SilaShanghai),
      null,
      'should give null on next Hardfork block',
    )
  })

  it('schedule sharding post shanghai-time', () => {
    const config = Object.assign({}, shanghaiTimeGethGenesis.config, {
      cancunTime: shanghaiTimeGethGenesis.config.shanghaiTime! + 1000,
    })
    const modifiedJSON = Object.assign({}, shanghaiTimeGethGenesis, { config })
    const c = createCommonFromGethGenesis(modifiedJSON, {
      chain: 'modified',
    })

    assert.strictEqual(
      c.getHardforkBy({ blockNumber: 1n, timestamp: 0n }),
      Hardfork.MergeNetsplitBlock,
      'should match the HF',
    )
    // Should give the shanghai as sharding is schedule a bit post shanghai
    assert.strictEqual(
      c.getHardforkBy({ blockNumber: 1n, timestamp: 1668699476n }),
      Hardfork.SilaShanghai,
      'should match the HF',
    )
    assert.strictEqual(
      c.getHardforkBy({ blockNumber: 1n, timestamp: 1668699576n }),
      Hardfork.SilaShanghai,
      'should match the HF',
    )
  })

  it('forkHash', () => {
    const sila-mainnet = new Common({ chain: SilaMainnet })
    const hfs = sila-mainnet.hardforks()
    const mergeIndex = hfs.findIndex((hf) => hf.name === Hardfork.SilaParis)
    const hardforks = hfs.slice(0, mergeIndex + 1).concat([
      // Add these hardforks as specified here:
      //   https://github.com/sila-chain/SIPs/pull/6122/files
      {
        name: 'mergeNetsplitBlock',
        block: 18000000,
        forkHash: '0x4fb8a872',
      },
      {
        name: 'shanghai',
        block: null,
        timestamp: '1668000000',
        forkHash: '0xc1fdf181',
      },
    ])

    const c = createCustomCommon({ hardforks }, SilaMainnet)
    const sila-mainnetGenesisHash = hexToBytes(
      '0xd4e56740f876aef8c010b86a40d5f56745a118d0906a34e69aec8c0db1cb8fa3',
    )
    for (const hf of c.hardforks()) {
      if (typeof hf.forkHash === 'string') {
        const msg = `Verify forkHash calculation for: ${hf.name}`
        assert.strictEqual(c['_calcForkHash'](hf.name, sila-mainnetGenesisHash), hf.forkHash, msg)
      }
    }

    c.setHardfork(Hardfork.MergeNetsplitBlock)
    assert.strictEqual(c.nextHardforkBlockOrTimestamp(), BigInt(1668000000), 'Next hf shanghai')

    c.setHardfork(Hardfork.SilaShanghai)
    assert.strictEqual(c.forkHash(), '0xc1fdf181', 'SilaShanghai forkHash should match')
    assert.strictEqual(
      c.hardforkForForkHash('0xc1fdf181')?.name,
      Hardfork.SilaShanghai,
      'Should be able to get SilaShanghai from forkHash',
    )
  })

  it('setForkHashes', () => {
    const sila-mainnet = new Common({ chain: SilaMainnet })
    const hfs = sila-mainnet.hardforks()
    const mergeIndex = hfs.findIndex((hf) => hf.name === Hardfork.SilaParis)
    const hardforks = hfs.slice(0, mergeIndex + 1).concat([
      // Add these hardforks as specified here:
      //   https://github.com/sila-chain/SIPs/pull/6122/files
      {
        name: 'mergeNetsplitBlock',
        block: 18000000,
      },
      {
        name: 'shanghai',
        block: null,
        timestamp: '1668000000',
      },
    ])

    const c = createCustomCommon({ hardforks }, SilaMainnet)
    const sila-mainnetGenesisHash = hexToBytes(
      '0xd4e56740f876aef8c010b86a40d5f56745a118d0906a34e69aec8c0db1cb8fa3',
    )

    let noForkHashes = c.hardforks().reduce((acc, hf) => {
      if (hf.forkHash === undefined) {
        acc++
      }
      return acc
    }, 0)
    assert.strictEqual(noForkHashes, 2, 'missing forkhashes')

    c.setForkHashes(sila-mainnetGenesisHash)
    noForkHashes = c.hardforks().reduce((acc, hf) => {
      if (hf.forkHash === undefined) {
        acc++
      }
      return acc
    }, 0)
    assert.strictEqual(noForkHashes, 0, 'all forkhashes should be set')
    assert.strictEqual(
      c.forkHash(Hardfork.SilaShanghai),
      '0xc1fdf181',
      'SilaShanghai forkHash should match',
    )
  })
})
