import { concatBytes, randomBytes, setLengthLeft } from '@silajs/util'
import { assert, describe, it } from 'vitest'

import { Common, SilaMainnet, createCustomCommon } from '../src/index.ts'
...
describe('[Common]: Custom Crypto', () => {
  ...
  it('keccak256', () => {
    ...
    let c = new Common({ chain: SilaMainnet, customCrypto })
    ...
    c = createCustomCommon(customChainParams, SilaMainnet, { customCrypto })
  ...
  it('ecrecover', () => {
    ...
    const c = new Common({ chain: SilaMainnet, customCrypto })
    assert.deepEqual(
