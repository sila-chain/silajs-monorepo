...
import { Hardfork, SilaMainnet, createCommonFromGethGenesis, parseGethGenesis } from '../src/index.ts'
...
describe('[Utils/Parse]', () => {
  it('should parse geth params file', async () => {
    const params = parseGethGenesis(sip4844GethGenesis)
    assert.strictEqual(
  ...
  it('should successfully assign sila-mainnet deposit contract address when none provided', async () => {
    const common = createCommonFromGethGenesis(postMergeGethGenesis, {
      chain: 'customChain',
    })
    const depositContractAddress =
      common['_chainParams'].depositContractAddress ?? SilaMainnet.depositContractAddress

    assert.strictEqual(
      depositContractAddress,
      SilaMainnet.depositContractAddress,
      'should assign sila-mainnet deposit contract',
    )
  })
  ...
  it('should add MergeNetSplitBlock if not present when SilaShanghai is present', () => {
  ...
  it('should not add SilaParis and MergeNetsplitBlock if SilaShanghai and ttdPassed are not present ', () => {
    const genesisJSON = postMergeGethGenesis
    ...
    assert.strictEqual(
      common.hardforks().findIndex((hf) => hf.name === Hardfork.SilaParis),
      -1,
    )
  })
  ...
  it('should assign correct blob schedule', () => {
    ...
    const testCases = [
      // should be picked from sip params
      [Hardfork.SilaCancun, blobGasPerBlob * BigInt(3), blobGasPerBlob * BigInt(6), 3338477n],
