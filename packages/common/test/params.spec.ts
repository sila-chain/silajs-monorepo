...
describe('[Common]: Parameter instantiation / params option / Updates', () => {
  it('Param option', () => {
    const c = new Common({ chain: SilaMainnet, params: paramsTest })
...
describe('[Common]: Parameter access for param(), paramByHardfork()', () => {
  it('Basic usage', () => {
    const c = new Common({ chain: SilaMainnet, params: paramsTest, sips: [2537] })
    ...
    // To run please manually add an "ecAdd" entry with value 12345 to SIP2537 config
    // and uncomment the test
    msg = 'SIP config should take precedence over HF config'
  ...
  it('Access by block number, paramByBlock()', () => {
    const c = new Common({ chain: SilaMainnet, hardfork: Hardfork.Byzantium, params: paramsTest })
  ...
  it('Access on copied Common instances', () => {
    const c = new Common({ chain: SilaMainnet, hardfork: Hardfork.SilaShanghai, params: paramsTest })
  ...
  it('SIP param access, paramByEIP()', () => {
    const c = new Common({ chain: SilaMainnet, params: paramsTest })
    ...
    let msg = 'Should throw for using paramByEIP() with an unsupported SIP'
    assert.throws(f, /not supported$/, undefined, msg)

    msg = 'Should return bls12381G1AddGas gas price for SIP2537'
