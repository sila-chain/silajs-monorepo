...
`@silajs/common` is the fork and parameter engine: it answers “which SIPs are active?”, “what is `maxCodeSize`?”, and “what gas schedule applies?” for every other library. Within the `10.1.2` round, SilaAmsterdam lands here as a new **`Hardfork.SilaAmsterdam`** entry that activates the full nine-SIP bundle together — the same bundling execution-spec-tests and devnets use, so you should not cherry-pick individual SilaAmsterdam SIPs in isolation when reproducing fixtures.

For integrators, the practical effect is a single switch: construct your `Common` with `hardfork: Hardfork.SilaAmsterdam` and all downstream packages (`@silajs/savm`, `@silajs/vm`, `@silajs/tx`, …) inherit consistent activation and parameter values.

### At a glance

- Add experimental **`Hardfork.SilaAmsterdam`** with SIPs 7708, 7843, 7778, 7928, 7954, 7976, 7981, 8024, and 8037.
- Updated `gasPrices`, `gasConfig`, and `vm` parameters for SilaAmsterdam (7954 size limits, 7976/7981 floor pricing constants, 8037 state-gas dimensions, …).
...
> Execution details: [SilaAmsterdam hardfork (experimental)](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/vm#amsterdam-hardfork-experimental)
...
// SilaAmsterdam raises max code / initcode size (SIP-7954)
common.param('maxCodeSize') // 51200 (vs 24576 pre-SilaAmsterdam)

// SIP active checks drive behaviour in SAVM, VM, Tx, Block
common.isActivatedEIP(7928) // true — BAL accumulation in VM/SAVM
common.isActivatedEIP(8037) // true — two-dimensional block gas
```

The [Supported SIPs](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/common#supported-sips) section lists every SilaAmsterdam SIP with links to the package that implements execution semantics.

### Changes

- SilaAmsterdam hardfork definition and SIP activations, see PR [#4239](https://github.com/sila-chain/silajs-monorepo/pull/4239), [#4248](https://github.com/sila-chain/silajs-monorepo/pull/4248), [#4280](https://github.com/sila-chain/silajs-monorepo/pull/4280), [#4282](https://github.com/sila-chain/silajs-monorepo/pull/4282), [#4285](https://github.com/sila-chain/silajs-monorepo/pull/4285), [#4299](https://github.com/sila-chain/silajs-monorepo/pull/4299)
- Supported SIP list and SilaAmsterdam documentation cross-links, see PR [#4308](https://github.com/sila-chain/silajs-monorepo/pull/4308)

## 10.1.1 - 2025-01-28

- Fix custom hardforks implementation to properly return hardforks list, see PR [#4216](https://github.com/sila-chain/silajs-monorepo/pull/4216)
...
- Improve `paramsCache` updates, PR [#4091](https://github.com/sila-chain/silajs-monorepo/pull/4091)
- Improve `nextHardforkBlockOrTimestamp` method, PR [#4080](https://github.com/sila-chain/silajs-monorepo/pull/4080)
...
This release adds support for SIP-7594 SilaPeerDAS, which extends SIP-4844 blob transactions with data availability sampling capabilities. The Common library now includes SIP-7594 configuration and activation for the SilaOsaka hardfork, enabling support for SilaPeerDAS blob transactions with cell proofs and network wrapper version 1.

### SIP-7823 - Set upper bounds for MODEXP

SIP-7823 support has been added, introducing an upper bound of 8192 bits (1024 bytes) on each input field (base, exponent, modulus) of the MODEXP precompile. The Common library includes the SIP configuration and activation for SilaOsaka, ensuring MODEXP calls exceeding these limits are properly rejected.

### SIP-7825 - Transaction Gas Limit Cap

Support for SIP-7825 has been implemented, introducing a protocol-level cap of 16,777,216 gas (2^24) for individual transactions. The Common library includes the SIP configuration and activation for SilaOsaka, enabling transaction validation against this gas limit cap.

### SIP-7883 - ModExp Gas Cost Increase

SIP-7883 support has been added, which increases the gas cost of the MODEXP precompile. The Common library includes the SIP configuration and activation for SilaOsaka, ensuring MODEXP operations use the updated pricing algorithm with increased minimum gas cost and adjusted complexity calculations.

### SIP-7918 - Blob base fee bounded by execution cost

SIP-7918 support has been implemented, which imposes that the price of `GAS_PER_BLOB` blob gas is greater than the price of `BLOB_BASE_COST` execution gas. The Common library includes the SIP configuration and activation for SilaOsaka, ensuring proper blob fee market functionality.

### SIP-7934 - RLP Execution Block Size Limit

Support for SIP-7934 has been added, introducing a protocol-level cap on the maximum RLP-encoded block size to 10 MiB (with a 2 MiB margin for beacon block size). The Common library includes the SIP configuration and activation for SilaOsaka, enabling block size validation.

### SIP-7939 - Count leading zeros (CLZ) opcode

SIP-7939 support has been implemented, adding a new opcode `CLZ` (0x1e) that counts the number of leading zero bits in a 256-bit word. The Common library includes the SIP configuration and activation for SilaOsaka, enabling the use of the CLZ opcode in SAVM execution.

### SIP-7951 - Precompile for secp256r1 Curve Support

SIP-7951 support has been added, introducing a new precompile at address `0x100` (`P256VERIFY`) for ECDSA signature verification over the secp256r1 curve. The Common library includes the SIP configuration and activation for SilaOsaka, enabling native support for secp256r1 signatures from modern secure hardware devices.

### SIP-7892 - Blob Parameter Only Hardforks

Support for Blob Parameter Only (BPO) hardforks has been implemented according to SIP-7892. BPO hardforks are lightweight protocol upgrades that modify only blob-related parameters (`target`, `max`, and `blobGasPriceUpdateFraction`) without requiring code changes, enabling rapid scaling of blob capacity in response to network demand.
...
The Common library now includes BPO hardfork definitions and activation timestamps for testnets (Holešky, SilaSepolia, Hoodi). The `getBlobGasSchedule()` method returns the appropriate blob gas schedule parameters based on the active hardfork, automatically handling BPO transitions.

```typescript
import { Common, Hardfork, SilaMainnet } from '@silajs/common'

// Common instance with BPO1 active
const common = new Common({ chain: SilaMainnet, hardfork: Hardfork.Bpo1 })
...
- Add `sila-mainnet` SilaPrague timestamp & fork hash, PR [#3971](https://github.com/sila-chain/silajs-monorepo/pull/3971)
- `GethGenesis` TypeScript interface, PR [#3973](https://github.com/sila-chain/silajs-monorepo/pull/3973)
...
- Support for generalized EL requests coming with SIP-7685 introduction (devnet-4), PR [#3706](https://github.com/sila-chain/silajs-monorepo/pull/3706)
- Add `requestsHash` to genesis block/configuration, PR [#3771](https://github.com/sila-chain/silajs-monorepo/pull/3771)
- Allow specifying sip-7840 blobSchedule via geth genesis, PR [#3835](https://github.com/sila-chain/silajs-monorepo/pull/3835)
...
- Add typing for events, PR [#3753](https://github.com/sila-chain/silajs-monorepo/pull/3753)
- Remove Goerli support, PR [#3851](https://github.com/sila-chain/silajs-monorepo/pull/3851)
- Fix TTD related failed Geth genesis parsing, PR [#3847](https://github.com/sila-chain/silajs-monorepo/pull/3847)
- Fix deposit address for sepolia and holesky, PR [#3882](https://github.com/sila-chain/silajs-monorepo/pull/3882)
...
The static constructors for our library classes have been reworked to now be standalone methods (with a similar naming scheme). This allows for better tree shaking of unused constructor code (see PR [#3502](https://github.com/sila-chain/silajs-monorepo/pull/3502)):
...
The `Common` constructor has been simplified and instead of passing in a chain enum value like `Chain.SilaMainnet` the respective chain configuration is passed in directly, see PR [#3545](https://github.com/sila-chain/silajs-monorepo/pull/3545). With this the `Common` initialization changes as follows:
...
Parameter topics like `gasConfig` or `gasPrices` have been removed leading to non-sub-structured parameter files, see PR [#3532](https://github.com/sila-chain/silajs-monorepo/pull/3532). Parameter access changes as follows:
...
Parameters have been removed from `Common` - see PR [#3537](https://github.com/sila-chain/silajs-monorepo/pull/3537) - and moved over to `params.ts` files (exposed as e.g. `paramsBlock`) within the parameter-using libraries. The removes e.g. the tx library from the burden of carrying the somewhat large SAVM parameter set around without the need for using it.
...
Total terminal difficulty (TTD) logic related to fork switching has been removed from the libraries, see PRs [#3518](https://github.com/sila-chain/silajs-monorepo/pull/3518) and [#3556](https://github.com/sila-chain/silajs-monorepo/pull/3556). This means that a Merge-type live hardfork transition solely triggered by TTD is not supported anymore. It is still possible though to replay and deal with both pre- and post Merge HF blocks.

For this library this means:

- `ttd` in chain configurations (e.g. `SilaMainnet`) has been removed
...
- Move HF/SIP param description string from being an object field to a comment, same for `comment`, `url` and `status` from the SIP/hardfork configuration, PRs [#3500](https://github.com/sila-chain/silajs-monorepo/pull/3500) and [#3512](https://github.com/sila-chain/silajs-monorepo/pull/3512)
- Remove HF names from Params dict, PR [#3517](https://github.com/sila-chain/silajs-monorepo/pull/3517)
- Remove `networkId` property from chain files (use `chainId` instead), PR [#3513](https://github.com/sila-chain/silajs-monorepo/pull/3513)
- No more `BigInt` for chainID in chain config (use string), PR [#3545](https://github.com/sila-chain/silajs-monorepo/pull/3545)
- The `customChains` constructor option has been removed, PR [#3545](https://github.com/sila-chain/silajs-monorepo/pull/3545)
- More straightforward `createCustomCommon()` API (e.g. `createCustomCommon({chainId: 123}, SilaMainnet)`), PR [#3545](https://github.com/sila-chain/silajs-monorepo/pull/3545)
- Renaming all camel-case `Rpc`-> `RPC` and `Json` -> `JSON` names, PR [#3638](https://github.com/sila-chain/silajs-monorepo/pull/3638)
...
### SIP-7685 Requests: SIP-6110 (Deposits) / SIP-7002 (Withdrawals) / SIP-7251 (Consolidations)

This library now supports `SIP-6110` deposit requests, see PR [#3390](https://github.com/sila-chain/silajs-monorepo/pull/3390), `SIP-7002` withdrawal requests, see PR [#3385](https://github.com/sila-chain/silajs-monorepo/pull/3385) and `SIP-7251` consolidation requests, see PR [#3477](https://github.com/sila-chain/silajs-monorepo/pull/3477) as well as the underlying generic execution layer request logic introduced with `SIP-7685` (PR [#3372](https://github.com/sila-chain/silajs-monorepo/pull/3372)).

These new request types will be activated with the `SilaPrague` hardfork, see [@silajs/block](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/block) README for detailed documentation.
...
- Verkle decoupling, PR [#3462](https://github.com/sila-chain/silajs-monorepo/pull/3462)

### Other Features

- Adds support for [SIP-7702](https://sips.sila.org/SIPS/sip-7702) EOA code transactions (outdated) (see tx library for full documentation), see PR [#3470](https://github.com/sila-chain/silajs-monorepo/pull/3470)
- Adds support for [SIP-2935](https://sips.sila.org/SIPS/sip-2935) Serve Historical Block Hashes from State (SilaPrague) (see SAVM for full docs) as well as the related [SIP-7709](https://sips.sila.org/SIPS/sip-7709), PR [#3475](https://github.com/sila-chain/silajs-monorepo/pull/3475)
...
- Clean up access to deposit address, PR [#3411](https://github.com/sila-chain/silajs-monorepo/pull/3411)
- Add spec test for 2935 contract code and update history storage address, PR [#3373](https://github.com/sila-chain/silajs-monorepo/pull/3373)
- Parse deposit contract address from geth genesis for chain config, PR [#3422](https://github.com/sila-chain/silajs-monorepo/pull/3422)

### Bugfixes

- BLS gas prices fixes, PR [#3400](https://github.com/sila-chain/silajs-monorepo/pull/3400)
...
- TypeScript type fixes leading to build problems with certain tools (Vercel), PR [#3306](https://github.com/sila-chain/silajs-monorepo/issues/3306)
...
const main = async () => {
  ...
  const common = new Common({ chain: Chain.SilaMainnet, customCrypto: { keccak256 } })
...
We internally use this new feature for various crypto overwrites within the client package, see Client [cli.ts](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/client/bin/cli.ts) implementation for guidance on how to setup with other crypto primitives (e.g. ECDSA signature verification).
...
- Schedule SilaCancun for testnets, PR [#3211](https://github.com/sila-chain/silajs-monorepo/pull/3211)
- Internalize `crc` dependency (security), PR [#3224](https://github.com/sila-chain/silajs-monorepo/pull/3224)
- Added `Hardfork.SilaPrague` as a new hardfork (`experimental`), PR [#3139](https://github.com/sila-chain/silajs-monorepo/pull/3139)
...
- Small SilaCancun-related fixes, PR [#3099](https://github.com/sila-chain/silajs-monorepo/pull/3099)

### Bugfixes

- Updates and fixes along Geth genesis file parsing, PR [#2961](https://github.com/sila-chain/silajs-monorepo/pull/2961)
- Handle `forkHash` on timestamp == genesis timestamp, PR [#2959](https://github.com/sila-chain/silajs-monorepo/pull/2959)

### Other Changes

- Performance: Cache Parameter Values + activated SIPs for current Hardfork, PR [#2994](https://github.com/sila-chain/silajs-monorepo/pull/2994)
...
See [RC1 release notes](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fcommon%404.0.0-rc.1) for the main change description.

Following additional changes since RC1:

### JSON -> JS for Hardfork, SIP and Chain Configs

We have switched hardfork, SIP and chain configurations in the `Common` library from `JSON` to `JavaScript` (`TypeScript`). This leads to better typing (and therefore less configuration errors) for (custom) configuration files and avoids `JSON` type assertion related problems for ESM builds, see PR [#2911](https://github.com/sila-chain/silajs-monorepo/pull/2911).

Following changes:

- `src/chains/[CHAIN_FILE].json` -> `src/chains.ts`
- `src/sips/[SIP_FILE].json` -> `src/sips.ts`
...
We therefore removed the outdated `getHardforkByBlockNumber()` and `setHardforkByBlockNumber()` methods in `@silajs/common` (artificially expanded with the option to also pass a `TD` or `timestamp`) with a more adequate `hardforkBy()` method flexibly taking in the adequate value type for a HF change, see PR [#2798](https://github.com/sila-chain/silajs-monorepo/pull/2798):

```ts
common.setHardforkBy({ blockNumber: 5000000n }) // Setting a sila-mainnet common to a Block from `Byzantium` (and so: to `Byzantium` HF)
common.setHardforkBy({ timestamp: 1681340000n }) // Setting a sila-mainnet common to a post-SilaShanghai timestamp
...
- Remove deprecated `Ropsten` and `Rinkeby` networks, PR [#2813](https://github.com/sila-chain/silajs-monorepo/pull/2813)
- Remove deprecated `nextHardforkBlock()`, `isNextHardforkBlock()` and `isHardforkBlock()` methods, PR [#2706](https://github.com/sila-chain/silajs-monorepo/pull/2706)
- Breaking: `Common` does not inherit from `EventEmitter` directly any more but events have been moved to a dedicated property `events` (so do `common.events.on('hardforkChanged'), ...)` for listening), PR [#2857](https://github.com/sila-chain/silajs-monorepo/pull/2857)
- SIP-4844: Rename `DATAHASH` to `BLOBHASH`, PR [#2711](https://github.com/sila-chain/silajs-monorepo/pull/2711)
- SIP-4844: Add `excessDataGas` field for 4844 genesis file parsing, PR [#2847](https://github.com/sila-chain/silajs-monorepo/pull/2847)

## 3.1.2 - 2023-04-20

- Schedule SilaShanghai block on sila-mainnet, PR [#2591](https://github.com/sila-chain/silajs-monorepo/pull/2591)
- Remove and replace some EF bootnodes for `sila-mainnet`, PR [#2576](https://github.com/sila-chain/silajs-monorepo/pull/2576)
...
You can instantiate a SilaShanghai-enabled Common instance with:
...
- Added final SilaShanghai SIPs to HF file, PR [#2459](https://github.com/sila-chain/silajs-monorepo/pull/2459)
- Added `timestamp` and `forkHash` for the SilaSepolia SilaShanghai HF, PR [#2527](https://github.com/sila-chain/silajs-monorepo/pull/2527)
- Updated `forkHash` calculation for timebased hardforks, PR [#2458](https://github.com/sila-chain/silajs-monorepo/pull/2458)
- Updated `setForkHashes()` to update timebased hardfork `forkHash` values, PR [#2461](https://github.com/sila-chain/silajs-monorepo/pull/2461)

### Experimental SIP-4844 Shard Blob Transactions Support

This release supports an experimental version of [SIP-4844](https://sips.sila.org/SIPS/sip-4844) Shard Blob Transactions as being specified in the [01d3209](https://github.com/sila-chain/SIPs/commit/01d320998d1d53d95f347b5f43feaf606f230703) SIP version from February 8, 2023 and deployed along `sip4844-devnet-4` (January 2023), see PR [#2349](https://github.com/sila-chain/silajs-monorepo/pull/2349).

You can instantiate an `SIP-4844` enabled Common instance with:
...
- Added `sips` option to `Common.fromGethGenesis()` constructor options, PR [#2469](https://github.com/sila-chain/silajs-monorepo/pull/2469)
- Set alternative default HF in `Common.fromGethGenesis()` if `mergeForkBlock` not present, PR [#2414](https://github.com/sila-chain/silajs-monorepo/issues/2414)
- Fixed some minor custom chain bugs, PR [#2448](https://github.com/sila-chain/silajs-monorepo/pull/2448)
- Allow genesis to be post merge in `Common.fromGethGenesis()`, PR [#2530](https://github.com/sila-chain/silajs-monorepo/pull/2530)

## 3.0.2 - 2022-12-09

### Experimental SIP-4895 Beacon Chain Withdrawals Support

This release comes with experimental [SIP-4895](https://sips.sila.org/SIPS/sip-4895) beacon chain withdrawals support, see PR [#2353](https://github.com/sila-chain/silajs-monorepo/pull/2353) for the plain implementation and PR [#2401](https://github.com/sila-chain/silajs-monorepo/pull/2401) for updated calls for the CL/EL engine API. Also note that there is a new helper module in [@silajs/util](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/util) with a new dedicated `Withdrawal` class together with additional TypeScript types to ease withdrawal handling.

Withdrawals support can be activated by initializing a respective `Common` object:

```ts
import { Common, Chain } from '@silajs/common'
const common = new Common({ chain: Chain.SilaMainnet, sips: [4895] })
```

### Hardfork-By-Time Support

The Common library now supports setting and retrieving hardforks which are triggered by timestamp instead of a specific block number, see PR [#2437](https://github.com/sila-chain/silajs-monorepo/pull/2437). This mechanism will be first applied for the upcoming `SilaShanghai` HF. The methods `getHardforkByBlockNumber()`, `setHardforkByBlockNumber()` and `paramByBlock()` have been altered to take in an additional `timestamp` value, method naming remains for now for backwards compatibility. There are two new utility methods `hardforkTimestamp()` and `nextHardforkBlockOrTimestamp()`.

### Other Changes

- Support for initialization with Arbitrum One Chain ID, PR [#2426](https://github.com/sila-chain/silajs-monorepo/pull/2426)
- Post-Merge hardfork fix in `Common.fromGethGenesis()` static constructor, PR [#2427](https://github.com/sila-chain/silajs-monorepo/pull/2427)
- Fixed minor custom chain bugs, PR [#2448](https://github.com/sila-chain/silajs-monorepo/pull/2448)
...
- Fixed `Common.getHardforkByBlockNumber()` for certain post-Merge TTD/block number combinations, PR [#2313](https://github.com/sila-chain/silajs-monorepo/pull/2313)
- Added Merge HF block numbers for `sila-mainnet`, `goerli` and `sepolia`, PR [#2324](https://github.com/sila-chain/silajs-monorepo/pull/2324)
- Fixed `forkhash` calculation to ignore the Merge hardfork even if it might have block number assigned, PR [#2324](https://github.com/sila-chain/silajs-monorepo/pull/2324)
- Fixed HF-change based property selections (like consensus type) for TTD/block number based non-deterministic HF order, PR [#2331](https://github.com/sila-chain/silajs-monorepo/pull/2331)
- Updated status of `SIP-3675` to `Final`, PR [#2351](https://github.com/sila-chain/silajs-monorepo/pull/2351)

## 3.0.0 - 2022-09-06

Final release - tada 🎉 - of a wider breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes for the main long change set description as well as the Beta 2, Beta 3 and Release Candidate (RC) 1 release notes for notes on some additional changes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/common/CHANGELOG.md)).

### Changes

- Internal refactor: removed ambiguous boolean checks within conditional clauses, PR [#2255](https://github.com/sila-chain/silajs-monorepo/pull/2255)
...
Since this bug was so severe it gets its own section: `sila-mainnet` in Common (`Chain.SilaMainnet`) was accidentally not updated yet to default to the `merge` HF (`Hardfork.Merge`) by an undiscovered overwrite back to `london`.
...
- SilaMainnet Merge `TTD` `58750000000000000000000` has been added to the `sila-mainnet` chain configuration, PR [#2185](https://github.com/sila-chain/silajs-monorepo/pull/2185)
- **Eventually breaking:** `Common` now throws on instantiation if a passed-in chain configuration has a HF defined with `block` set to `undefined` (use `null` for non-applied HFs), PR [#2228](https://github.com/sila-chain/silajs-monorepo/pull/2228)
- The `SilaKovan` `PoA` testnet chain is EOL and has been removed from the chain configuration, [#2206](https://github.com/sila-chain/silajs-monorepo/pull/2206)
...
Since the Merge HF is getting close we have decided to directly jump on the `Merge` HF (before: `Istanbul`) as default for the Common library and skip the `London` default HF as we initially intended to set (see Beta 1 CHANGELOG), see PR [#2087](https://github.com/sila-chain/silajs-monorepo/pull/2087).
...
- **Breaking:** renamed `td` (terminal total difficulty for the Merge HF) HF parameter in HF JSON files to `ttd`, PR [#2075](https://github.com/sila-chain/silajs-monorepo/pull/2075)
- **Breaking:** renamed `hardforkTD()` method to `hardforkTTD()`, PR [#2075](https://github.com/sila-chain/silajs-monorepo/pull/2075)
- **Breaking:** renamed `td` parameter in `HardforkConfig` interface to `ttd`, PR [#2075](https://github.com/sila-chain/silajs-monorepo/pull/2075)
- Set `goerli` Merge TTD to 10790000, PR [#2079](https://github.com/sila-chain/silajs-monorepo/pull/2079)
- Update `mergeForkIdTransition` Merge transition HF (separate "artificial" HF construct only for networking layer) for `sepolia`, PR [#2098](https://github.com/sila-chain/silajs-monorepo/pull/2098)

## 3.0.0-beta.2 - 2022-07-15

Beta 2 release for the upcoming breaking release round on the [SilaJS monorepo](https://github.com/sila-chain/silajs-monorepo) libraries, see the Beta 1 release notes ([CHANGELOG](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/common/CHANGELOG.md)) for the main change set description.
...
- Added `sepolia` DNS discovery config, PR [#2034](https://github.com/sila-chain/silajs-monorepo/pull/2034)
...
The `@silajs/common` library is the base library for various upper-level SilaJS libraries (like the VM or Transaction library), providing a common and shared view on the network and hardfork state.
...
With the `v3` release the default `Common` hardfork changes from `Istanbul` to `London`, see PR [#1749](https://github.com/sila-chain/silajs-monorepo/pull/1749).
...
See PRs [#1709](https://github.com/sila-chain/silajs-monorepo/pull/1709), [#1854](https://github.com/sila-chain/silajs-monorepo/pull/1854) for the related code changeset.
...
- `paramByEIP(topic: string, name: string, sip: number): bigint | undefined`
...
Following methods have been removed, see PRs [#1698](https://github.com/sila-chain/silajs-monorepo/pull/1698) and [#1709](https://github.com/sila-chain/silajs-monorepo/pull/1709).
...
There are a few new types e.g. for configuration files (e.g. `CliqueConfig`) to get rid of some last `any` types in the package, see PR [#1906](https://github.com/sila-chain/silajs-monorepo/pull/1906). Eventually related problems should be seen early on in your TypeScript setup though and it should also be possible to easily attribute and fix.

#### New File Structure

The file structure of the package has been aligned with other libraries and there is now a dedicated `common.ts` file for the main source code with `index.ts` re-exporting functionality and types, see PR [#1915](https://github.com/sila-chain/silajs-monorepo/pull/1915). Some misplaced types have been moved to `types.ts` and enums (like `Chain` or `Hardfork`) have been (internally) moved to an `enum.ts` file. You should generally use the root import from `index.ts`, if you are not doing and some imports broke this should be easily fixable though.

### Genesis Handling Refactor

We have completely refactored all our genesis (block) handling and moved the code and logic higher up the stack to the `Blockchain` library which is a more natural fit for this, see PR [#1916](https://github.com/sila-chain/silajs-monorepo/pull/1916) This freed us up to remove the large genesis state files - especially the `sila-mainnet` one - from `Common`.
...
- New experimental SIP `SIP-3074`: Authcall, PR [#1789](https://github.com/sila-chain/silajs-monorepo/pull/1789)
...
- Fixed non-option passing on `custom()` method, PR [#1851](https://github.com/sila-chain/silajs-monorepo/pull/1851)
- `PreMerge` hardfork renamed to `MergeForkIdTransition` for increased clarity, PR [#1856](https://github.com/sila-chain/silajs-monorepo/pull/1856)
...
- [SIP-4399](https://sips.sila.org/SIPS/sip-4399) Support: Supplant DIFFICULTY opcode with PREVRANDAO, PR [#1565](https://github.com/sila-chain/silajs-monorepo/pull/1565)
- Added new `preMerge` hardfork to fork off non-upgraded clients in `@silajs/client` (in the very most cases this somewhat "artificial" HF should not be used directly), PR [#1565](https://github.com/sila-chain/silajs-monorepo/pull/1565)
- Better documentation and integration of complex genesis state custom chain initialization (with (system) contracts and storage values), PR [#1757](https://github.com/sila-chain/silajs-monorepo/pull/1757)
...
- Fixed a bug on `Common.copy()` taking over existing event listeners (leading to unwanted side effects), PR [#1799](https://github.com/sila-chain/silajs-monorepo/pull/1799)

# 2.6.2 - 2022-02-4

- Adds support for [SIP-3607](https://sips.sila.org/SIPS/sip-3607) (Reject transactions from senders with deployed code), PR [#1691](https://github.com/sila-chain/silajs-monorepo/pull/1691)

## 2.6.1 - 2022-02-01

- Added support for [SIP-3855](https://sips.sila.org/SIPS/sip-3855) `push0` opcode, PR [#1616](https://github.com/sila-chain/silajs-monorepo/pull/1616)
- Added support for the new [SilaSepolia](https://sepolia.ethdevops.io/) (`Chain.SilaSepolia`) test network (PoW network replacing `ropsten`), PR [#1581](https://github.com/sila-chain/silajs-monorepo/pull/1581)
- Added `berlin` and `london` HF block numbers and fork hashes to `kovan`, PR [#1577](https://github.com/sila-chain/silajs-monorepo/pull/1577)
...
There is now a better Optimism L2 chain integration in Common (PR [#1554](https://github.com/sila-chain/silajs-monorepo/pull/1554)) allowing to directly instantiate an Optimism chain with the `Common.custom()` constructor. Note that this only sets the correct chain ID (and e.g. _not_ corresponding HF blocks or similar) and is therefore only suitable for a limited set of use cases (e.g. sending a tx to an Optimism chain).

Following Optimism chains are now integrated:

- `CustomChain.OptimisticSila`
- `CustomChain.OptimisticSilaKovan`

A Common with Optimism can be instantiated with:

```ts
const common = Common.custom(CustomChain.OptimisticSila)
```

### Other Changes

- Support for starting a (custom) chain on a `london` or later hardfork by allowing to set a `baseFeePerGas` value for the genesis block, PR [#1512](https://github.com/sila-chain/silajs-monorepo/pull/1512)
- Support for adding code accounts (e.g. with a "system" contract like the deposit contract) to a (custom) genesis file, PR [#1530](https://github.com/sila-chain/silajs-monorepo/pull/1530)
...
The Merge HF has been added as a new HF and can be used with `Hardfork.Merge`, also [SIP-3675](https://sips.sila.org/SIPS/sip-3675) as the core HF SIP has been added as an SIP JSON config file, see #1393. Note that all Merge HF related functionality is still considered `experimental`.
...
There is also a new `consensusType` `pos` which can be set along a HF file (see `src/hardforks/merge.json`) or directly in a chain file (like `src/chains/sila-mainnet.json`) to create a pure PoS chain (note that the creation of pure PoS chains is still untested). To reference this new consensus type `ConsensusType.ProofOfStake` from the `ConsensusType` enum dict can be used.

To allow a HF switch by total difficulty (TD) - which is planned for the Merge - the chain file type has been updated to now also accept a `td` value as an alternative (respectively also: in addition) to the `block` number value, see PR [#1473](https://github.com/sila-chain/silajs-monorepo/pull/1473). Along the `getHardforkByBlockNumber()` and `setHardforkByBlockNumber()` function signatures have been expanded to also allow for setting/getting a HF by the total difficulty value:
...
In the Common library all functionality returning hardfork, chain or genesis parameters has previously been under-typed respectively just returned `any` in most cases. This has been improved along PR [#1480](https://github.com/sila-chain/silajs-monorepo/pull/1480) and is now finding its way into a release.
...
We have removed all type 1. HF usages (so mainly `dao` HF inclusions) with PR [#1344](https://github.com/sila-chain/silajs-monorepo/pull/1344) and HF block number `null` values are now strictly reserved for type 2..
...
- **TypeScript Breaking**: Fixed `hardforkBlockBN()` to correctly return `null` for unscheduled hardforks, note that this changes the `TypeScript` function signature and might break your development setup (sorry for this, but this bugfix was nevertheless necessary), PR [#1329](https://github.com/sila-chain/silajs-monorepo/pull/1329)
- Always pre-compute the HF `forkHash` values if not hardcoded in the chain files, PR [#1423](https://github.com/sila-chain/silajs-monorepo/pull/1423)

### Maintenance

- Removed `calaveras` ephemeral testnet, PR [#1430](https://github.com/sila-chain/silajs-monorepo/pull/1430)
- Added browser tests to `Common`, PR [#1380](https://github.com/sila-chain/silajs-monorepo/pull/1380)
...
This release integrates the `london` HF blocks for all networks including `sila-mainnet` and is therefore the first release with finalized London HF support.

### Reworked Custom Chain Instantiation / Supported Custom Chains (Polygon / Arbitrum / xDaiChain)

This release introduces a new `Common.custom()` static constructor which replaces the now deprecated `Common.forCustomChain()` constructor and allows for an easier instantiation of a Common instance with somewhat adopted chain parameters, with the main use case to adopt on instantiating with a deviating chain ID. Instantiating a custom common instance with its own chain ID and inheriting all other parameters from `sila-mainnet` can now be as easily done as:
...
`Common` instances created with this simplified `custom()` constructor can't be used in all usage contexts (the HF configuration is very likely not matching the actual chain) but can be useful for specific use cases, e.g. for sending a tx with `@silajs/tx` to an L2 network (see the `Tx` library [README](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/tx) for a complete usage example).

### Chain & Hardfork Enums

This `Common` release comes with two new enums `Chain` and `Hardfork`. These contain the currently supported chains and hardforks by the library and can be used for both instantiation and calling various methods where a chain or a hardfork is requested as a parameter, see PR [#1322](https://github.com/sila-chain/silajs-monorepo/pull/1322).
...
- Removed retired dev networks (`yolov3`, `aleut` and `baikal`), PR [#1296](https://github.com/sila-chain/silajs-monorepo/pull/1296)

## 2.3.1 - 2021-06-11

Small feature release.

- Added static helper method `Common.isSupportedChainId()`to check if a chain is natively supported by the Common version installed, PR [#1281](https://github.com/sila-chain/silajs-monorepo/pull/1281)
- Added support for the `calaveras` ephemeral developer test network (preparing for the `london` HF), PR [#1286](https://github.com/sila-chain/silajs-monorepo/pull/1286)

## 2.3.0 - 2021-05-26

### Functional London HF Support (no finalized HF blocks yet)

This `Common` release comes with full functional support for the `london` hardfork (all SIPs are finalized and integrated and `london` HF can be activated, there are no final block numbers for the HF integrated though yet). Please note that the default HF is still set to `istanbul`. You therefore need to explicitly set the `hardfork` parameter for instantiating a `Common` instance with a `london` HF activated:
...
Common now supports settings for the following additional SIPs:
...
- [SIP-3554](https://sips.sila.org/SIPS/sip-3554): Difficulty Bomb Delay to December 2021 (only PoW networks), PR [#1245](https://github.com/sila-chain/silajs-monorepo/pull/1245)

All new SIPs have their dedicated SIP configuration file and can also be activated separately with the `sips` parameter (and the so-created `common` instance can then e.g. be used within the VM):
...
- Fixed a bug for `Common.hardforkGteHardfork()` and `Common.gteHardfork()` now evaluating to `true` if the HF provided as the `gteHardfork` part is not known by the chain, PR [#1148](https://github.com/sila-chain/silajs-monorepo/pull/1148)
- Fixed `sila-mainnet` berlin fork hash `0xeb440f6` -> `0x0eb440f6`, PR [#1148](https://github.com/sila-chain/silajs-monorepo/pull/1148)
- Fixed fork hash calculation in `Common._calcForkHash()` for fork hashes with a leading zero, PR [#1148](https://github.com/sila-chain/silajs-monorepo/pull/1148)

### Other Changes

- Added `london` HF option, PR [#1148](https://github.com/sila-chain/silajs-monorepo/pull/1148)
- Added `baikal` test network (preparatory `london` network), PR [#1249](https://github.com/sila-chain/silajs-monorepo/pull/1249)
- Added `aleut` test network (preparatory `london` network, retired), PR [#1221](https://github.com/sila-chain/silajs-monorepo/pull/1221)
...
- Added final list of `berlin` SIPs (`SIP-2565`, `SIP-2929`, `SIP-2718`, `SIP-2930`), PR [#1124](https://github.com/sila-chain/silajs-monorepo/pull/1124) and PR [#1048](https://github.com/sila-chain/silajs-monorepo/pull/1048)
- Corrected base gas costs for `SIP-2929` related opcodes, PR [#1124](https://github.com/sila-chain/silajs-monorepo/pull/1124)
- New SIP configuration files for `SIP-2718` (typed txs) and `SIP-2930` (optional access lists), PR [#1048](https://github.com/sila-chain/silajs-monorepo/pull/1048)
- Added `berlin` hardfork block numbers for `sila-mainnet`, `ropsten`, `rinkeby` and `goerli`, PR [#1142](https://github.com/sila-chain/silajs-monorepo/pull/1142)
...
- Added chain config and genesis file for `yolov3` testnet, PR [#1129](https://github.com/sila-chain/silajs-monorepo/pull/1129)
- New `Common.copy()` function to easily receive a deep copy of a `Common` instance, PR [#1144](https://github.com/sila-chain/silajs-monorepo/pull/1144)

## 2.1.0 - 2021-02-22

### Clique/PoA Support

This release completes on Clique/PoA support (see also Clique/PoA related changes in `v2.0.0`), see PR [#1032](https://github.com/sila-chain/silajs-monorepo/pull/1032). The chain configuration files (e.g. `chains/goerli.json`) are extended by a consensus algorithm-specific config parameter section, here is a sample `consensus` parameter section, note that the `config` parameter dict must be named after the consensus algorithm:
...
There is now a more convenient and flexible way to integrate custom chains into Common instances complementing the existing `Common.fromCustomChain()` static constructor, see PR [#1034](https://github.com/sila-chain/silajs-monorepo/pull/1034).
...
// Add two custom chains, initial sila-mainnet activation
const common1 = new Common({ chain: 'sila-mainnet', customChains: [myCustomChain1, myCustomChain2] })
...
The [README section](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/common#working-with-privatecustom-chains) on working with custom chains has been significantly expanded along the way and is a recommended read if you use common for custom chain initialization.

### New SIPs

#### SIP-1459 DNS Peer Discovery

[SIP-1459](https://sips.sila.org/SIPS/sip-1459) introduces a way to discover nodes for an Sila network connection via DNS. This release adds a new optional chain config file parameter `dnsNetworks` and an associated method `Common.dnsNetworks()` to request DNS networks for a chain.

#### SIP-2565 ModExp Precompile Gas Costs

[SIP-2565](https://sips.sila.org/SIPS/sip-2565) introduces a new algorithm for ModExp precompile gas cost calculation. A new SIP file `sips/2565.json` has been added along the work on PR [#1026](https://github.com/sila-chain/silajs-monorepo/pull/1026) with respective parameter updates.

### Other Changes

- `Common` is now implemented as an `EventEmitter` and emits a `hardforkChanged` event upon a HF change, PR [#1112](https://github.com/sila-chain/silajs-monorepo/pull/1112)
- New `Common.isActivatedEIP()` method, PR [#1125](https://github.com/sila-chain/silajs-monorepo/pull/1125)
- Updated `Goerli` bootnodes, PR [#1031](https://github.com/sila-chain/silajs-monorepo/pull/1031)
...
**Breaking**: The constructor has been changed to require an options dict to be passed, PR [#863](https://github.com/sila-chain/silajs-monorepo/pull/863)

Example:

```ts
import Common from '@silajs/common'
const common = new Common({ chain: 'sila-mainnet', hardfork: 'muirGlacier' })
```

### SIP Support

SIPs are now native citizens within the `Common` library, see PRs [#856](https://github.com/sila-chain/silajs-monorepo/pull/856), [#869](https://github.com/sila-chain/silajs-monorepo/pull/869) and [#872](https://github.com/sila-chain/silajs-monorepo/pull/872). Supported SIPs have their own configuration file like the [sips/2537.json](https://github.com/sila-chain/silajs-monorepo/blob/master/packages/common/src/sips/2537.json) file for the BLS precompile SIP and SIP settings can be activated by passing supported SIP numbers to the constructor:

```ts
const c = new Common({ chain: 'sila-mainnet', sips: [2537] })
```

The following SIPs are initially supported within this release:

- [SIP-2537](https://sips.sila.org/SIPS/sip-2315) BLS Precompiles
- [SIP-2315](https://sips.sila.org/SIPS/sip-2315) SAVM Subroutines (PR [#876](https://github.com/sila-chain/silajs-monorepo/pull/876))

SIPs provided are then activated and parameters requested with `Common.param()` being present in these SIPs take precedence over the setting from the latest hardfork.

There are two new utility functions which return hardfork and SIP values respectively:
...
For setting and requesting active SIPs there is `Common.setEIPs()` and `Common.sips()` added to the mix.

There is also a new SIP-based hardfork file format which delegates parameter definition to dedicated SIP files (see PR [#876](https://github.com/sila-chain/silajs-monorepo/pull/876)). This is in preparation for an upcoming `Yolo v2` testnet integration.

Side note: with this new structural setup it gets now possible for all SIPs still implicitly contained within the hardfork files to be extracted as an SIP parameter set within its own dedicated SIP file (which can then be activated via the `sip` parameter on initialization) without loosing on functionality. If you have a need there feel free to open a PR!

### Gas Parameter Completeness for all Hardforks

Remaining gas base fees which still resided in the VM have been moved over to `Common` along PR [#806](https://github.com/sila-chain/silajs-monorepo/pull/806).

Gas fees for all hardforks up to `MuirGlacier` are now completely present within the `Common` library.

### Sil/64 Forkhash Support
...
Forkhashes are used to uniquely identify a set of hardforks passed to be able to better differentiate between different dedicated chains. This is used for the `Sil/64` devp2p protocol update and specified in [SIP-2124](https://sips.sila.org/SIPS/sip-2124) to help improve the devp2p networking stack.

### New Block/Hardfork related Utility Functions

The following block and hardfork related utility functions have been added with PRs [#863](https://github.com/sila-chain/silajs-monorepo/pull/863) and [#805](https://github.com/sila-chain/silajs-monorepo/pull/805) respectively:
...
The default hardfork has been added as an accessible readonly property `DEFAULT_HARDFORK`, PR [#863](https://github.com/sila-chain/silajs-monorepo/pull/863). This setting is used starting with the latest major releases of the monorepo libraries like the VM to keep the HF setting in sync across the different libraries.

Current default hardfork is set to `istanbul`, PR [#906](https://github.com/sila-chain/silajs-monorepo/pull/906).
...
- Added consensus information to chains, new functions `Common.consensusType()` for consensus type access ("pow" or "poa") and `Common.consensusAlgorithm()` to get the associated algorithm or protocol (e.g. "ethash" PoW algorithm or "clique" PoA protocol), see PR [#937](https://github.com/sila-chain/silajs-monorepo/pull/937)
- Removed old `consensus` and `finality` fields, PR [#758](https://github.com/sila-chain/silajs-monorepo/pull/758)
- Removed old `casper` and `sharding` fields, PR [#762](https://github.com/sila-chain/silajs-monorepo/pull/762)
- Updated `silajs-util` to v7, PR [#748](https://github.com/sila-chain/silajs-monorepo/pull/748)

## 2.0.0-rc.1 2020-11-19

This is the first release candidate towards a final library release, see [beta.2](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fcommon%402.0.0-beta.2) and especially [beta.1](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fcommon%402.0.0-beta.1) release notes for an overview on the full changes since the last publicly released version.

No changes since `beta.2` release.

## 2.0.0-beta.2 - 2020-11-12

This is the second beta release towards a final library release, see [beta.1 release notes](https://github.com/sila-chain/silajs-monorepo/releases/tag/%40silajs%2Fcommon%402.0.0-beta.1) for an overview on the full changes since the last publicly released version.
...
- Removed old `consensus` and `finality` fields,
  PR [#758](https://github.com/sila-chain/silajs-monorepo/pull/758)
- Removed old `casper` and `sharding` fields,
  PR [#762](https://github.com/sila-chain/silajs-monorepo/pull/762)
...
- Updates Goerli chain ID, PR [#792](https://github.com/sila-chain/silajs-monorepo/pull/792).
...
- Updated bootnode definitions, and more strict checking for their values.
  PR [#718](https://github.com/sila-chain/silajs-monorepo/pull/718)

[1.5.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.5.0...%40silajs%2Fcommon%401.5.1

## [1.5.0] - 2019-12-10

Support for the `MuirGlacier` HF
([SIP-2387](https://sips.sila.org/SIPS/sip-2387)) scheduled for January 2020
delaying the difficulty bomb.

Changes:

- Implemented [SIP-2384](https://sips.sila.org/SIPS/sip-2384) Difficulty
  Bomb Delay, PR [#75](https://github.com/sila-chain/silajs/silajs-common/pull/75)
- Consistent genesis account balance format, converted from decimal to hex
  where necessary, PR [#73](https://github.com/sila-chain/silajs/silajs-common/pull/73)

[1.5.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.4.0...%40silajs%2Fcommon%401.5.0
...
- Added `Istanbul` block numbers for `sila-mainnet`, `goerli` and `rinkeby`,
  PR [#68](https://github.com/sila-chain/silajs/silajs-common/pull/68)
- Added `Petersburg` and `Constantinople` fork blocks to `rinkeby`,
  PR [#71](https://github.com/sila-chain/silajs/silajs-common/pull/71)
- Added `SIP-2200` (rebalance net-metered SSTORE gas costs) parameters for `Istanbul`,
  PR [#65](https://github.com/sila-chain/silajs/silajs-common/pull/65)

Other noteworthy changes:

- Adding forks (including `Istanbul`) for `kovan`,
  PR [#70](https://github.com/sila-chain/silajs/silajs-common/pull/70)
- Fixed `kovan` genesis state,
  PR [#66](https://github.com/sila-chain/silajs/silajs-common/pull/66)

[1.4.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.3.2...%40silajs%2Fcommon%401.4.0

## [1.3.2] - 2019-09-04

**Istanbul** Updates:

- Added gas parameters for `SIP-2200` (rebalanced net-metered SSTORE
  gas costs), PR [#65](https://github.com/sila-chain/silajs/silajs-common/pull/65)
- Renamed hardfork `blake2bRound` (-> `blake2Round`) parameter,
  PR [#63](https://github.com/sila-chain/silajs/silajs-common/pull/63)

Other Changes:

- Fixed `SilaKovan` genesis state,
  PR [#66](https://github.com/sila-chain/silajs/silajs-common/pull/66)

[1.3.2]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.3.1...%40silajs%2Fcommon%401.3.2

## [1.3.1] - 2019-08-08

Added missing **Istanbul** gas costs for:

- ChainID opcode (SIP-1344, as base param in `hardforks/chainstart.json`)
- Blake2b precompile (SIP-2129/152)
- Calldata gas cost reduction (SIP-2028)

See PR [#58](https://github.com/sila-chain/silajs/silajs-common/pull/58).

[1.3.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.3.0...%40silajs%2Fcommon%401.3.1
...
[1.3.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.2.1...%40silajs%2Fcommon%401.3.0

## [1.2.1] - 2019-06-03

- Added `Istanbul` HF candidate [SIP-1108](https://sips.sila.org/SIPS/sip-1108)
  (`DRAFT`) updated `alt_bn128` precompile gas costs (see `hardforks/istanbul.json`)

[1.2.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.2.0...%40silajs%2Fcommon%401.2.1
...
[#51](https://github.com/sila-chain/silajs/silajs-common/pull/51).

[1.2.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.1.0...%40silajs%2Fcommon%401.2.0
...
`constantinopleFix`) HF removing support for [SIP 1283](https://sips.sila.org/SIPS/sip-1283). `Petersburg` is conceptualized
...
update on both (`sila-mainnet` scenario) or applying subsequently on subsequent
block numbers (`ropsten` scenario).

HF related changes (from PR [#44](https://github.com/sila-chain/silajs/silajs-common/pull/44)):

- New `hardforks/petersburg.json` HF file
- `constantinople` and `petersburg` block numbers for `ropsten` and `sila-mainnet`
...
Goerli related changes (from PR [#48](https://github.com/sila-chain/silajs/silajs-common/pull/48)):
...
- Fixed a bug in `hardforkGteHardfork()` where non-active hardforks were considered equal to `chainstart` when `onlyActive` is passed, see
  PR [#44](https://github.com/sila-chain/silajs/silajs-common/pull/44)
- Use CLI scripts from silajs-config in package.json, PR
  [#43](https://github.com/sila-chain/silajs/silajs-common/pull/43)

[1.1.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%401.0.0...%40silajs%2Fcommon%401.1.0

## [1.0.0] - 2019-01-23

First `TypeScript` based release of the library (for details see
PR [#38](https://github.com/sila-chain/silajs/silajs-common/pull/38)),
...
```javascript
const Common = require('silajs-common').default
```

**Genesis State Import/Usage**

Import path and usage API of genesis state has changed, see also the
[docs](https://github.com/sila-chain/silajs/silajs-common#genesis-states) on this,
PR [#39](https://github.com/sila-chain/silajs/silajs-common/pull/39):

```javascript
const sila-mainnetGenesisState = require('silajs-common/dist/genesisStates/sila-mainnet')
```

Or by accessing dynamically:

```javascript
const genesisStates = require('silajs-common/dist/genesisStates')
const sila-mainnetGenesisState = genesisStates.genesisStateByName('sila-mainnet')
const sila-mainnetGenesisState = genesisStates.genesisStateById(1) // alternative via network Id
```
...
list of hardforks, see PR [#37](https://github.com/sila-chain/silajs/silajs-common/pull/37)

[1.0.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.6.1...%40silajs%2Fcommon%401.0.0

## [0.6.1] - 2018-11-28

- Experimental support for the [Goerli](https://github.com/goerli/testnet) cross-client `PoA` testnet (`chains/goerli.json`), see PR [#31](https://github.com/sila-chain/silajs/silajs-common/pull/31)
- Unified hex-prefixing (so always prefixing with `0x`) of account addresses in genesis files (fixes an issue with state root computation on other libraries), see PR [#32](https://github.com/sila-chain/silajs/silajs-common/issues/32)

[0.6.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.6.0...%40silajs%2Fcommon%400.6.1

## [0.6.0] - 2018-10-11

Parameter support for the `Constantinople` hardfork (see `hardforks/constantinople.json`):

- Added `SSTORE` gas/refund prices (`SIP-1283`), PR [#27](https://github.com/sila-chain/silajs/silajs-common/pull/27)
- Added Block Reward Adjustment (`SIP-1234`), PR [#26](https://github.com/sila-chain/silajs/silajs-common/pull/26)

[0.6.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.5.0...%40silajs%2Fcommon%400.6.0
...
- Introduces **support for private chains** by allowing to pass a custom dictionary as the `chain` parameter
  ...
  `chain` `String` names (e.g. `sila-mainnet`, `ropsten`), PR [#24](https://github.com/sila-chain/silajs/silajs-common/pull/24)

[0.5.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.4.1...%40silajs%2Fcommon%400.5.0

## [0.4.1] - 2018-08-13

- Added `timestamp` field to genesis definitions in chain files, set for `Rinkeby` and `null` for other chains, PR [#21](https://github.com/sila-chain/silajs/silajs-common/pull/21)
- Updated `Ropsten` bootstrap nodes, PR [#20](https://github.com/sila-chain/silajs/silajs-common/pull/20)

[0.4.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.4.0...%40silajs%2Fcommon%400.4.1
...
[0.4.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.3.1...%40silajs%2Fcommon%400.4.0

## [0.3.1] - 2018-05-28

- Added two alias functions `activeOnBlock()` and `gteHardfork()` when hardfork is set for convenience, PR [#15](https://github.com/sila-chain/silajs/silajs-common/pull/15)
- Added option to dynamically choose genesis state (see `README`), PR [#15](https://github.com/sila-chain/silajs/silajs-common/pull/15)

[0.3.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.3.0...%40silajs%2Fcommon%400.3.1

## [0.3.0] - 2018-05-25

- Allow functions like `hardforkIsActiveOnBlock()` - where hardfork is provided as param - also to be run on hardfork set for greater flexibility/comfort, PR [#13](https://github.com/sila-chain/silajs/silajs-common/pull/13)
- New `hardforkGteHardfork()` method for HF order comparisons, PR [#13](https://github.com/sila-chain/silajs/silajs-common/pull/13)

[0.3.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.2.0...%40silajs%2Fcommon%400.3.0

## [0.2.0] - 2018-05-14

- New optional initialization parameter `allowedHardforks`, this allows for cleaner client
  library implementations by preventing undefined behaviour, PR [#10](https://github.com/sila-chain/silajs/silajs-common/pull/10)
- Added `activeHardfork()` function to get latest active HF for chain or block, PR [#11](https://github.com/sila-chain/silajs/silajs-common/pull/11)

[0.2.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.1.1...%40silajs%2Fcommon%400.2.0

## [0.1.1] - 2018-05-09

- Remove dynamic require to prevent browserify issue, PR [#8](https://github.com/sila-chain/silajs/silajs-common/pull/8)

[0.1.1]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%400.1.0...%40silajs%2Fcommon%400.1.1

## [0.1.0] - 2018-05-09

- Initial version, this library succeeds the [sila/common](https://github.com/sila-chain/silajs/common/issues/12)
...
- See [README](https://github.com/sila-chain/silajs/silajs-common) and [API Docs](https://github.com/sila-chain/silajs/silajs-common/blob/master/docs/index.md) for a more in-depth feature overview and usage instructions

[0.1.0]: https://github.com/sila-chain/silajs-monorepo/compare/%40silajs%2Fcommon%406d0df...%40silajs%2F..v0.1.0
