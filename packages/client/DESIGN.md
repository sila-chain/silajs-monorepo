...
We will now briefly describe the directory structure and main components of the Silajs client
to help contributors better understand how the project is organized.

**Directory structure**

- `/bin` Contains the CLI script for the `silajs` command.
...
- `/src/service` Contains the main Sila services (`FullSilaService` and `LightSilaService`).
...
- `Chain` [**In Progress**] This class represents the blockchain and is a wrapper around
  `@silajs/blockchain`. It handles creation of the data directory, provides basic blockchain operations
...
- `Server` This class represents a server that discovers new peers and handles incoming and dropped
  ...
  if a new peer understands the `sil` protocol, it will contain an `sil` property that provides all `sil`
  protocol methods (for example: `peer.sil.getBlockHeaders()`)
...
- `Protocol` [**In Progress**] This class and subclasses provide a user-friendly wrapper around the
  low level sila protocols `sil/66` and `les/4`. Subclasses must define the messages provided by the protocol.
  - `SilProtocol` [**In Progress**] Implements `sil/66` protocol.
...
- `Handler` Subclasses of this class implements a protocol message handler. Handlers respond to incoming requests from peers.
  - `SilHandler` [**In Progress**] Handles incoming SIL requests
  - `LesHandler` [**In Progress**] Handles incoming LES requests
- `Service` Subclasses of `Service` will implement specific functionality of a `Client`. For example, the `SilaService` subclasses will synchronize the blockchain using the full or light sync protocols. Each service must specify which protocols it needs and define a `start()` and `stop()` function.
  - `FullSilaService` [**In Progress**] Implementation of sila full sync.
  - `LightSilaService` [**In Progress**] Implementation of sila light sync.
- `Client` [**In Progress**] Represents the top-level sila client, and is responsible for managing the lifecycle of included services.
