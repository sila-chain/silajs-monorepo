...
import { TransactionType, createTx } from '@silajs/tx'
import { equalsBytes, hexToBytes, randomBytes } from '@silajs/util'
...
vi.mock('../../src/net/protocol/ethprotocol.ts', () => {
  const SilProtocol = vi.fn()
  SilProtocol.prototype.name = 'sil'
  return { SilProtocol }
})
...
const { FullSilaService } = await import('../../src/service/fullsilaservice.ts')

describe('initialize', async () => {
  const config = new Config({ accountCache: 10000, storageCache: 1000 })
  const chain = await Chain.create({ config })
  const service = new FullSilaService({ config, chain })

  it('should initialize correctly', async () => {
    assert.strictEqual('full', service.synchronizer?.type, 'full mode')
    assert.strictEqual(service.name, 'sil', 'got name')
  })

  it('should get protocols', async () => {
    let config = new Config({ accountCache: 10000, storageCache: 1000 })
    const chain = await Chain.create({ config })
    let service = new FullSilaService({ config, chain })
    assert.isNotEmpty(
      service.protocols.filter((p) => p.name === 'sil'),
      'full protocol',
    )
    assert.isEmpty(
    ...
    config = new Config({})
    service = new FullSilaService({ config, chain })
    assert.isNotEmpty(
      service.protocols.filter((p) => p.name === 'sil'),
      'full protocol',
    )
...
describe('should call handleNewBlock on NewBlock and handleNewBlockHashes on NewBlockHashes', async () => {
  ...
  it('should handle new block', async () => {
    await service.handle({ name: 'NewBlock', data: [{}, BigInt(1)] }, 'sil', undefined as any)
    expect((service.synchronizer as any).handleNewBlock).toBeCalled()
  })
  it('should handle new blockhashes', async () => {
    await service.handle({ name: 'NewBlockHashes', data: [{}, BigInt(1)] }, 'sil', undefined as any)
...
describe('should ban peer for sending NewBlock/NewBlockHashes after merge', async () => {
  ...
  const chain = await Chain.create({ config })
  chain.config.chainCommon.setHardfork(Hardfork.SilaParis)
  const service = new FullSilaService({ config, chain })
  service.pool.ban = () => {
    it('should ban peer', () => {
  ...
  await service.handle({ name: 'NewBlock', data: [{}, BigInt(1)] }, 'sil', { id: 1 } as any)
  await service.handle({ name: 'NewBlockHashes', data: [] }, 'sil', { id: 1 } as any)
})
...
describe('should send Receipts on GetReceipts', async () => {
  ...
  const peer = { sil: { send: vi.fn() } } as any
  it('should handle getReceipts', async () => {
    await service.handle({ name: 'GetReceipts', data: [BigInt(1), [blockHash]] }, 'sil', peer)
    expect(peer.sil.send).toBeCalledWith('Receipts', { reqId: BigInt(1), receipts })
  })
})
...
describe('should handle decoding NewPooledTransactionHashes with sil/68 message format', async () => {
...
describe.skip('should handle structuring NewPooledTransactionHashes with sil/68 message format', async () => {
  const txHash = randomBytes(32)
  const config = new Config({ accountCache: 10000, storageCache: 1000 })
  const chain = await Chain.create({ config })
  const service = new FullSilaService({ config, chain })
...
describe('should start on beacon sync when past merge', async () => {
  ...
  it('should not be available', () => {
    ...
    const service = new FullSilaService({ config: configDisableBeaconSync, chain })
