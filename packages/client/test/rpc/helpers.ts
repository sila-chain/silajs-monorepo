import { createBlockHeader, paramsBlock } from '@silajs/block'
...
import { buildBlock } from '@silajs/vm'
...
import type { CustomCrypto, GenesisState, GethGenesis, Hardfork } from '@silajs/common'
...
export function createManager(client: SilaClient) {
  return new Manager(client, client.config)
}

export async function createClient(clientOpts: Partial<CreateClientOptions> = {}) {
  const common: Common = clientOpts.commonChain ?? new Common({ chain: SilaMainnet })
