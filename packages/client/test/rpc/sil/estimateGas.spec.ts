...
import { Units, bigIntToHex, createAddressFromString } from '@silajs/util'
import { runBlock, runTx } from '@silajs/vm'
...
describe(method, () => {
  it('call with valid arguments', async () => {
    // Use custom genesis so we can test SIP1559 txs more easily
    const { RPCTestnetData } = await import('@silajs/testdata')
  ...
  */
    ...
    // Setup chain to run an SIP1559 tx
    ...
    // Test SIP1559 tx
    const SIP1559res = await rpc.request(method, [
      { ...estimateTxData, type: 2, maxFeePerGas: '0x' + 10000000000n.toString(16) },
    ])
    assert.strictEqual(
      SIP1559res.result,
      '0x' + totalGasSpent.toString(16),
      'should return the correct gas estimate for SIP1559 tx',
    )

    // Test SIP1559 tx with no maxFeePerGas
    const SIP1559reqNoGas = await rpc.request(method, [
      {
        ...estimateTxData,
        type: 2,
