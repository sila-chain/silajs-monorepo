import { paramsBlock } from '@silajs/block'
import { Common, Hardfork, SilaMainnet } from '@silajs/common'
import { createTx } from '@silajs/tx'
import { BIGINT_0, bigIntToHex, bytesToBigInt, hexToBytes } from '@silajs/util'
import { buildBlock } from '@silajs/vm'
import { assert, describe, it } from 'vitest'

import { SIGNER_G, powData } from '@silajs/testdata'
...
describe(method, () => {
  ...
  it(`${method}: should return 0x0 for base fees requested before sip-1559`, async () => {
  ...
  /*it(
    ...
    async () => {
      const { chain, execution, server } = await setupChain(sip4844GethGenesis, 'post-merge', {
        engine: true,
