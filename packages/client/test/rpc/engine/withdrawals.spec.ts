import { genWithdrawalsTrieRoot } from '@silajs/block'
import { MerklePatriciaTrie } from '@silajs/mpt'
import { withdrawalsGethGenesis } from '@silajs/testdata'
import { bigIntToHex, bytesToHex, createWithdrawal, intToHex } from '@silajs/util'
...
for (const { name, withdrawals, withdrawalsRoot, gethBlockRlp } of testCases) {
  ...
  it(name, async () => {
    ...
    assert.isTrue(
      res.error.message.includes('PayloadAttributesV2 MUST be used after SilaShanghai is activated'),
    )
