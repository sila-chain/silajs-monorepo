import { beaconData, postMergeGethGenesis } from '@silajs/testdata'
import { bigIntToHex } from '@silajs/util'
...
describe(`${method}: call with executionPayloadV3`, () => {
  it('invalid call before SilaCancun', async () => {
    ...
    assert.isTrue(
      res.error.message.includes('NewPayloadV{1|2} MUST be used before SilaCancun is activated'),
    )
  })
  ...
  it('valid data', async () => {
    ...
    const expectedErrors = [
      'NewPayloadV2 MUST be used after SilaShanghai is activated',
      'NewPayloadV3 MUST be used after SilaCancun is activated',
    ]
