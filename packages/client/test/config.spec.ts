...
describe('[Config]', () => {
  ...
  it('Chain data default directory', () => {
    const config = new Config()
    assert.strictEqual(config.getDataDirectory(DataDirectory.Chain), './datadir/sila-mainnet/chain')
  })

  it('State data default directory', () => {
    const config = new Config()
    assert.strictEqual(config.getDataDirectory(DataDirectory.State), './datadir/sila-mainnet/state')
  })

  it('Meta data default directory', () => {
    const config = new Config()
    assert.strictEqual(config.getDataDirectory(DataDirectory.Meta), './datadir/sila-mainnet/meta')
  })

  it('peer discovery default sila-mainnet setting', () => {
    const common = new Common({ chain: SilaMainnet })
    const config = new Config({ common, accountCache: 10000, storageCache: 1000 })
    assert.strictEqual(config.discDns, false, 'disables DNS peer discovery for sila-mainnet')
    assert.strictEqual(config.discV4, true, 'enables DNS peer discovery for sila-mainnet')
  })
