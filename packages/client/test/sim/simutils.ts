...
import { executionPayloadFromBeaconPayload } from '@silajs/block'
import { type Common } from '@silajs/common'
import { createBlob4844Tx, createFeeMarket1559Tx } from '@silajs/tx'
import {
  BIGINT_1,
  blobsToCommitments,
  blobsToProofs,
...
import type { TransactionType, TxData, TxOptions } from '@silajs/tx'
import type { PrefixedHexString } from '@silajs/util'
import type { Client } from 'jayson/promise/index.js'
import type { SilaClient } from '../../src/client.ts'
...
/**
 * Sila Beacon API requires the query with format:
...
export async function validateBlockHashesInclusionInBeacon(
  ...
  for (let i = from; i <= to; i++) {
    const res = await (await fetch(`${beaconUrl}/sil/v2/beacon/blocks/${i}`)).json()
...
export async function setupEngineUpdateRelay(client: SilaClient, peerBeaconUrl: string) {
  // track head
  const topics = ['head']
  const EventSource = await getEventSource()
  const query = stringifyQuery({ topics })
  console.log({ query })
  const eventSource = new EventSource(`${peerBeaconUrl}/sil/v1/events?${query}`)
  ...
  eventSource.addEventListener(topics[0], async (_event: MessageEvent) => {
    ...
    try {
      ...
      const beaconFinalized = await (
        await fetch(`${peerBeaconUrl}/sil/v1/beacon/light_client/finality_update`)
      ).json()
      ...
      const beaconHead = await (await fetch(`${peerBeaconUrl}/sil/v2/beacon/blocks/head`)).json()
