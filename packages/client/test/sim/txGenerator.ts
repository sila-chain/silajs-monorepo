// Adapted from - https://github.com/Inphi/sip4844-interop/blob/master/blob_tx_generator/blob.js
...
import type { TransactionType, TxData } from '@silajs/tx'
...
async function run(data: any) {
  ...
  while (!done) {
    ...
    console.log(`waiting for sip4844 proc.... bn=${num}`)
    await sleep(1000)
  }
  ...
  try {
    const res = (
      await (await fetch('http://127.0.0.1:9596/sil/v1/beacon/headers', { method: 'get' })).json()
    ).data[0].header.message.slot
    const start = parseInt(res)
    for (let i = 0; i < 5; i++) {
      const res = (
        await (await fetch(`http://127.0.0.1:9596/sil/v2/beacon/blocks/${start + i}`)).json()
      ...
      while (!done) {
        const current =
          (await (await fetch('http://127.0.0.1:9596/sil/v1/beacon/headers')).json()).data[0].header
            .message.slot - 1
        if (current > start + i) {
          done = true
        }
