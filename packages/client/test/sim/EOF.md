...
- https://github.com/sila-chain/silajs-monorepo/issues/2298

This sim test is to setup a single silajs<>lodestar instance to allow executing testvectors for the SIPs targeting SilaShanghai hardfork.

### SIP(s) testing

Target SIPs are: `3540,3651,3670,3855,3860`
...
3. `jq` & `curl` installed
4. `silajs-monorepo` codebase build via `npm i` (for e.g. at `ethjs/`)
...
1. `npm install` the `silajs-monorepo`
...
This command run should start both silajs and lodestar in terminal. Soon you should see lodestar driving silajs in PoS configuration.
...
1. Clean DATADIR (`rm -rf data/eof/*`) in start of each run and do `mkdir data/eof/silajs && mkdir data/eof/lodestar`
2. Start silajs: `npm run client:start -- --datadir data/eof/silajs --gethGenesis test/sim/configs/geth-genesis.json --rpc --rpcEngine --rpcEngineAuth false`
3. Get genesis hash from `silajs` client:
...
You may want to do your own custom transaction development/testing to target the SIP functionality in the hardfork. Using **Semi Auto** network script or via **Manual network setup** steps, you may start `silajs<>lodestar` custom network.
Silajs rpc endpoint should now be available at `http://127.0.0.1:8545` for you to play with!

happy Testing the SilaShanghai!
...
2. Find the silajs process by doing `ps -a | grep client` and do `kill <process id>`
