...
describe('sharding/sip4844 hardfork tests', async () => {
  ...
  it('Simple blob tx', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const res = await (
        await fetch(`http://127.0.0.1:9596/sil/v2/beacon/blocks/${start + i}`)
      ).json()
      let done = false
  ...
  it('point precompile contract test', async () => {
    ...
    //https://github.com/Inphi/sip4844-interop/blob/master/point_evaluation_tx/PointEvaluationTest.sol
