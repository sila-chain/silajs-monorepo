...
import { bytesToHex, createAddressFromString, hexToBytes, privateToAddress } from '@silajs/util'
...
let ejsClient: SilaClient | null = null
...
describe('simple sila-mainnet test run', async () => {
  ...
  it.skipIf(process.env.ADD_EOA_STATE === undefined)(
    ...
    async () => {
      ...
      balance = await client.request('eth_getBalance', [EOATransferToAccount, 'latest'])
      assert.strictEqual(BigInt(balance.result), EOATransferToBalance, 'sent a simple SIL transfer')
      await runTx('', EOATransferToAccount, 1000000n)
      EOATransferToBalance += 1000000n
  ...
  it.skipIf(process.env.SNAP_SYNC === undefined)(
    ...
    async () => {
      ...
      assert.isTrue(ejsClient !== null, 'silajs client started')

      const enode = ejsClient!.server()!.getRlpxInfo().enode
      const res = await client.request('admin_addPeer', [enode])
      assert.strictEqual(res.result, true, 'successfully requested Geth add SilaJS as peer')
  ...
  it.skipIf(process.env.SNAP_SYNC === undefined)(
    ...
    async () => {
      if (ejsClient !== null && snapCompleted !== undefined && beaconSyncRelayer !== null) {
        ...
        assert.fail('silajs client not setup properly for snap sync')
      }
    },
    10 * 60_000,
  )
