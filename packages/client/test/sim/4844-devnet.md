...
Step 1. To run a single SilaJS client <> Lodestar CL client for testing, run the following command:
...
Step 2. (Optional) To run a second SilaJS <> Lodestar pair, use this command:
...
`tsx test/sim/txGenerator 8545 'hello'`. The first argument is the port number of the SilaJS client you which to submit the transaction to and the second is any data to include in the blob.

This script was adapted from the [interop repo blob script](https://github.com/Inphi/sip4844-interop/blob/master/blob_tx_generator/blob.js)
...
Note, these tests are adapted from the specification tests contained in the [SIP-4844 Interop repo](https://github.com/Inphi/sip4844-interop)
