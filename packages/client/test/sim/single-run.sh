...
then
  ...
  then
    case $ELCLIENT in 
      silajs)
        echo "ELCLIENT=$ELCLIENT using local silajs binary from packages/client"
        ;;
      geth)
        if [ ! -n "$NETWORKID" ]
        then
        ...
        ELCLIENT_IMAGE="sila/client-go:v1.12.2"
  ...
  * )
    ...
    case $ELCLIENT in 
      ...
      geth)
        ...
        EL_PORT_ARGS="--datadir /data/geth --authrpc.jwtsecret /data/jwtsecret --http --http.api engine,net,sil,web3,debug,admin --http.corsdomain \"*\" --http.port 8545 --http.addr 0.0.0.0 --http.vhosts \"*\" --authrpc.addr 0.0.0.0 --authrpc.vhosts \"*\" --authrpc.port=8551 --syncmode full --networkid $NETWORKID --nodiscover"
        ;;
      *)
        echo "ELCLIENT=$ELCLIENT not implemented"
    esac
...
cleanup() {
  ...
  if [ -n "$ejsPid" ] 
  then
    ejsPidBySearch=$(ps x | grep "tsx bin/cli.ts --dataDir $DATADIR/silajs" | grep -v grep | awk '{print $1}')
    echo "cleaning silajs pid:${ejsPid} ejsPidBySearch:${ejsPidBySearch}..."
    if [ -n "$ELCLIENT_IMAGE" ]
    then
...
then
  case $ELCLIENT in 
    silajs)
      ejsCmd="npm run client:start:ts -- --dataDir $DATADIR/silajs --gethGenesis $scriptDir/configs/$NETWORK.json --rpc --rpcEngine --rpcEngineAuth false $EL_PORT_ARGS"
      ;;
    geth)
      # geth will be mounted in docker with DATADIR to /data
  ...
  then
    ...
    do
      sleep 3
      echo "Fetching genesis hash from silajs ..."
      ejsId=$(( ejsId +1 ))
  ...
  do
    ...
    ejsId=$(( ejsId +1 ))
    responseCmd="curl --location --request GET 'http://localhost:9596/sil/v1/beacon/headers/genesis' --header 'Content-Type: application/json'  2>/dev/null | jq \".data.root\""
  ...
  do
    ...
    ejsId=$(( ejsId +1 ))
    responseCmd="curl --location --request GET 'http://localhost:9596/sil/v1/node/identity' --header 'Content-Type: application/json'  2>/dev/null | jq \".data.enr\""
    bootEnrs=$(eval "$responseCmd")
  done;

  elBootnode=$(cat "$origDataDir/peer1/silajs/$NETWORK/rlpx");
  ...
  case $ELCLIENT in 
    silajs)
      ejsCmd="npm run client:start -- --dataDir $DATADIR/silajs --gethGenesis $scriptDir/configs/$NETWORK.json --rpc --rpcEngine --rpcEngineAuth false $EL_PORT_ARGS"
      ;;
    geth)
