import { createBlock } from '@silajs/block'
import { createBlockchain } from '@silajs/blockchain'
import { Hardfork } from '@silajs/common'
import { MerkleStateManager } from '@silajs/statemanager'
import { createFeeMarket1559TxFromRLP } from '@silajs/tx'
import { Account, equalsBytes, hexToBytes, toBytes } from '@silajs/util'
import { assert, describe, it, vi } from 'vitest'

import { Config } from '../../src/config.ts'
import { FullSilaService } from '../../src/service/index.ts'
import { Event } from '../../src/types.ts'

import { MockChain } from './mocks/mockchain.ts'
import { MockServer } from './mocks/mockserver.ts'
import { destroy } from './util.ts'

const config = new Config({ accountCache: 10000, storageCache: 1000 })

// Stub out setStateRoot since correct state root doesn't exist in mock state.

MerkleStateManager.prototype.setStateRoot = (): any => {}

MerkleStateManager.prototype.shallowCopy = function () {
  return this
}
async function setup(): Promise<[MockServer, FullSilaService]> {
  const server = new MockServer({ config }) as any
  const blockchain = await createBlockchain({
    common: config.chainCommon,
    validateBlocks: false,
    validateConsensus: false,
  })
  const chain = new MockChain({ config, blockchain })
  const serviceConfig = new Config({ server })
  const service = new FullSilaService({
    config: serviceConfig,
    chain,
  })

  await service.open()
  await server.start()
  await service.start()
  service.txPool.start()
  return [server, service]
}

describe('should handle SIL requests', { timeout: 30000 }, async () => {
  const [server, service] = await setup()
  const peer = await server.accept('peer0')
  const hash = hexToBytes('0xa321d27cd2743617c1c1b0d7ecb607dd14febcdfca8f01b79c3f0249505ea069')
  const [reqId1, headers] = await peer.sil!.getBlockHeaders({ block: BigInt(1), max: 2 })
  it('handled getBlockHeaders', async () => {
    assert.strictEqual(reqId1, BigInt(1), 'handled GetBlockHeaders')
    assert.isTrue(equalsBytes(headers![1].hash(), hash), 'handled GetBlockHeaders')
  })
  const res = await peer.sil!.getBlockBodies({ hashes: [hash] })
  it('handled getBlockBodies', async () => {
    const [reqId2, bodies] = res
    assert.strictEqual(reqId2, BigInt(2), 'handled GetBlockBodies')
    assert.deepEqual(bodies, [[[], []]], 'handled GetBlockBodies')
  })
  service.config.events.on(Event.PROTOCOL_MESSAGE, async (msg) => {
    switch (msg.name) {
      case 'NewBlockHashes': {
        it('should handle newBlockHashes', () => {
          assert.isTrue(true, 'handled NewBlockHashes')
        })
        break
      }
      case 'NewBlock': {
        it('should handle NewBlock', () => {
          assert.isTrue(true, 'handled NewBlock')
        })
        await destroy(server, service)
        break
      }
    }
  })
  peer.sil!.send('NewBlockHashes', [[hash, BigInt(2)]])

  const block = createBlock(
    {
      header: {
        number: 1,
        difficulty: 1,
      },
    },
    { common: config.chainCommon },
  )
  peer.sil!.send('NewBlock', [block, BigInt(1)])

  const txData =
    ''
  const tx = createFeeMarket1559TxFromRLP(toBytes(txData))
  await service.execution.vm.stateManager.putAccount(
    tx.getSenderAddress(),
    new Account(BigInt(0), BigInt('40000000000100000')),
  )
  await service.txPool.add(tx)
  service.config.chainCommon.getHardforkBy = vi.fn().mockReturnValue(Hardfork.London)
  const [_, txs] = await peer.sil!.getPooledTransactions({ hashes: [tx.hash()] })
  it('should handle GetPooledTransactions', async () => {
    assert.isTrue(equalsBytes(txs[0].hash(), tx.hash()), 'handled GetPooledTransactions')
  })

  peer.sil!.send('Transactions', [tx])
})
