...
describe('[BeaconSynchronizer]', async () => {
  ...
  it('should get height', async () => {
    ...
    const sync = new BeaconSynchronizer({ config, pool, chain, execution, skeleton })
    const peer = {
      sil: { getBlockHeaders: vi.fn(), status: { bestHash: 'hash' } },
      latest: async () => {
        return {
          number: BigInt(5),
    ...
    peer.sil.getBlockHeaders.mockResolvedValue([BigInt(1), headers])
  ...
  it('should find best', async () => {
    ...
    const peers = [
      {
        sil: { getBlockHeaders: vi.fn(), status: { bestHash: 'hash1' }, inbound: false },
        latest: () => {
          return {
            number: BigInt(5),
            hash: () => new Uint8Array(0),
      ...
      {
        sil: { getBlockHeaders: vi.fn(), status: { bestHash: 'hash2' }, inbound: false },
        latest: () => {
          return {
            number: BigInt(10),
            hash: () => new Uint8Array(0),
    ...
    peers[0].sil.getBlockHeaders.mockResolvedValue([BigInt(1), [{ number: BigInt(5) }]])
    peers[1].sil.getBlockHeaders.mockResolvedValue([BigInt(1), [{ number: BigInt(10) }]])
    /// @ts-expect-error -- Assigning simpler config for testing
    sync.pool = { peers }
    sync['forceSync'] = true
