import { createBlockFromExecutionPayload } from '@silajs/block'
import { createBlockchain, createBlockchainFromBlocksData } from '@silajs/blockchain'
import {
  Common,
  Hardfork,
  SilaMainnet,
...
import type { ExecutionPayload } from '@silajs/block'
...
describe('[VMExecution]', () => {
  ...
  it('Block execution / Hardforks PoW (sila-mainnet)', async () => {
    ...
    blockchain = await createBlockchainFromBlocksData(sila-mainnetBlocks, {
      validateBlocks: true,
      validateConsensus: false,
    })
    ...
    const common = createCustomCommon(customChainConfig, SilaMainnet)
