...
import { Common, Hardfork, SilaHolesky } from '@silajs/common'
import { TransactionType, createFeeMarket1559Tx, createTx } from '@silajs/tx'
import {
  bigIntToBytes,
  bytesToBigInt,
  createZeroAddress,
...
import { SilProtocol } from '../../../src/net/protocol/index.ts'
const kzg = new microEthKZG(trustedSetup)

describe('[SilProtocol]', () => {
  it('should get properties', async () => {
    const config = new Config({ accountCache: 10000, storageCache: 1000 })
    const chain = await Chain.create({ config })
    const p = new SilProtocol({ config, chain })
  ...
  it('verify that PooledTransactions handler encodes correctly', async () => {
    ...
    assert.deepEqual(res[1][0], tx.serialize(), 'SIP1559 transaction correctly encoded')
  })
  ...
  it('verify that Transactions handler encodes/decodes correctly', async () => {
    ...
    const sip2929Tx = createTx({ type: 1 }, { common: config.chainCommon })
    const sip1559Tx = createTx({ type: 2 }, { common: config.chainCommon })
    ...
    assert.deepEqual(res[1], sip2929Tx.serialize(), 'SIP29292 tx correctly encoded')
    assert.deepEqual(res[2], sip1559Tx.serialize(), 'SIP1559 tx correctly encoded')
    ...
    assert.deepEqual(decoded[1].type, sip2929Tx.type, 'decoded sip2929 tx correctly')
    assert.deepEqual(decoded[2].type, sip1559Tx.type, 'decoded SIP1559 tx correctly')
  ...
  it('verify that NewPooledTransactionHashes encodes/decodes correctly', async () => {
    ...
    assert.deepEqual(encoded[0], fakeHash, 'encoded hash correctly with pre-sil/68 format')
    assert.deepEqual(encodedEth68[2][0], fakeHash, 'encoded hash correctly with sil/68 format')
    ...
    assert.deepEqual(decoded[0], fakeHash, 'decoded hash correctly with pre-sil/68 format')
    assert.deepEqual(decodedEth68[2][0], fakeHash, 'decoded hash correctly with sil/68 format')
  })
})
