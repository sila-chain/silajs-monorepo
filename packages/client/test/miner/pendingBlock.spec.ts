import { BlockHeader, createBlockHeader } from '@silajs/block'
...
import { SIGNER_A, SIGNER_B, sip4844GethGenesis, goerliChainConfig } from '@silajs/testdata'
import {
  NetworkWrapperType,
  createBlob4844Tx,
...
import type { Blockchain } from '@silajs/blockchain'
...
describe('[PendingBlock]', async () => {
  ...
  const txA011 = createTx() // SIGNER_A -> SIGNER_B, nonce: 0, value: 1, normal gasPrice
  ...
  const txB011 = createTx(SIGNER_B, SIGNER_A, 0, 1, 2500000000) // SIGNER_B -> SIGNER_A, nonce: 0, value: 1, 2.5x gasPrice

  it('should start and build', async () => {
    const { txPool } = setup()
    const blockchain = await createBlockchain({ common })
    const vm = await createVM({ common, blockchain })
    await setBalance(vm, SIGNER_A.address, BigInt(5000000000000000))
    await setBalance(vm, SIGNER_B.address, BigInt(5000000000000000))
  ...
  it('should include txs with mismatching hardforks that can still be executed', async () => {
    ...
    txA011.common.setHardfork(Hardfork.SilaParis)
    ...
    txB011.common.setHardfork(Hardfork.SilaParis)
  ...
  it('should stop adding txs when block is full', async () => {
    ...
    const txA012 = createTx(SIGNER_A, SIGNER_B, 0, 1, 1000000000, 25000)
    const txA022 = createTx(SIGNER_A, SIGNER_B, 1, 1, 2000000000, 25000) // SIGNER_A -> SIGNER_B, nonce: 1, value: 1, 2x gasPrice
  ...
  it('construct blob bundles', async () => {
    ...
    const { txPool } = setup()
    txPool['config'].chainCommon.setHardfork(Hardfork.SilaCancun)
    ...
    for (let x = 0; x <= 2; x++) {
      ...
      const txA01 = createBlob4844Tx(
        {
          networkWrapperVersion: NetworkWrapperType.SIP4844,
    ...
    const blockchain = await createBlockchain({ common })
    const vm = await createVM({ common, blockchain })
    await setBalance(vm, SIGNER_A.address, BigInt(500000000000000000))
    const parentBlock = await (vm.blockchain as Blockchain).getCanonicalHeadBlock!()
