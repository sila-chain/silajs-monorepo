...
import type { Common, GenesisState } from '@silajs/common'
import type { Config } from '../src/config.ts'
import type { SilaClient } from '../src/index.ts'
  ...
  replServer.defineCommand('logLevel', {
    ...
    action(params) {
      ...
      if (['debug', 'info', 'warn', 'error'].includes(level)) {
        const logger = (replServer.context.client as SilaClient).config.logger
        if (logger === undefined) {
          this.displayPrompt()
          return
        }
        for (const transport of (replServer.context.client as SilaClient).config.logger!
          .transports) {
