...
import { createBlockFromBytesArray } from '@silajs/block'
import { CliqueConsensus, createBlockchain } from '@silajs/blockchain'
import { ConsensusAlgorithm, Hardfork } from '@silajs/common'
import { RLP } from '@silajs/rlp'
import { SilaJSErrorWithoutCode, bytesToHex, short } from '@silajs/util'
...
import type { Block, BlockBytes } from '@silajs/block'
...
import type { FullSilaService } from '../src/service/index.ts'
 ...
 */
async function executeBlocks(client: SilaClient) {
  let first = 0
  let last = 0
  let txHashes = []
  try {
    ...
    if ((blockRange[0][1] as string[]).length > 0 && blockRange.length === 2) {
      throw SilaJSErrorWithoutCode('wrong input')
    }
  } catch {
  ...
  if (execution === undefined) throw SilaJSErrorWithoutCode('executeBlocks requires execution')
 ...
 */
async function startBlock(client: SilaClient) {
  ...
  if (height < startBlock) {
    throw SilaJSErrorWithoutCode(`Cannot start chain higher than current height ${height}`)
  }
  try {
    ...
    throw SilaJSErrorWithoutCode(`Error setting back chain in startBlock: ${err}`)
  }
}

/**
 * Starts and returns the {@link SilaClient}
 */
async function startClient(
  ...
  const client = await SilaClient.create({
    config,
    blockchain,
    ...genesisMeta,
    ...dbs,
  ...
  if (client.config.synchronized === true) {
    ...
    // The service might not be FullSilaService even if we cast it as one,
    // so txPool might not exist on it
    ;(fullService as FullSilaService).txPool?.checkRunState()
  }
    ...
    .then((client) => {
      const servers: (RPCServer | http.Server)[] =
        args.rpc === true || args.rpcEngine === true || args.ws === true
          ...
          : []
      if (
        client.config.chainCommon.gteHardfork(Hardfork.SilaParis) &&
