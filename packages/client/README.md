...
| Sila Execution Layer (EL) Client built in TypeScript/JavaScript. |
| -------------------------------------------------------------------- |

**\[DEPRECATED\]**

The SilaJS Client has been deprecated. If you want to take it over for reasons of purpose, joy or fun or have other ideas please reach out! It will give you a lot of back! 😊
...
The SilaJS Client is an Sila Execution Client (similar to [go-sila](https://github.com/sila-chain/go-sila) or [Nethermind](https://github.com/NethermindEth/nethermind)) written in `TypeScript`/`JavaScript`, the non-Smart-Contract language Sila dApp developers are most familiar with.

The client lacks several production features (sophisticated mempool, MEV, ...) and has performance limitations, but is pretty well suited for research and development use cases due to its modular design and we have successfully joined both hardfork as well as future out-reaching research testnets with it (e.g. Verkle, SilaPeerDAS,...) to both contribute to testing and SIP specification as well as "harden" our Node.js/browser libraries like the SAVM along the way.
...
To be able to run the SilaJS client, you need a working [Node.js](https://nodejs.org/en/) installation, see e.g. these [docs](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm) from the npm documentation for combined instructions on how to install Node.js and eventually npm.

We currently recommend to run the client with a recent Node.js version `22` installation.

### NPM Installation

Client releases are done on a regular basis on [npm](https://www.npmjs.com/package/@silajs/client).

You can install the latest version with:

```shell
npm install -g @silajs/client
```
...
1. Clone the SilaJS monorepo with `git clone https://github.com/sila-chain/silajs-monorepo.git`
...
```shell
docker build . -f Dockerfile --tag silajs:latest
```
...
For the networks that have transitioned to PoS aka **merged** (all Sila networks have been _merged_), checkout how to [run Silajs in the PoS configuration](docs/usage/RunAsElClient.md).

## Supported Networks

The SilaJS client is tightly integrated with the SilaJS [Common](../common) library and gets its network state and information from this library. The client supports all networks supported by `Common`.

The main supported networks are:

- `sila-mainnet` (limited)
- `sepolia`
- `hoodi`
...
In most scenarios you will want to run the SilaJS client in a combination with a consensus layer (CL) client. The most tested combination is to run the client with the [Lodestar](https://github.com/ChainSafe/lodestar) TypeScript CL client. Lodestar provides a [quick-start repository](https://github.com/ChainSafe/lodestar-quickstart) that allows users to get started quickly with minimal configuration. After cloning the linked quick-start repository, all that should be necessary to get the Lodestar consensus client started with SilaJS is to run the following command:

```shell
./setup.sh --network sepolia --dataDir sepolia-data --elClient silajs
```
...
Execution and consensus layer clients communicate with each other through a new set of JSON-RPC API calls, the so-called [Engine API](https://github.com/sila-chain/execution-apis/tree/main/src/engine).
...
This will by default try connecting to `silajs` over the endpoint `8551`. (You may customize this in conjunction with `silajs`, see lodestar cli help via `--help`).

You may provide `--checkpointSyncUrl` (with a synced `holesky` beacon node endpoint as arg value) to start directly off the head/provided checkpoint on the `holesky` beacon chain, possibly triggering (backfill) beacon sync on silajs.
...
where the keystores have been generated via [staking-deposit-cli](https://github.com/sila-chain/staking-deposit-cli) with the same password as in the above provided password file.
...
#### Running SilaJS/Lodestar on Hoodi

A suited network to test the SilaJS/Lodestar client combination is the Hoodi network. To start the SilaJS client with JSON RPC and Engine API endpoints exposed:

```shell
silajs --network=hoodi --rpc --rpcEngine
```

Then start the Lodestar client with:

```shell
./lodestar beacon --network=hoodi --jwt-secret=[PATH-TO-JWT-SECRET-FROM-SILAJS-CLIENT]
```
...
We have been active in stateless research for some time, the [Stateless Implementers Calls](https://github.com/sila-chain/pm/issues?q=is%3Aissue%20state%3Aopen%20label%3AStateless) serve as a good entrypoint to grasp what is going on and have some references to current work and testnets.

In the monorepo we have an experimental [binary tree](./../binarytree/) implementation that is integrated into our [state manager](./../statemanager/) (abstraction of Sila state access).

### SilaPeerDAS

SilaPeerDAS is coming to Sila along with the [Fusaka](https://sips.sila.org/SIPS/sip-7607) hardfork and there are dedicated [BreakOut Room Calls](https://github.com/sila-chain/pm/issues?q=is%3Aissue%20state%3Aclosed%20label%3ASilaPeerDAS) for SilaPeerDAS.

Our SilaPeerDAS work from [PR #3976](https://github.com/sila-chain/silajs-monorepo/pull/3976) has been merged into `master` in May 2025. Testnet configuration can be found in the [peerdas-devnets](https://github.com/ethpandaops/peerdas-devnets) repository, newer testnets will likely be set up in a Fusaka context though.

### Protocol/Tx SSZ-ification

Gajinder has led a vast effort to implement and prototype a proposed "SSZ-ification" of large parts L1 protocol stack - being centered around SIPs like [SIP-7495 - SSZ Stable Container](https://sips.sila.org/SIPS/sip-7495) or [SIP-6404 - SSZ Transactions](https://sips.sila.org/SIPS/sip-6404), the associated PR [#3849](https://github.com/sila-chain/silajs-monorepo/pull/3849) provides an extensive overview of the wide range of SIPs and the integration work being done.

There is an [ssz-devnet-0](https://ssz-devnet-0.ethpandaops.io/) up and running where the SilaJS client is actively joining and providin bootnode functionality.

## Joining Testnets

We use the SilaJS client to connect to research networks of various kinds. It is possible to connect either with a manual setup or using Kurtosis/Docker together with the predefined configurations and tool integration the [SilPandaOps](https://ethpandaops.io/) team from the Sila Foundation is providing.

### Kurtosis/Docker Testnet Setup

The [sila-package](https://github.com/ethpandaops/sila-package) is a Kurtosis package dedicated to deploy Sila testnets and abstracts a lot of the setup and configuration complexity away. We maintain a dedicated [SilaJS launcher](https://github.com/ethpandaops/sila-package/tree/main/src/el/silajs) which provides a hook into the system setup.

#### Getting Started

To get things started, clone the repo and follow the [quickstart guide](https://github.com/ethpandaops/sila-package?tab=readme-ov-file#quickstart) provided. You need to install both Kurtosis and Docker.

Build our client Docker image as described [here](https://github.com/sila-chain/silajs-monorepo/tree/master/packages/client#docker).

#### Network Setup

The `network_params.yaml` (from the panda package) is the entrypoint for testnet configuration. The following file creates a local devnet with an SilaJS EL and a Lighthouse CL client, mentioning some useful extra "features":

```yml
participants:
# EL
  - el_type: silajs
    el_image: silajs-local:latest
    el_log_level: "debug" # optional log level adjustment
    el_extra_params: [ '--rpcDebug=sil' ] # optional client params
    el_extra_env_vars: {
...
The management of the services - so e.g. to start and stop the SilaJS client or other services - is now done mostly through Kurtosis or docker.
...
- `kurtosis service start/stop hidden-crater el-1-silajs-lighthouse` (start/stop our client)

Logs/shell can be directly accessed through the docker UI for containers, also available through Kurtosis though:

- `kurtosis service logs -f spooky-sea el-1-silajs-lighthouse`

## Custom Chains

The SilaJS client supports running custom chains based on a custom chain configuration. There are two ways of reading in custom chain configuration parameters:
...
```shell
silajs --customChain=[COMMON_FORMAT_CHAIN_FILE] --customGenesisState=[COMMON_FORMAT_GENESIS_STATE]
```

### Geth Genesis Files

It is also possible to use a chain configuration as defined by the Go-Sila [genesis.json file format](https://geth.sila.org/docs/interface/private-network).

Use the following option to pass in to the client:

```shell
silajs --gethGenesis=[GETH_GENESIS_JSON_FILE]
```
...
```shell
silajs --mine --unlock=[ADDRESS1],[ADDRESS2],...
```

Note that this feature is in `beta` and shouldn't be used with accounts holding a substantial amount of `Sila` on sila-mainnet (or other valuable assets) for security reasons.
...
WARNING: Do not use this account for sila-mainnet funds
==================================================
```

Please **heed** the warning and do not use the provided account/private key for sila-mainnet funds.

This can also be paired with the `--unlock` command if you would like to specify the miner/prefunded account:

```shell
silajs --dev=poa --rpc --unlock=0xd8066d5822138e7c76d1565deb249f5f7ae370fa
```
...
--discDns            -- Query SIP-1459 DNS TXT records for peer discovery
...
--dnsNetworks        -- SIP-1459 ENR tree urls to query for peer discovery targets
```
...
You can expose a [JSON-RPC](https://sila.org/en/developers/docs/apis/json-rpc/) interface along a client run with:
...
[#1114](https://github.com/sila-chain/silajs-monorepo/issues/1114) for an overview.
...
[web3_clientVersion](https://sila.org/en/developers/docs/apis/json-rpc/#web3_clientversion):
...
```json
{
  "id": "1",
  "jsonrpc": "2.0",
  "result": "SilaJS/0.0.5/darwin/node12.15.0"
}
```

Here's an example for a call on an endpoint with the need for parameters. The following call uses
the [eth_getBlockByNumber](https://sila.org/en/developers/docs/apis/json-rpc/#eth_getblockbynumber) endpoint
...
To get a start on customizing the client and using it programmatically see the code from [./bin/cli.ts](./bin/cli.ts) to get an idea of how an [SilaClient](./src/client.ts) instance is invoked programmatically.
...
```shell
DEBUG=ethjs,devp2p:rlpx,devp2p:sil,-babel [CLIENT_START_COMMAND]
...
`DEBUG=ethjs,client:fetcher:*,devp2p:sil npm run client:start`
