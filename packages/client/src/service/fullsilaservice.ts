...
import { concatBytes, hexToBytes } from '@silajs/util'
import { encodeReceipt } from '@silajs/vm'

import { SyncMode } from '../config.ts'
import { VMExecution } from '../execution/index.ts'
import { Miner } from '../miner/index.ts'
import { SilProtocol } from '../net/protocol/ethprotocol.ts'
...
import type { Blob4844Tx } from '@silajs/tx'
 ...
 */
export class FullSilaService extends Service {
  ...
  constructor(options: ServiceOptions) {
    ...
    if (this.config.syncmode === SyncMode.Full) {
      if (this.config.chainCommon.gteHardfork(Hardfork.SilaParis)) {
  ...
  async open() {
    if (this.synchronizer !== undefined) {
      this.config.logger?.info(
        `Preparing for sync using FullSilaService with ${
      ...
      this.config.logger?.info('Starting FullSilaService with no syncing.')
    }
  ...
  /**
   * Handles incoming SIL message from connected peer
   * @param message message object
   * @param peer peer
   */
  async handleEth(message: any, peer: Peer): Promise<void> {
    switch (message.name) {
      case 'GetBlockHeaders': {
        ...
        if (typeof block === 'bigint') {
          if (
            ...
            peer.sil!.send('BlockHeaders', { reqId, headers: [] })
            return
          }
        }
      ...
      case 'GetBlockBodies': {
        ...
        peer.sil!.send('BlockBodies', { reqId, bodies })
        break
      }
      case 'NewBlockHashes': {
        if (this.config.chainCommon.gteHardfork(Hardfork.SilaParis)) {
          this.config.logger?.debug(
            `Dropping peer ${peer.id} for sending NewBlockHashes after merge (SIP-3675)`,
          )
      ...
      case 'NewBlock': {
        if (this.config.chainCommon.gteHardfork(Hardfork.SilaParis)) {
          this.config.logger?.debug(
            `Dropping peer ${peer.id} for sending NewBlock after merge (SIP-3675)`,
          )
          this.pool.ban(peer, 9000000)
        } else if (this.synchronizer instanceof FullSynchronizer) {
      ...
      case 'NewPooledTransactionHashes': {
        let hashes = []
        if (peer.sil!['versions'].includes(68)) {
          // sil/68 message data format - [tx_types: number[], tx_sizes: number[], tx_hashes: uint8array[]]
          // With sil/68, we can check transaction types and transaction sizes to
          ...
          // transactions - though this is not prescribed in sil/68 (SIP 5793)
          // https://sips.sila.org/SIPS/sip-5793
      ...
      case 'GetPooledTransactions': {
        ...
        peer.sil?.send('PooledTransactions', { reqId, txs })
        break
      }
      case 'GetReceipts': {
        const [reqId, hashes] = message.data
        ...
        peer.sil?.send('Receipts', { reqId, receipts })
        break
      }
    }
  }
