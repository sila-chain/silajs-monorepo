...
import type { FullSilaService } from './fullsilaservice.ts'
...
export class TxPool {
  ...
  private async validate(tx: TypedTransaction, isLocalTransaction: boolean = false) {
    if (!tx.isSigned()) {
      throw SilaJSErrorWithoutCode('Attempting to add tx to txpool which is not signed')
    }
    ...
    if (!isLocalTransaction) {
      ...
      if (txsInPool >= MAX_POOL_SIZE) {
        throw SilaJSErrorWithoutCode('Cannot add tx: pool is full')
      }
    ...
    if (tx.common.isActivatedEIP(7825)) {
      ...
      if (tx.gasLimit > maxGasLimit) {
        throw SilaJSErrorWithoutCode(
          `Transaction gas limit ${tx.gasLimit} exceeds the maximum allowed by SIP-7825 (${maxGasLimit})`,
        )
      }
    }
  ...
  sendNewTxHashes(txs: [number[], number[], Uint8Array[]], peers: Peer[]) {
    ...
    for (const peer of peers) {
      ...
      if (hashesToSend.length > 0) {
        if (
          peer.sil !== undefined &&
          peer.sil['versions'] !== undefined &&
          peer.sil['versions'].includes(68)
        ) {
          // If peer supports sil/68, send sil/68 formatted message (tx_types[], tx_sizes[], hashes[])
          ...
          try {
            peer.sil?.send('NewPooledTransactionHashes', txsToSend.slice(0, 4096))
          } catch (e) {
            this.markFailedSends(peer, hashesToSend, e as Error)
          }
        }
        // If peer doesn't support sil/68, just send tx hashes
        else
          try {
            ...
            peer.sil?.send('NewPooledTransactionHashes', hashesToSend.slice(0, 4096))
          } catch (e) {
            this.markFailedSends(peer, hashesToSend, e as Error)
          }
      }
  ...
  sendTransactions(txs: TypedTransaction[], peers: Peer[]) {
    if (txs.length > 0) {
      ...
      for (const peer of peers) {
        ...
        peer.sil?.request('Transactions', newTxs).catch((e) => {
  ...
  async handleAnnouncedTxHashes(txHashes: Uint8Array[], peer: Peer, peerPool: PeerPool) {
    ...
    const getPooledTxs = await peer.sil?.getPooledTransactions({
  ...
  private normalizedGasPrice(tx: TypedTransaction, baseFee?: bigint) {
    const supports1559 = tx.supports(Capability.SIP1559FeeMarket)
  ...
  private txGasPrice(tx: TypedTransaction): GasPrice {
    ...
    if (isFeeMarket1559Tx(tx) || isBlob4844Tx(tx)) {
      ...
      throw SilaJSErrorWithoutCode(`tx of type ${(tx as TypedTransaction).type} unknown`)
    }
  }
  ...
  async txsByPriceAndNonce(
    ...
    if (vm.common.isActivatedEIP(7594)) {
      ...
      for (const [_address, poolObjects] of this.pool) {
        for (const txObj of poolObjects) {
          const tx = txObj.tx
          if (isBlob4844Tx(tx) && tx.networkWrapperVersion === NetworkWrapperType.SIP4844) {
    ...
    while (byPrice.length > 0) {
      ...
      // Skip the best tx if this is a future 7594 blob tx
        ...
        best instanceof Blob4844Tx &&
        best.networkWrapperVersion === NetworkWrapperType.SIP7594 &&
