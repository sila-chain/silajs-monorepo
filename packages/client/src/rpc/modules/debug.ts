...
import { type VM, encodeReceipt, runTx } from '@silajs/vm'
...
export class Debug {
  ...
  constructor(client: SilaClient, rpcDebug: boolean) {
    ...
    this.vm = (this.service as FullSilaService).execution?.vm
  ...
  async traceTransaction(params: [PrefixedHexString, tracerOpts]) {
    ...
    if (!this.service.execution.txIndex) throw SilaJSErrorWithoutCode('missing txIndex')
    ...
    vmCopy.savm.events?.on('step', async (step, next) => {
      const memory = []
      let storage = {}
    ...
    vmCopy.savm.events?.on('afterMessage', (data, next) => {
  ...
  async traceCall(params: [RPCTx, string, tracerOpts]) {
    ...
    vm.savm.events?.on('step', async (step, next) => {
      const memory = []
      let storage = {}
      if (opts.disableStorage === false) {
    ...
    vm.savm.events?.on('afterMessage', (data, next) => {
      if (data.execResult.exceptionError !== undefined && trace.structLogs.length > 0) {
        // Mark last opcode trace as error if exception occurs
        trace.structLogs[trace.structLogs.length - 1].error = true
      }
    ...
    const res = await vm.savm.runCall(runCallOpts)
  ...
  /**
   * Returns an array of SIP-2718 binary-encoded receipts
   * @param blockOpt Block number or tag
   */
  async getRawReceipts(params: [string]) {
    const [blockOpt] = params
    if (!this.service.execution.receiptsManager)
      throw SilaJSErrorWithoutCode('missing receiptsManager')
    const block = await getBlockByOption(blockOpt, this.chain)
