...
export class CLConnectionManager {
  ...
  /** Track silajs client shutdown status */
  ...
  private connectionCheck() {
    if (this.connectionStatus === ConnectionStatus.Connected) {
      ...
      if (
        this.config.chainCommon.gteHardfork(Hardfork.SilaParis) &&
    ...
    if (
      ...
      this.config.chainCommon.hardfork() === Hardfork.SilaParis
    ) {
      if (this.connectionStatus === ConnectionStatus.Disconnected) {
        logCLStatus(
          this.config.logger,
          'SilaParis (Merge) HF activated, CL client connection is needed for continued block processing',
          logLevel.INFO,
        )
        logCLStatus(
          this.config.logger,
