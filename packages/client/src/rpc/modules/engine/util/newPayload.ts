import { createBlockFromExecutionPayload, genRequestsRoot } from '@silajs/block'
...
export const validateAndGen7685RequestsHash = (
  ...
  for (const request of executionRequests) {
    ...
    if (bytes.length === 0) {
      throw SilaJSErrorWithoutCode('Got a request without a request-identifier')
    }
    switch (bytes[0]) {
      case CLRequestType.Deposit:
        if (!common.isActivatedEIP(6110)) {
          throw SilaJSErrorWithoutCode(`Deposit requests are not active`)
        }
        requests.push(new CLRequest(CLRequestType.Deposit, bytes.slice(1)))
        break
      case CLRequestType.Withdrawal:
        if (!common.isActivatedEIP(7002)) {
          throw SilaJSErrorWithoutCode(`Withdrawal requests are not active`)
        }
        requests.push(new CLRequest(CLRequestType.Withdrawal, bytes.slice(1)))
        break
      case CLRequestType.Consolidation:
        if (!common.isActivatedEIP(7251)) {
          throw SilaJSErrorWithoutCode(`Consolidation requests are not active`)
        }
        requests.push(new CLRequest(CLRequestType.Consolidation, bytes.slice(1)))
        break
      default:
        throw SilaJSErrorWithoutCode(`Unknown request identifier: got ${bytes[0]}`)
    }
  }
...
export const assembleBlock = async (
  ...
  try {
    ...
    if (executionRequests !== undefined) {
      ...
      throw `Invalid executionRequests=undefined for SIP-7685 activated block`
    }
    ...
    /**
     * Validate blob versioned hashes in the context of SIP-4844 blob transactions
     */
    if (block.common.isActivatedEIP(4844)) {
      ...
      const validationError = `Invalid blobVersionedHashes before SIP-4844 is activated`
      throw validationError
    }
