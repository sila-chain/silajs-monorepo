...
import { MerkleStateManager, getMerkleStateProof } from '@silajs/statemanager'
import {
  Capability,
  NetworkWrapperType,
...
import type { Block, JSONRPCBlock } from '@silajs/block'
import type { Log } from '@silajs/savm'
import type { Proof } from '@silajs/statemanager'
import type { FeeMarket1559Tx, LegacyTx, TypedTransaction } from '@silajs/tx'
...
import type { SilProtocol } from '../../net/protocol/index.ts'
import type { FullSilaService, Service } from '../../service/index.ts'
...
type GetLogsParams = {
  ...
  blockHash?: PrefixedHexString // DATA, 32 Bytes. With the addition of SIP-234,
...
const calculateRewards = async (
  ...
  const txsWithGasUsed = txs.map((tx, i) => ({
    ...
    // Can assume baseFee exists, since if SIP1559/SIP4844 txs are included, this is a post-SIP-1559 block.
...
export class Sil {
  ...
  constructor(client: SilaClient, rpcDebug: boolean) {
    ...
    this._vm = (this.service as FullSilaService).execution?.vm
    this.receiptsManager = (this.service as FullSilaService).execution?.receiptsManager
    this.txIndex = (this.service as FullSilaService).execution?.txIndex
    this._rpcDebug = rpcDebug

    const ethProtocol = this.service.protocols.find((p) => p.name === 'sil') as SilProtocol
  ...
  async call(params: [RPCTx, string]) {
    ...
    if (this._vm === undefined) {
      throw SilaJSErrorWithoutCode('missing vm')
    }
    ...
    const { execResult } = await vm.savm.runCall(runCallOpts)
  ...
  /**
   * Returns the currently configured chain id, a value used in replay-protected transaction signing as introduced by SIP-155.
  ...
  async maxPriorityFeePerGas() {
    ...
    for (const block of blocks) {
      ...
      for (const tx of block.transactions) {
        // Only SIP-1559 transactions have maxPriorityFeePerGas
        if (tx.supports(Capability.SIP1559FeeMarket) === true) {
   ...
   * for a variety of reasons including SAVM mechanics and node performance.
   * @param params An array of two parameters:
   *   1. The transaction object
   *       * from (optional) - The address the transaction is sent from
   *       * to - The address the transaction is directed to
  ...
  async getTransactionByHash(params: [PrefixedHexString]) {
    const [txHash] = params
    if (!this.receiptsManager) throw SilaJSErrorWithoutCode('missing receiptsManager')
    if (!this.txIndex) throw SilaJSErrorWithoutCode('missing txIndex')
  ...
  async getTransactionCount(params: [string, string]) {
    ...
    if (blockOpt === 'pending') {
      pendingTxsCount = BigInt(
        (this.service as FullSilaService).txPool.pool.get(addressHex.slice(2))?.length ?? 0,
      )
    }
  ...
  /**
   * Returns the current sila protocol version as a hex-encoded string
   */
  protocolVersion() {
  ...
  async getBlockReceipts(params: [string]) {
    ...
    const receipts = await Promise.all(
      result.map(async (r, i) => {
        ...
        const { blobGasPrice, blobGasUsed } = runBlockResult.receipts[i] as SIP4844BlobTxReceipt
        const effectiveGasPrice =
          tx.supports(Capability.SIP1559FeeMarket) === true
  ...
  async getTransactionReceipt(params: [PrefixedHexString]) {
    ...
    const parentBlock = await this._chain.getBlock(block.header.parentHash)
    const tx = block.transactions[txIndex]
    const effectiveGasPrice = tx.supports(Capability.SIP1559FeeMarket)
      ? (tx as FeeMarket1559Tx).maxPriorityFeePerGas <
        (tx as FeeMarket1559Tx).maxFeePerGas - block.header.baseFeePerGas!
        ? (tx as FeeMarket1559Tx).maxPriorityFeePerGas
        : (tx as FeeMarket1559Tx).maxFeePerGas -
    ...
    const { blobGasPrice, blobGasUsed } = runBlockResult.receipts[txIndex] as SIP4844BlobTxReceipt
    return toJSONRPCReceipt(
      receipt,
      totalGasSpent,
      effectiveGasPrice,
  ...
  async sendRawTransaction(params: [PrefixedHexString]) {
    ...
    try {
      ...
      if (txBuf[0] === 0x03) {
        ...
        if (
          ...
          tx.networkWrapperVersion !== NetworkWrapperType.SIP7594
        ) {
          throw Error(
            `tx with networkWrapperVersion=${tx.networkWrapperVersion} sent for SIP-7594 activated hardfork=${common.hardfork()}`,
          )
        }
    ...
    const { txPool, pool } = this.service as FullSilaService
  ...
  async gasPrice() {
    ...
    if (this._vm !== undefined && this._vm.common.isActivatedEIP(1559)) {
      ...
      // For chains that don't support SIP-1559 we iterate over the last 20
