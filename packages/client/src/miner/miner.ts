import { type BlockHeader, createSealedCliqueBlockHeader } from '@silajs/block'
import { ConsensusType, Hardfork } from '@silajs/common'
import { Ethash } from '@silajs/ethash'
import { BIGINT_0, BIGINT_1, BIGINT_2, bytesToHex, equalsBytes } from '@silajs/util'
import { type TxReceipt, buildBlock } from '@silajs/vm'
import { MemoryLevel } from 'memory-level'

import { LevelDB } from '../execution/level.ts'
import { Event } from '../types.ts'

import type { Blockchain, CliqueConsensus } from '@silajs/blockchain'
import type { CliqueConfig } from '@silajs/common'
import type { Miner as EthashMiner, Solution } from '@silajs/ethash'
...
/**
 * Implements Sila block creation and mining.
...
export class Miner {
  ...
  private async queueNextAssembly(timeout?: number) {
    ...
    if (this.config.chainCommon.hardforkGteHardfork(nextBlockHf, Hardfork.SilaParis)) {
    ...
    if (this.config.chainCommon.consensusType() === ConsensusType.ProofOfAuthority) {
      // SIP-225 spec: If the signer is out-of-turn,
  ...
  async assembleBlock() {
    ...
    if (
      ...
      // Set initial SIP1559 block gas limit to 2x parent gas limit per logic in `block.validateGasLimit`
