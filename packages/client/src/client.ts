...
import { FullSilaService } from './service/index.ts'
...
export interface SilaClientOptions {
...
export class SilaClient {
  ...
  public chain: Chain
  public service: FullSilaService
   ...
   */
  public static async create(options: SilaClientOptions) {
   ...
   * Create new node
   */
  protected constructor(chain: Chain, options: SilaClientOptions) {
    this.config = options.config
    this.chain = chain
    this.service = new FullSilaService({
      config: this.config,
  ...
  async open() {
    ...
    this.config.logger?.info(
      `Initializing Silajs client version=v${packageJSON.version} network=${name} chainId=${chainId}`,
    )
