import { URL } from 'url'
import { SilaJSErrorWithoutCode, hexToBytes } from '@silajs/util'
...
export function parseMultiaddrs(input: MultiaddrLike): Multiaddr[] {
  ...
  try {
    return input.map((s) => {
      ...
      throw SilaJSErrorWithoutCode(`Unable to parse bootnode URL: ${s}`)
    })
  } catch (e: any) {
    throw SilaJSErrorWithoutCode(`Invalid bootnode URLs: ${e.message}`)
  }
}
