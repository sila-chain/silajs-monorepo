...
import { BIGINT_0, SilaJSErrorWithoutCode, equalsBytes } from '@silajs/util'

import { LevelDB } from '../execution/level.ts'
import { Event } from '../types.ts'

import type { Block, BlockHeader } from '@silajs/block'
import type { Blockchain, ConsensusDict } from '@silajs/blockchain'
import type { GenesisState } from '@silajs/common'
import type { DB, DBObject } from '@silajs/util'
...
export class Chain {
  ...
  async getBlocks(
    ...
    if (!this.opened) throw SilaJSErrorWithoutCode('Chain closed')
  ...
  async putBlocks(blocks: Block[], fromEngine = false, skipUpdateEmit = false): Promise<number> {
    ...
    for (const [i, b] of newBlocks.entries()) {
      if (!fromEngine && this.config.chainCommon.gteHardfork(Hardfork.SilaParis)) {
        if (i > 0) {
  ...
  async putHeaders(headers: BlockHeader[], mergeIncludes = false): Promise<number> {
    ...
    for (const [i, h] of headers.entries()) {
      if (!mergeIncludes && this.config.chainCommon.gteHardfork(Hardfork.SilaParis)) {
        if (i > 0) {
          // emitOnLast below won't be reached, so run an update here
