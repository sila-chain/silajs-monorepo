import { BIGINT_0, SilaJSErrorWithoutCode, short } from '@silajs/util'
...
export abstract class Peer extends EventEmitter {
  ...
  */
  ...
  async latest(): Promise<BlockHeader | undefined> {
    ...
    if (!this.sil!.updatedBestHeader) {
      // If there is no updated best header stored yet, start with the status hash
      block = this.sil!.status.bestHash
    } else {
      block = this.getPotentialBestHeaderNum()
    }
    const result = await this.sil!.getBlockHeaders({
      block,
      max: 1,
    })
    if (result !== undefined) {
      const latest = result[1][0]
      this.sil!.updatedBestHeader = latest
      if (latest !== undefined) {
    ...
    return this.sil!.updatedBestHeader
  }
  ...
  getPotentialBestHeaderNum(): bigint {
    ...
    if (this.sil?.updatedBestHeader !== undefined) {
      const bestHeaderNum = this.sil!.updatedBestHeader.number
      const nowSec = Math.floor(Date.now() / 1000)
      const diffSec = nowSec - Number(this.sil!.updatedBestHeader.timestamp)
      const SLOT_TIME = 12
  ...
  async addProtocol(sender: Sender, protocol: Protocol): Promise<void> {
    ...
    if (protocol.name === 'sil') {
      ...
      throw SilaJSErrorWithoutCode(`addProtocol: ${protocol.name} protocol not supported`)
    }
