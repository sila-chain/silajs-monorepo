import { SIL as Devp2pETH, RLPx as Devp2pRLPx, SNAP as Devp2pSNAP } from '@silajs/devp2p'
import { randomBytes, unprefixedHexToBytes } from '@silajs/util'
...
import type { Capabilities as Devp2pCapabilities, Peer as Devp2pRlpxPeer } from '@silajs/devp2p'
 ...
 * import { SilProtocol } from './src/net/protocol'
 *
 * const chain = await Chain.create()
 * const protocols = [ new SilProtocol({ chain })]
  ...
  private async bindProtocols(rlpxPeer: Devp2pRlpxPeer): Promise<void> {
    ...
    await Promise.all(
      rlpxPeer.getProtocols().map((rlpxProtocol) => {
        ...
        // Since snap is running atop/besides sil, it doesn't need a separate sender
        // handshake, and can just use the sil handshake
