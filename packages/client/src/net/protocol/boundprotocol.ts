import { SilaJSErrorWithoutCode, Lock } from '@silajs/util'

import { Event } from '../../types.ts'

import type { BlockBodyBytes, BlockHeader } from '@silajs/block'
import type { TypedTransaction } from '@silajs/tx'
import type { TxReceipt } from '@silajs/vm'
import type { Config } from '../../config.ts'
import type { Peer } from '../peer/peer.ts'
import type { SilProtocolMethods } from './ethprotocol.ts'
...
export class BoundProtocol {
      ...
      try {
        if (this.peer.pooled) {
          ...
          if (this.messageQueue.length >= 50) {
            const error = SilaJSErrorWithoutCode('unexpected message queue growth for peer')
  ...
  private handle(incoming: Message) {
    ...
    try {
      ...
      error = SilaJSErrorWithoutCode(`Could not decode message ${message.name}: ${e}`)
    }
  ...
  send(name: string, args?: any) {
    ...
    if (message) {
      ...
      throw SilaJSErrorWithoutCode(`Unknown message: ${name}`)
    }
    return message
  }
  ...
  async request(name: string, args: any): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      resolver.timeout = setTimeout(() => {
        ...
        resolver.reject(SilaJSErrorWithoutCode(`Request timed out after ${this.timeout}ms`))
      }, this.timeout)
    })
  }
}

export class BoundEthProtocol extends BoundProtocol implements SilProtocolMethods {
  name = 'sil' // public name: string
