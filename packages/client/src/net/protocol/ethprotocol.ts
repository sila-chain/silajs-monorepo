...
import type { PostByzantiumTxReceipt, PreByzantiumTxReceipt, TxReceipt } from '@silajs/vm'
...
interface SilProtocolOptions extends ProtocolOptions {
  /* Blockchain */
  chain: Chain
}
 ...
 */
export interface SilProtocolMethods {
...
function exhaustiveTypeGuard(_value: never, errorMsg: string): never {
  throw SilaJSErrorWithoutCode(errorMsg)
}

/**
 * Implements sil/66 protocol
 * @memberof module:net/protocol
 */
export class SilProtocol extends Protocol {
  private chain: Chain
  ...
  private protocolMessages: Message[] = [
    ...
    },
    {
      ...
      code: 0x08,
      // If sil protocol is sil/68, the parameter list for `NewPooledTransactionHashes` changes from
   ...
   * Create sil protocol
   */
  constructor(options: SilProtocolOptions) {
    super(options)
  ...
  /**
   * Encodes status into SIL status message payload
   */
  encodeStatus(): any {
    return {
  ...
  /**
   * Decodes SIL status message payload into a status object
