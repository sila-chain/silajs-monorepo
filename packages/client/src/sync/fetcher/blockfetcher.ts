...
import { KECCAK256_RLP, KECCAK256_RLP_ARRAY, equalsBytes } from '@silajs/util'
...
/**
 * Implements an sil/66 based block fetcher
...
export class BlockFetcher extends BlockFetcherBase<Block[], Block> {
  ...
  async request(job: Job<JobTask, Block[], Block>): Promise<Block[]> {
    ...
    // 2. Properly implement SIL request IDs -> allow to call on non-idle in Peer Pool
    await peer?.latest()
    ...
    const headersResult = await peer!.sil!.getBlockHeaders({
      block: first,
      max: count,
    ...
    const bodiesResult = await peer?.sil?.getBlockBodies({ hashes: headers.map((h) => h.hash()) })
    if (
   ...
   */
  peer(): Peer | undefined {
    return this.pool.idle((peer) => 'sil' in peer)
  }
}
