...
import { BIGINT_0, BIGINT_1, equalsBytes } from '@silajs/util'
...
type PeerId = string
/**
 * Implements an sila full sync synchronizer
...
export class FullSynchronizer extends Synchronizer {
  ...
  async best(): Promise<Peer | undefined> {
    ...
    if (
      ...
      this.config.chainCommon.hardforkBlock(Hardfork.SilaParis) === null
    ) {
      // For pure non-Merge HF Ethash/Clique chains we want to select the peer with the highest TD
      let best
      for (const peer of peers) {
        if (peer.sil?.status !== undefined) {
          const td = peer.sil.status.td
          if (
            (!best && td >= this.chain.blocks.td) ||
            (best && best.sil && best.sil.status.td < td)
          ) {
            best = peer
          }
        }
      ...
      // Take a random peer which advertises the sil protocol (and did handshake with, `status !== undefined`)
      const peersWithEth = peers.filter((peer) => peer.sil?.status !== undefined)
  ...
  async processBlocks(blocks: Block[]) {
    if (this.config.chainCommon.gteHardfork(Hardfork.SilaParis)) {
      if (this.fetcher !== null) {
        ...
        this.config.logger?.info('SilaParis (Merge) hardfork reached, stopping block fetcher')
        this.clearFetcher()
      }
    }
  ...
  async sendNewBlock(block: Block, peers: Peer[]) {
    for (const peer of peers) {
      ...
      if (!alreadyKnownByPeer) {
        peer.sil?.send('NewBlock', [block, this.chain.blocks.td])
      }
    }
  }
  ...
  async handleNewBlock(block: Block, peer?: Peer) {
    ...
    // https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#block-propagation
    ...
    for (const peer of this.pool.peers.slice(numPeersToShareWith)) {
      ...
      const alreadyKnownByPeer = this.addToKnownByPeer(block.hash(), peer)
      if (!alreadyKnownByPeer) {
        peer.sil?.send('NewBlockHashes', [[block.hash(), block.header.number]])
      }
    }
  }
