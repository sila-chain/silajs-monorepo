import {
  ...
  DBSetHashToNumber,
  DBSetTD,
} from '@silajs/blockchain'
import { ConsensusType, Hardfork } from '@silajs/common'
import { MCLBLS, RustBN254 } from '@silajs/savm'
import { getGenesis } from '@silajs/genesis'
import { createMPT } from '@silajs/mpt'
import { CacheType, Caches, MerkleStateManager } from '@silajs/statemanager'
import {
  BIGINT_0,
  BIGINT_1,
...
import type { RunBlockOpts, TxReceipt, VM } from '@silajs/vm'
...
export class VMExecution extends Execution {
    ...
    return this.runWithLock<void>(async () => {
      ...
      if (number === BIGINT_0) {
        ...
        if (!genesisState && !('generateCanonicalGenesis' in this.vm.stateManager)) {
          throw SilaJSErrorWithoutCode('genesisState not available')
        } else {
        ...
        while (
          ...
          if (numExecuted !== null) {
            ...
            if (typeof numExecuted === 'number' && numExecuted > 0) {
              ...
              const tdAdd = this.config.execCommon.gteHardfork(Hardfork.SilaParis)
                ? ''
              ...
              if (this.config.execCommon.gteHardfork(Hardfork.SilaParis)) {
  ...
  async start(): Promise<boolean> {
    ...
    if (
      !this.config.execCommon.gteHardfork(Hardfork.SilaParis) &&
