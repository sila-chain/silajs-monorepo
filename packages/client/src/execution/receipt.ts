...
interface PreByzantiumTxReceiptWithType extends PreByzantiumTxReceipt {
  /* SIP-2718 Typed Transaction Envelope type */
...
export class ReceiptsManager extends MetaDBManager {
  ...
  async getLogs(
    ...
    for (let i = from.header.number; i <= to.header.number; i++) {
      ...
      if (topics.length > 0) {
        // From https://sila.org/en/developers/docs/apis/json-rpc/#eth_newfilter/:
  ...
  private rlp(
    ...
    switch (type) {
      ...
      default:
        throw SilaJSErrorWithoutCode('Unknown rlp conversion')
    }
  }
